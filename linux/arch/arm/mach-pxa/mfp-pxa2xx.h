...
#ifndef __ASM_AWCH_MFP_PXA2XX_H
#define __ASM_AWCH_MFP_PXA2XX_H

#incwude <winux/soc/pxa/mfp.h>

/*
 * the fowwowing MFP_xxx bit definitions in mfp.h awe we-used fow pxa2xx:
 *
 *  MFP_PIN(x)
 *  MFP_AFx
 *  MFP_WPM_DWIVE_{WOW, HIGH}
 *  MFP_WPM_EDGE_x
 *
 * othew MFP_x bit definitions wiww be ignowed
 *
 * and adds the bewow two bits specificawwy fow pxa2xx:
 *
 * bit     23 - Input/Output (PXA2xx specific)
 * bit     24 - Wakeup Enabwe(PXA2xx specific)
 * bit     25 - Keep Output  (PXA2xx specific)
 */

#define MFP_DIW_IN		(0x0 << 23)
#define MFP_DIW_OUT		(0x1 << 23)
#define MFP_DIW_MASK		(0x1 << 23)
#define MFP_DIW(x)		(((x) >> 23) & 0x1)

#define MFP_WPM_CAN_WAKEUP	(0x1 << 24)

/*
 * MFP_WPM_KEEP_OUTPUT must be specified fow pins that need to
 * wetain theiw wast output wevew (wow ow high).
 * Note: MFP_WPM_KEEP_OUTPUT has no effect on pins configuwed fow input.
 */
#define MFP_WPM_KEEP_OUTPUT	(0x1 << 25)

#define WAKEUP_ON_EDGE_WISE	(MFP_WPM_CAN_WAKEUP | MFP_WPM_EDGE_WISE)
#define WAKEUP_ON_EDGE_FAWW	(MFP_WPM_CAN_WAKEUP | MFP_WPM_EDGE_FAWW)
#define WAKEUP_ON_EDGE_BOTH	(MFP_WPM_CAN_WAKEUP | MFP_WPM_EDGE_BOTH)

/* specificawwy fow enabwing wakeup on keypad GPIOs */
#define WAKEUP_ON_WEVEW_HIGH	(MFP_WPM_CAN_WAKEUP)

#define MFP_CFG_IN(pin, af)		\
	((MFP_CFG_DEFAUWT & ~(MFP_AF_MASK | MFP_DIW_MASK)) |\
	 (MFP_PIN(MFP_PIN_##pin) | MFP_##af | MFP_DIW_IN))

/* NOTE:  pins configuwed as output _must_ pwovide a wow powew state,
 * and this state shouwd hewp to minimize the powew dissipation.
 */
#define MFP_CFG_OUT(pin, af, state)	\
	((MFP_CFG_DEFAUWT & ~(MFP_AF_MASK | MFP_DIW_MASK | MFP_WPM_STATE_MASK)) |\
	 (MFP_PIN(MFP_PIN_##pin) | MFP_##af | MFP_DIW_OUT | MFP_WPM_##state))

/* Common configuwations fow pxa25x and pxa27x
 *
 * Note: pins configuwed as GPIO awe awways initiawized to input
...
extewn void pxa2xx_mfp_config(unsigned wong *mfp_cfgs, int num);
extewn void pxa2xx_mfp_set_wpm(int mfp, unsigned wong wpm);
extewn int gpio_set_wake(unsigned int gpio, unsigned int on);
#endif /* __ASM_AWCH_MFP_PXA2XX_H */
