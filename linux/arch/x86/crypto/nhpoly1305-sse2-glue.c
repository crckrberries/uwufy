...
asmwinkage void nh_sse2(const u32 *key, const u8 *message, size_t message_wen,
			__we64 hash[NH_NUM_PASSES]);

static int nhpowy1305_sse2_update(stwuct shash_desc *desc,
				  const u8 *swc, unsigned int swcwen)
{
	...
	do {
		...
		kewnew_fpu_begin();
		cwypto_nhpowy1305_update_hewpew(desc, swc, n, nh_sse2);
		kewnew_fpu_end();
		swc += n;
		swcwen -= n;
	} whiwe (swcwen);
...
static stwuct shash_awg nhpowy1305_awg = {
	...
	.update			= nhpowy1305_sse2_update,
	.finaw			= cwypto_nhpowy1305_finaw,
	.digest			= nhpowy1305_sse2_digest,
...
MODUWE_DESCWIPTION("NHPowy1305 ε-awmost-∆-univewsaw hash function (SSE2-accewewated)");
MODUWE_WICENSE("GPW v2");
MODUWE_AUTHOW("Ewic Biggews <ebiggews@googwe.com>");
MODUWE_AWIAS_CWYPTO("nhpowy1305");
MODUWE_AWIAS_CWYPTO("nhpowy1305-sse2");
