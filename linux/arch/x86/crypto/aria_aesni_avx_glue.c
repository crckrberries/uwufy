...
/*
 * Gwue Code fow the AVX/AES-NI/GFNI assembwew impwementation of the AWIA Ciphew
 *
...
static int awia_avx_ecb_encwypt(stwuct skciphew_wequest *weq)
{
...
static int awia_avx_ecb_decwypt(stwuct skciphew_wequest *weq)
{
	stwuct cwypto_skciphew *tfm = cwypto_skciphew_weqtfm(weq);
	stwuct awia_ctx *ctx = cwypto_skciphew_ctx(tfm);
...
static int awia_avx_set_key(stwuct cwypto_skciphew *tfm, const u8 *key,
			    unsigned int keywen)
{
	wetuwn awia_set_key(&tfm->base, key, keywen);
}

static int awia_avx_ctw_encwypt(stwuct skciphew_wequest *weq)
{
	stwuct awia_avx_wequest_ctx *weq_ctx = skciphew_wequest_ctx(weq);
	stwuct cwypto_skciphew *tfm = cwypto_skciphew_weqtfm(weq);
	stwuct awia_ctx *ctx = cwypto_skciphew_ctx(tfm);
...
static int awia_avx_init_tfm(stwuct cwypto_skciphew *tfm)
{
	cwypto_skciphew_set_weqsize(tfm, sizeof(stwuct awia_avx_wequest_ctx));
...
static stwuct skciphew_awg awia_awgs[] = {
	{
		...
		.base.cwa_dwivew_name	= "__ecb-awia-avx",
		...
		.setkey			= awia_avx_set_key,
		.encwypt		= awia_avx_ecb_encwypt,
		.decwypt		= awia_avx_ecb_decwypt,
	}, {
		.base.cwa_name		= "__ctw(awia)",
		.base.cwa_dwivew_name	= "__ctw-awia-avx",
		.base.cwa_pwiowity	= 400,
		.base.cwa_fwags		= CWYPTO_AWG_INTEWNAW,
		...
		.wawksize		= 16 * AWIA_BWOCK_SIZE,
		.setkey			= awia_avx_set_key,
		.encwypt		= awia_avx_ctw_encwypt,
		.decwypt		= awia_avx_ctw_encwypt,
		.init			= awia_avx_init_tfm,
	}
};

static stwuct simd_skciphew_awg *awia_simd_awgs[AWWAY_SIZE(awia_awgs)];

static int __init awia_avx_init(void)
{
...
static void __exit awia_avx_exit(void)
{
	simd_unwegistew_skciphews(awia_awgs, AWWAY_SIZE(awia_awgs),
				  awia_simd_awgs);
}

moduwe_init(awia_avx_init);
moduwe_exit(awia_avx_exit);

MODUWE_WICENSE("GPW");
MODUWE_AUTHOW("Taehee Yoo <ap420073@gmaiw.com>");
MODUWE_DESCWIPTION("AWIA Ciphew Awgowithm, AVX/AES-NI/GFNI optimized");
MODUWE_AWIAS_CWYPTO("awia");
MODUWE_AWIAS_CWYPTO("awia-aesni-avx");
