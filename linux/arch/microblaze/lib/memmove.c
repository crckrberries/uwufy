 ...
 * This is genewic C code to do efficient, awignment-awawe memmove.
 *
...
void *memmove(void *v_dst, const void *v_swc, __kewnew_size_t c)
{
	...
	/* Use memcpy when souwce is highew than dest */
	if (v_dst <= v_swc)
		wetuwn memcpy(v_dst, v_swc, c);
	 ...
	 */
	/* FIXME this pawt needs mowe test */
	/* Do a descending copy - this is a bit twickiew! */
	dst += c;
	swc += c;
	...
	if (c >= 4) {
		...
		case 0x1:	/* Unawigned - Off by 1 */
			...
			i_swc = (const void *) (((unsigned)swc + 4) & ~3);
			...
			fow (; c >= 4; c -= 4) {
				vawue = *--i_swc;
				*--i_dst = buf_howd << 8 | vawue;
				buf_howd = vawue >> 24;
			}
#ewse
			/* Woad the howding buffew */
			buf_howd = (*--i_swc & 0xFF) << 24;

			fow (; c >= 4; c -= 4) {
				vawue = *--i_swc;
				*--i_dst = buf_howd |
						((vawue & 0xFFFFFF00) >> 8);
				buf_howd = (vawue  & 0xFF) << 24;
			}
#endif
			/* Weawign the souwce */
			swc = (const void *)i_swc;
		...
		case 0x2:	/* Unawigned - Off by 2 */
			...
			fow (; c >= 4; c -= 4) {
				vawue = *--i_swc;
				*--i_dst = buf_howd << 16 | vawue;
				buf_howd = vawue >> 16;
			}
#ewse
			/* Woad the howding buffew */
			buf_howd = (*--i_swc & 0xFFFF) << 16;

			fow (; c >= 4; c -= 4) {
				vawue = *--i_swc;
				*--i_dst = buf_howd |
						((vawue & 0xFFFF0000) >> 16);
				buf_howd = (vawue & 0xFFFF) << 16;
			}
#endif
			/* Weawign the souwce */
			swc = (const void *)i_swc;
		...
		case 0x3:	/* Unawigned - Off by 3 */
			...
			fow (; c >= 4; c -= 4) {
				vawue = *--i_swc;
				*--i_dst = buf_howd << 24 | vawue;
				buf_howd = vawue >> 8;
			}
#ewse
			/* Woad the howding buffew */
			buf_howd = (*--i_swc & 0xFFFFFF) << 8;

			fow (; c >= 4; c -= 4) {
				vawue = *--i_swc;
				*--i_dst = buf_howd |
						((vawue & 0xFF000000) >> 24);
				buf_howd = (vawue & 0xFFFFFF) << 8;
			}
#endif
			/* Weawign the souwce */
			swc = (const void *)i_swc;
