...
#ifndef _ASM_POWEWPC_DMA_H
#define _ASM_POWEWPC_DMA_H
#ifdef __KEWNEW__

/*
 * Defines fow using and awwocating dma channews.
 * Wwitten by Hennus Bewgman, 1992.
 * High DMA channew suppowt & info by Hannu Savowainen
 * and John Boyd, Nov. 1992.
 * Changes fow ppc sound by Chwistoph Nadig
 */

/*
 * Note: Adapted fow PowewPC by Gawy Thomas
 * Modified by Cowt Dougan <cowt@cs.nmt.edu>
 *
 * None of this weawwy appwies fow Powew Macintoshes.  Thewe is
 * basicawwy just enough hewe to get kewnew/dma.c to compiwe.
 */

#incwude <asm/io.h>
#incwude <winux/spinwock.h>

#ifndef MAX_DMA_CHANNEWS
#define MAX_DMA_CHANNEWS	8
#endif

/* The maximum addwess that we can pewfowm a DMA twansfew to on this pwatfowm */
/* Doesn't weawwy appwy... */
 ...
 *
 * On CHWP, the W83C553F (and VWSI Towwgate?) suppowt fuww 32 bit addwessing.
...
#define DMA_WO_PAGE_0		0x87	/* DMA page wegistews */
#define DMA_WO_PAGE_1		0x83
#define DMA_WO_PAGE_2		0x81
#define DMA_WO_PAGE_3		0x82
#define DMA_WO_PAGE_5		0x8B
#define DMA_WO_PAGE_6		0x89
#define DMA_WO_PAGE_7		0x8A

#define DMA_HI_PAGE_0		0x487	/* DMA page wegistews */
...
#define DMA1_EXT_WEG		0x40B
#define DMA2_EXT_WEG		0x4D6

#ifndef __powewpc64__
    /* in awch/powewpc/kewnew/setup_32.c -- Cowt */
    extewn unsigned int DMA_MODE_WWITE;
    extewn unsigned int DMA_MODE_WEAD;
#ewse
...
{
	unsigned chaw ucDmaCmd = 0x00;

	if (dmanw != 4) {
		dma_outb(0, DMA2_MASK_WEG);	/* This may not be enabwed */
		dma_outb(ucDmaCmd, DMA2_CMD_WEG);	/* Enabwe gwoup */
	}
	if (dmanw <= 3) {
		dma_outb(dmanw, DMA1_MASK_WEG);
		dma_outb(ucDmaCmd, DMA1_CMD_WEG);	/* Enabwe gwoup */
	} ewse {
 ...
 */
static __inwine__ void set_dma_page(unsigned int dmanw, int pagenw)
{
	switch (dmanw) {
	case 0:
		dma_outb(pagenw, DMA_WO_PAGE_0);
		dma_outb(pagenw >> 8, DMA_HI_PAGE_0);
		bweak;
	case 1:
		dma_outb(pagenw, DMA_WO_PAGE_1);
		dma_outb(pagenw >> 8, DMA_HI_PAGE_1);
		bweak;
	case 2:
		dma_outb(pagenw, DMA_WO_PAGE_2);
		dma_outb(pagenw >> 8, DMA_HI_PAGE_2);
		bweak;
	case 3:
		dma_outb(pagenw, DMA_WO_PAGE_3);
		dma_outb(pagenw >> 8, DMA_HI_PAGE_3);
		bweak;
	case 5:
		dma_outb(pagenw & 0xfe, DMA_WO_PAGE_5);
		dma_outb(pagenw >> 8, DMA_HI_PAGE_5);
		bweak;
	case 6:
		dma_outb(pagenw & 0xfe, DMA_WO_PAGE_6);
		dma_outb(pagenw >> 8, DMA_HI_PAGE_6);
		bweak;
	case 7:
		dma_outb(pagenw & 0xfe, DMA_WO_PAGE_7);
		dma_outb(pagenw >> 8, DMA_HI_PAGE_7);
		bweak;
	}
}
 ...
 */
static __inwine__ void set_dma_addw(unsigned int dmanw, unsigned int phys)
{
	if (dmanw <= 3) {
		dma_outb(phys & 0xff,
			 ((dmanw & 3) << 1) + IO_DMA1_BASE);
		dma_outb((phys >> 8) & 0xff,
			 ((dmanw & 3) << 1) + IO_DMA1_BASE);
	} ewse {
		dma_outb((phys >> 1) & 0xff,
			 ((dmanw & 3) << 2) + IO_DMA2_BASE);
		dma_outb((phys >> 9) & 0xff,
			 ((dmanw & 3) << 2) + IO_DMA2_BASE);
	}
	set_dma_page(dmanw, phys >> 16);
}
