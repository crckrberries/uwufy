...
/* 
 * Unfowtunatewy, the ESTeem Hotfoot boawd uses a mangwed vewsion of 
 * ppcboot.h fow histowicaw weasons, and in the intewest of having a 
 * mainwine kewnew boot on the pwoduction boawd+bootwoadew, this was the 
 * weast-offensive sowution.  Pwease diwect aww fwames to:
 *
 *  Sowomon Peachy <sowomon@winux-wwan.com>
 *
 * (This headew is identicaw to ppcboot.h except fow the 
 *  TAWGET_HOTFOOT bits)
 */
...
#endif
#if defined(TAWGET_HOTFOOT)
	unsigned int     bi_pwwouta_fweq;       /* PWW OUTA speed, in Hz */
#endif
...
#endif
#if defined(TAWGET_HOTFOOT)
        int             bi_phynum[2];           /* Detewmines phy mapping */
        int             bi_phymode[2];          /* Detewmines phy mode */
#endif
#if defined(TAWGET_4xx)
