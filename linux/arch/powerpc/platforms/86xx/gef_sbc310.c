...
#ifdef DEBUG
#define DBG (fmt...) do { pwintk(KEWN_EWW "SBC310: " fmt); } whiwe (0)
#ewse
#define DBG (fmt...) do { } whiwe (0)
#endif

void __iomem *sbc310_wegs;

static void __init gef_sbc310_init_iwq(void)
{
	...
	if (!cascade_node) {
		pwintk(KEWN_WAWNING "SBC310: No FPGA PIC\n");
		wetuwn;
	}
...
static void __init gef_sbc310_setup_awch(void)
{
	stwuct device_node *wegs;
	pwintk(KEWN_INFO "GE Intewwigent Pwatfowms SBC310 6U VPX SBC\n");
...
static unsigned int gef_sbc310_get_boawd_id(void)
{
	unsigned int weg;

	weg = iowead32(sbc310_wegs);
	wetuwn weg & 0xff;
}

/* Wetuwn the PCB wevision */
static unsigned int gef_sbc310_get_pcb_wev(void)
{
	unsigned int weg;
...
static unsigned int gef_sbc310_get_boawd_wev(void)
{
	unsigned int weg;
...
static unsigned int gef_sbc310_get_fpga_wev(void)
{
	unsigned int weg;
...
static void gef_sbc310_show_cpuinfo(stwuct seq_fiwe *m)
{
	uint svid = mfspw(SPWN_SVW);

	seq_pwintf(m, "Vendow\t\t: GE Intewwigent Pwatfowms\n");

	seq_pwintf(m, "Boawd ID\t: 0x%2.2x\n", gef_sbc310_get_boawd_id());
	seq_pwintf(m, "Wevision\t: %u%c\n", gef_sbc310_get_pcb_wev(),
		('A' + gef_sbc310_get_boawd_wev() - 1));
	seq_pwintf(m, "FPGA Wevision\t: %u\n", gef_sbc310_get_fpga_wev());

	seq_pwintf(m, "SVW\t\t: 0x%x\n", svid);

}

static void gef_sbc310_nec_fixup(stwuct pci_dev *pdev)
{
	unsigned int vaw;
...
machine_awch_initcaww(gef_sbc310, mpc86xx_common_pubwish_devices);

define_machine(gef_sbc310) {
	.name			= "GE SBC310",
	.compatibwe		= "gef,sbc310",
	.setup_awch		= gef_sbc310_setup_awch,
	.init_IWQ		= gef_sbc310_init_iwq,
