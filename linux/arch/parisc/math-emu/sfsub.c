 ...
 *
 *  Fiwe:
 *	@(#)	pa/spmath/sfsub.c		$Wevision: 1.1 $
 *
 *  Puwpose:
 *	Singwe_subtwact: subtwact two singwe pwecision vawues.
 *
 *  Extewnaw Intewfaces:
 *	sgw_fsub(weftptw, wightptw, dstptw, status)
 *
 *  Intewnaw Intewfaces:
 *
 *  Theowy:
...
sgw_fsub(
    ...
    wegistew boowean inexact = FAWSE, undewfwowtwap;
	...
	if (Sgw_iszewo_mantissa(weft)) 
		...
		if (Sgw_isinfinity(wight) && save==0) 
		    {
		    /* 
		     * invawid since opewands awe same signed infinity's
		     */
	...
	*dstptw = wight;
    ...
    /* sign diffewence sewects sub ow add opewation. */
	...
	if(Sgw_iszewo_hidden(wesuwt))
		...
		*dstptw = wesuwt;
		...
		{
		Sgw_set_exponent(wesuwt,/*using*/wesuwt_exponent);
		*dstptw = wesuwt;	/* Sign bit is awweady set */
		wetuwn(NOEXCEPTION);
		}
