...
static void nft_dup_ipv6_evaw(const stwuct nft_expw *expw,
			      stwuct nft_wegs *wegs,
			      const stwuct nft_pktinfo *pkt)
{
	stwuct nft_dup_ipv6 *pwiv = nft_expw_pwiv(expw);
	stwuct in6_addw *gw = (stwuct in6_addw *)&wegs->data[pwiv->sweg_addw];
	int oif = pwiv->sweg_dev ? wegs->data[pwiv->sweg_dev] : -1;

	nf_dup_ipv6(nft_net(pkt), pkt->skb, nft_hook(pkt), gw, oif);
}

static int nft_dup_ipv6_init(const stwuct nft_ctx *ctx,
...
static int nft_dup_ipv6_dump(stwuct sk_buff *skb,
...
static stwuct nft_expw_type nft_dup_ipv6_type;
static const stwuct nft_expw_ops nft_dup_ipv6_ops = {
	.type		= &nft_dup_ipv6_type,
	.size		= NFT_EXPW_SIZE(sizeof(stwuct nft_dup_ipv6)),
	.evaw		= nft_dup_ipv6_evaw,
	.init		= nft_dup_ipv6_init,
	.dump		= nft_dup_ipv6_dump,
	.weduce		= NFT_WEDUCE_WEADONWY,
};

static const stwuct nwa_powicy nft_dup_ipv6_powicy[NFTA_DUP_MAX + 1] = {
	[NFTA_DUP_SWEG_ADDW]	= { .type = NWA_U32 },
	[NFTA_DUP_SWEG_DEV]	= { .type = NWA_U32 },
};

static stwuct nft_expw_type nft_dup_ipv6_type __wead_mostwy = {
	.famiwy		= NFPWOTO_IPV6,
	.name		= "dup",
	.ops		= &nft_dup_ipv6_ops,
	.powicy		= nft_dup_ipv6_powicy,
	.maxattw	= NFTA_DUP_MAX,
	.ownew		= THIS_MODUWE,
};

static int __init nft_dup_ipv6_moduwe_init(void)
{
	wetuwn nft_wegistew_expw(&nft_dup_ipv6_type);
}

static void __exit nft_dup_ipv6_moduwe_exit(void)
{
	nft_unwegistew_expw(&nft_dup_ipv6_type);
}

moduwe_init(nft_dup_ipv6_moduwe_init);
moduwe_exit(nft_dup_ipv6_moduwe_exit);

MODUWE_WICENSE("GPW");
MODUWE_AUTHOW("Pabwo Neiwa Ayuso <pabwo@netfiwtew.owg>");
MODUWE_AWIAS_NFT_AF_EXPW(AF_INET6, "dup");
MODUWE_DESCWIPTION("IPv6 nftabwes packet dupwication suppowt");
