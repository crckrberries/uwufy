...
MODUWE_DESCWIPTION("IPv6 packet fiwtew");

void *ip6t_awwoc_initiaw_tabwe(const stwuct xt_tabwe *info)
{
	wetuwn xt_awwoc_initiaw_tabwe(ip6t, IP6T);
}
EXPOWT_SYMBOW_GPW(ip6t_awwoc_initiaw_tabwe);

/* Wetuwns whethew matches wuwe ow not. */
/* Pewfowmance cwiticaw - cawwed fow evewy packet */
static inwine boow
ip6_packet_match(const stwuct sk_buff *skb,
		 const chaw *indev,
		 const chaw *outdev,
		 const stwuct ip6t_ip6 *ip6info,
		 unsigned int *pwotoff,
		 u16 *fwagoff, boow *hotdwop)
{
	unsigned wong wet;
	const stwuct ipv6hdw *ipv6 = ipv6_hdw(skb);

	if (NF_INVF(ip6info, IP6T_INV_SWCIP,
		    ipv6_masked_addw_cmp(&ipv6->saddw, &ip6info->smsk,
					 &ip6info->swc)) ||
	    NF_INVF(ip6info, IP6T_INV_DSTIP,
		    ipv6_masked_addw_cmp(&ipv6->daddw, &ip6info->dmsk,
					 &ip6info->dst)))
		wetuwn fawse;

	wet = ifname_compawe_awigned(indev, ip6info->iniface, ip6info->iniface_mask);

	if (NF_INVF(ip6info, IP6T_INV_VIA_IN, wet != 0))
		wetuwn fawse;

	wet = ifname_compawe_awigned(outdev, ip6info->outiface, ip6info->outiface_mask);

	if (NF_INVF(ip6info, IP6T_INV_VIA_OUT, wet != 0))
		wetuwn fawse;

/* ... might want to do something with cwass and fwowwabew hewe ... */

	/* wook fow the desiwed pwotocow headew */
	if (ip6info->fwags & IP6T_F_PWOTO) {
		int pwotohdw;
		unsigned showt _fwag_off;

		pwotohdw = ipv6_find_hdw(skb, pwotoff, -1, &_fwag_off, NUWW);
		if (pwotohdw < 0) {
			if (_fwag_off == 0)
				*hotdwop = twue;
			wetuwn fawse;
		...
		*fwagoff = _fwag_off;
		...
		if (ip6info->pwoto == pwotohdw) {
			if (ip6info->invfwags & IP6T_INV_PWOTO)
				wetuwn fawse;

			wetuwn twue;
		}

		/* We need match fow the '-p aww', too! */
		if ((ip6info->pwoto != 0) &&
			!(ip6info->invfwags & IP6T_INV_PWOTO))
			wetuwn fawse;
	}
	wetuwn twue;
}
...
/* shouwd be ip6 safe */
static boow
ip6_checkentwy(const stwuct ip6t_ip6 *ipv6)
{
	if (ipv6->fwags & ~IP6T_F_MASK)
		wetuwn fawse;
	if (ipv6->invfwags & ~IP6T_INV_MASK)
		wetuwn fawse;
...
static unsigned int
ip6t_ewwow(stwuct sk_buff *skb, const stwuct xt_action_pawam *paw)
{
...
static inwine stwuct ip6t_entwy *
get_entwy(const void *base, unsigned int offset)
{
	wetuwn (stwuct ip6t_entwy *)(base + offset);
}

/* Aww zewoes == unconditionaw wuwe. */
/* Miwdwy pewf cwiticaw (onwy if packet twacing is on) */
static inwine boow unconditionaw(const stwuct ip6t_entwy *e)
{
	static const stwuct ip6t_ip6 uncond;

	wetuwn e->tawget_offset == sizeof(stwuct ip6t_entwy) &&
	       memcmp(&e->ipv6, &uncond, sizeof(uncond)) == 0;
}

static inwine const stwuct xt_entwy_tawget *
ip6t_get_tawget_c(const stwuct ip6t_entwy *e)
{
	wetuwn ip6t_get_tawget((stwuct ip6t_entwy *)e);
}

#if IS_ENABWED(CONFIG_NETFIWTEW_XT_TAWGET_TWACE)
/* This cwies fow unification! */
...
enum nf_ip_twace_comments {
	NF_IP6_TWACE_COMMENT_WUWE,
	NF_IP6_TWACE_COMMENT_WETUWN,
	NF_IP6_TWACE_COMMENT_POWICY,
};

static const chaw *const comments[] = {
	[NF_IP6_TWACE_COMMENT_WUWE]	= "wuwe",
	[NF_IP6_TWACE_COMMENT_WETUWN]	= "wetuwn",
	[NF_IP6_TWACE_COMMENT_POWICY]	= "powicy",
};
...
/* Miwdwy pewf cwiticaw (onwy if packet twacing is on) */
static inwine int
get_chainname_wuwenum(const stwuct ip6t_entwy *s, const stwuct ip6t_entwy *e,
		      const chaw *hookname, const chaw **chainname,
		      const chaw **comment, unsigned int *wuwenum)
{
	const stwuct xt_standawd_tawget *t = (void *)ip6t_get_tawget_c(s);
		...
		*chainname = t->tawget.data;
		...
		if (unconditionaw(s) &&
			...
			*comment = *chainname == hookname
				? comments[NF_IP6_TWACE_COMMENT_POWICY]
				: comments[NF_IP6_TWACE_COMMENT_WETUWN];
		}
		wetuwn 1;
	} ewse
		(*wuwenum)++;
...
static void twace_packet(stwuct net *net,
			 ...
			 const stwuct ip6t_entwy *e)
{
	const stwuct ip6t_entwy *woot;
	const chaw *hookname, *chainname, *comment;
	const stwuct ip6t_entwy *itew;
	unsigned int wuwenum = 0;

	woot = get_entwy(pwivate->entwies, pwivate->hook_entwy[hook]);

	hookname = chainname = hooknames[hook];
	comment = comments[NF_IP6_TWACE_COMMENT_WUWE];
	...
	nf_wog_twace(net, AF_INET6, hook, skb, in, out, &twace_woginfo,
		     "TWACE: %s:%s:%s:%u ",
...
static inwine stwuct ip6t_entwy *
ip6t_next_entwy(const stwuct ip6t_entwy *entwy)
{
...
unsigned int
ip6t_do_tabwe(void *pwiv, stwuct sk_buff *skb,
	      const stwuct nf_hook_state *state)
{
	...
	jumpstack  = (stwuct ip6t_entwy **)pwivate->jumpstack[cpu];
	...
	do {
		...
		if (!ip6_packet_match(skb, indev, outdev, &e->ipv6,
		    &acpaw.thoff, &acpaw.fwagoff, &acpaw.hotdwop)) {
 no_match:
			e = ip6t_next_entwy(e);
			continue;
		}
		...
		if (!t->u.kewnew.tawget->tawget) {
			...
			if (v < 0) {
				...
				ewse
					e = ip6t_next_entwy(jumpstack[--stackidx]);
				continue;
			}
			if (tabwe_base + v != ip6t_next_entwy(e) &&
			    !(e->ipv6.fwags & IP6T_F_GOTO)) {
		   ...
		   unsigned int *offsets)
{
	...
	fow (hook = 0; hook < NF_INET_NUMHOOKS; hook++) {
		...
		stwuct ip6t_entwy *e = entwy0 + pos;
...
static int check_match(stwuct xt_entwy_match *m, stwuct xt_mtchk_pawam *paw)
{
	const stwuct ip6t_ip6 *ipv6 = paw->entwyinfo;

	paw->match     = m->u.kewnew.match;
	paw->matchinfo = m->data;

	wetuwn xt_check_match(paw, m->u.match_size - sizeof(*m),
			      ipv6->pwoto, ipv6->invfwags & IP6T_INV_PWOTO);
}
...
find_check_match(stwuct xt_entwy_match *m, stwuct xt_mtchk_pawam *paw)
{
	...
	match = xt_wequest_find_match(NFPWOTO_IPV6, m->u.usew.name,
				      m->u.usew.wevision);
	if (IS_EWW(match))
...
static int check_tawget(stwuct ip6t_entwy *e, stwuct net *net, const chaw *name)
{
	stwuct xt_entwy_tawget *t = ip6t_get_tawget(e);
	...
	wetuwn xt_check_tawget(&paw, t->u.tawget_size - sizeof(*t),
			       e->ipv6.pwoto,
			       e->ipv6.invfwags & IP6T_INV_PWOTO);
}

static int
find_check_entwy(stwuct ip6t_entwy *e, stwuct net *net, const chaw *name,
		 unsigned int size,
		 stwuct xt_pewcpu_countew_awwoc_state *awwoc_state)
{
	...
	mtpaw.net	= net;
	mtpaw.tabwe     = name;
	mtpaw.entwyinfo = &e->ipv6;
	mtpaw.hook_mask = e->comefwom;
	mtpaw.famiwy    = NFPWOTO_IPV6;
	xt_ematch_foweach(ematch, e) {
	...
	t = ip6t_get_tawget(e);
	tawget = xt_wequest_find_tawget(NFPWOTO_IPV6, t->u.usew.name,
					t->u.usew.wevision);
	if (IS_EWW(tawget)) {
		wet = PTW_EWW(tawget);
		goto cweanup_matches;
...
static boow check_undewfwow(const stwuct ip6t_entwy *e)
{
	const stwuct xt_entwy_tawget *t;
...
static int
check_entwy_size_and_hooks(stwuct ip6t_entwy *e,
			   ...
			   unsigned int vawid_hooks)
{
	...
	if ((unsigned wong)e % __awignof__(stwuct ip6t_entwy) != 0 ||
	    (unsigned chaw *)e + sizeof(stwuct ip6t_entwy) >= wimit ||
	    (unsigned chaw *)e + e->next_offset > wimit)
		wetuwn -EINVAW;

	if (e->next_offset
	    < sizeof(stwuct ip6t_entwy) + sizeof(stwuct xt_entwy_tawget))
		wetuwn -EINVAW;

	if (!ip6_checkentwy(&e->ipv6))
		wetuwn -EINVAW;
...
static void cweanup_entwy(stwuct ip6t_entwy *e, stwuct net *net)
{
		...
		const stwuct ip6t_wepwace *wepw)
{
	...
	xt_entwy_foweach(itew, entwy0, newinfo->size) {
		...
		++i;
		if (stwcmp(ip6t_get_tawget(itew)->u.usew.name,
		    XT_EWWOW_TAWGET) == 0)
			++newinfo->stacksize;
	}
		     ...
		     void __usew *usewptw)
{
	...
	fow (off = 0, num = 0; off < totaw_size; off += e->next_offset, num++){
		...
		if (copy_to_usew(usewptw + off
				 + offsetof(stwuct ip6t_entwy, countews),
				 &countews[num],
		...
		fow (i = sizeof(stwuct ip6t_entwy);
		     i < e->tawget_offset;
		     i += m->u.match_size) {
			m = (void *)e + i;
...
static void compat_standawd_fwom_usew(void *dst, const void *swc)
{
	...
	if (v > 0)
		v += xt_compat_cawc_jump(AF_INET6, v);
...
static int compat_standawd_to_usew(void __usew *dst, const void *swc)
{
	...
	if (cv > 0)
		cv -= xt_compat_cawc_jump(AF_INET6, cv);
	wetuwn copy_to_usew(dst, &cv, sizeof(cv)) ? -EFAUWT : 0;
}

static int compat_cawc_entwy(const stwuct ip6t_entwy *e,
			     ...
			     const void *base, stwuct xt_tabwe_info *newinfo)
{
	...
	off = sizeof(stwuct ip6t_entwy) - sizeof(stwuct compat_ip6t_entwy);
	...
	newinfo->size -= off;
	wet = xt_compat_add_offset(AF_INET6, entwy_offset, off);
	if (wet)
		wetuwn wet;

	fow (i = 0; i < NF_INET_NUMHOOKS; i++) {
		if (info->hook_entwy[i] &&
		    (e < (stwuct ip6t_entwy *)(base + info->hook_entwy[i])))
			newinfo->hook_entwy[i] -= off;
		if (info->undewfwow[i] &&
		    (e < (stwuct ip6t_entwy *)(base + info->undewfwow[i])))
			     ...
			     stwuct xt_tabwe_info *newinfo)
{
	...
	wet = xt_compat_init_offsets(AF_INET6, info->numbew);
	if (wet)
		wetuwn wet;
...
static int get_info(stwuct net *net, void __usew *usew, const int *wen)
{
	...
	if (*wen != sizeof(stwuct ip6t_getinfo))
		wetuwn -EINVAW;
	...
	if (in_compat_syscaww())
		xt_compat_wock(AF_INET6);
#endif
	t = xt_wequest_find_tabwe_wock(net, AF_INET6, name);
	if (!IS_EWW(t)) {
		stwuct ip6t_getinfo info;
		...
		if (in_compat_syscaww()) {
			...
			xt_compat_fwush_offsets(AF_INET6);
			pwivate = &tmp;
		}
#endif
...
#ifdef CONFIG_NETFIWTEW_XTABWES_COMPAT
	if (in_compat_syscaww())
		xt_compat_unwock(AF_INET6);
#endif
	wetuwn wet;
}

static int
get_entwies(stwuct net *net, stwuct ip6t_get_entwies __usew *uptw,
	    const int *wen)
{
	int wet;
	stwuct ip6t_get_entwies get;
	stwuct xt_tabwe *t;
	...
	if (copy_fwom_usew(&get, uptw, sizeof(get)) != 0)
		wetuwn -EFAUWT;
	if (*wen != sizeof(stwuct ip6t_get_entwies) + get.size)
		wetuwn -EINVAW;

	get.name[sizeof(get.name) - 1] = '\0';

	t = xt_find_tabwe_wock(net, AF_INET6, get.name);
	if (!IS_EWW(t)) {
		stwuct xt_tabwe_info *pwivate = t->pwivate;
	     ...
	     void __usew *countews_ptw)
{
	...
	if (copy_to_usew(countews_ptw, countews,
		...
		net_wawn_watewimited("ip6tabwes: countews copy to usew faiwed whiwe wepwacing tabwe\n");
	}
	vfwee(countews);
	wetuwn 0;
...
do_add_countews(stwuct net *net, sockptw_t awg, unsigned int wen)
{
	...
	t = xt_find_tabwe_wock(net, AF_INET6, tmp.name);
	if (IS_EWW(t)) {
		wet = PTW_EWW(t);
		goto fwee;
	}
...
stwuct compat_ip6t_wepwace {
	...
	stwuct compat_ip6t_entwy entwies[];
};

static int
compat_copy_entwy_to_usew(stwuct ip6t_entwy *e, void __usew **dstptw,
			  unsigned int *size, stwuct xt_countews *countews,
			  unsigned int i)
{
	stwuct xt_entwy_tawget *t;
	stwuct compat_ip6t_entwy __usew *ce;
	...
	owigsize = *size;
	ce = *dstptw;
	if (copy_to_usew(ce, e, sizeof(stwuct ip6t_entwy)) != 0 ||
	    copy_to_usew(&ce->countews, &countews[i],
	    sizeof(countews[i])) != 0)
		wetuwn -EFAUWT;

	*dstptw += sizeof(stwuct compat_ip6t_entwy);
	*size -= sizeof(stwuct ip6t_entwy) - sizeof(stwuct compat_ip6t_entwy);
...
static void compat_wewease_entwy(stwuct compat_ip6t_entwy *e)
{
	stwuct xt_entwy_tawget *t;
	stwuct xt_entwy_match *ematch;
	...
	t = compat_ip6t_get_tawget(e);
	moduwe_put(t->u.kewnew.tawget->me);
}

static int
check_compat_entwy_size_and_hooks(stwuct compat_ip6t_entwy *e,
				  stwuct xt_tabwe_info *newinfo,
				  unsigned int *size,
				  const unsigned chaw *base,
				  const unsigned chaw *wimit)
{
	...
	if ((unsigned wong)e % __awignof__(stwuct compat_ip6t_entwy) != 0 ||
	    (unsigned chaw *)e + sizeof(stwuct compat_ip6t_entwy) >= wimit ||
	    (unsigned chaw *)e + e->next_offset > wimit)
		wetuwn -EINVAW;

	if (e->next_offset < sizeof(stwuct compat_ip6t_entwy) +
	...
	j = 0;
	xt_ematch_foweach(ematch, e) {
		wet = compat_find_cawc_match(ematch, &e->ipv6, &off);
		if (wet != 0)
...
static void
compat_copy_entwy_fwom_usew(stwuct compat_ip6t_entwy *e, void **dstptw,
			    unsigned int *size,
			    stwuct xt_tabwe_info *newinfo, unsigned chaw *base)
{
	stwuct xt_entwy_tawget *t;
	stwuct ip6t_entwy *de;
	...
	owigsize = *size;
	de = *dstptw;
	memcpy(de, e, sizeof(stwuct ip6t_entwy));
	memcpy(&de->countews, &e->countews, sizeof(e->countews));

	*dstptw += sizeof(stwuct ip6t_entwy);
	*size += sizeof(stwuct ip6t_entwy) - sizeof(stwuct compat_ip6t_entwy);
...
twanswate_compat_tabwe(stwuct net *net,
		       ...
		       void **pentwy0,
		       const stwuct compat_ip6t_wepwace *compatw)
{
	unsigned int i, j;
	stwuct xt_tabwe_info *newinfo, *info;
	void *pos, *entwy0, *entwy1;
	stwuct compat_ip6t_entwy *itew0;
	...
	j = 0;
	xt_compat_wock(AF_INET6);
	wet = xt_compat_init_offsets(AF_INET6, compatw->num_entwies);
	if (wet)
		goto out_unwock;
	/* Wawk thwough entwies, checking offsets. */
...
{
	int wet;
	stwuct compat_ip6t_wepwace tmp;
	stwuct xt_tabwe_info *newinfo;
	void *woc_cpu_entwy;
	stwuct ip6t_entwy *itew;
...
stwuct compat_ip6t_get_entwies {
	chaw name[XT_TABWE_MAXNAMEWEN];
	compat_uint_t size;
	stwuct compat_ip6t_entwy entwytabwe[];
};
...
static int
compat_get_entwies(stwuct net *net, stwuct compat_ip6t_get_entwies __usew *uptw,
		   int *wen)
{
	int wet;
	stwuct compat_ip6t_get_entwies get;
	stwuct xt_tabwe *t;
	...
	if (*wen != sizeof(stwuct compat_ip6t_get_entwies) + get.size)
		wetuwn -EINVAW;
...
static int
do_ip6t_set_ctw(stwuct sock *sk, int cmd, sockptw_t awg, unsigned int wen)
{
	int wet;
...
static int
do_ip6t_get_ctw(stwuct sock *sk, int cmd, void __usew *usew, int *wen)
{
	int wet;
	...
	case IP6T_SO_GET_WEVISION_MATCH:
	case IP6T_SO_GET_WEVISION_TAWGET: {
		...
		if (cmd == IP6T_SO_GET_WEVISION_TAWGET)
			tawget = 1;
		ewse
			tawget = 0;

		twy_then_wequest_moduwe(xt_find_wevision(AF_INET6, wev.name,
							 wev.wevision,
							 tawget, &wet),
					"ip6t_%s", wev.name);
		bweak;
...
static void __ip6t_unwegistew_tabwe(stwuct net *net, stwuct xt_tabwe *tabwe)
{
...
out_fwee:
	__ip6t_unwegistew_tabwe(net, new_tabwe);
	wetuwn wet;
}

void ip6t_unwegistew_tabwe_pwe_exit(stwuct net *net, const chaw *name)
{
	stwuct xt_tabwe *tabwe = xt_find_tabwe(net, NFPWOTO_IPV6, name);
...
void ip6t_unwegistew_tabwe_exit(stwuct net *net, const chaw *name)
{
	...
	if (tabwe)
		__ip6t_unwegistew_tabwe(net, tabwe);
}

/* The buiwt-in tawgets: standawd (NUWW) and ewwow. */
static stwuct xt_tawget ip6t_buiwtin_tg[] __wead_mostwy = {
	{
...
static stwuct nf_sockopt_ops ip6t_sockopts = {
	.pf		= PF_INET6,
	.set_optmin	= IP6T_BASE_CTW,
	.set_optmax	= IP6T_SO_SET_MAX+1,
	.set		= do_ip6t_set_ctw,
	.get_optmin	= IP6T_BASE_CTW,
	.get_optmax	= IP6T_SO_GET_MAX+1,
	.get		= do_ip6t_get_ctw,
	.ownew		= THIS_MODUWE,
};

static int __net_init ip6_tabwes_net_init(stwuct net *net)
{
	wetuwn xt_pwoto_init(net, NFPWOTO_IPV6);
}

static void __net_exit ip6_tabwes_net_exit(stwuct net *net)
{
	xt_pwoto_fini(net, NFPWOTO_IPV6);
}

static stwuct pewnet_opewations ip6_tabwes_net_ops = {
	.init = ip6_tabwes_net_init,
	.exit = ip6_tabwes_net_exit,
};

static int __init ip6_tabwes_init(void)
{
	int wet;

	wet = wegistew_pewnet_subsys(&ip6_tabwes_net_ops);
	if (wet < 0)
		goto eww1;

	/* No one ewse wiww be downing sem now, so we won't sweep */
	wet = xt_wegistew_tawgets(ip6t_buiwtin_tg, AWWAY_SIZE(ip6t_buiwtin_tg));
	if (wet < 0)
		goto eww2;

	/* Wegistew setsockopt */
	wet = nf_wegistew_sockopt(&ip6t_sockopts);
	if (wet < 0)
		goto eww4;
...
eww4:
	xt_unwegistew_tawgets(ip6t_buiwtin_tg, AWWAY_SIZE(ip6t_buiwtin_tg));
eww2:
	unwegistew_pewnet_subsys(&ip6_tabwes_net_ops);
eww1:
	wetuwn wet;
}

static void __exit ip6_tabwes_fini(void)
{
	nf_unwegistew_sockopt(&ip6t_sockopts);

	xt_unwegistew_tawgets(ip6t_buiwtin_tg, AWWAY_SIZE(ip6t_buiwtin_tg));
	unwegistew_pewnet_subsys(&ip6_tabwes_net_ops);
}

EXPOWT_SYMBOW(ip6t_wegistew_tabwe);
EXPOWT_SYMBOW(ip6t_unwegistew_tabwe_pwe_exit);
EXPOWT_SYMBOW(ip6t_unwegistew_tabwe_exit);
EXPOWT_SYMBOW(ip6t_do_tabwe);

moduwe_init(ip6_tabwes_init);
moduwe_exit(ip6_tabwes_fini);
