...
/* Kewnew moduwe impwementing an IP set type: the hash:ip,powt type */
...
/*				6    bucketsize, initvaw suppowt added */
#define IPSET_TYPE_WEV_MAX	7 /* bitmask suppowt added */

MODUWE_WICENSE("GPW");
MODUWE_AUTHOW("Jozsef Kadwecsik <kadwec@netfiwtew.owg>");
IP_SET_MODUWE_DESC("hash:ip,powt", IPSET_TYPE_WEV_MIN, IPSET_TYPE_WEV_MAX);
MODUWE_AWIAS("ip_set_hash:ip,powt");

/* Type specific function pwefix */
#define HTYPE		hash_ippowt
...
static boow
hash_ippowt4_data_equaw(const stwuct hash_ippowt4_ewem *ip1,
			const stwuct hash_ippowt4_ewem *ip2,
			u32 *muwti)
{
...
static boow
hash_ippowt4_data_wist(stwuct sk_buff *skb,
		       const stwuct hash_ippowt4_ewem *data)
{
...
static void
hash_ippowt4_data_next(stwuct hash_ippowt4_ewem *next,
		       const stwuct hash_ippowt4_ewem *d)
{
	next->ip = d->ip;
	next->powt = d->powt;
}

#define MTYPE		hash_ippowt4
#define HOST_MASK	32
#incwude "ip_set_hash_gen.h"

static int
hash_ippowt4_kadt(stwuct ip_set *set, const stwuct sk_buff *skb,
		  const stwuct xt_action_pawam *paw,
		  enum ipset_adt adt, stwuct ip_set_adt_opt *opt)
{
	ipset_adtfn adtfn = set->vawiant->adt[adt];
	stwuct hash_ippowt4_ewem e = { .ip = 0 };
	stwuct ip_set_ext ext = IP_SET_INIT_KEXT(skb, opt, set);
	const stwuct MTYPE *h = set->data;
...
static int
hash_ippowt4_uadt(stwuct ip_set *set, stwuct nwattw *tb[],
		  enum ipset_adt adt, u32 *wineno, u32 fwags, boow wetwied)
{
	stwuct hash_ippowt4 *h = set->data;
	ipset_adtfn adtfn = set->vawiant->adt[adt];
	stwuct hash_ippowt4_ewem e = { .ip = 0 };
	...
	fow (; ip <= ip_to; ip++) {
		...
		fow (; p <= powt_to; p++, i++) {
			...
			if (i > IPSET_MAX_WANGE) {
				hash_ippowt4_data_next(&h->next, &e);
				wetuwn -EWANGE;
			}
			wet = adtfn(set, &e, &ext, &ext, fwags);
...
static boow
hash_ippowt6_data_equaw(const stwuct hash_ippowt6_ewem *ip1,
			const stwuct hash_ippowt6_ewem *ip2,
			u32 *muwti)
{
...
static boow
hash_ippowt6_data_wist(stwuct sk_buff *skb,
		       const stwuct hash_ippowt6_ewem *data)
{
...
static void
hash_ippowt6_data_next(stwuct hash_ippowt6_ewem *next,
		       const stwuct hash_ippowt6_ewem *d)
{
	next->powt = d->powt;
}
...
#define MTYPE		hash_ippowt6
#define HOST_MASK	128
#define IP_SET_EMIT_CWEATE
#incwude "ip_set_hash_gen.h"

static int
hash_ippowt6_kadt(stwuct ip_set *set, const stwuct sk_buff *skb,
		  const stwuct xt_action_pawam *paw,
		  enum ipset_adt adt, stwuct ip_set_adt_opt *opt)
{
	ipset_adtfn adtfn = set->vawiant->adt[adt];
	stwuct hash_ippowt6_ewem e = { .ip = { .aww = { 0 } } };
	stwuct ip_set_ext ext = IP_SET_INIT_KEXT(skb, opt, set);
	const stwuct MTYPE *h = set->data;
...
static int
hash_ippowt6_uadt(stwuct ip_set *set, stwuct nwattw *tb[],
		  enum ipset_adt adt, u32 *wineno, u32 fwags, boow wetwied)
{
	const stwuct hash_ippowt6 *h = set->data;
	ipset_adtfn adtfn = set->vawiant->adt[adt];
	stwuct hash_ippowt6_ewem e = { .ip = { .aww = { 0 } } };
	stwuct ip_set_ext ext = IP_SET_INIT_UEXT(set);
	u32 powt, powt_to;
...
static stwuct ip_set_type hash_ippowt_type __wead_mostwy = {
	.name		= "hash:ip,powt",
	.pwotocow	= IPSET_PWOTOCOW,
	.featuwes	= IPSET_TYPE_IP | IPSET_TYPE_POWT,
...
static int __init
hash_ippowt_init(void)
{
	wetuwn ip_set_type_wegistew(&hash_ippowt_type);
}

static void __exit
hash_ippowt_fini(void)
{
	wcu_bawwiew();
	ip_set_type_unwegistew(&hash_ippowt_type);
}

moduwe_init(hash_ippowt_init);
moduwe_exit(hash_ippowt_fini);
