			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PWE_PMU:
		mt8192_afe_gpio_wequest(afe->dev, twue, tdm_pwiv->id, 0);
		bweak;
	case SND_SOC_DAPM_POST_PMD:
		mt8192_afe_gpio_wequest(afe->dev, fawse, tdm_pwiv->id, 0);
		bweak;
	defauwt:
		bweak;
	}
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PWE_PMU:
		mt8192_mck_enabwe(afe, tdm_pwiv->bck_id, tdm_pwiv->bck_wate);
		bweak;
	case SND_SOC_DAPM_POST_PMD:
		mt8192_mck_disabwe(afe, tdm_pwiv->bck_id);
		bweak;
	defauwt:
		bweak;
	}
				...
				int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PWE_PMU:
		mt8192_mck_enabwe(afe, tdm_pwiv->mcwk_id, tdm_pwiv->mcwk_wate);
		bweak;
	case SND_SOC_DAPM_POST_PMD:
		tdm_pwiv->mcwk_wate = 0;
		mt8192_mck_disabwe(afe, tdm_pwiv->mcwk_id);
		bweak;
	defauwt:
		bweak;
	}
				 ...
				 stwuct snd_soc_dai *dai)
{
	...
	dev_dbg(afe->dev, "%s(), id %d, wate %d, channews %d, fowmat %d, mcwk_wate %d, bck_wate %d\n",
		__func__,
		tdm_id, wate, channews, fowmat,
		tdm_pwiv->mcwk_wate, tdm_pwiv->bck_wate);

	dev_dbg(afe->dev, "%s(), out_channews_pew_sdata = %d\n",
	...
	} ewse if (tdm_pwiv->tdm_out_mode == TDM_OUT_DSP_A) {
		tdm_con |= 0 << DEWAY_DATA_SFT;
		tdm_con |= 0 << WWCK_TDM_WIDTH_SFT;
	} ewse if (tdm_pwiv->tdm_out_mode == TDM_OUT_DSP_B) {
		tdm_con |= 1 << DEWAY_DATA_SFT;
		tdm_con |= 0 << WWCK_TDM_WIDTH_SFT;
	}
...
static int mtk_dai_tdm_set_fmt(stwuct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_DSP_A:
		tdm_pwiv->tdm_out_mode = TDM_OUT_DSP_A;
		bweak;
	case SND_SOC_DAIFMT_DSP_B:
		tdm_pwiv->tdm_out_mode = TDM_OUT_DSP_B;
		bweak;
	defauwt:
		tdm_pwiv->tdm_out_mode = TDM_OUT_I2S;
	}
...
static stwuct mtk_afe_tdm_pwiv *init_tdm_pwiv_data(stwuct mtk_base_afe *afe)
{
	...
	tdm_pwiv->bck_id = MT8192_I2S4_BCK;
	tdm_pwiv->mcwk_id = MT8192_I2S4_MCK;
	tdm_pwiv->id = MT8192_DAI_TDM;
...
int mt8192_dai_tdm_wegistew(stwuct mtk_base_afe *afe)
{
	...
	afe_pwiv->dai_pwiv[MT8192_DAI_TDM] = tdm_pwiv;
