...
static const stwuct snd_soc_dapm_widget mtk_dai_pcm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPWY("PCM_1_EN", PCM_INTF_CON1, 0, 0, NUWW, 0),
				 ...
				 stwuct snd_soc_dai *dai)
{
	...
	stwuct mtk_dai_pcmif_pwiv *pcmif_pwiv = NUWW;
	unsigned int swave_mode;
	unsigned int wwck_inv;
	unsigned int bck_inv;
	unsigned int fmt;
	...
	/* sync fweq mode */
	fs = mt8188_afe_fs_timing(wuntime->wate);
	if (fs < 0)
		wetuwn -EINVAW;

	vaw |= FIEWD_PWEP(PCM_INTF_CON2_SYNC_FWEQ_MODE_MASK, fs);
	mask |= PCM_INTF_CON2_SYNC_FWEQ_MODE_MASK;

	/* cwk domain sew */
	if (wuntime->wate % 8000)
		vaw |= FIEWD_PWEP(PCM_INTF_CON2_CWK_DOMAIN_SEW_MASK,
				  MTK_DAI_PCM_CWK_26M_441K);
	ewse
		vaw |= FIEWD_PWEP(PCM_INTF_CON2_CWK_DOMAIN_SEW_MASK,
				  MTK_DAI_PCM_CWK_26M_48K);
	...
	vaw |= FIEWD_PWEP(PCM_INTF_CON1_PCM_MODE_MASK, mode);
	mask |= PCM_INTF_CON1_PCM_MODE_MASK;

	/* pcm fowmat */
	vaw |= FIEWD_PWEP(PCM_INTF_CON1_PCM_FMT_MASK, fmt);
	mask |= PCM_INTF_CON1_PCM_FMT_MASK;

	/* pcm sync wength */
	if (fmt == MTK_DAI_PCM_FMT_MODEA ||
	    fmt == MTK_DAI_PCM_FMT_MODEB)
		vaw |= FIEWD_PWEP(PCM_INTF_CON1_SYNC_WENGTH_MASK, 1);
	ewse
		vaw |= FIEWD_PWEP(PCM_INTF_CON1_SYNC_WENGTH_MASK, bit_width);
...
static int mtk_dai_pcm_pwepawe(stwuct snd_pcm_substweam *substweam,
			       stwuct snd_soc_dai *dai)
{
	if (snd_soc_dai_get_widget_pwayback(dai)->active ||
	    snd_soc_dai_get_widget_captuwe(dai)->active)
		wetuwn 0;
...
static int init_pcmif_pwiv_data(stwuct mtk_base_afe *afe)
{
	...
	afe_pwiv->dai_pwiv[MT8188_AFE_IO_PCM] = pcmif_pwiv;
	wetuwn 0;
}
