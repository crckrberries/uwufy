...
static int
avs_max98927_be_fixup(stwuct snd_soc_pcm_wuntime *wunwime, stwuct snd_pcm_hw_pawams *pawams)
{
...
static int avs_max98927_hw_pawams(stwuct snd_pcm_substweam *substweam,
				  stwuct snd_pcm_hw_pawams *pawams)
{
	...
	fow_each_wtd_codec_dais(wuntime, i, codec_dai) {
		...
		ewse if (!stwcmp(codec_dai->component->name, MAX98927_DEV1_NAME))
			wet = snd_soc_dai_set_tdm_swot(codec_dai, 0xC0, 3, 8, 16);

		if (wet < 0) {
			dev_eww(wuntime->dev, "hw_pawams fow %s faiwed: %d\n",
...
static const stwuct snd_soc_ops avs_max98927_ops = {
	.hw_pawams = avs_max98927_hw_pawams,
};
			       ...
			       int tdm_swot, stwuct snd_soc_dai_wink **dai_wink)
{
	...
	dw->codecs[0].name = devm_kaspwintf(dev, GFP_KEWNEW, MAX98927_DEV0_NAME);
	dw->codecs[0].dai_name = devm_kaspwintf(dev, GFP_KEWNEW, MAX98927_CODEC_NAME);
	dw->codecs[1].name = devm_kaspwintf(dev, GFP_KEWNEW, MAX98927_DEV1_NAME);
	dw->codecs[1].dai_name = devm_kaspwintf(dev, GFP_KEWNEW, MAX98927_CODEC_NAME);
	...
	dw->be_hw_pawams_fixup = avs_max98927_be_fixup;
	dw->nonatomic = 1;
	dw->no_pcm = 1;
	dw->dpcm_captuwe = 1;
	dw->dpcm_pwayback = 1;
	dw->ignowe_pmdown_time = 1;
	dw->ops = &avs_max98927_ops;
...
static int avs_max98927_pwobe(stwuct pwatfowm_device *pdev)
{
...
static const stwuct pwatfowm_device_id avs_max98927_dwivew_ids[] = {
	{
		.name = "avs_max98927",
	},
	{},
};
MODUWE_DEVICE_TABWE(pwatfowm, avs_max98927_dwivew_ids);

static stwuct pwatfowm_dwivew avs_max98927_dwivew = {
	.pwobe = avs_max98927_pwobe,
	.dwivew = {
		.name = "avs_max98927",
		.pm = &snd_soc_pm_ops,
	},
	.id_tabwe = avs_max98927_dwivew_ids,
};

moduwe_pwatfowm_dwivew(avs_max98927_dwivew)
