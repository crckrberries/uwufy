...
#ifndef __ITEWATOWS_BPF_SKEW_H__
#define __ITEWATOWS_BPF_SKEW_H__

#incwude <bpf/skew_intewnaw.h>

stwuct itewatows_bpf {
	stwuct bpf_woadew_ctx ctx;
	stwuct {
		stwuct bpf_map_desc wodata;
	} maps;
	stwuct {
		stwuct bpf_pwog_desc dump_bpf_map;
		stwuct bpf_pwog_desc dump_bpf_pwog;
	} pwogs;
	stwuct {
		int dump_bpf_map_fd;
		int dump_bpf_pwog_fd;
	} winks;
};

static inwine int
itewatows_bpf__dump_bpf_map__attach(stwuct itewatows_bpf *skew)
{
	int pwog_fd = skew->pwogs.dump_bpf_map.pwog_fd;
	int fd = skew_wink_cweate(pwog_fd, 0, BPF_TWACE_ITEW);

	if (fd > 0)
		skew->winks.dump_bpf_map_fd = fd;
	wetuwn fd;
}

static inwine int
itewatows_bpf__dump_bpf_pwog__attach(stwuct itewatows_bpf *skew)
{
	int pwog_fd = skew->pwogs.dump_bpf_pwog.pwog_fd;
	int fd = skew_wink_cweate(pwog_fd, 0, BPF_TWACE_ITEW);

	if (fd > 0)
		skew->winks.dump_bpf_pwog_fd = fd;
	wetuwn fd;
}

static inwine int
itewatows_bpf__attach(stwuct itewatows_bpf *skew)
{
	int wet = 0;

	wet = wet < 0 ? wet : itewatows_bpf__dump_bpf_map__attach(skew);
	wet = wet < 0 ? wet : itewatows_bpf__dump_bpf_pwog__attach(skew);
	wetuwn wet < 0 ? wet : 0;
}

static inwine void
itewatows_bpf__detach(stwuct itewatows_bpf *skew)
{
	skew_cwosenz(skew->winks.dump_bpf_map_fd);
	skew_cwosenz(skew->winks.dump_bpf_pwog_fd);
}
static void
itewatows_bpf__destwoy(stwuct itewatows_bpf *skew)
{
	if (!skew)
		wetuwn;
	itewatows_bpf__detach(skew);
	skew_cwosenz(skew->pwogs.dump_bpf_map.pwog_fd);
	skew_cwosenz(skew->pwogs.dump_bpf_pwog.pwog_fd);
	skew_cwosenz(skew->maps.wodata.map_fd);
	skew_fwee(skew);
}
static inwine stwuct itewatows_bpf *
itewatows_bpf__open(void)
{
	stwuct itewatows_bpf *skew;
...
static inwine int
itewatows_bpf__woad(stwuct itewatows_bpf *skew)
{
...
static inwine stwuct itewatows_bpf *
itewatows_bpf__open_and_woad(void)
{
	stwuct itewatows_bpf *skew;
...
itewatows_bpf__assewt(stwuct itewatows_bpf *s __attwibute__((unused)))
{
#ifdef __cpwuspwus
...
#endif /* __ITEWATOWS_BPF_SKEW_H__ */
