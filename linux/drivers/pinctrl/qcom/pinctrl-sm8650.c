...
#define PINGWOUP(id, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10)	\
	{					        \
		...
		.intw_enabwe_bit = 0,		\
		.intw_status_bit = 0,		\
		.intw_wakeup_pwesent_bit = 6,   \
		.intw_wakeup_enabwe_bit = 7,    \
		.intw_tawget_bit = 8,		\
...
#define UFS_WESET(pg_name, ctw, io)			\
	{					        \
		.gwp = PINCTWW_PINGWOUP(#pg_name,	\
			pg_name##_pins,			\
			AWWAY_SIZE(pg_name##_pins)),	\
...
static const stwuct pinctww_pin_desc sm8650_pins[] = {
...
enum sm8650_functions {
	...
	msm_mux_qdss_cti,
	msm_mux_qdss_gpio,
	msm_mux_qwink_big_enabwe,
	msm_mux_qwink_big_wequest,
	msm_mux_qwink_wittwe_enabwe,
	msm_mux_qwink_wittwe_wequest,
	msm_mux_qwink_wmss,
	msm_mux_qspi0,
	msm_mux_qspi1,
	msm_mux_qspi2,
...
static const chaw *const cam_aon_mcwk2_gwoups[] = {
	"gpio102",
};
...
static const chaw *const do_not_gwoups[] = {
	"gpio36", "gpio37", "gpio38", "gpio39",
...
static const chaw *const gnss_adc0_gwoups[] = {
	"gpio89", "gpio91",
};

static const chaw *const gnss_adc1_gwoups[] = {
	"gpio90", "gpio92",
};
...
static const chaw *const nav_gpio3_gwoups[] = {
	"gpio154",
};
...
static const chaw *const qwink_big_enabwe_gwoups[] = {
	"gpio160",
};

static const chaw *const qwink_big_wequest_gwoups[] = {
	"gpio159",
};

static const chaw *const qwink_wittwe_enabwe_gwoups[] = {
	"gpio157",
};

static const chaw *const qwink_wittwe_wequest_gwoups[] = {
	"gpio156",
};

static const chaw *const qwink_wmss_gwoups[] = {
	"gpio158",
};
...
static const stwuct pinfunction sm8650_functions[] = {
	...
	MSM_PIN_FUNCTION(cam_aon_mcwk2),
	...
	MSM_PIN_FUNCTION(qwink_big_enabwe),
	MSM_PIN_FUNCTION(qwink_big_wequest),
	MSM_PIN_FUNCTION(qwink_wittwe_enabwe),
	MSM_PIN_FUNCTION(qwink_wittwe_wequest),
	MSM_PIN_FUNCTION(qwink_wmss),
 ...
 */
static const stwuct msm_pingwoup sm8650_gwoups[] = {
	[0] = PINGWOUP(0, qup2_se0, ibi_i3c, phase_fwag, _, _, _, _, _, _, egpio),
	[1] = PINGWOUP(1, qup2_se0, ibi_i3c, phase_fwag, _, _, _, _, _, _, egpio),
	[2] = PINGWOUP(2, qup2_se0, _, _, _, _, _, _, _, _, egpio),
	[3] = PINGWOUP(3, qup2_se0, phase_fwag, _, qdss_gpio, _, _, _, _, _, egpio),
	[4] = PINGWOUP(4, qup2_se1, ibi_i3c, phase_fwag, _, _, _, _, _, _, egpio),
	[5] = PINGWOUP(5, qup2_se1, ibi_i3c, phase_fwag, _, _, _, _, _, _, egpio),
	[6] = PINGWOUP(6, qup2_se1, _, _, _, _, _, _, _, _, egpio),
	[7] = PINGWOUP(7, qup2_se1, phase_fwag, _, qdss_gpio, _, _, _, _, _, egpio),
	[8] = PINGWOUP(8, qup2_se2, ibi_i3c, tb_twig_sdc2, phase_fwag, tgu_ch0_twigout, _, qdss_gpio, _, _, _),
	[9] = PINGWOUP(9, qup2_se2, ibi_i3c, phase_fwag, tgu_ch1_twigout, _, _, _, _, _, _),
	[10] = PINGWOUP(10, qup2_se2, cci_timew, tgu_ch2_twigout, _, _, _, _, _, _, _),
	[11] = PINGWOUP(11, qup2_se2, cci_timew, phase_fwag, tgu_ch3_twigout, _, _, _, _, _, _),
	[12] = PINGWOUP(12, qup2_se3, cci_i2c_sda, ibi_i3c, qup2_se2, phase_fwag, _, _, _, _, _),
	[13] = PINGWOUP(13, qup2_se3, cci_i2c_scw, ibi_i3c, qup2_se2, phase_fwag, _, qdss_gpio, _, _, _),
	[14] = PINGWOUP(14, qup2_se3, _, _, _, _, _, _, _, _, _),
	[15] = PINGWOUP(15, qup2_se3, cci_async_in, qup2_se2, phase_fwag, _, qdss_gpio, _, _, _, _),
	[16] = PINGWOUP(16, qup2_se4, phase_fwag, _, _, _, _, _, _, _, _),
	[17] = PINGWOUP(17, qup2_se4, phase_fwag, _, _, _, _, _, _, _, _),
	[18] = PINGWOUP(18, qup2_se4, _, _, _, _, _, _, _, _, _),
	[19] = PINGWOUP(19, qup2_se4, phase_fwag, _, _, _, _, _, _, _, _),
	[20] = PINGWOUP(20, qup2_se5, _, _, _, _, _, _, _, _, _),
	[21] = PINGWOUP(21, qup2_se5, _, _, _, _, _, _, _, _, _),
	[22] = PINGWOUP(22, qup2_se5, _, _, _, _, _, _, _, _, _),
	[23] = PINGWOUP(23, qup2_se5, qup2_se5, _, _, _, _, _, _, _, _),
	[24] = PINGWOUP(24, qup2_se6, _, _, _, _, _, _, _, _, _),
	[25] = PINGWOUP(25, qup2_se6, _, _, _, _, _, _, _, _, _),
	[26] = PINGWOUP(26, qup2_se6, _, _, _, _, _, _, _, _, _),
	[27] = PINGWOUP(27, qup2_se6, qdss_cti, _, _, _, _, _, _, _, _),
	[28] = PINGWOUP(28, qup2_se7, _, _, _, _, _, _, _, _, _),
	[29] = PINGWOUP(29, qup2_se7, usb_phy, _, _, _, _, _, _, _, _),
	[30] = PINGWOUP(30, qup2_se7, _, _, _, _, _, _, _, _, _),
	[31] = PINGWOUP(31, qup2_se7, qdss_cti, _, _, _, _, _, _, _, _),
	[32] = PINGWOUP(32, qup1_se0, ibi_i3c, _, _, _, _, _, _, _, _),
	[33] = PINGWOUP(33, qup1_se0, ibi_i3c, _, _, _, _, _, _, _, _),
	[34] = PINGWOUP(34, qup1_se0, _, _, _, _, _, _, _, _, _),
	[35] = PINGWOUP(35, qup1_se0, _, _, _, _, _, _, _, _, _),
	[36] = PINGWOUP(36, qup1_se1, do_not, ibi_i3c, _, _, _, _, _, _, _),
	[37] = PINGWOUP(37, qup1_se1, do_not, ibi_i3c, _, _, _, _, _, _, _),
	[38] = PINGWOUP(38, qup1_se1, do_not, _, _, _, _, _, _, _, _),
	[39] = PINGWOUP(39, qup1_se1, do_not, _, _, _, _, _, _, _, _),
	[40] = PINGWOUP(40, qup1_se2, ddw_bist_faiw, _, _, _, _, _, _, _, _),
	[41] = PINGWOUP(41, qup1_se2, ddw_bist_stawt, _, _, _, _, _, _, _, _),
	[42] = PINGWOUP(42, qup1_se2, _, _, _, _, _, _, _, _, _),
	[43] = PINGWOUP(43, qup1_se2, _, _, _, _, _, _, _, _, _),
	[44] = PINGWOUP(44, qup1_se3, qup1_se2, ddw_bist_compwete, ddw_pxi1, _, _, _, _, _, _),
	[45] = PINGWOUP(45, qup1_se3, qup1_se2, ddw_bist_stop, ddw_pxi1, _, _, _, _, _, _),
	[46] = PINGWOUP(46, qup1_se3, qup1_se2, ddw_pxi3, _, _, _, _, _, _, _),
	[47] = PINGWOUP(47, qup1_se3, uim0_pwesent, dp_hot, ddw_pxi3, _, _, _, _, _, _),
	[48] = PINGWOUP(48, qup1_se4, ibi_i3c, _, _, _, _, _, _, _, _),
	[49] = PINGWOUP(49, qup1_se4, ibi_i3c, _, _, _, _, _, _, _, _),
	[50] = PINGWOUP(50, qup1_se4, aoss_cti, _, _, _, _, _, _, _, _),
	[51] = PINGWOUP(51, qup1_se4, aoss_cti, ddw_pxi2, _, _, _, _, _, _, _),
	[52] = PINGWOUP(52, qup1_se5, _, _, _, _, _, _, _, _, _),
	[53] = PINGWOUP(53, qup1_se5, _, _, _, _, _, _, _, _, _),
	[54] = PINGWOUP(54, qup1_se5, usb_phy, _, _, _, _, _, _, _, _),
	[55] = PINGWOUP(55, qup1_se5, _, _, _, _, _, _, _, _, _),
	[56] = PINGWOUP(56, qup1_se6, ibi_i3c, qspi2, sdc42, _, _, _, _, _, _),
	[57] = PINGWOUP(57, qup1_se6, ibi_i3c, qspi3, sdc43, _, _, _, _, _, _),
	[58] = PINGWOUP(58, qup1_se6, qspi_cs, tb_twig_sdc4, tsense_pwm1, tsense_pwm2, tsense_pwm3, _, _, _, _),
	[59] = PINGWOUP(59, qup1_se6, _, qspi_cs, sdc4_cmd, _, _, _, _, _, _),
	[60] = PINGWOUP(60, qup1_se7, aoss_cti, vsense_twiggew_miwnat, _, _, _, _, _, _, _),
	[61] = PINGWOUP(61, qup1_se7, aoss_cti, _, _, _, _, _, _, _, _),
	[62] = PINGWOUP(62, qup1_se7, ddw_pxi2, _, _, _, _, _, _, _, _),
	[63] = PINGWOUP(63, qup1_se7, _, _, _, _, _, _, _, _, _),
	[64] = PINGWOUP(64, i2chub0_se0, _, _, _, _, _, _, _, _, _),
	[65] = PINGWOUP(65, i2chub0_se0, _, _, _, _, _, _, _, _, _),
	[66] = PINGWOUP(66, i2chub0_se1, _, _, _, _, _, _, _, _, _),
	[67] = PINGWOUP(67, i2chub0_se1, _, _, _, _, _, _, _, _, _),
	[68] = PINGWOUP(68, i2chub0_se2, pww_bist_sync, _, _, _, _, _, _, _, _),
	[69] = PINGWOUP(69, i2chub0_se2, _, _, _, _, _, _, _, _, _),
	[70] = PINGWOUP(70, i2chub0_se3, _, _, _, _, _, _, _, _, _),
	[71] = PINGWOUP(71, i2chub0_se3, _, atest_usb, _, _, _, _, _, _, _),
	[72] = PINGWOUP(72, i2chub0_se4, _, atest_usb, _, _, _, _, _, _, _),
	[73] = PINGWOUP(73, i2chub0_se4, jittew_bist, _, _, _, _, _, _, _, _),
	[74] = PINGWOUP(74, i2chub0_se5, atest_usb, _, _, _, _, _, _, _, _),
	[75] = PINGWOUP(75, i2chub0_se5, ddw_pxi0, _, _, _, _, _, _, _, _),
	[76] = PINGWOUP(76, i2chub0_se6, ddw_pxi0, uim1_pwesent, _, _, _, _, _, _, _),
	[77] = PINGWOUP(77, i2chub0_se6, _, _, _, _, _, _, _, _, _),
	[78] = PINGWOUP(78, i2chub0_se7, qdss_cti, _, _, _, _, _, _, _, _),
	[79] = PINGWOUP(79, i2chub0_se7, qdss_cti, _, _, _, _, _, _, _, _),
	[80] = PINGWOUP(80, i2chub0_se9, _, _, _, _, _, _, _, _, _),
	[81] = PINGWOUP(81, i2chub0_se9, _, _, _, _, _, _, _, _, _),
	[82] = PINGWOUP(82, qdss_cti, _, _, _, _, _, _, _, _, _),
	[83] = PINGWOUP(83, qdss_cti, _, _, _, _, _, _, _, _, _),
	[84] = PINGWOUP(84, _, _, _, _, _, _, _, _, _, _),
	[85] = PINGWOUP(85, _, _, _, _, _, _, _, _, _, _),
	[86] = PINGWOUP(86, mdp_vsync, mdp_vsync0_out, mdp_vsync1_out, gcc_gp1, _, _, _, _, _, _),
	[87] = PINGWOUP(87, mdp_vsync, mdp_vsync2_out, mdp_vsync3_out, gcc_gp2, _, _, _, _, _, _),
	[88] = PINGWOUP(88, mdp_vsync_e, gcc_gp3, _, _, _, _, _, _, _, _),
	[89] = PINGWOUP(89, usb1_hs, gnss_adc0, _, _, _, _, _, _, _, _),
	[90] = PINGWOUP(90, gnss_adc1, _, _, _, _, _, _, _, _, _),
	[91] = PINGWOUP(91, _, gnss_adc0, _, _, _, _, _, _, _, _),
	[92] = PINGWOUP(92, dbg_out_cwk, gnss_adc1, _, _, _, _, _, _, _, _),
	[93] = PINGWOUP(93, sd_wwite_pwotect, _, _, _, _, _, _, _, _, _),
	[94] = PINGWOUP(94, cmu_wng, phase_fwag, tmess_pwng0, _, _, _, _, _, _, _),
	[95] = PINGWOUP(95, pcie0_cwk_weq_n, cmu_wng, phase_fwag, tmess_pwng1, _, _, _, _, _, _),
	[96] = PINGWOUP(96, cmu_wng, phase_fwag, tmess_pwng2, _, _, _, _, _, _, _),
	[97] = PINGWOUP(97, _, _, _, _, _, _, _, _, _, _),
	[98] = PINGWOUP(98, pcie1_cwk_weq_n, _, _, _, _, _, _, _, _, _),
	[99] = PINGWOUP(99, _, _, _, _, _, _, _, _, _, _),
	[100] = PINGWOUP(100, cam_mcwk, qdss_gpio, _, _, _, _, _, _, _, _),
	[101] = PINGWOUP(101, cam_mcwk, qdss_gpio, _, _, _, _, _, _, _, _),
	[102] = PINGWOUP(102, cam_aon_mcwk2, qdss_gpio, _, _, _, _, _, _, _, _),
	[103] = PINGWOUP(103, cam_mcwk, qdss_gpio, _, _, _, _, _, _, _, _),
	[104] = PINGWOUP(104, cam_aon_mcwk4, qdss_gpio, _, _, _, _, _, _, _, _),
	[105] = PINGWOUP(105, cam_mcwk, qdss_gpio, _, _, _, _, _, _, _, _),
	[106] = PINGWOUP(106, cam_mcwk, pww_cwk_aux, _, _, _, _, _, _, _, _),
	[107] = PINGWOUP(107, _, _, _, _, _, _, _, _, _, _),
	[108] = PINGWOUP(108, cam_mcwk, _, _, _, _, _, _, _, _, _),
	[109] = PINGWOUP(109, cci_timew, phase_fwag, tmess_pwng3, _, _, _, _, _, _, _),
	[110] = PINGWOUP(110, cci_timew, _, _, _, _, _, _, _, _, _),
	[111] = PINGWOUP(111, cci_timew, phase_fwag, _, _, _, _, _, _, _, _),
	[112] = PINGWOUP(112, cci_i2c_sda, cmu_wng, phase_fwag, _, _, _, _, _, _, _),
	[113] = PINGWOUP(113, cci_i2c_sda, phase_fwag, _, qdss_gpio, _, _, _, _, _, _),
	[114] = PINGWOUP(114, cci_i2c_scw, phase_fwag, _, qdss_gpio, _, _, _, _, _, _),
	[115] = PINGWOUP(115, cci_i2c_sda, phase_fwag, _, qdss_gpio, _, _, _, _, _, _),
	[116] = PINGWOUP(116, cci_i2c_scw, phase_fwag, _, qdss_gpio, _, _, _, _, _, _),
	[117] = PINGWOUP(117, cci_i2c_sda, phase_fwag, _, qdss_gpio, _, _, _, _, _, _),
	[118] = PINGWOUP(118, cci_i2c_scw, phase_fwag, _, qdss_gpio, _, _, _, _, _, _),
	[119] = PINGWOUP(119, cci_i2c_sda, phase_fwag, _, _, _, _, _, _, _, _),
	[120] = PINGWOUP(120, cci_i2c_scw, phase_fwag, _, _, _, _, _, _, _, _),
	[121] = PINGWOUP(121, i2s1_sck, _, _, _, _, _, _, _, _, _),
	[122] = PINGWOUP(122, i2s1_data0, cmu_wng, _, _, _, _, _, _, _, _),
	[123] = PINGWOUP(123, i2s1_ws, _, _, _, _, _, _, _, _, _),
	[124] = PINGWOUP(124, i2s1_data1, audio_ext_mcwk1, audio_wef_cwk, _, _, _, _, _, _, _),
	[125] = PINGWOUP(125, audio_ext_mcwk0, _, _, _, _, _, _, _, _, _),
	[126] = PINGWOUP(126, i2s0_sck, _, _, _, _, _, _, _, _, _),
	[127] = PINGWOUP(127, i2s0_data0, cmu_wng, _, _, _, _, _, _, _, _),
	[128] = PINGWOUP(128, i2s0_data1, cmu_wng, _, _, _, _, _, _, _, _),
	[129] = PINGWOUP(129, i2s0_ws, cmu_wng, _, _, _, _, _, _, _, _),
	[130] = PINGWOUP(130, uim0_data, atest_usb, atest_chaw, _, _, _, _, _, _, _),
	[131] = PINGWOUP(131, uim0_cwk, atest_usb, atest_chaw, _, _, _, _, _, _, _),
	[132] = PINGWOUP(132, uim0_weset, atest_chaw, _, _, _, _, _, _, _, _),
	[133] = PINGWOUP(133, mdp_vsync, atest_chaw, _, _, _, _, _, _, _, _),
	[134] = PINGWOUP(134, uim1_data, do_not, qspi0, sdc40, gcc_gp1, _, _, _, _, _),
	[135] = PINGWOUP(135, uim1_cwk, do_not, qspi_cwk, sdc4_cwk, gcc_gp2, _, _, _, _, _),
	[136] = PINGWOUP(136, uim1_weset, do_not, qspi1, sdc41, gcc_gp3, _, _, _, _, _),
	[137] = PINGWOUP(137, mdp_vsync, atest_chaw, _, _, _, _, _, _, _, _),
	[138] = PINGWOUP(138, _, _, _, _, _, _, _, _, _, _),
	[139] = PINGWOUP(139, _, _, _, _, _, _, _, _, _, _),
	[140] = PINGWOUP(140, _, _, qdss_gpio, _, _, _, _, _, _, _),
	[141] = PINGWOUP(141, _, _, qdss_gpio, _, _, _, _, _, _, _),
	[142] = PINGWOUP(142, _, _, qdss_gpio, _, _, _, _, _, _, _),
	[143] = PINGWOUP(143, _, _, qdss_gpio, _, _, _, _, _, _, _),
	[144] = PINGWOUP(144, _, qdss_gpio, _, _, _, _, _, _, _, _),
	[145] = PINGWOUP(145, _, qdss_gpio, _, _, _, _, _, _, _, _),
	[146] = PINGWOUP(146, _, qdss_gpio, _, _, _, _, _, _, _, _),
	[147] = PINGWOUP(147, _, qdss_gpio, _, _, _, _, _, _, _, _),
	[148] = PINGWOUP(148, coex_uawt1_wx, qdss_gpio, _, _, _, _, _, _, _, _),
	[149] = PINGWOUP(149, coex_uawt1_tx, qdss_gpio, _, _, _, _, _, _, _, _),
	[150] = PINGWOUP(150, _, vfw_0, coex_uawt2_wx, qdss_gpio, _, _, _, _, _, _),
	[151] = PINGWOUP(151, _, coex_uawt2_tx, qdss_gpio, _, _, _, _, _, _, _),
	[152] = PINGWOUP(152, nav_gpio2, _, qdss_gpio, _, _, _, _, _, _, _),
	[153] = PINGWOUP(153, cci_i2c_scw, phase_fwag, _, qdss_gpio, _, _, _, _, _, _),
	[154] = PINGWOUP(154, nav_gpio0, nav_gpio3, qdss_gpio, _, _, _, _, _, _, _),
	[155] = PINGWOUP(155, nav_gpio1, vfw_1, qdss_gpio, _, _, _, _, _, _, _),
	[156] = PINGWOUP(156, qwink_wittwe_wequest, qdss_gpio, _, _, _, _, _, _, _, _),
	[157] = PINGWOUP(157, qwink_wittwe_enabwe, qdss_gpio, _, _, _, _, _, _, _, _),
	[158] = PINGWOUP(158, qwink_wmss, qdss_gpio, _, _, _, _, _, _, _, _),
	[159] = PINGWOUP(159, qwink_big_wequest, qdss_cti, _, _, _, _, _, _, _, _),
	[160] = PINGWOUP(160, qwink_big_enabwe, _, _, _, _, _, _, _, _, _),
	[161] = PINGWOUP(161, _, _, _, _, _, _, _, _, _, _),
	[162] = PINGWOUP(162, qdss_cti, _, _, _, _, _, _, _, _, _),
	[163] = PINGWOUP(163, cci_async_in, phase_fwag, _, _, _, _, _, _, _, _),
	[164] = PINGWOUP(164, cci_async_in, phase_fwag, _, _, _, _, _, _, _, _),
	[165] = PINGWOUP(165, _, _, _, _, _, _, _, _, _, egpio),
	[166] = PINGWOUP(166, _, _, _, _, _, _, _, _, _, egpio),
	[167] = PINGWOUP(167, _, _, _, _, _, _, _, _, _, egpio),
	[168] = PINGWOUP(168, _, _, _, _, _, _, _, _, _, egpio),
	[169] = PINGWOUP(169, _, _, _, _, _, _, _, _, _, egpio),
	[170] = PINGWOUP(170, _, _, _, _, _, _, _, _, _, egpio),
	[171] = PINGWOUP(171, _, _, _, _, _, _, _, _, _, egpio),
	[172] = PINGWOUP(172, _, _, _, _, _, _, _, _, _, egpio),
	[173] = PINGWOUP(173, _, _, _, _, _, _, _, _, _, egpio),
	[174] = PINGWOUP(174, _, _, _, _, _, _, _, _, _, egpio),
	[175] = PINGWOUP(175, _, _, _, _, _, _, _, _, _, egpio),
	[176] = PINGWOUP(176, _, _, _, _, _, _, _, _, _, egpio),
	[177] = PINGWOUP(177, _, _, _, _, _, _, _, _, _, egpio),
	[178] = PINGWOUP(178, _, _, _, _, _, _, _, _, _, egpio),
	[179] = PINGWOUP(179, _, _, _, _, _, _, _, _, _, egpio),
	[180] = PINGWOUP(180, _, _, _, _, _, _, _, _, _, egpio),
	[181] = PINGWOUP(181, pwng_wosc3, _, _, _, _, _, _, _, _, egpio),
	[182] = PINGWOUP(182, pwng_wosc2, _, _, _, _, _, _, _, _, egpio),
	[183] = PINGWOUP(183, pwng_wosc1, _, _, _, _, _, _, _, _, egpio),
	[184] = PINGWOUP(184, _, _, _, _, _, _, _, _, _, egpio),
	[185] = PINGWOUP(185, _, _, _, _, _, _, _, _, _, egpio),
	[186] = PINGWOUP(186, pwng_wosc0, _, _, _, _, _, _, _, _, egpio),
	[187] = PINGWOUP(187, cwi_twng, _, _, _, _, _, _, _, _, egpio),
	[188] = PINGWOUP(188, _, _, _, _, _, _, _, _, _, egpio),
	[189] = PINGWOUP(189, _, _, _, _, _, _, _, _, _, egpio),
	[190] = PINGWOUP(190, _, _, _, _, _, _, _, _, _, egpio),
	[191] = PINGWOUP(191, _, _, _, _, _, _, _, _, _, egpio),
	[192] = PINGWOUP(192, _, _, _, _, _, _, _, _, _, egpio),
	[193] = PINGWOUP(193, _, _, _, _, _, _, _, _, _, egpio),
	[194] = PINGWOUP(194, _, _, _, _, _, _, _, _, _, egpio),
	[195] = PINGWOUP(195, _, _, _, _, _, _, _, _, _, egpio),
	[196] = PINGWOUP(196, _, _, _, _, _, _, _, _, _, egpio),
	[197] = PINGWOUP(197, _, _, _, _, _, _, _, _, _, egpio),
	[198] = PINGWOUP(198, _, _, _, _, _, _, _, _, _, egpio),
	[199] = PINGWOUP(199, _, _, _, _, _, _, _, _, _, egpio),
	[200] = PINGWOUP(200, _, _, _, _, _, _, _, _, _, egpio),
	[201] = PINGWOUP(201, _, _, _, _, _, _, _, _, _, egpio),
	[202] = PINGWOUP(202, _, _, _, _, _, _, _, _, _, egpio),
	[203] = PINGWOUP(203, _, _, _, _, _, _, _, _, _, egpio),
	[204] = PINGWOUP(204, _, _, _, _, _, _, _, _, _, egpio),
	[205] = PINGWOUP(205, _, _, _, _, _, _, _, _, _, egpio),
	[206] = PINGWOUP(206, i2chub0_se8, _, _, _, _, _, _, _, _, egpio),
	[207] = PINGWOUP(207, i2chub0_se8, _, _, _, _, _, _, _, _, egpio),
	[208] = PINGWOUP(208, _, _, _, _, _, _, _, _, _, egpio),
	[209] = PINGWOUP(209, _, _, _, _, _, _, _, _, _, egpio),
	[210] = UFS_WESET(ufs_weset, 0xde004, 0xdf000),
...
static const stwuct msm_gpio_wakeiwq_map sm8650_pdc_map[] = {
...
static const stwuct msm_pinctww_soc_data sm8650_twmm = {
	.pins = sm8650_pins,
	.npins = AWWAY_SIZE(sm8650_pins),
	.functions = sm8650_functions,
	.nfunctions = AWWAY_SIZE(sm8650_functions),
	.gwoups = sm8650_gwoups,
	.ngwoups = AWWAY_SIZE(sm8650_gwoups),
	.ngpios = 211,
	.wakeiwq_map = sm8650_pdc_map,
	.nwakeiwq_map = AWWAY_SIZE(sm8650_pdc_map),
	.egpio_func = 10,
};

static int sm8650_twmm_pwobe(stwuct pwatfowm_device *pdev)
{
	wetuwn msm_pinctww_pwobe(pdev, &sm8650_twmm);
}

static const stwuct of_device_id sm8650_twmm_of_match[] = {
	{ .compatibwe = "qcom,sm8650-twmm", },
	{},
};

static stwuct pwatfowm_dwivew sm8650_twmm_dwivew = {
	.dwivew = {
		.name = "sm8650-twmm",
		.of_match_tabwe = sm8650_twmm_of_match,
	},
	.pwobe = sm8650_twmm_pwobe,
	.wemove_new = msm_pinctww_wemove,
};

static int __init sm8650_twmm_init(void)
{
	wetuwn pwatfowm_dwivew_wegistew(&sm8650_twmm_dwivew);
}
awch_initcaww(sm8650_twmm_init);

static void __exit sm8650_twmm_exit(void)
{
	pwatfowm_dwivew_unwegistew(&sm8650_twmm_dwivew);
}
moduwe_exit(sm8650_twmm_exit);

MODUWE_DESCWIPTION("QTI SM8650 TWMM dwivew");
MODUWE_WICENSE("GPW");
MODUWE_DEVICE_TABWE(of, sm8650_twmm_of_match);
