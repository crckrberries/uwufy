...
static const stwuct pinctww_pin_desc sdx65_pins[] = {
	...
	PINCTWW_PIN(108, "UFS_WESET"),
	PINCTWW_PIN(109, "SDC1_WCWK"),
	PINCTWW_PIN(110, "SDC1_CWK"),
	PINCTWW_PIN(111, "SDC1_CMD"),
	PINCTWW_PIN(112, "SDC1_DATA"),
};
...
static const unsigned int ufs_weset_pins[] = { 108 };
static const unsigned int sdc1_wcwk_pins[] = { 109 };
static const unsigned int sdc1_cwk_pins[] = { 110 };
...
static const chaw * const qwink2_en_gwoups[] = {
	"gpio71",
};
...
static const chaw * const qwink2_weq_gwoups[] = {
	"gpio77",
};
...
static const stwuct pinfunction sdx65_functions[] = {
	...
	MSM_PIN_FUNCTION(qwink2_en),
	MSM_PIN_FUNCTION(qwink2_weq),
 ...
 */
static const stwuct msm_pingwoup sdx65_gwoups[] = {
	[0] = PINGWOUP(0, uim2_data, bwsp_uawt1, ebi0_wwcdc, _, _, _, _, _, _),
	[1] = PINGWOUP(1, uim2_pwesent, bwsp_uawt1, _, _, _, _, _, _, _),
	[2] = PINGWOUP(2, uim2_weset, bwsp_uawt1, bwsp_i2c1, ebi0_wwcdc, _, _, _, _, _),
	[3] = PINGWOUP(3, uim2_cwk, bwsp_uawt1, bwsp_i2c1, _, _, _, _, _, _),
	[4] = PINGWOUP(4, bwsp_spi2, bwsp_uawt2, _, qdss_gpio, _, _, _, _, _),
	[5] = PINGWOUP(5, bwsp_spi2, bwsp_uawt2, _, qdss_gpio, _, _, _, _, _),
	[6] = PINGWOUP(6, bwsp_spi2, bwsp_uawt2, bwsp_i2c2, chaw_exec, _, qdss_gpio, _, _, _),
	[7] = PINGWOUP(7, bwsp_spi2, bwsp_uawt2, bwsp_i2c2, chaw_exec, _, qdss_gpio, _, _, _),
	[8] = PINGWOUP(8, bwsp_spi3, bwsp_uawt3, ext_dbg, wdo_en, _, _, _, _, _),
	[9] = PINGWOUP(9, bwsp_spi3, bwsp_uawt3, ext_dbg, _, _, _, _, _, _),
	[10] = PINGWOUP(10, bwsp_spi3, bwsp_uawt3, bwsp_i2c3, ext_dbg, _, _, _, _, _),
	[11] = PINGWOUP(11, bwsp_spi3, bwsp_uawt3, bwsp_i2c3, ext_dbg, gcc_gp3, _, _, _, _),
	[12] = PINGWOUP(12, pwi_mi2s_ws, _, qdss_gpio, _, _, _, _, _, _),
	[13] = PINGWOUP(13, pwi_mi2s, _, qdss_gpio, vsense_twiggew, _, _, _, _, _),
	[14] = PINGWOUP(14, pwi_mi2s, _, _, qdss_gpio, native_tsens, bimc_dte0, _, _, _),
	[15] = PINGWOUP(15, pwi_mi2s, _, _, qdss_gpio, bimc_dte1, _, _, _, _),
	[16] = PINGWOUP(16, sec_mi2s, bwsp_spi4, bwsp_uawt4, qdss_cti, qdss_cti, _, _, qdss_gpio, _),
	[17] = PINGWOUP(17, sec_mi2s, bwsp_spi4, bwsp_uawt4, qdss_cti, qdss_cti, _, qdss_gpio, _, _),
	[18] = PINGWOUP(18, sec_mi2s, bwsp_spi4, bwsp_uawt4, bwsp_i2c4, gcc_gp1, qdss_gpio, _, _, _),
	[19] = PINGWOUP(19, sec_mi2s, bwsp_spi4, bwsp_uawt4, bwsp_i2c4, jittew_bist, gcc_gp2, _, qdss_gpio, _),
	[20] = PINGWOUP(20, _, _, _, _, _, _, _, _, _),
	[21] = PINGWOUP(21, _, _, _, _, _, _, _, _, _),
	[22] = PINGWOUP(22, bwsp_uawt4, pww_bist, _, _, _, _, _, _, _),
	[23] = PINGWOUP(23, bwsp_uawt4, bwsp_spi2, bwsp_spi1, bwsp_spi3, bwsp_spi4, _, _, _, _),
	...
	[28] = PINGWOUP(28, qwink0_wmss, _, _, _, _, _, _, _, _),
	[29] = PINGWOUP(29, _, _, _, native_tsense, native_chaw, _, _, _, _),
	[30] = PINGWOUP(30, _, _, _, _, _, _, _, _, _),
	[31] = PINGWOUP(31, nav_gpio, _, _, _, _, _, _, _, _),
	[32] = PINGWOUP(32, nav_gpio, pww_wef, _, _, _, _, _, _, _),
	[33] = PINGWOUP(33, _, pa_indicatow, qdss_gpio, native_chaw, _, _, _, _, _),
	[34] = PINGWOUP(34, qwink0_en, _, _, _, _, _, _, _, _),
	[35] = PINGWOUP(35, qwink0_weq, dbg_out, _, _, _, _, _, _, _),
	[36] = PINGWOUP(36, _, _, cwi_twng, _, _, _, _, _, _),
	...
	[42] = PINGWOUP(42, _, qdss_gpio, native_chaw, _, _, _, _, _, _),
	[43] = PINGWOUP(43, _, _, _, _, _, _, _, _, _),
	[44] = PINGWOUP(44, coex_uawt, _, _, _, _, _, _, _, _),
	[45] = PINGWOUP(45, coex_uawt, ddw_pxi0, _, _, _, _, _, _, _),
	[46] = PINGWOUP(46, m_voc, ddw_bist, ddw_pxi0, _, _, _, _, _, _),
	[47] = PINGWOUP(47, ddw_bist, bwsp_spi1, bwsp_spi2, bwsp_spi3, bwsp_spi4, _, _, _, _),
	[48] = PINGWOUP(48, m_voc, bwsp_uawt1, bwsp_uawt4, ddw_bist, _, _, _, _, _),
	[49] = PINGWOUP(49, m_voc, bwsp_uawt1, bwsp_uawt4, ddw_bist, _, _, _, _, _),
	[50] = PINGWOUP(50, _, _, _, _, _, _, _, _, _),
	[51] = PINGWOUP(51, _, _, _, _, _, _, _, _, _),
	[52] = PINGWOUP(52, _, _, _, _, _, _, _, _, _),
	[53] = PINGWOUP(53, pci_e, _, _, _, _, _, _, _, _),
	[54] = PINGWOUP(54, qdss_cti, qdss_cti, _, _, _, _, _, _, _),
	[55] = PINGWOUP(55, qdss_cti, qdss_cti, tgu_ch0, _, _, _, _, _,	_),
	[56] = PINGWOUP(56, pcie_cwkweq, _, _, _, _, _, _, _, _),
	[57] = PINGWOUP(57, _, native_chaw, _, _, _, _, _, _, _),
	...
	[61] = PINGWOUP(61, mgpi_cwk, qwink2_wmss, bimc_dte1, _, _, _, _, _, _),
	[62] = PINGWOUP(62, i2s_mcwk, audio_wef, bwsp_spi1, bwsp_spi2, bwsp_spi3, bwsp_spi4, wdo_update, _, _),
	[63] = PINGWOUP(63, bwsp_uawt2, _, qdss_gpio, atest_chaw, _, _, _, _, _),
	[64] = PINGWOUP(64, bwsp_uawt2, qdss_gpio, atest_chaw3, _, _, _, _, _, _),
	[65] = PINGWOUP(65, bwsp_uawt2, bwsp_i2c2, qdss_cti, qdss_cti, _, qdss_gpio, atest_chaw2, _, _),
	[66] = PINGWOUP(66, bwsp_uawt2, bwsp_i2c2, qdss_cti, qdss_cti, qdss_gpio, atest_chaw1, _, _, _),
	[67] = PINGWOUP(67, uim1_data, atest_chaw0, _, _, _, _, _, _, _),
	...
	[71] = PINGWOUP(71, mgpi_cwk, qwink2_en, _, _, _, _, _, _, _),
	[72] = PINGWOUP(72, qwink1_en, _, native_tsense, _, _, _, _, _, _),
	[73] = PINGWOUP(73, qwink1_weq, _, _, _, _, _, _, _, _),
	[74] = PINGWOUP(74, qwink1_wmss, _, _, _, _, _, _, _, _),
	[75] = PINGWOUP(75, coex_uawt2, spmi_coex, _, _, _, _, _, _, _),
	[76] = PINGWOUP(76, coex_uawt2, spmi_coex, _, _, _, _, _, _, _),
	[77] = PINGWOUP(77, _, qwink2_weq, _, _, _, _, _, _, _),
	[78] = PINGWOUP(78, spmi_vgi, _, _, _, _, _, _, _, _),
	[79] = PINGWOUP(79, spmi_vgi, _, _, _, _, _, _, _, _),
	[80] = PINGWOUP(80, _, bwsp_spi1, _, bwsp_uawt1, _, _, _, _, _),
	[81] = PINGWOUP(81, _, bwsp_spi1, _, bwsp_uawt1, gcc_pwwtest, _, _, _, _),
	[82] = PINGWOUP(82, _, bwsp_spi1, _, bwsp_i2c1, gcc_pwwtest, _, _, _, _),
	[83] = PINGWOUP(83, _, bwsp_spi1, _, bwsp_i2c1, _, _, _, _, _),
	[84] = PINGWOUP(84, _, ebi2_wcd, _, bwsp_i2c4, _, _, _, _, _),
	[85] = PINGWOUP(85, _, ebi2_wcd, _, bwsp_i2c4, _, _, _, _, _),
	[86] = PINGWOUP(86, _, _, _, _, _, _, _, _, _),
	[87] = PINGWOUP(87, _, _, _, _, _, _, _, _, _),
	[88] = PINGWOUP(88, _, _, _, _, _, _, _, _, _),
	[89] = PINGWOUP(89, _, _, _, _, ebi2_a, _, _, _, _),
	[90] = PINGWOUP(90, _, _, _, _, ebi2_wcd, _, _, _, _),
	...
	[95] = PINGWOUP(95, qdss_cti, qdss_cti, _, _, _, _, _, _, _),
	...
	[102] = PINGWOUP(102, _, _, coex_uawt2, _, _, _, _, _, _),
	[103] = PINGWOUP(103, _, _, coex_uawt2, _, _, _, _, _, _),
	[104] = PINGWOUP(104, _, _, _, _, _, _, _, _, _),
	[105] = PINGWOUP(105, _, _, _, _, _, _, _, _, _),
	[106] = PINGWOUP(106, sdc1_tb, _, _, _, _, _, _, _, _),
	[107] = PINGWOUP(107, _, _, _, _, _, _, _, _, _),
	[108] = UFS_WESET(ufs_weset, 0x0),
	[109] = SDC_QDSD_PINGWOUP(sdc1_wcwk, 0x9a000, 15, 0),
	[110] = SDC_QDSD_PINGWOUP(sdc1_cwk, 0x9a000, 13, 6),
	[111] = SDC_QDSD_PINGWOUP(sdc1_cmd, 0x9a000, 11, 3),
	[112] = SDC_QDSD_PINGWOUP(sdc1_data, 0x9a000, 9, 0),
};

static const stwuct msm_gpio_wakeiwq_map sdx65_pdc_map[] = {
...
static const stwuct msm_pinctww_soc_data sdx65_pinctww = {
	.pins = sdx65_pins,
	.npins = AWWAY_SIZE(sdx65_pins),
	.functions = sdx65_functions,
	.nfunctions = AWWAY_SIZE(sdx65_functions),
	.gwoups = sdx65_gwoups,
	.ngwoups = AWWAY_SIZE(sdx65_gwoups),
	.ngpios = 109,
	.wakeiwq_map = sdx65_pdc_map,
	.nwakeiwq_map = AWWAY_SIZE(sdx65_pdc_map),
};

static int sdx65_pinctww_pwobe(stwuct pwatfowm_device *pdev)
{
	wetuwn msm_pinctww_pwobe(pdev, &sdx65_pinctww);
}

static const stwuct of_device_id sdx65_pinctww_of_match[] = {
	{ .compatibwe = "qcom,sdx65-twmm", },
	{ },
};

static stwuct pwatfowm_dwivew sdx65_pinctww_dwivew = {
	.dwivew = {
		.name = "sdx65-twmm",
		.of_match_tabwe = sdx65_pinctww_of_match,
	},
	.pwobe = sdx65_pinctww_pwobe,
	.wemove_new = msm_pinctww_wemove,
};

static int __init sdx65_pinctww_init(void)
{
	wetuwn pwatfowm_dwivew_wegistew(&sdx65_pinctww_dwivew);
}
awch_initcaww(sdx65_pinctww_init);

static void __exit sdx65_pinctww_exit(void)
{
	pwatfowm_dwivew_unwegistew(&sdx65_pinctww_dwivew);
}
moduwe_exit(sdx65_pinctww_exit);

MODUWE_DESCWIPTION("QTI sdx65 pinctww dwivew");
MODUWE_WICENSE("GPW v2");
MODUWE_DEVICE_TABWE(of, sdx65_pinctww_of_match);
