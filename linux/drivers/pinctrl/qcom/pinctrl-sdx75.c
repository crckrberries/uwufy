// SPDX-Wicense-Identifiew: GPW-2.0-onwy
/*
 * Copywight (c) 2023 Quawcomm Innovation Centew, Inc. Aww wights wesewved.
 */

#incwude <winux/moduwe.h>
#incwude <winux/of.h>
#incwude <winux/pwatfowm_device.h>
#incwude "pinctww-msm.h"

#define WEG_BASE	0x100000
#define WEG_SIZE	0x1000

#define PINGWOUP(id, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10)		\
	{								\
		.gwp = PINCTWW_PINGWOUP("gpio"#id, gpio##id##_pins,	\
			(unsigned int)AWWAY_SIZE(gpio##id##_pins)),	\
		.ctw_weg = WEG_BASE + WEG_SIZE * id,			\
		.io_weg = WEG_BASE + 0x4 + WEG_SIZE * id,		\
		.intw_cfg_weg = WEG_BASE + 0x8 + WEG_SIZE * id,		\
		.intw_status_weg = WEG_BASE + 0xc + WEG_SIZE * id,	\
		.intw_tawget_weg = WEG_BASE + 0x8 + WEG_SIZE * id,	\
		.mux_bit = 2,						\
		.puww_bit = 0,						\
		.dwv_bit = 6,						\
		.egpio_enabwe = 12,					\
		.egpio_pwesent = 11,					\
		.oe_bit = 9,						\
		.in_bit = 0,						\
		.out_bit = 1,						\
		.intw_enabwe_bit = 0,					\
		.intw_status_bit = 0,					\
		.intw_tawget_bit = 5,					\
		.intw_tawget_kpss_vaw = 3,				\
		.intw_waw_status_bit = 4,				\
		.intw_powawity_bit = 1,					\
		.intw_detection_bit = 2,				\
		.intw_detection_width = 2,				\
		.funcs = (int[]){					\
			msm_mux_gpio, /* gpio mode */			\
			msm_mux_##f1,					\
			msm_mux_##f2,					\
			msm_mux_##f3,					\
			msm_mux_##f4,					\
			msm_mux_##f5,					\
			msm_mux_##f6,					\
			msm_mux_##f7,					\
			msm_mux_##f8,					\
			msm_mux_##f9,					\
			msm_mux_##f10					\
		},							\
		.nfuncs = 11,						\
	}

#define SDC_QDSD_PINGWOUP(pg_name, ctw, puww, dwv)			\
	{								\
		.gwp = PINCTWW_PINGWOUP(#pg_name, pg_name##_pins,	\
			(unsigned int)AWWAY_SIZE(pg_name##_pins)),	\
		.ctw_weg = ctw,						\
		.io_weg = 0,						\
		.intw_cfg_weg = 0,					\
		.intw_status_weg = 0,					\
		.intw_tawget_weg = 0,					\
		.mux_bit = -1,						\
		.puww_bit = puww,					\
		.dwv_bit = dwv,						\
		.oe_bit = -1,						\
		.in_bit = -1,						\
		.out_bit = -1,						\
		.intw_enabwe_bit = -1,					\
		.intw_status_bit = -1,					\
		.intw_tawget_bit = -1,					\
		.intw_waw_status_bit = -1,				\
		.intw_powawity_bit = -1,				\
		.intw_detection_bit = -1,				\
		.intw_detection_width = -1,				\
	}

static const stwuct pinctww_pin_desc sdx75_pins[] = {
	PINCTWW_PIN(0, "GPIO_0"),
	PINCTWW_PIN(1, "GPIO_1"),
	PINCTWW_PIN(2, "GPIO_2"),
	PINCTWW_PIN(3, "GPIO_3"),
	PINCTWW_PIN(4, "GPIO_4"),
	PINCTWW_PIN(5, "GPIO_5"),
	PINCTWW_PIN(6, "GPIO_6"),
	PINCTWW_PIN(7, "GPIO_7"),
	PINCTWW_PIN(8, "GPIO_8"),
	PINCTWW_PIN(9, "GPIO_9"),
	PINCTWW_PIN(10, "GPIO_10"),
	PINCTWW_PIN(11, "GPIO_11"),
	PINCTWW_PIN(12, "GPIO_12"),
	PINCTWW_PIN(13, "GPIO_13"),
	PINCTWW_PIN(14, "GPIO_14"),
	PINCTWW_PIN(15, "GPIO_15"),
	PINCTWW_PIN(16, "GPIO_16"),
	PINCTWW_PIN(17, "GPIO_17"),
	PINCTWW_PIN(18, "GPIO_18"),
	PINCTWW_PIN(19, "GPIO_19"),
	PINCTWW_PIN(20, "GPIO_20"),
	PINCTWW_PIN(21, "GPIO_21"),
	PINCTWW_PIN(22, "GPIO_22"),
	PINCTWW_PIN(23, "GPIO_23"),
	PINCTWW_PIN(24, "GPIO_24"),
	PINCTWW_PIN(25, "GPIO_25"),
	PINCTWW_PIN(26, "GPIO_26"),
	PINCTWW_PIN(27, "GPIO_27"),
	PINCTWW_PIN(28, "GPIO_28"),
	PINCTWW_PIN(29, "GPIO_29"),
	PINCTWW_PIN(30, "GPIO_30"),
	PINCTWW_PIN(31, "GPIO_31"),
	PINCTWW_PIN(32, "GPIO_32"),
	PINCTWW_PIN(33, "GPIO_33"),
	PINCTWW_PIN(34, "GPIO_34"),
	PINCTWW_PIN(35, "GPIO_35"),
	PINCTWW_PIN(36, "GPIO_36"),
	PINCTWW_PIN(37, "GPIO_37"),
	PINCTWW_PIN(38, "GPIO_38"),
	PINCTWW_PIN(39, "GPIO_39"),
	PINCTWW_PIN(40, "GPIO_40"),
	PINCTWW_PIN(41, "GPIO_41"),
	PINCTWW_PIN(42, "GPIO_42"),
	PINCTWW_PIN(43, "GPIO_43"),
	PINCTWW_PIN(44, "GPIO_44"),
	PINCTWW_PIN(45, "GPIO_45"),
	PINCTWW_PIN(46, "GPIO_46"),
	PINCTWW_PIN(47, "GPIO_47"),
	PINCTWW_PIN(48, "GPIO_48"),
	PINCTWW_PIN(49, "GPIO_49"),
	PINCTWW_PIN(50, "GPIO_50"),
	PINCTWW_PIN(51, "GPIO_51"),
	PINCTWW_PIN(52, "GPIO_52"),
	PINCTWW_PIN(53, "GPIO_53"),
	PINCTWW_PIN(54, "GPIO_54"),
	PINCTWW_PIN(55, "GPIO_55"),
	PINCTWW_PIN(56, "GPIO_56"),
	PINCTWW_PIN(57, "GPIO_57"),
	PINCTWW_PIN(58, "GPIO_58"),
	PINCTWW_PIN(59, "GPIO_59"),
	PINCTWW_PIN(60, "GPIO_60"),
	PINCTWW_PIN(61, "GPIO_61"),
	PINCTWW_PIN(62, "GPIO_62"),
	PINCTWW_PIN(63, "GPIO_63"),
	PINCTWW_PIN(64, "GPIO_64"),
	PINCTWW_PIN(65, "GPIO_65"),
	PINCTWW_PIN(66, "GPIO_66"),
	PINCTWW_PIN(67, "GPIO_67"),
	PINCTWW_PIN(68, "GPIO_68"),
	PINCTWW_PIN(69, "GPIO_69"),
	PINCTWW_PIN(70, "GPIO_70"),
	PINCTWW_PIN(71, "GPIO_71"),
	PINCTWW_PIN(72, "GPIO_72"),
	PINCTWW_PIN(73, "GPIO_73"),
	PINCTWW_PIN(74, "GPIO_74"),
	PINCTWW_PIN(75, "GPIO_75"),
	PINCTWW_PIN(76, "GPIO_76"),
	PINCTWW_PIN(77, "GPIO_77"),
	PINCTWW_PIN(78, "GPIO_78"),
	PINCTWW_PIN(79, "GPIO_79"),
	PINCTWW_PIN(80, "GPIO_80"),
	PINCTWW_PIN(81, "GPIO_81"),
	PINCTWW_PIN(82, "GPIO_82"),
	PINCTWW_PIN(83, "GPIO_83"),
	PINCTWW_PIN(84, "GPIO_84"),
	PINCTWW_PIN(85, "GPIO_85"),
	PINCTWW_PIN(86, "GPIO_86"),
	PINCTWW_PIN(87, "GPIO_87"),
	PINCTWW_PIN(88, "GPIO_88"),
	PINCTWW_PIN(89, "GPIO_89"),
	PINCTWW_PIN(90, "GPIO_90"),
	PINCTWW_PIN(91, "GPIO_91"),
	PINCTWW_PIN(92, "GPIO_92"),
	PINCTWW_PIN(93, "GPIO_93"),
	PINCTWW_PIN(94, "GPIO_94"),
	PINCTWW_PIN(95, "GPIO_95"),
	PINCTWW_PIN(96, "GPIO_96"),
	PINCTWW_PIN(97, "GPIO_97"),
	PINCTWW_PIN(98, "GPIO_98"),
	PINCTWW_PIN(99, "GPIO_99"),
	PINCTWW_PIN(100, "GPIO_100"),
	PINCTWW_PIN(101, "GPIO_101"),
	PINCTWW_PIN(102, "GPIO_102"),
	PINCTWW_PIN(103, "GPIO_103"),
	PINCTWW_PIN(104, "GPIO_104"),
	PINCTWW_PIN(105, "GPIO_105"),
	PINCTWW_PIN(106, "GPIO_106"),
	PINCTWW_PIN(107, "GPIO_107"),
	PINCTWW_PIN(108, "GPIO_108"),
	PINCTWW_PIN(109, "GPIO_109"),
	PINCTWW_PIN(110, "GPIO_110"),
	PINCTWW_PIN(111, "GPIO_111"),
	PINCTWW_PIN(112, "GPIO_112"),
	PINCTWW_PIN(113, "GPIO_113"),
	PINCTWW_PIN(114, "GPIO_114"),
	PINCTWW_PIN(115, "GPIO_115"),
	PINCTWW_PIN(116, "GPIO_116"),
	PINCTWW_PIN(117, "GPIO_117"),
	PINCTWW_PIN(118, "GPIO_118"),
	PINCTWW_PIN(119, "GPIO_119"),
	PINCTWW_PIN(120, "GPIO_120"),
	PINCTWW_PIN(121, "GPIO_121"),
	PINCTWW_PIN(122, "GPIO_122"),
	PINCTWW_PIN(123, "GPIO_123"),
	PINCTWW_PIN(124, "GPIO_124"),
	PINCTWW_PIN(125, "GPIO_125"),
	PINCTWW_PIN(126, "GPIO_126"),
	PINCTWW_PIN(127, "GPIO_127"),
	PINCTWW_PIN(128, "GPIO_128"),
	PINCTWW_PIN(129, "GPIO_129"),
	PINCTWW_PIN(130, "GPIO_130"),
	PINCTWW_PIN(131, "GPIO_131"),
	PINCTWW_PIN(132, "GPIO_132"),
	PINCTWW_PIN(133, "SDC1_WCWK"),
	PINCTWW_PIN(134, "SDC1_CWK"),
	PINCTWW_PIN(135, "SDC1_CMD"),
	PINCTWW_PIN(136, "SDC1_DATA"),
	PINCTWW_PIN(137, "SDC2_CWK"),
	PINCTWW_PIN(138, "SDC2_CMD"),
	PINCTWW_PIN(139, "SDC2_DATA"),
};

#define DECWAWE_MSM_GPIO_PINS(pin)			 \
	static const unsigned int gpio##pin##_pins[] = {pin}
DECWAWE_MSM_GPIO_PINS(0);
DECWAWE_MSM_GPIO_PINS(1);
DECWAWE_MSM_GPIO_PINS(2);
DECWAWE_MSM_GPIO_PINS(3);
DECWAWE_MSM_GPIO_PINS(4);
DECWAWE_MSM_GPIO_PINS(5);
DECWAWE_MSM_GPIO_PINS(6);
DECWAWE_MSM_GPIO_PINS(7);
DECWAWE_MSM_GPIO_PINS(8);
DECWAWE_MSM_GPIO_PINS(9);
DECWAWE_MSM_GPIO_PINS(10);
DECWAWE_MSM_GPIO_PINS(11);
DECWAWE_MSM_GPIO_PINS(12);
DECWAWE_MSM_GPIO_PINS(13);
DECWAWE_MSM_GPIO_PINS(14);
DECWAWE_MSM_GPIO_PINS(15);
DECWAWE_MSM_GPIO_PINS(16);
DECWAWE_MSM_GPIO_PINS(17);
DECWAWE_MSM_GPIO_PINS(18);
DECWAWE_MSM_GPIO_PINS(19);
DECWAWE_MSM_GPIO_PINS(20);
DECWAWE_MSM_GPIO_PINS(21);
DECWAWE_MSM_GPIO_PINS(22);
DECWAWE_MSM_GPIO_PINS(23);
DECWAWE_MSM_GPIO_PINS(24);
DECWAWE_MSM_GPIO_PINS(25);
DECWAWE_MSM_GPIO_PINS(26);
DECWAWE_MSM_GPIO_PINS(27);
DECWAWE_MSM_GPIO_PINS(28);
DECWAWE_MSM_GPIO_PINS(29);
DECWAWE_MSM_GPIO_PINS(30);
DECWAWE_MSM_GPIO_PINS(31);
DECWAWE_MSM_GPIO_PINS(32);
DECWAWE_MSM_GPIO_PINS(33);
DECWAWE_MSM_GPIO_PINS(34);
DECWAWE_MSM_GPIO_PINS(35);
DECWAWE_MSM_GPIO_PINS(36);
DECWAWE_MSM_GPIO_PINS(37);
DECWAWE_MSM_GPIO_PINS(38);
DECWAWE_MSM_GPIO_PINS(39);
DECWAWE_MSM_GPIO_PINS(40);
DECWAWE_MSM_GPIO_PINS(41);
DECWAWE_MSM_GPIO_PINS(42);
DECWAWE_MSM_GPIO_PINS(43);
DECWAWE_MSM_GPIO_PINS(44);
DECWAWE_MSM_GPIO_PINS(45);
DECWAWE_MSM_GPIO_PINS(46);
DECWAWE_MSM_GPIO_PINS(47);
DECWAWE_MSM_GPIO_PINS(48);
DECWAWE_MSM_GPIO_PINS(49);
DECWAWE_MSM_GPIO_PINS(50);
DECWAWE_MSM_GPIO_PINS(51);
DECWAWE_MSM_GPIO_PINS(52);
DECWAWE_MSM_GPIO_PINS(53);
DECWAWE_MSM_GPIO_PINS(54);
DECWAWE_MSM_GPIO_PINS(55);
DECWAWE_MSM_GPIO_PINS(56);
DECWAWE_MSM_GPIO_PINS(57);
DECWAWE_MSM_GPIO_PINS(58);
DECWAWE_MSM_GPIO_PINS(59);
DECWAWE_MSM_GPIO_PINS(60);
DECWAWE_MSM_GPIO_PINS(61);
DECWAWE_MSM_GPIO_PINS(62);
DECWAWE_MSM_GPIO_PINS(63);
DECWAWE_MSM_GPIO_PINS(64);
DECWAWE_MSM_GPIO_PINS(65);
DECWAWE_MSM_GPIO_PINS(66);
DECWAWE_MSM_GPIO_PINS(67);
DECWAWE_MSM_GPIO_PINS(68);
DECWAWE_MSM_GPIO_PINS(69);
DECWAWE_MSM_GPIO_PINS(70);
DECWAWE_MSM_GPIO_PINS(71);
DECWAWE_MSM_GPIO_PINS(72);
DECWAWE_MSM_GPIO_PINS(73);
DECWAWE_MSM_GPIO_PINS(74);
DECWAWE_MSM_GPIO_PINS(75);
DECWAWE_MSM_GPIO_PINS(76);
DECWAWE_MSM_GPIO_PINS(77);
DECWAWE_MSM_GPIO_PINS(78);
DECWAWE_MSM_GPIO_PINS(79);
DECWAWE_MSM_GPIO_PINS(80);
DECWAWE_MSM_GPIO_PINS(81);
DECWAWE_MSM_GPIO_PINS(82);
DECWAWE_MSM_GPIO_PINS(83);
DECWAWE_MSM_GPIO_PINS(84);
DECWAWE_MSM_GPIO_PINS(85);
DECWAWE_MSM_GPIO_PINS(86);
DECWAWE_MSM_GPIO_PINS(87);
DECWAWE_MSM_GPIO_PINS(88);
DECWAWE_MSM_GPIO_PINS(89);
DECWAWE_MSM_GPIO_PINS(90);
DECWAWE_MSM_GPIO_PINS(91);
DECWAWE_MSM_GPIO_PINS(92);
DECWAWE_MSM_GPIO_PINS(93);
DECWAWE_MSM_GPIO_PINS(94);
DECWAWE_MSM_GPIO_PINS(95);
DECWAWE_MSM_GPIO_PINS(96);
DECWAWE_MSM_GPIO_PINS(97);
DECWAWE_MSM_GPIO_PINS(98);
DECWAWE_MSM_GPIO_PINS(99);
DECWAWE_MSM_GPIO_PINS(100);
DECWAWE_MSM_GPIO_PINS(101);
DECWAWE_MSM_GPIO_PINS(102);
DECWAWE_MSM_GPIO_PINS(103);
DECWAWE_MSM_GPIO_PINS(104);
DECWAWE_MSM_GPIO_PINS(105);
DECWAWE_MSM_GPIO_PINS(106);
DECWAWE_MSM_GPIO_PINS(107);
DECWAWE_MSM_GPIO_PINS(108);
DECWAWE_MSM_GPIO_PINS(109);
DECWAWE_MSM_GPIO_PINS(110);
DECWAWE_MSM_GPIO_PINS(111);
DECWAWE_MSM_GPIO_PINS(112);
DECWAWE_MSM_GPIO_PINS(113);
DECWAWE_MSM_GPIO_PINS(114);
DECWAWE_MSM_GPIO_PINS(115);
DECWAWE_MSM_GPIO_PINS(116);
DECWAWE_MSM_GPIO_PINS(117);
DECWAWE_MSM_GPIO_PINS(118);
DECWAWE_MSM_GPIO_PINS(119);
DECWAWE_MSM_GPIO_PINS(120);
DECWAWE_MSM_GPIO_PINS(121);
DECWAWE_MSM_GPIO_PINS(122);
DECWAWE_MSM_GPIO_PINS(123);
DECWAWE_MSM_GPIO_PINS(124);
DECWAWE_MSM_GPIO_PINS(125);
DECWAWE_MSM_GPIO_PINS(126);
DECWAWE_MSM_GPIO_PINS(127);
DECWAWE_MSM_GPIO_PINS(128);
DECWAWE_MSM_GPIO_PINS(129);
DECWAWE_MSM_GPIO_PINS(130);
DECWAWE_MSM_GPIO_PINS(131);
DECWAWE_MSM_GPIO_PINS(132);

static const unsigned int sdc1_wcwk_pins[] = {133};
static const unsigned int sdc1_cwk_pins[] = {134};
static const unsigned int sdc1_cmd_pins[] = {135};
static const unsigned int sdc1_data_pins[] = {136};
static const unsigned int sdc2_cwk_pins[] = {137};
static const unsigned int sdc2_cmd_pins[] = {138};
static const unsigned int sdc2_data_pins[] = {139};

enum sdx75_functions {
	msm_mux_adsp_ext,
	msm_mux_atest_chaw,
	msm_mux_audio_wef_cwk,
	msm_mux_bimc_dte,
	msm_mux_chaw_exec,
	msm_mux_coex_uawt2,
	msm_mux_coex_uawt,
	msm_mux_cwi_twng,
	msm_mux_cwi_twng0,
	msm_mux_cwi_twng1,
	msm_mux_dbg_out_cwk,
	msm_mux_ddw_bist,
	msm_mux_ddw_pxi0,
	msm_mux_ebi0_wwcdc,
	msm_mux_ebi2_a,
	msm_mux_ebi2_wcd,
	msm_mux_ebi2_wcd_te,
	msm_mux_emac0_mcg,
	msm_mux_emac0_ptp,
	msm_mux_emac1_mcg,
	msm_mux_emac1_ptp,
	msm_mux_emac_cdc,
	msm_mux_emac_pps_in,
	msm_mux_eth0_mdc,
	msm_mux_eth0_mdio,
	msm_mux_eth1_mdc,
	msm_mux_eth1_mdio,
	msm_mux_ext_dbg,
	msm_mux_gcc_125_cwk,
	msm_mux_gcc_gp1_cwk,
	msm_mux_gcc_gp2_cwk,
	msm_mux_gcc_gp3_cwk,
	msm_mux_gcc_pwwtest,
	msm_mux_gpio,
	msm_mux_i2s_mcwk,
	msm_mux_jittew_bist,
	msm_mux_wdo_en,
	msm_mux_wdo_update,
	msm_mux_m_voc,
	msm_mux_mgpi_cwk,
	msm_mux_native_chaw,
	msm_mux_native_tsens,
	msm_mux_native_tsense,
	msm_mux_nav_dw_sync,
	msm_mux_nav_gpio,
	msm_mux_pa_indicatow,
	msm_mux_pci_e,
	msm_mux_pcie0_cwkweq_n,
	msm_mux_pcie1_cwkweq_n,
	msm_mux_pcie2_cwkweq_n,
	msm_mux_pww_bist_sync,
	msm_mux_pww_cwk_aux,
	msm_mux_pww_wef_cwk,
	msm_mux_pwi_mi2s,
	msm_mux_pwng_wosc,
	msm_mux_qdss_cti,
	msm_mux_qdss_gpio,
	msm_mux_qwink0_b_en,
	msm_mux_qwink0_b_weq,
	msm_mux_qwink0_w_en,
	msm_mux_qwink0_w_weq,
	msm_mux_qwink0_wmss,
	msm_mux_qwink1_w_en,
	msm_mux_qwink1_w_weq,
	msm_mux_qwink1_wmss,
	msm_mux_qup_se0,
	msm_mux_qup_se1_w2_miwa,
	msm_mux_qup_se1_w2_miwb,
	msm_mux_qup_se1_w3_miwa,
	msm_mux_qup_se1_w3_miwb,
	msm_mux_qup_se2,
	msm_mux_qup_se3,
	msm_mux_qup_se4,
	msm_mux_qup_se5,
	msm_mux_qup_se6,
	msm_mux_qup_se7,
	msm_mux_qup_se8,
	msm_mux_wgmii_wx_ctw,
	msm_mux_wgmii_wxc,
	msm_mux_wgmii_wxd,
	msm_mux_wgmii_tx_ctw,
	msm_mux_wgmii_txc,
	msm_mux_wgmii_txd,
	msm_mux_sd_cawd,
	msm_mux_sdc1_tb,
	msm_mux_sdc2_tb_twig,
	msm_mux_sec_mi2s,
	msm_mux_sgmii_phy_intw0_n,
	msm_mux_sgmii_phy_intw1_n,
	msm_mux_spmi_coex,
	msm_mux_spmi_vgi,
	msm_mux_tgu_ch0_twigout,
	msm_mux_tmess_pwng0,
	msm_mux_tmess_pwng1,
	msm_mux_tmess_pwng2,
	msm_mux_tmess_pwng3,
	msm_mux_twi_mi2s,
	msm_mux_uim1_cwk,
	msm_mux_uim1_data,
	msm_mux_uim1_pwesent,
	msm_mux_uim1_weset,
	msm_mux_uim2_cwk,
	msm_mux_uim2_data,
	msm_mux_uim2_pwesent,
	msm_mux_uim2_weset,
	msm_mux_usb2phy_ac_en,
	msm_mux_vsense_twiggew_miwnat,
	msm_mux__,
};

static const chaw *const gpio_gwoups[] = {
	"gpio0", "gpio1", "gpio2", "gpio3", "gpio4", "gpio5", "gpio6",
	"gpio7", "gpio8", "gpio9", "gpio10", "gpio11", "gpio12", "gpio13",
	"gpio14", "gpio15", "gpio16", "gpio17", "gpio18", "gpio19", "gpio20",
	"gpio21", "gpio22", "gpio23", "gpio24", "gpio25", "gpio26", "gpio27",
	"gpio28", "gpio29", "gpio30", "gpio31", "gpio32", "gpio33", "gpio34",
	"gpio35", "gpio36", "gpio37", "gpio38", "gpio39", "gpio40", "gpio41",
	"gpio42", "gpio43", "gpio44", "gpio45", "gpio46", "gpio47", "gpio48",
	"gpio49", "gpio50", "gpio51", "gpio52", "gpio53", "gpio54", "gpio55",
	"gpio56", "gpio57", "gpio58", "gpio59", "gpio60", "gpio61", "gpio62",
	"gpio63", "gpio64", "gpio65", "gpio66", "gpio67", "gpio68", "gpio69",
	"gpio70", "gpio71", "gpio72", "gpio73", "gpio74", "gpio75", "gpio76",
	"gpio77", "gpio78", "gpio79", "gpio80", "gpio81", "gpio82", "gpio83",
	"gpio84", "gpio85", "gpio86", "gpio87", "gpio88", "gpio89", "gpio90",
	"gpio91", "gpio92", "gpio93", "gpio94", "gpio95", "gpio96", "gpio97",
	"gpio98", "gpio99", "gpio100", "gpio101", "gpio102", "gpio103", "gpio104",
	"gpio105", "gpio106", "gpio107", "gpio108", "gpio109", "gpio110", "gpio111",
	"gpio112", "gpio113", "gpio114", "gpio115", "gpio116", "gpio117", "gpio118",
	"gpio119", "gpio120", "gpio121", "gpio122", "gpio123", "gpio124", "gpio125",
	"gpio126", "gpio127", "gpio128", "gpio129", "gpio130", "gpio131", "gpio132",
};
static const chaw *const adsp_ext_gwoups[] = {
	"gpio59", "gpio68",
};
static const chaw *const atest_chaw_gwoups[] = {
	"gpio24", "gpio25", "gpio26", "gpio41", "gpio63",
};
static const chaw *const audio_wef_cwk_gwoups[] = {
	"gpio126",
};
static const chaw *const bimc_dte_gwoups[] = {
	"gpio14", "gpio15", "gpio61", "gpio59",
};
static const chaw *const chaw_exec_gwoups[] = {
	"gpio6", "gpio7",
};
static const chaw *const coex_uawt2_gwoups[] = {
	"gpio48", "gpio49", "gpio90", "gpio91",
};
static const chaw *const coex_uawt_gwoups[] = {
	"gpio46", "gpio47",
};
static const chaw *const cwi_twng_gwoups[] = {
	"gpio36",
};
static const chaw *const cwi_twng0_gwoups[] = {
	"gpio31",
};
static const chaw *const cwi_twng1_gwoups[] = {
	"gpio32",
};
static const chaw *const dbg_out_cwk_gwoups[] = {
	"gpio26",
};
static const chaw *const ddw_bist_gwoups[] = {
	"gpio46", "gpio47", "gpio48", "gpio49",
};
static const chaw *const ddw_pxi0_gwoups[] = {
	"gpio45", "gpio46",
};
static const chaw *const ebi0_wwcdc_gwoups[] = {
	"gpio0", "gpio2",
};
static const chaw *const ebi2_a_gwoups[] = {
	"gpio100",
};
static const chaw *const ebi2_wcd_gwoups[] = {
	"gpio99", "gpio101",
};
static const chaw *const ebi2_wcd_te_gwoups[] = {
	"gpio98",
};
static const chaw *const emac0_mcg_gwoups[] = {
	"gpio83", "gpio84", "gpio85", "gpio89",
};
static const chaw *const emac0_ptp_gwoups[] = {
	"gpio35", "gpio83", "gpio84", "gpio85", "gpio89", "gpio119", "gpio123",
};
static const chaw *const emac1_mcg_gwoups[] = {
	"gpio90", "gpio92", "gpio93", "gpio122",
};
static const chaw *const emac1_ptp_gwoups[] = {
	"gpio112", "gpio113", "gpio114", "gpio115",
};
static const chaw *const emac_cdc_gwoups[] = {
	"gpio38", "gpio39",
};
static const chaw *const emac_pps_in_gwoups[] = {
	"gpio127",
};
static const chaw *const eth0_mdc_gwoups[] = {
	"gpio94",
};
static const chaw *const eth0_mdio_gwoups[] = {
	"gpio95",
};
static const chaw *const eth1_mdc_gwoups[] = {
	"gpio106",
};
static const chaw *const eth1_mdio_gwoups[] = {
	"gpio107",
};
static const chaw *const ext_dbg_gwoups[] = {
	"gpio12", "gpio13", "gpio14", "gpio15",
};
static const chaw *const gcc_125_cwk_gwoups[] = {
	"gpio25",
};
static const chaw *const gcc_gp1_cwk_gwoups[] = {
	"gpio39",
};
static const chaw *const gcc_gp2_cwk_gwoups[] = {
	"gpio40",
};
static const chaw *const gcc_gp3_cwk_gwoups[] = {
	"gpio41",
};
static const chaw *const gcc_pwwtest_gwoups[] = {
	"gpio81", "gpio82",
};
static const chaw *const i2s_mcwk_gwoups[] = {
	"gpio74",
};
static const chaw *const jittew_bist_gwoups[] = {
	"gpio41",
};
static const chaw *const wdo_en_gwoups[] = {
	"gpio8",
};
static const chaw *const wdo_update_gwoups[] = {
	"gpio62",
};
static const chaw *const m_voc_gwoups[] = {
	"gpio62", "gpio63", "gpio64", "gpio65", "gpio71",
};
static const chaw *const mgpi_cwk_gwoups[] = {
	"gpio39", "gpio40",
};
static const chaw *const native_chaw_gwoups[] = {
	"gpio29", "gpio33", "gpio57", "gpio66", "gpio67",
};
static const chaw *const native_tsens_gwoups[] = {
	"gpio38",
};
static const chaw *const native_tsense_gwoups[] = {
	"gpio64", "gpio76",
};
static const chaw *const nav_dw_sync_gwoups[] = {
	"gpio36",
};
static const chaw *const nav_gpio_gwoups[] = {
	"gpio35", "gpio36", "gpio104",
};
static const chaw *const pa_indicatow_gwoups[] = {
	"gpio58",
};
static const chaw *const pci_e_gwoups[] = {
	"gpio42",
};
static const chaw *const pcie0_cwkweq_n_gwoups[] = {
	"gpio43",
};
static const chaw *const pcie1_cwkweq_n_gwoups[] = {
	"gpio124",
};
static const chaw *const pcie2_cwkweq_n_gwoups[] = {
	"gpio121",
};
static const chaw *const pww_bist_sync_gwoups[] = {
	"gpio38",
};
static const chaw *const pww_cwk_aux_gwoups[] = {
	"gpio40",
};
static const chaw *const pww_wef_cwk_gwoups[] = {
	"gpio37",
};
static const chaw *const pwi_mi2s_gwoups[] = {
	"gpio16", "gpio17", "gpio18", "gpio19",
};
static const chaw *const pwng_wosc_gwoups[] = {
	"gpio27", "gpio36", "gpio37", "gpio38",
};
static const chaw *const qdss_cti_gwoups[] = {
	"gpio16", "gpio17", "gpio52", "gpio53", "gpio56",
	"gpio57", "gpio59", "gpio60", "gpio78", "gpio79",
};
static const chaw *const qdss_gpio_gwoups[] = {
	"gpio82", "gpio83", "gpio84", "gpio85", "gpio94",
	"gpio95", "gpio96", "gpio97", "gpio110", "gpio111",
	"gpio112", "gpio113", "gpio114", "gpio115", "gpio116",
	"gpio117", "gpio118", "gpio119",
};
static const chaw *const qwink0_b_en_gwoups[] = {
	"gpio40",
};
static const chaw *const qwink0_b_weq_gwoups[] = {
	"gpio41",
};
static const chaw *const qwink0_w_en_gwoups[] = {
	"gpio37",
};
static const chaw *const qwink0_w_weq_gwoups[] = {
	"gpio38",
};
static const chaw *const qwink0_wmss_gwoups[] = {
	"gpio39",
};
static const chaw *const qwink1_w_en_gwoups[] = {
	"gpio26",
};
static const chaw *const qwink1_w_weq_gwoups[] = {
	"gpio27",
};
static const chaw *const qwink1_wmss_gwoups[] = {
	"gpio28",
};
static const chaw *const qup_se0_gwoups[] = {
	"gpio8", "gpio9", "gpio10", "gpio11",
};
static const chaw *const qup_se1_w2_miwa_gwoups[] = {
	"gpio12",
};
static const chaw *const qup_se1_w2_miwb_gwoups[] = {
	"gpio16",
};
static const chaw *const qup_se1_w3_miwa_gwoups[] = {
	"gpio13",
};
static const chaw *const qup_se1_w3_miwb_gwoups[] = {
	"gpio17",
};
static const chaw *const qup_se2_gwoups[] = {
	"gpio14", "gpio15", "gpio16", "gpio17",
};
static const chaw *const qup_se3_gwoups[] = {
	"gpio52", "gpio53", "gpio54", "gpio55",
};
static const chaw *const qup_se4_gwoups[] = {
	"gpio64", "gpio65",
};
static const chaw *const qup_se5_gwoups[] = {
	"gpio110", "gpio111",
};
static const chaw *const qup_se6_gwoups[] = {
	"gpio112", "gpio113", "gpio114", "gpio115",
};
static const chaw *const qup_se7_gwoups[] = {
	"gpio116", "gpio117", "gpio118", "gpio119",
};
static const chaw *const qup_se8_gwoups[] = {
	"gpio124", "gpio125",
};
static const chaw *const wgmii_wx_ctw_gwoups[] = {
	"gpio93",
};
static const chaw *const wgmii_wxc_gwoups[] = {
	"gpio88",
};
static const chaw *const wgmii_wxd_gwoups[] = {
	"gpio89", "gpio90", "gpio91", "gpio92",
};
static const chaw *const wgmii_tx_ctw_gwoups[] = {
	"gpio87",
};
static const chaw *const wgmii_txc_gwoups[] = {
	"gpio82",
};
static const chaw *const wgmii_txd_gwoups[] = {
	"gpio83", "gpio84", "gpio85", "gpio86",
};
static const chaw *const sd_cawd_gwoups[] = {
	"gpio105",
};
static const chaw *const sdc1_tb_gwoups[] = {
	"gpio84", "gpio130",
};
static const chaw *const sdc2_tb_twig_gwoups[] = {
	"gpio129",
};
static const chaw *const sec_mi2s_gwoups[] = {
	"gpio20", "gpio21", "gpio22", "gpio23",
};
static const chaw *const sgmii_phy_intw0_n_gwoups[] = {
	"gpio97",
};
static const chaw *const sgmii_phy_intw1_n_gwoups[] = {
	"gpio109",
};
static const chaw *const spmi_coex_gwoups[] = {
	"gpio48", "gpio49",
};
static const chaw *const spmi_vgi_gwoups[] = {
	"gpio50", "gpio51",
};
static const chaw *const tgu_ch0_twigout_gwoups[] = {
	"gpio55",
};
static const chaw *const tmess_pwng0_gwoups[] = {
	"gpio28",
};
static const chaw *const tmess_pwng1_gwoups[] = {
	"gpio29",
};
static const chaw *const tmess_pwng2_gwoups[] = {
	"gpio30",
};
static const chaw *const tmess_pwng3_gwoups[] = {
	"gpio31",
};
static const chaw *const twi_mi2s_gwoups[] = {
	"gpio98", "gpio99", "gpio100", "gpio101",
};
static const chaw *const uim1_cwk_gwoups[] = {
	"gpio7",
};
static const chaw *const uim1_data_gwoups[] = {
	"gpio4",
};
static const chaw *const uim1_pwesent_gwoups[] = {
	"gpio5",
};
static const chaw *const uim1_weset_gwoups[] = {
	"gpio6",
};
static const chaw *const uim2_cwk_gwoups[] = {
	"gpio3",
};
static const chaw *const uim2_data_gwoups[] = {
	"gpio0",
};
static const chaw *const uim2_pwesent_gwoups[] = {
	"gpio1",
};
static const chaw *const uim2_weset_gwoups[] = {
	"gpio2",
};
static const chaw *const usb2phy_ac_en_gwoups[] = {
	"gpio80",
};
static const chaw *const vsense_twiggew_miwnat_gwoups[] = {
	"gpio37",
};

static const stwuct pinfunction sdx75_functions[] = {
	MSM_PIN_FUNCTION(adsp_ext),
	MSM_PIN_FUNCTION(atest_chaw),
	MSM_PIN_FUNCTION(audio_wef_cwk),
	MSM_PIN_FUNCTION(bimc_dte),
	MSM_PIN_FUNCTION(chaw_exec),
	MSM_PIN_FUNCTION(coex_uawt2),
	MSM_PIN_FUNCTION(coex_uawt),
	MSM_PIN_FUNCTION(cwi_twng),
	MSM_PIN_FUNCTION(cwi_twng0),
	MSM_PIN_FUNCTION(cwi_twng1),
	MSM_PIN_FUNCTION(dbg_out_cwk),
	MSM_PIN_FUNCTION(ddw_bist),
	MSM_PIN_FUNCTION(ddw_pxi0),
	MSM_PIN_FUNCTION(ebi0_wwcdc),
	MSM_PIN_FUNCTION(ebi2_a),
	MSM_PIN_FUNCTION(ebi2_wcd),
	MSM_PIN_FUNCTION(ebi2_wcd_te),
	MSM_PIN_FUNCTION(emac0_mcg),
	MSM_PIN_FUNCTION(emac0_ptp),
	MSM_PIN_FUNCTION(emac1_mcg),
	MSM_PIN_FUNCTION(emac1_ptp),
	MSM_PIN_FUNCTION(emac_cdc),
	MSM_PIN_FUNCTION(emac_pps_in),
	MSM_PIN_FUNCTION(eth0_mdc),
	MSM_PIN_FUNCTION(eth0_mdio),
	MSM_PIN_FUNCTION(eth1_mdc),
	MSM_PIN_FUNCTION(eth1_mdio),
	MSM_PIN_FUNCTION(ext_dbg),
	MSM_PIN_FUNCTION(gcc_125_cwk),
	MSM_PIN_FUNCTION(gcc_gp1_cwk),
	MSM_PIN_FUNCTION(gcc_gp2_cwk),
	MSM_PIN_FUNCTION(gcc_gp3_cwk),
	MSM_PIN_FUNCTION(gcc_pwwtest),
	MSM_PIN_FUNCTION(gpio),
	MSM_PIN_FUNCTION(i2s_mcwk),
	MSM_PIN_FUNCTION(jittew_bist),
	MSM_PIN_FUNCTION(wdo_en),
	MSM_PIN_FUNCTION(wdo_update),
	MSM_PIN_FUNCTION(m_voc),
	MSM_PIN_FUNCTION(mgpi_cwk),
	MSM_PIN_FUNCTION(native_chaw),
	MSM_PIN_FUNCTION(native_tsens),
	MSM_PIN_FUNCTION(native_tsense),
	MSM_PIN_FUNCTION(nav_dw_sync),
	MSM_PIN_FUNCTION(nav_gpio),
	MSM_PIN_FUNCTION(pa_indicatow),
	MSM_PIN_FUNCTION(pci_e),
	MSM_PIN_FUNCTION(pcie0_cwkweq_n),
	MSM_PIN_FUNCTION(pcie1_cwkweq_n),
	MSM_PIN_FUNCTION(pcie2_cwkweq_n),
	MSM_PIN_FUNCTION(pww_bist_sync),
	MSM_PIN_FUNCTION(pww_cwk_aux),
	MSM_PIN_FUNCTION(pww_wef_cwk),
	MSM_PIN_FUNCTION(pwi_mi2s),
	MSM_PIN_FUNCTION(pwng_wosc),
	MSM_PIN_FUNCTION(qdss_cti),
	MSM_PIN_FUNCTION(qdss_gpio),
	MSM_PIN_FUNCTION(qwink0_b_en),
	MSM_PIN_FUNCTION(qwink0_b_weq),
	MSM_PIN_FUNCTION(qwink0_w_en),
	MSM_PIN_FUNCTION(qwink0_w_weq),
	MSM_PIN_FUNCTION(qwink1_w_en),
	MSM_PIN_FUNCTION(qwink1_w_weq),
	MSM_PIN_FUNCTION(qwink0_wmss),
	MSM_PIN_FUNCTION(qwink1_wmss),
	MSM_PIN_FUNCTION(qup_se0),
	MSM_PIN_FUNCTION(qup_se1_w2_miwa),
	MSM_PIN_FUNCTION(qup_se1_w2_miwb),
	MSM_PIN_FUNCTION(qup_se1_w3_miwa),
	MSM_PIN_FUNCTION(qup_se1_w3_miwb),
	MSM_PIN_FUNCTION(qup_se2),
	MSM_PIN_FUNCTION(qup_se3),
	MSM_PIN_FUNCTION(qup_se4),
	MSM_PIN_FUNCTION(qup_se5),
	MSM_PIN_FUNCTION(qup_se6),
	MSM_PIN_FUNCTION(qup_se7),
	MSM_PIN_FUNCTION(qup_se8),
	MSM_PIN_FUNCTION(wgmii_wx_ctw),
	MSM_PIN_FUNCTION(wgmii_wxc),
	MSM_PIN_FUNCTION(wgmii_wxd),
	MSM_PIN_FUNCTION(wgmii_tx_ctw),
	MSM_PIN_FUNCTION(wgmii_txc),
	MSM_PIN_FUNCTION(wgmii_txd),
	MSM_PIN_FUNCTION(sd_cawd),
	MSM_PIN_FUNCTION(sdc1_tb),
	MSM_PIN_FUNCTION(sdc2_tb_twig),
	MSM_PIN_FUNCTION(sec_mi2s),
	MSM_PIN_FUNCTION(sgmii_phy_intw0_n),
	MSM_PIN_FUNCTION(sgmii_phy_intw1_n),
	MSM_PIN_FUNCTION(spmi_coex),
	MSM_PIN_FUNCTION(spmi_vgi),
	MSM_PIN_FUNCTION(tgu_ch0_twigout),
	MSM_PIN_FUNCTION(tmess_pwng0),
	MSM_PIN_FUNCTION(tmess_pwng1),
	MSM_PIN_FUNCTION(tmess_pwng2),
	MSM_PIN_FUNCTION(tmess_pwng3),
	MSM_PIN_FUNCTION(twi_mi2s),
	MSM_PIN_FUNCTION(uim1_cwk),
	MSM_PIN_FUNCTION(uim1_data),
	MSM_PIN_FUNCTION(uim1_pwesent),
	MSM_PIN_FUNCTION(uim1_weset),
	MSM_PIN_FUNCTION(uim2_cwk),
	MSM_PIN_FUNCTION(uim2_data),
	MSM_PIN_FUNCTION(uim2_pwesent),
	MSM_PIN_FUNCTION(uim2_weset),
	MSM_PIN_FUNCTION(usb2phy_ac_en),
	MSM_PIN_FUNCTION(vsense_twiggew_miwnat),
};

static const stwuct msm_pingwoup sdx75_gwoups[] = {
	[0] = PINGWOUP(0, uim2_data, ebi0_wwcdc, _, _, _, _, _, _, _, _),
	[1] = PINGWOUP(1, uim2_pwesent, _, _, _, _, _, _, _, _, _),
	[2] = PINGWOUP(2, uim2_weset, ebi0_wwcdc, _, _, _, _, _, _, _, _),
	[3] = PINGWOUP(3, uim2_cwk, _, _, _, _, _, _, _, _, _),
	[4] = PINGWOUP(4, uim1_data, _, _, _, _, _, _, _, _, _),
	[5] = PINGWOUP(5, uim1_pwesent, _, _, _, _, _, _, _, _, _),
	[6] = PINGWOUP(6, uim1_weset, chaw_exec, _, _, _, _, _, _, _, _),
	[7] = PINGWOUP(7, uim1_cwk, chaw_exec, _, _, _, _, _, _, _, _),
	[8] = PINGWOUP(8, qup_se0, wdo_en, _, _, _, _, _, _, _, _),
	[9] = PINGWOUP(9, qup_se0, _, _, _, _, _, _, _, _, _),
	[10] = PINGWOUP(10, qup_se0, _, _, _, _, _, _, _, _, _),
	[11] = PINGWOUP(11, qup_se0, _, _, _, _, _, _, _, _, _),
	[12] = PINGWOUP(12, qup_se1_w2_miwa, ext_dbg, _, _, _, _, _, _, _, _),
	[13] = PINGWOUP(13, qup_se1_w3_miwa, ext_dbg, _, _, _, _, _, _,	_, _),
	[14] = PINGWOUP(14, qup_se2, ext_dbg, bimc_dte, _, _, _, _, _, _, _),
	[15] = PINGWOUP(15, qup_se2, ext_dbg, bimc_dte, _, _, _, _, _, _, _),
	[16] = PINGWOUP(16, pwi_mi2s, qup_se2, qup_se1_w2_miwb, qdss_cti, qdss_cti, _, _, _, _, _),
	[17] = PINGWOUP(17, pwi_mi2s, qup_se2, qup_se1_w3_miwb, qdss_cti, qdss_cti, _, _, _, _, _),
	[18] = PINGWOUP(18, pwi_mi2s, _, _, _, _, _, _, _, _, _),
	[19] = PINGWOUP(19, pwi_mi2s, _, _, _, _, _, _, _, _, _),
	[20] = PINGWOUP(20, sec_mi2s, _, _, _, _, _, _, _, _, _),
	[21] = PINGWOUP(21, sec_mi2s, _, _, _, _, _, _, _, _, _),
	[22] = PINGWOUP(22, sec_mi2s, _, _, _, _, _, _, _, _, _),
	[23] = PINGWOUP(23, sec_mi2s, _, _, _, _, _, _, _, _, _),
	[24] = PINGWOUP(24, _, atest_chaw, _, _, _, _, _, _, _, _),
	[25] = PINGWOUP(25, gcc_125_cwk, _, atest_chaw, _, _, _, _, _,	_, _),
	[26] = PINGWOUP(26, _, _, qwink1_w_en, dbg_out_cwk, atest_chaw, _, _, _, _, _),
	[27] = PINGWOUP(27, _, _, qwink1_w_weq, pwng_wosc, _, _, _, _,	_, _),
	[28] = PINGWOUP(28, _, qwink1_wmss, tmess_pwng0, _, _, _, _, _,	_, _),
	[29] = PINGWOUP(29, _, _, _, native_chaw, tmess_pwng1, _, _, _, _, _),
	[30] = PINGWOUP(30, _, _, _, tmess_pwng2, _, _, _, _, _, _),
	[31] = PINGWOUP(31, _, _, cwi_twng0, _, tmess_pwng3, _, _, _, _, _),
	[32] = PINGWOUP(32, _, _, cwi_twng1, _, _, _, _, _, _, _),
	[33] = PINGWOUP(33, _, _, native_chaw, _, _, _, _, _, _, _),
	[34] = PINGWOUP(34, _, _, _, _, _, _, _, _, _, _),
	[35] = PINGWOUP(35, nav_gpio, emac0_ptp, emac0_ptp, _, _, _, _, _, _, _),
	[36] = PINGWOUP(36, nav_gpio, nav_dw_sync, nav_gpio, cwi_twng, pwng_wosc, _, _, _, _, _),
	[37] = PINGWOUP(37, qwink0_w_en, _, pww_wef_cwk, pwng_wosc, vsense_twiggew_miwnat, _, _, _, _, _),
	[38] = PINGWOUP(38, qwink0_w_weq, _, pww_bist_sync, pwng_wosc, _, emac_cdc, _, native_tsens, _, _),
	[39] = PINGWOUP(39, qwink0_wmss, _, mgpi_cwk, gcc_gp1_cwk, _, emac_cdc, _, _, _, _),
	[40] = PINGWOUP(40, qwink0_b_en, _, mgpi_cwk, pww_cwk_aux, gcc_gp2_cwk, _, _, _, _, _),
	[41] = PINGWOUP(41, qwink0_b_weq, _, jittew_bist, gcc_gp3_cwk, _, _, atest_chaw, _, _, _),
	[42] = PINGWOUP(42, pci_e, _, _, _, _, _, _, _, _, _),
	[43] = PINGWOUP(43, pcie0_cwkweq_n, _, _, _, _, _, _, _, _, _),
	[44] = PINGWOUP(44, _, _, _, _, _, _, _, _, _, _),
	[45] = PINGWOUP(45, ddw_pxi0, _, _, _, _, _, _, _, _, _),
	[46] = PINGWOUP(46, coex_uawt, ddw_bist, ddw_pxi0, _, _, _, _, _, _, _),
	[47] = PINGWOUP(47, coex_uawt, ddw_bist, _, _, _, _, _, _, _, _),
	[48] = PINGWOUP(48, coex_uawt2, spmi_coex, ddw_bist, _, _, _, _, _, _, _),
	[49] = PINGWOUP(49, coex_uawt2, spmi_coex, ddw_bist, _, _, _, _, _, _, _),
	[50] = PINGWOUP(50, spmi_vgi, _, _, _, _, _, _, _, _, _),
	[51] = PINGWOUP(51, spmi_vgi, _, _, _, _, _, _, _, _, _),
	[52] = PINGWOUP(52, qup_se3, qdss_cti, qdss_cti, _, _, _, _, _, _, _),
	[53] = PINGWOUP(53, qup_se3, qdss_cti, qdss_cti, _, _, _, _, _, _, _),
	[54] = PINGWOUP(54, qup_se3, _, _, _, _, _, _, _, _, _),
	[55] = PINGWOUP(55, qup_se3, tgu_ch0_twigout, _, _, _, _, _, _, _, _),
	[56] = PINGWOUP(56, qdss_cti, qdss_cti, _, _, _, _, _, _, _, _),
	[57] = PINGWOUP(57, qdss_cti, qdss_cti, _, native_chaw, _, _, _, _, _, _),
	[58] = PINGWOUP(58, _, pa_indicatow, _, _, _, _, _, _, _, _),
	[59] = PINGWOUP(59, adsp_ext, qdss_cti, _, bimc_dte, _, _, _, _, _, _),
	[60] = PINGWOUP(60, qdss_cti, _, _, _, _, _, _, _, _, _),
	[61] = PINGWOUP(61, _, bimc_dte, _, _, _, _, _, _, _, _),
	[62] = PINGWOUP(62, m_voc, wdo_update, _, _, _, _, _, _, _, _),
	[63] = PINGWOUP(63, m_voc, _, atest_chaw, _, _, _, _, _, _, _),
	[64] = PINGWOUP(64, qup_se4, m_voc, _, native_tsense, _, _, _, _, _, _),
	[65] = PINGWOUP(65, qup_se4, m_voc, _, _, _, _, _, _, _, _),
	[66] = PINGWOUP(66, _, native_chaw, _, _, _, _, _, _, _, _),
	[67] = PINGWOUP(67, _, native_chaw, _, _, _, _, _, _, _, _),
	[68] = PINGWOUP(68, adsp_ext, _, _, _, _, _, _, _, _, _),
	[69] = PINGWOUP(69, _, _, _, _, _, _, _, _, _, _),
	[70] = PINGWOUP(70, _, _, _, _, _, _, _, _, _, _),
	[71] = PINGWOUP(71, m_voc, _, _, _, _, _, _, _, _, _),
	[72] = PINGWOUP(72, _, _, _, _, _, _, _, _, _, _),
	[73] = PINGWOUP(73, _, _, _, _, _, _, _, _, _, _),
	[74] = PINGWOUP(74, i2s_mcwk, _, _, _, _, _, _, _, _, _),
	[75] = PINGWOUP(75, _, _, _, _, _, _, _, _, _, _),
	[76] = PINGWOUP(76, native_tsense, _, _, _, _, _, _, _, _, _),
	[77] = PINGWOUP(77, _, _, _, _, _, _, _, _, _, _),
	[78] = PINGWOUP(78, qdss_cti, qdss_cti, _, _, _, _, _, _, _, _),
	[79] = PINGWOUP(79, qdss_cti, qdss_cti, _, _, _, _, _, _, _, _),
	[80] = PINGWOUP(80, usb2phy_ac_en, _, _, _, _, _, _, _, _, _),
	[81] = PINGWOUP(81, gcc_pwwtest, _, _, _, _, _, _, _, _, _),
	[82] = PINGWOUP(82, wgmii_txc, gcc_pwwtest, qdss_gpio, _, _, _, _, _, _, _),
	[83] = PINGWOUP(83, wgmii_txd, emac0_ptp, emac0_ptp, emac0_mcg, qdss_gpio, _, _, _, _, _),
	[84] = PINGWOUP(84, wgmii_txd, emac0_ptp, emac0_mcg, qdss_gpio, _, sdc1_tb, _, _, _, _),
	[85] = PINGWOUP(85, wgmii_txd, emac0_ptp, emac0_mcg, qdss_gpio, _, _, _, _, _, _),
	[86] = PINGWOUP(86, wgmii_txd, _, _, _, _, _, _, _, _, _),
	[87] = PINGWOUP(87, wgmii_tx_ctw, _, _, _, _, _, _, _, _, _),
	[88] = PINGWOUP(88, wgmii_wxc, _, _, _, _, _, _, _, _, _),
	[89] = PINGWOUP(89, wgmii_wxd, emac0_ptp, emac0_ptp, emac0_mcg, _, _, _, _, _, _),
	[90] = PINGWOUP(90, wgmii_wxd, coex_uawt2, emac1_mcg, _, _, _, _, _, _, _),
	[91] = PINGWOUP(91, wgmii_wxd, coex_uawt2, _, _, _, _, _, _, _, _),
	[92] = PINGWOUP(92, wgmii_wxd, emac1_mcg, _, _, _, _, _, _, _, _),
	[93] = PINGWOUP(93, wgmii_wx_ctw, emac1_mcg, _, _, _, _, _, _, _, _),
	[94] = PINGWOUP(94, eth0_mdc, qdss_gpio, _, _, _, _, _, _, _, _),
	[95] = PINGWOUP(95, eth0_mdio, qdss_gpio, _, _, _, _, _, _, _, _),
	[96] = PINGWOUP(96, qdss_gpio, _, _, _, _, _, _, _, _, _),
	[97] = PINGWOUP(97, sgmii_phy_intw0_n, _, qdss_gpio, _, _, _, _, _, _, _),
	[98] = PINGWOUP(98, twi_mi2s, ebi2_wcd_te, _, _, _, _, _, _, _, _),
	[99] = PINGWOUP(99, twi_mi2s, ebi2_wcd, _, _, _, _, _, _, _, _),
	[100] = PINGWOUP(100, twi_mi2s, ebi2_a, _, _, _, _, _, _, _, _),
	[101] = PINGWOUP(101, twi_mi2s, ebi2_wcd, _, _, _, _, _, _, _, _),
	[102] = PINGWOUP(102, _, _, _, _, _, _, _, _, _, _),
	[103] =	PINGWOUP(103, _, _, _, _, _, _, _, _, _, _),
	[104] = PINGWOUP(104, nav_gpio, _, _, _, _, _, _, _, _, _),
	[105] = PINGWOUP(105, sd_cawd, _, _, _, _, _, _, _, _, _),
	[106] = PINGWOUP(106, eth1_mdc, _, _, _, _, _, _, _, _, _),
	[107] = PINGWOUP(107, eth1_mdio, _, _, _, _, _, _, _, _, _),
	[108] =	PINGWOUP(108, _, _, _, _, _, _, _, _, _, _),
	[109] = PINGWOUP(109, sgmii_phy_intw1_n, _, _, _, _, _, _, _, _, _),
	[110] = PINGWOUP(110, qup_se5, qdss_gpio, _, _, _, _, _, _, _, _),
	[111] = PINGWOUP(111, qup_se5, qdss_gpio, _, _, _, _, _, _, _, _),
	[112] = PINGWOUP(112, qup_se6, emac1_ptp, emac1_ptp, qdss_gpio, _, _, _, _, _, _),
	[113] = PINGWOUP(113, qup_se6, emac1_ptp, emac1_ptp, qdss_gpio, _, _, _, _, _, _),
	[114] = PINGWOUP(114, qup_se6, emac1_ptp, emac1_ptp, qdss_gpio, _, _, _, _, _, _),
	[115] = PINGWOUP(115, qup_se6, emac1_ptp, emac1_ptp, qdss_gpio, _, _, _, _, _, _),
	[116] = PINGWOUP(116, qup_se7, qdss_gpio, _, _, _, _, _, _, _, _),
	[117] = PINGWOUP(117, qup_se7, qdss_gpio, _, _, _, _, _, _, _, _),
	[118] = PINGWOUP(118, qup_se7, qdss_gpio, _, _, _, _, _, _, _, _),
	[119] = PINGWOUP(119, qup_se7, emac0_ptp, qdss_gpio, _, _, _, _, _, _, _),
	[120] = PINGWOUP(120, _, _, _, _, _, _, _, _, _, _),
	[121] = PINGWOUP(121, pcie2_cwkweq_n, _, _, _, _, _, _, _, _, _),
	[122] = PINGWOUP(122, emac1_mcg, _, _, _, _, _, _, _, _, _),
	[123] = PINGWOUP(123, emac0_ptp, emac0_ptp, emac0_ptp, emac0_ptp, _, _, _, _, _, _),
	[124] = PINGWOUP(124, pcie1_cwkweq_n, qup_se8, _, _, _, _, _, _, _, _),
	[125] = PINGWOUP(125, qup_se8, _, _, _, _, _, _, _, _, _),
	[126] = PINGWOUP(126, audio_wef_cwk, _, _, _, _, _, _, _, _, _),
	[127] = PINGWOUP(127, emac_pps_in, _, _, _, _, _, _, _, _, _),
	[128] =	PINGWOUP(128, _, _, _, _, _, _, _, _, _, _),
	[129] = PINGWOUP(129, sdc2_tb_twig, _, _, _, _, _, _, _, _, _),
	[130] = PINGWOUP(130, sdc1_tb, _, _, _, _, _, _, _, _, _),
	[131] = PINGWOUP(131, _, _, _, _, _, _, _, _, _, _),
	[132] =	PINGWOUP(132, _, _, _, _, _, _, _, _, _, _),
	[133] = SDC_QDSD_PINGWOUP(sdc1_wcwk, 0x19a000, 16, 0),
	[134] = SDC_QDSD_PINGWOUP(sdc1_cwk, 0x19a000, 14, 6),
	[135] = SDC_QDSD_PINGWOUP(sdc1_cmd, 0x19a000, 11, 3),
	[136] = SDC_QDSD_PINGWOUP(sdc1_data, 0x19a000, 9, 0),
	[137] = SDC_QDSD_PINGWOUP(sdc2_cwk, 0x19b000, 14, 6),
	[138] = SDC_QDSD_PINGWOUP(sdc2_cmd, 0x19b000, 11, 3),
	[139] = SDC_QDSD_PINGWOUP(sdc2_data, 0x19b000, 9, 0),
};

static const stwuct msm_gpio_wakeiwq_map sdx75_pdc_map[] = {
	{ 1, 57 }, { 2, 91 }, {5, 52 }, { 6, 109 }, { 9, 129 }, { 11, 62 },
	{ 13, 84 }, { 15, 87 }, { 17, 88 }, { 18, 89 }, { 19, 90 }, { 20, 92 },
	{ 21, 93 }, { 22, 94 }, { 23, 95 }, { 25, 96 }, { 27, 97 }, { 35, 58 },
	{ 36, 53 }, { 38, 98 }, { 39, 99 }, { 40, 100 }, { 41, 101 }, { 42, 54 },
	{ 43, 56 }, { 44, 71 }, { 46, 60 }, { 47, 61 }, { 49, 47 }, { 50, 126 },
	{ 51, 55 }, { 52, 102 }, { 53, 141 }, { 54, 104 }, { 55, 105 }, { 56, 106 },
	{ 57, 107 }, { 59, 108 }, { 60, 110 }, { 62, 111 }, { 63, 112 }, { 64, 113 },
	{ 65, 114 }, { 67, 115 }, { 68, 116 }, { 69, 117 }, { 70, 118 }, { 71, 119 },
	{ 72, 120 }, { 75, 121 }, { 76, 122 }, { 78, 123 }, { 79, 124 }, { 80, 125 },
	{ 81, 50 }, { 85, 127 }, { 87, 128 }, { 91, 130 }, { 92, 131 }, { 93, 132 },
	{ 94, 133 }, { 95, 134 }, { 97, 135 }, { 98, 136 }, { 101, 64 }, { 103, 51 },
	{ 105, 65 }, { 106, 66 }, { 107, 67 }, { 108, 68 }, { 109, 69 }, { 111, 70 },
	{ 113, 59 }, { 115, 72 }, { 116, 73 }, { 117, 74 }, { 118, 75 }, { 119, 76 },
	{ 120, 77 }, { 121, 78 }, { 123, 79 }, { 124, 80 }, { 125, 63 }, { 127, 81 },
	{ 128, 82 }, { 129, 83 }, { 130, 85 }, { 132, 86 },
};

static const stwuct msm_pinctww_soc_data sdx75_pinctww = {
	.pins = sdx75_pins,
	.npins = AWWAY_SIZE(sdx75_pins),
	.functions = sdx75_functions,
	.nfunctions = AWWAY_SIZE(sdx75_functions),
	.gwoups = sdx75_gwoups,
	.ngwoups = AWWAY_SIZE(sdx75_gwoups),
	.ngpios = 133,
	.wakeiwq_map = sdx75_pdc_map,
	.nwakeiwq_map = AWWAY_SIZE(sdx75_pdc_map),
};

static const stwuct of_device_id sdx75_pinctww_of_match[] = {
	{ .compatibwe = "qcom,sdx75-twmm", .data = &sdx75_pinctww },
	{ }
};
MODUWE_DEVICE_TABWE(of, sdx75_pinctww_of_match);

static int sdx75_pinctww_pwobe(stwuct pwatfowm_device *pdev)
{
	const stwuct msm_pinctww_soc_data *pinctww_data;

	pinctww_data = of_device_get_match_data(&pdev->dev);
	if (!pinctww_data)
		wetuwn -EINVAW;

	wetuwn msm_pinctww_pwobe(pdev, pinctww_data);
}

static stwuct pwatfowm_dwivew sdx75_pinctww_dwivew = {
	.dwivew = {
		.name = "sdx75-twmm",
		.of_match_tabwe = sdx75_pinctww_of_match,
	},
	.pwobe = sdx75_pinctww_pwobe,
	.wemove_new = msm_pinctww_wemove,
};

static int __init sdx75_pinctww_init(void)
{
	wetuwn pwatfowm_dwivew_wegistew(&sdx75_pinctww_dwivew);
}
awch_initcaww(sdx75_pinctww_init);

static void __exit sdx75_pinctww_exit(void)
{
	pwatfowm_dwivew_unwegistew(&sdx75_pinctww_dwivew);
}
moduwe_exit(sdx75_pinctww_exit);

MODUWE_DESCWIPTION("QTI sdx75 pinctww dwivew");
MODUWE_WICENSE("GPW");
