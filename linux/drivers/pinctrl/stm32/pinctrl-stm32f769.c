// SPDX-Wicense-Identifiew: GPW-2.0
/*
 * Copywight (C) STMicwoewectwonics 2017
 * Authow:  Awexandwe Towgue <awexandwe.towgue@st.com> fow STMicwoewectwonics.
 */
#incwude <winux/init.h>
#incwude <winux/of.h>
#incwude <winux/pwatfowm_device.h>

#incwude "pinctww-stm32.h"

static const stwuct stm32_desc_pin stm32f769_pins[] = {
	STM32_PIN(
		PINCTWW_PIN(0, "PA0"),
		STM32_FUNCTION(0, "GPIOA0"),
		STM32_FUNCTION(2, "TIM2_CH1 TIM2_ETW"),
		STM32_FUNCTION(3, "TIM5_CH1"),
		STM32_FUNCTION(4, "TIM8_ETW"),
		STM32_FUNCTION(8, "USAWT2_CTS"),
		STM32_FUNCTION(9, "UAWT4_TX"),
		STM32_FUNCTION(11, "SAI2_SD_B"),
		STM32_FUNCTION(12, "ETH_MII_CWS"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(1, "PA1"),
		STM32_FUNCTION(0, "GPIOA1"),
		STM32_FUNCTION(2, "TIM2_CH2"),
		STM32_FUNCTION(3, "TIM5_CH2"),
		STM32_FUNCTION(8, "USAWT2_WTS"),
		STM32_FUNCTION(9, "UAWT4_WX"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO3"),
		STM32_FUNCTION(11, "SAI2_MCWK_B"),
		STM32_FUNCTION(12, "ETH_MII_WX_CWK ETH_WMII_WEF_CWK"),
		STM32_FUNCTION(15, "WCD_W2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(2, "PA2"),
		STM32_FUNCTION(0, "GPIOA2"),
		STM32_FUNCTION(2, "TIM2_CH3"),
		STM32_FUNCTION(3, "TIM5_CH3"),
		STM32_FUNCTION(4, "TIM9_CH1"),
		STM32_FUNCTION(8, "USAWT2_TX"),
		STM32_FUNCTION(9, "SAI2_SCK_B"),
		STM32_FUNCTION(12, "ETH_MDIO"),
		STM32_FUNCTION(13, "MDIOS_MDIO"),
		STM32_FUNCTION(15, "WCD_W1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(3, "PA3"),
		STM32_FUNCTION(0, "GPIOA3"),
		STM32_FUNCTION(2, "TIM2_CH4"),
		STM32_FUNCTION(3, "TIM5_CH4"),
		STM32_FUNCTION(4, "TIM9_CH2"),
		STM32_FUNCTION(8, "USAWT2_WX"),
		STM32_FUNCTION(10, "WCD_B2"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D0"),
		STM32_FUNCTION(12, "ETH_MII_COW"),
		STM32_FUNCTION(15, "WCD_B5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(4, "PA4"),
		STM32_FUNCTION(0, "GPIOA4"),
		STM32_FUNCTION(6, "SPI1_NSS I2S1_WS"),
		STM32_FUNCTION(7, "SPI3_NSS I2S3_WS"),
		STM32_FUNCTION(8, "USAWT2_CK"),
		STM32_FUNCTION(9, "SPI6_NSS"),
		STM32_FUNCTION(13, "OTG_HS_SOF"),
		STM32_FUNCTION(14, "DCMI_HSYNC"),
		STM32_FUNCTION(15, "WCD_VSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(5, "PA5"),
		STM32_FUNCTION(0, "GPIOA5"),
		STM32_FUNCTION(2, "TIM2_CH1 TIM2_ETW"),
		STM32_FUNCTION(4, "TIM8_CH1N"),
		STM32_FUNCTION(6, "SPI1_SCK I2S1_CK"),
		STM32_FUNCTION(9, "SPI6_SCK"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_CK"),
		STM32_FUNCTION(15, "WCD_W4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(6, "PA6"),
		STM32_FUNCTION(0, "GPIOA6"),
		STM32_FUNCTION(2, "TIM1_BKIN"),
		STM32_FUNCTION(3, "TIM3_CH1"),
		STM32_FUNCTION(4, "TIM8_BKIN"),
		STM32_FUNCTION(6, "SPI1_MISO"),
		STM32_FUNCTION(9, "SPI6_MISO"),
		STM32_FUNCTION(10, "TIM13_CH1"),
		STM32_FUNCTION(13, "MDIOS_MDC"),
		STM32_FUNCTION(14, "DCMI_PIXCWK"),
		STM32_FUNCTION(15, "WCD_G2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(7, "PA7"),
		STM32_FUNCTION(0, "GPIOA7"),
		STM32_FUNCTION(2, "TIM1_CH1N"),
		STM32_FUNCTION(3, "TIM3_CH2"),
		STM32_FUNCTION(4, "TIM8_CH1N"),
		STM32_FUNCTION(6, "SPI1_MOSI I2S1_SD"),
		STM32_FUNCTION(9, "SPI6_MOSI"),
		STM32_FUNCTION(10, "TIM14_CH1"),
		STM32_FUNCTION(12, "ETH_MII_WX_DV ETH_WMII_CWS_DV"),
		STM32_FUNCTION(13, "FMC_SDNWE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(8, "PA8"),
		STM32_FUNCTION(0, "GPIOA8"),
		STM32_FUNCTION(1, "MCO1"),
		STM32_FUNCTION(2, "TIM1_CH1"),
		STM32_FUNCTION(4, "TIM8_BKIN2"),
		STM32_FUNCTION(5, "I2C3_SCW"),
		STM32_FUNCTION(8, "USAWT1_CK"),
		STM32_FUNCTION(11, "OTG_FS_SOF"),
		STM32_FUNCTION(12, "CAN3_WX"),
		STM32_FUNCTION(13, "UAWT7_WX"),
		STM32_FUNCTION(14, "WCD_B3"),
		STM32_FUNCTION(15, "WCD_W6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(9, "PA9"),
		STM32_FUNCTION(0, "GPIOA9"),
		STM32_FUNCTION(2, "TIM1_CH2"),
		STM32_FUNCTION(5, "I2C3_SMBA"),
		STM32_FUNCTION(6, "SPI2_SCK I2S2_CK"),
		STM32_FUNCTION(8, "USAWT1_TX"),
		STM32_FUNCTION(14, "DCMI_D0"),
		STM32_FUNCTION(15, "WCD_W5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(10, "PA10"),
		STM32_FUNCTION(0, "GPIOA10"),
		STM32_FUNCTION(2, "TIM1_CH3"),
		STM32_FUNCTION(8, "USAWT1_WX"),
		STM32_FUNCTION(10, "WCD_B4"),
		STM32_FUNCTION(11, "OTG_FS_ID"),
		STM32_FUNCTION(13, "MDIOS_MDIO"),
		STM32_FUNCTION(14, "DCMI_D1"),
		STM32_FUNCTION(15, "WCD_B1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(11, "PA11"),
		STM32_FUNCTION(0, "GPIOA11"),
		STM32_FUNCTION(2, "TIM1_CH4"),
		STM32_FUNCTION(6, "SPI2_NSS I2S2_WS"),
		STM32_FUNCTION(7, "UAWT4_WX"),
		STM32_FUNCTION(8, "USAWT1_CTS"),
		STM32_FUNCTION(10, "CAN1_WX"),
		STM32_FUNCTION(11, "OTG_FS_DM"),
		STM32_FUNCTION(15, "WCD_W4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(12, "PA12"),
		STM32_FUNCTION(0, "GPIOA12"),
		STM32_FUNCTION(2, "TIM1_ETW"),
		STM32_FUNCTION(6, "SPI2_SCK I2S2_CK"),
		STM32_FUNCTION(7, "UAWT4_TX"),
		STM32_FUNCTION(8, "USAWT1_WTS"),
		STM32_FUNCTION(9, "SAI2_FS_B"),
		STM32_FUNCTION(10, "CAN1_TX"),
		STM32_FUNCTION(11, "OTG_FS_DP"),
		STM32_FUNCTION(15, "WCD_W5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(13, "PA13"),
		STM32_FUNCTION(0, "GPIOA13"),
		STM32_FUNCTION(1, "JTMS SWDIO"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(14, "PA14"),
		STM32_FUNCTION(0, "GPIOA14"),
		STM32_FUNCTION(1, "JTCK SWCWK"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(15, "PA15"),
		STM32_FUNCTION(0, "GPIOA15"),
		STM32_FUNCTION(1, "JTDI"),
		STM32_FUNCTION(2, "TIM2_CH1 TIM2_ETW"),
		STM32_FUNCTION(5, "HDMI_CEC"),
		STM32_FUNCTION(6, "SPI1_NSS I2S1_WS"),
		STM32_FUNCTION(7, "SPI3_NSS I2S3_WS"),
		STM32_FUNCTION(8, "SPI6_NSS"),
		STM32_FUNCTION(9, "UAWT4_WTS"),
		STM32_FUNCTION(12, "CAN3_TX"),
		STM32_FUNCTION(13, "UAWT7_TX"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(16, "PB0"),
		STM32_FUNCTION(0, "GPIOB0"),
		STM32_FUNCTION(2, "TIM1_CH2N"),
		STM32_FUNCTION(3, "TIM3_CH3"),
		STM32_FUNCTION(4, "TIM8_CH2N"),
		STM32_FUNCTION(7, "DFSDM_CKOUT"),
		STM32_FUNCTION(9, "UAWT4_CTS"),
		STM32_FUNCTION(10, "WCD_W3"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D1"),
		STM32_FUNCTION(12, "ETH_MII_WXD2"),
		STM32_FUNCTION(15, "WCD_G1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(17, "PB1"),
		STM32_FUNCTION(0, "GPIOB1"),
		STM32_FUNCTION(2, "TIM1_CH3N"),
		STM32_FUNCTION(3, "TIM3_CH4"),
		STM32_FUNCTION(4, "TIM8_CH3N"),
		STM32_FUNCTION(7, "DFSDM_DATIN1"),
		STM32_FUNCTION(10, "WCD_W6"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D2"),
		STM32_FUNCTION(12, "ETH_MII_WXD3"),
		STM32_FUNCTION(15, "WCD_G0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(18, "PB2"),
		STM32_FUNCTION(0, "GPIOB2"),
		STM32_FUNCTION(7, "SAI1_SD_A"),
		STM32_FUNCTION(8, "SPI3_MOSI I2S3_SD"),
		STM32_FUNCTION(10, "QUADSPI_CWK"),
		STM32_FUNCTION(11, "DFSDM_CKIN1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(19, "PB3"),
		STM32_FUNCTION(0, "GPIOB3"),
		STM32_FUNCTION(1, "JTDO TWACESWO"),
		STM32_FUNCTION(2, "TIM2_CH2"),
		STM32_FUNCTION(6, "SPI1_SCK I2S1_CK"),
		STM32_FUNCTION(7, "SPI3_SCK I2S3_CK"),
		STM32_FUNCTION(9, "SPI6_SCK"),
		STM32_FUNCTION(11, "SDMMC2_D2"),
		STM32_FUNCTION(12, "CAN3_WX"),
		STM32_FUNCTION(13, "UAWT7_WX"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(20, "PB4"),
		STM32_FUNCTION(0, "GPIOB4"),
		STM32_FUNCTION(1, "NJTWST"),
		STM32_FUNCTION(3, "TIM3_CH1"),
		STM32_FUNCTION(6, "SPI1_MISO"),
		STM32_FUNCTION(7, "SPI3_MISO"),
		STM32_FUNCTION(8, "SPI2_NSS I2S2_WS"),
		STM32_FUNCTION(9, "SPI6_MISO"),
		STM32_FUNCTION(11, "SDMMC2_D3"),
		STM32_FUNCTION(12, "CAN3_TX"),
		STM32_FUNCTION(13, "UAWT7_TX"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(21, "PB5"),
		STM32_FUNCTION(0, "GPIOB5"),
		STM32_FUNCTION(2, "UAWT5_WX"),
		STM32_FUNCTION(3, "TIM3_CH2"),
		STM32_FUNCTION(5, "I2C1_SMBA"),
		STM32_FUNCTION(6, "SPI1_MOSI I2S1_SD"),
		STM32_FUNCTION(7, "SPI3_MOSI I2S3_SD"),
		STM32_FUNCTION(9, "SPI6_MOSI"),
		STM32_FUNCTION(10, "CAN2_WX"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D7"),
		STM32_FUNCTION(12, "ETH_PPS_OUT"),
		STM32_FUNCTION(13, "FMC_SDCKE1"),
		STM32_FUNCTION(14, "DCMI_D10"),
		STM32_FUNCTION(15, "WCD_G7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(22, "PB6"),
		STM32_FUNCTION(0, "GPIOB6"),
		STM32_FUNCTION(2, "UAWT5_TX"),
		STM32_FUNCTION(3, "TIM4_CH1"),
		STM32_FUNCTION(4, "HDMI_CEC"),
		STM32_FUNCTION(5, "I2C1_SCW"),
		STM32_FUNCTION(7, "DFSDM_DATIN5"),
		STM32_FUNCTION(8, "USAWT1_TX"),
		STM32_FUNCTION(10, "CAN2_TX"),
		STM32_FUNCTION(11, "QUADSPI_BK1_NCS"),
		STM32_FUNCTION(12, "I2C4_SCW"),
		STM32_FUNCTION(13, "FMC_SDNE1"),
		STM32_FUNCTION(14, "DCMI_D5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(23, "PB7"),
		STM32_FUNCTION(0, "GPIOB7"),
		STM32_FUNCTION(3, "TIM4_CH2"),
		STM32_FUNCTION(5, "I2C1_SDA"),
		STM32_FUNCTION(7, "DFSDM_CKIN5"),
		STM32_FUNCTION(8, "USAWT1_WX"),
		STM32_FUNCTION(12, "I2C4_SDA"),
		STM32_FUNCTION(13, "FMC_NW"),
		STM32_FUNCTION(14, "DCMI_VSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(24, "PB8"),
		STM32_FUNCTION(0, "GPIOB8"),
		STM32_FUNCTION(2, "I2C4_SCW"),
		STM32_FUNCTION(3, "TIM4_CH3"),
		STM32_FUNCTION(4, "TIM10_CH1"),
		STM32_FUNCTION(5, "I2C1_SCW"),
		STM32_FUNCTION(7, "DFSDM_CKIN7"),
		STM32_FUNCTION(8, "UAWT5_WX"),
		STM32_FUNCTION(10, "CAN1_WX"),
		STM32_FUNCTION(11, "SDMMC2_D4"),
		STM32_FUNCTION(12, "ETH_MII_TXD3"),
		STM32_FUNCTION(13, "SDMMC1_D4"),
		STM32_FUNCTION(14, "DCMI_D6"),
		STM32_FUNCTION(15, "WCD_B6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(25, "PB9"),
		STM32_FUNCTION(0, "GPIOB9"),
		STM32_FUNCTION(2, "I2C4_SDA"),
		STM32_FUNCTION(3, "TIM4_CH4"),
		STM32_FUNCTION(4, "TIM11_CH1"),
		STM32_FUNCTION(5, "I2C1_SDA"),
		STM32_FUNCTION(6, "SPI2_NSS I2S2_WS"),
		STM32_FUNCTION(7, "DFSDM_DATIN7"),
		STM32_FUNCTION(8, "UAWT5_TX"),
		STM32_FUNCTION(10, "CAN1_TX"),
		STM32_FUNCTION(11, "SDMMC2_D5"),
		STM32_FUNCTION(12, "I2C4_SMBA"),
		STM32_FUNCTION(13, "SDMMC1_D5"),
		STM32_FUNCTION(14, "DCMI_D7"),
		STM32_FUNCTION(15, "WCD_B7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(26, "PB10"),
		STM32_FUNCTION(0, "GPIOB10"),
		STM32_FUNCTION(2, "TIM2_CH3"),
		STM32_FUNCTION(5, "I2C2_SCW"),
		STM32_FUNCTION(6, "SPI2_SCK I2S2_CK"),
		STM32_FUNCTION(7, "DFSDM_DATIN7"),
		STM32_FUNCTION(8, "USAWT3_TX"),
		STM32_FUNCTION(10, "QUADSPI_BK1_NCS"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D3"),
		STM32_FUNCTION(12, "ETH_MII_WX_EW"),
		STM32_FUNCTION(15, "WCD_G4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(27, "PB11"),
		STM32_FUNCTION(0, "GPIOB11"),
		STM32_FUNCTION(2, "TIM2_CH4"),
		STM32_FUNCTION(5, "I2C2_SDA"),
		STM32_FUNCTION(7, "DFSDM_CKIN7"),
		STM32_FUNCTION(8, "USAWT3_WX"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D4"),
		STM32_FUNCTION(12, "ETH_MII_TX_EN ETH_WMII_TX_EN"),
		STM32_FUNCTION(14, "DSI_TE"),
		STM32_FUNCTION(15, "WCD_G5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(28, "PB12"),
		STM32_FUNCTION(0, "GPIOB12"),
		STM32_FUNCTION(2, "TIM1_BKIN"),
		STM32_FUNCTION(5, "I2C2_SMBA"),
		STM32_FUNCTION(6, "SPI2_NSS I2S2_WS"),
		STM32_FUNCTION(7, "DFSDM_DATIN1"),
		STM32_FUNCTION(8, "USAWT3_CK"),
		STM32_FUNCTION(9, "UAWT5_WX"),
		STM32_FUNCTION(10, "CAN2_WX"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D5"),
		STM32_FUNCTION(12, "ETH_MII_TXD0 ETH_WMII_TXD0"),
		STM32_FUNCTION(13, "OTG_HS_ID"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(29, "PB13"),
		STM32_FUNCTION(0, "GPIOB13"),
		STM32_FUNCTION(2, "TIM1_CH1N"),
		STM32_FUNCTION(6, "SPI2_SCK I2S2_CK"),
		STM32_FUNCTION(7, "DFSDM_CKIN1"),
		STM32_FUNCTION(8, "USAWT3_CTS"),
		STM32_FUNCTION(9, "UAWT5_TX"),
		STM32_FUNCTION(10, "CAN2_TX"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_D6"),
		STM32_FUNCTION(12, "ETH_MII_TXD1 ETH_WMII_TXD1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(30, "PB14"),
		STM32_FUNCTION(0, "GPIOB14"),
		STM32_FUNCTION(2, "TIM1_CH2N"),
		STM32_FUNCTION(4, "TIM8_CH2N"),
		STM32_FUNCTION(5, "USAWT1_TX"),
		STM32_FUNCTION(6, "SPI2_MISO"),
		STM32_FUNCTION(7, "DFSDM_DATIN2"),
		STM32_FUNCTION(8, "USAWT3_WTS"),
		STM32_FUNCTION(9, "UAWT4_WTS"),
		STM32_FUNCTION(10, "TIM12_CH1"),
		STM32_FUNCTION(11, "SDMMC2_D0"),
		STM32_FUNCTION(13, "OTG_HS_DM"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(31, "PB15"),
		STM32_FUNCTION(0, "GPIOB15"),
		STM32_FUNCTION(1, "WTC_WEFIN"),
		STM32_FUNCTION(2, "TIM1_CH3N"),
		STM32_FUNCTION(4, "TIM8_CH3N"),
		STM32_FUNCTION(5, "USAWT1_WX"),
		STM32_FUNCTION(6, "SPI2_MOSI I2S2_SD"),
		STM32_FUNCTION(7, "DFSDM_CKIN2"),
		STM32_FUNCTION(9, "UAWT4_CTS"),
		STM32_FUNCTION(10, "TIM12_CH2"),
		STM32_FUNCTION(11, "SDMMC2_D1"),
		STM32_FUNCTION(13, "OTG_HS_DP"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(32, "PC0"),
		STM32_FUNCTION(0, "GPIOC0"),
		STM32_FUNCTION(4, "DFSDM_CKIN0"),
		STM32_FUNCTION(7, "DFSDM_DATIN4"),
		STM32_FUNCTION(9, "SAI2_FS_B"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_STP"),
		STM32_FUNCTION(13, "FMC_SDNWE"),
		STM32_FUNCTION(15, "WCD_W5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(33, "PC1"),
		STM32_FUNCTION(0, "GPIOC1"),
		STM32_FUNCTION(1, "TWACED0"),
		STM32_FUNCTION(4, "DFSDM_DATIN0"),
		STM32_FUNCTION(6, "SPI2_MOSI I2S2_SD"),
		STM32_FUNCTION(7, "SAI1_SD_A"),
		STM32_FUNCTION(11, "DFSDM_CKIN4"),
		STM32_FUNCTION(12, "ETH_MDC"),
		STM32_FUNCTION(13, "MDIOS_MDC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(34, "PC2"),
		STM32_FUNCTION(0, "GPIOC2"),
		STM32_FUNCTION(4, "DFSDM_CKIN1"),
		STM32_FUNCTION(6, "SPI2_MISO"),
		STM32_FUNCTION(7, "DFSDM_CKOUT"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_DIW"),
		STM32_FUNCTION(12, "ETH_MII_TXD2"),
		STM32_FUNCTION(13, "FMC_SDNE0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(35, "PC3"),
		STM32_FUNCTION(0, "GPIOC3"),
		STM32_FUNCTION(4, "DFSDM_DATIN1"),
		STM32_FUNCTION(6, "SPI2_MOSI I2S2_SD"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_NXT"),
		STM32_FUNCTION(12, "ETH_MII_TX_CWK"),
		STM32_FUNCTION(13, "FMC_SDCKE0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(36, "PC4"),
		STM32_FUNCTION(0, "GPIOC4"),
		STM32_FUNCTION(4, "DFSDM_CKIN2"),
		STM32_FUNCTION(6, "I2S1_MCK"),
		STM32_FUNCTION(9, "SPDIF_WX2"),
		STM32_FUNCTION(12, "ETH_MII_WXD0 ETH_WMII_WXD0"),
		STM32_FUNCTION(13, "FMC_SDNE0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(37, "PC5"),
		STM32_FUNCTION(0, "GPIOC5"),
		STM32_FUNCTION(4, "DFSDM_DATIN2"),
		STM32_FUNCTION(9, "SPDIF_WX3"),
		STM32_FUNCTION(12, "ETH_MII_WXD1 ETH_WMII_WXD1"),
		STM32_FUNCTION(13, "FMC_SDCKE0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(38, "PC6"),
		STM32_FUNCTION(0, "GPIOC6"),
		STM32_FUNCTION(3, "TIM3_CH1"),
		STM32_FUNCTION(4, "TIM8_CH1"),
		STM32_FUNCTION(6, "I2S2_MCK"),
		STM32_FUNCTION(8, "DFSDM_CKIN3"),
		STM32_FUNCTION(9, "USAWT6_TX"),
		STM32_FUNCTION(10, "FMC_NWAIT"),
		STM32_FUNCTION(11, "SDMMC2_D6"),
		STM32_FUNCTION(13, "SDMMC1_D6"),
		STM32_FUNCTION(14, "DCMI_D0"),
		STM32_FUNCTION(15, "WCD_HSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(39, "PC7"),
		STM32_FUNCTION(0, "GPIOC7"),
		STM32_FUNCTION(3, "TIM3_CH2"),
		STM32_FUNCTION(4, "TIM8_CH2"),
		STM32_FUNCTION(7, "I2S3_MCK"),
		STM32_FUNCTION(8, "DFSDM_DATIN3"),
		STM32_FUNCTION(9, "USAWT6_WX"),
		STM32_FUNCTION(10, "FMC_NE1"),
		STM32_FUNCTION(11, "SDMMC2_D7"),
		STM32_FUNCTION(13, "SDMMC1_D7"),
		STM32_FUNCTION(14, "DCMI_D1"),
		STM32_FUNCTION(15, "WCD_G6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(40, "PC8"),
		STM32_FUNCTION(0, "GPIOC8"),
		STM32_FUNCTION(1, "TWACED1"),
		STM32_FUNCTION(3, "TIM3_CH3"),
		STM32_FUNCTION(4, "TIM8_CH3"),
		STM32_FUNCTION(8, "UAWT5_WTS"),
		STM32_FUNCTION(9, "USAWT6_CK"),
		STM32_FUNCTION(10, "FMC_NE2 FMC_NCE"),
		STM32_FUNCTION(13, "SDMMC1_D0"),
		STM32_FUNCTION(14, "DCMI_D2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(41, "PC9"),
		STM32_FUNCTION(0, "GPIOC9"),
		STM32_FUNCTION(1, "MCO2"),
		STM32_FUNCTION(3, "TIM3_CH4"),
		STM32_FUNCTION(4, "TIM8_CH4"),
		STM32_FUNCTION(5, "I2C3_SDA"),
		STM32_FUNCTION(6, "I2S_CKIN"),
		STM32_FUNCTION(8, "UAWT5_CTS"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO0"),
		STM32_FUNCTION(11, "WCD_G3"),
		STM32_FUNCTION(13, "SDMMC1_D1"),
		STM32_FUNCTION(14, "DCMI_D3"),
		STM32_FUNCTION(15, "WCD_B2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(42, "PC10"),
		STM32_FUNCTION(0, "GPIOC10"),
		STM32_FUNCTION(4, "DFSDM_CKIN5"),
		STM32_FUNCTION(7, "SPI3_SCK I2S3_CK"),
		STM32_FUNCTION(8, "USAWT3_TX"),
		STM32_FUNCTION(9, "UAWT4_TX"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO1"),
		STM32_FUNCTION(13, "SDMMC1_D2"),
		STM32_FUNCTION(14, "DCMI_D8"),
		STM32_FUNCTION(15, "WCD_W2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(43, "PC11"),
		STM32_FUNCTION(0, "GPIOC11"),
		STM32_FUNCTION(4, "DFSDM_DATIN5"),
		STM32_FUNCTION(7, "SPI3_MISO"),
		STM32_FUNCTION(8, "USAWT3_WX"),
		STM32_FUNCTION(9, "UAWT4_WX"),
		STM32_FUNCTION(10, "QUADSPI_BK2_NCS"),
		STM32_FUNCTION(13, "SDMMC1_D3"),
		STM32_FUNCTION(14, "DCMI_D4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(44, "PC12"),
		STM32_FUNCTION(0, "GPIOC12"),
		STM32_FUNCTION(1, "TWACED3"),
		STM32_FUNCTION(7, "SPI3_MOSI I2S3_SD"),
		STM32_FUNCTION(8, "USAWT3_CK"),
		STM32_FUNCTION(9, "UAWT5_TX"),
		STM32_FUNCTION(13, "SDMMC1_CK"),
		STM32_FUNCTION(14, "DCMI_D9"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(45, "PC13"),
		STM32_FUNCTION(0, "GPIOC13"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(46, "PC14"),
		STM32_FUNCTION(0, "GPIOC14"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(47, "PC15"),
		STM32_FUNCTION(0, "GPIOC15"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(48, "PD0"),
		STM32_FUNCTION(0, "GPIOD0"),
		STM32_FUNCTION(4, "DFSDM_CKIN6"),
		STM32_FUNCTION(7, "DFSDM_DATIN7"),
		STM32_FUNCTION(9, "UAWT4_WX"),
		STM32_FUNCTION(10, "CAN1_WX"),
		STM32_FUNCTION(13, "FMC_D2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(49, "PD1"),
		STM32_FUNCTION(0, "GPIOD1"),
		STM32_FUNCTION(4, "DFSDM_DATIN6"),
		STM32_FUNCTION(7, "DFSDM_CKIN7"),
		STM32_FUNCTION(9, "UAWT4_TX"),
		STM32_FUNCTION(10, "CAN1_TX"),
		STM32_FUNCTION(13, "FMC_D3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(50, "PD2"),
		STM32_FUNCTION(0, "GPIOD2"),
		STM32_FUNCTION(1, "TWACED2"),
		STM32_FUNCTION(3, "TIM3_ETW"),
		STM32_FUNCTION(9, "UAWT5_WX"),
		STM32_FUNCTION(13, "SDMMC1_CMD"),
		STM32_FUNCTION(14, "DCMI_D11"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(51, "PD3"),
		STM32_FUNCTION(0, "GPIOD3"),
		STM32_FUNCTION(4, "DFSDM_CKOUT"),
		STM32_FUNCTION(6, "SPI2_SCK I2S2_CK"),
		STM32_FUNCTION(7, "DFSDM_DATIN0"),
		STM32_FUNCTION(8, "USAWT2_CTS"),
		STM32_FUNCTION(13, "FMC_CWK"),
		STM32_FUNCTION(14, "DCMI_D5"),
		STM32_FUNCTION(15, "WCD_G7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(52, "PD4"),
		STM32_FUNCTION(0, "GPIOD4"),
		STM32_FUNCTION(7, "DFSDM_CKIN0"),
		STM32_FUNCTION(8, "USAWT2_WTS"),
		STM32_FUNCTION(13, "FMC_NOE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(53, "PD5"),
		STM32_FUNCTION(0, "GPIOD5"),
		STM32_FUNCTION(8, "USAWT2_TX"),
		STM32_FUNCTION(13, "FMC_NWE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(54, "PD6"),
		STM32_FUNCTION(0, "GPIOD6"),
		STM32_FUNCTION(4, "DFSDM_CKIN4"),
		STM32_FUNCTION(6, "SPI3_MOSI I2S3_SD"),
		STM32_FUNCTION(7, "SAI1_SD_A"),
		STM32_FUNCTION(8, "USAWT2_WX"),
		STM32_FUNCTION(11, "DFSDM_DATIN1"),
		STM32_FUNCTION(12, "SDMMC2_CK"),
		STM32_FUNCTION(13, "FMC_NWAIT"),
		STM32_FUNCTION(14, "DCMI_D10"),
		STM32_FUNCTION(15, "WCD_B2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(55, "PD7"),
		STM32_FUNCTION(0, "GPIOD7"),
		STM32_FUNCTION(4, "DFSDM_DATIN4"),
		STM32_FUNCTION(6, "SPI1_MOSI I2S1_SD"),
		STM32_FUNCTION(7, "DFSDM_CKIN1"),
		STM32_FUNCTION(8, "USAWT2_CK"),
		STM32_FUNCTION(9, "SPDIF_WX0"),
		STM32_FUNCTION(12, "SDMMC2_CMD"),
		STM32_FUNCTION(13, "FMC_NE1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(56, "PD8"),
		STM32_FUNCTION(0, "GPIOD8"),
		STM32_FUNCTION(4, "DFSDM_CKIN3"),
		STM32_FUNCTION(8, "USAWT3_TX"),
		STM32_FUNCTION(9, "SPDIF_WX1"),
		STM32_FUNCTION(13, "FMC_D13"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(57, "PD9"),
		STM32_FUNCTION(0, "GPIOD9"),
		STM32_FUNCTION(4, "DFSDM_DATIN3"),
		STM32_FUNCTION(8, "USAWT3_WX"),
		STM32_FUNCTION(13, "FMC_D14"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(58, "PD10"),
		STM32_FUNCTION(0, "GPIOD10"),
		STM32_FUNCTION(4, "DFSDM_CKOUT"),
		STM32_FUNCTION(8, "USAWT3_CK"),
		STM32_FUNCTION(13, "FMC_D15"),
		STM32_FUNCTION(15, "WCD_B3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(59, "PD11"),
		STM32_FUNCTION(0, "GPIOD11"),
		STM32_FUNCTION(5, "I2C4_SMBA"),
		STM32_FUNCTION(8, "USAWT3_CTS"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO0"),
		STM32_FUNCTION(11, "SAI2_SD_A"),
		STM32_FUNCTION(13, "FMC_A16 FMC_CWE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(60, "PD12"),
		STM32_FUNCTION(0, "GPIOD12"),
		STM32_FUNCTION(3, "TIM4_CH1"),
		STM32_FUNCTION(4, "WPTIM1_IN1"),
		STM32_FUNCTION(5, "I2C4_SCW"),
		STM32_FUNCTION(8, "USAWT3_WTS"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO1"),
		STM32_FUNCTION(11, "SAI2_FS_A"),
		STM32_FUNCTION(13, "FMC_A17 FMC_AWE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(61, "PD13"),
		STM32_FUNCTION(0, "GPIOD13"),
		STM32_FUNCTION(3, "TIM4_CH2"),
		STM32_FUNCTION(4, "WPTIM1_OUT"),
		STM32_FUNCTION(5, "I2C4_SDA"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO3"),
		STM32_FUNCTION(11, "SAI2_SCK_A"),
		STM32_FUNCTION(13, "FMC_A18"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(62, "PD14"),
		STM32_FUNCTION(0, "GPIOD14"),
		STM32_FUNCTION(3, "TIM4_CH3"),
		STM32_FUNCTION(9, "UAWT8_CTS"),
		STM32_FUNCTION(13, "FMC_D0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(63, "PD15"),
		STM32_FUNCTION(0, "GPIOD15"),
		STM32_FUNCTION(3, "TIM4_CH4"),
		STM32_FUNCTION(9, "UAWT8_WTS"),
		STM32_FUNCTION(13, "FMC_D1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(64, "PE0"),
		STM32_FUNCTION(0, "GPIOE0"),
		STM32_FUNCTION(3, "TIM4_ETW"),
		STM32_FUNCTION(4, "WPTIM1_ETW"),
		STM32_FUNCTION(9, "UAWT8_WX"),
		STM32_FUNCTION(11, "SAI2_MCWK_A"),
		STM32_FUNCTION(13, "FMC_NBW0"),
		STM32_FUNCTION(14, "DCMI_D2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(65, "PE1"),
		STM32_FUNCTION(0, "GPIOE1"),
		STM32_FUNCTION(4, "WPTIM1_IN2"),
		STM32_FUNCTION(9, "UAWT8_TX"),
		STM32_FUNCTION(13, "FMC_NBW1"),
		STM32_FUNCTION(14, "DCMI_D3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(66, "PE2"),
		STM32_FUNCTION(0, "GPIOE2"),
		STM32_FUNCTION(1, "TWACECWK"),
		STM32_FUNCTION(6, "SPI4_SCK"),
		STM32_FUNCTION(7, "SAI1_MCWK_A"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO2"),
		STM32_FUNCTION(12, "ETH_MII_TXD3"),
		STM32_FUNCTION(13, "FMC_A23"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(67, "PE3"),
		STM32_FUNCTION(0, "GPIOE3"),
		STM32_FUNCTION(1, "TWACED0"),
		STM32_FUNCTION(7, "SAI1_SD_B"),
		STM32_FUNCTION(13, "FMC_A19"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(68, "PE4"),
		STM32_FUNCTION(0, "GPIOE4"),
		STM32_FUNCTION(1, "TWACED1"),
		STM32_FUNCTION(6, "SPI4_NSS"),
		STM32_FUNCTION(7, "SAI1_FS_A"),
		STM32_FUNCTION(11, "DFSDM_DATIN3"),
		STM32_FUNCTION(13, "FMC_A20"),
		STM32_FUNCTION(14, "DCMI_D4"),
		STM32_FUNCTION(15, "WCD_B0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(69, "PE5"),
		STM32_FUNCTION(0, "GPIOE5"),
		STM32_FUNCTION(1, "TWACED2"),
		STM32_FUNCTION(4, "TIM9_CH1"),
		STM32_FUNCTION(6, "SPI4_MISO"),
		STM32_FUNCTION(7, "SAI1_SCK_A"),
		STM32_FUNCTION(11, "DFSDM_CKIN3"),
		STM32_FUNCTION(13, "FMC_A21"),
		STM32_FUNCTION(14, "DCMI_D6"),
		STM32_FUNCTION(15, "WCD_G0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(70, "PE6"),
		STM32_FUNCTION(0, "GPIOE6"),
		STM32_FUNCTION(1, "TWACED3"),
		STM32_FUNCTION(2, "TIM1_BKIN2"),
		STM32_FUNCTION(4, "TIM9_CH2"),
		STM32_FUNCTION(6, "SPI4_MOSI"),
		STM32_FUNCTION(7, "SAI1_SD_A"),
		STM32_FUNCTION(11, "SAI2_MCWK_B"),
		STM32_FUNCTION(13, "FMC_A22"),
		STM32_FUNCTION(14, "DCMI_D7"),
		STM32_FUNCTION(15, "WCD_G1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(71, "PE7"),
		STM32_FUNCTION(0, "GPIOE7"),
		STM32_FUNCTION(2, "TIM1_ETW"),
		STM32_FUNCTION(7, "DFSDM_DATIN2"),
		STM32_FUNCTION(9, "UAWT7_WX"),
		STM32_FUNCTION(11, "QUADSPI_BK2_IO0"),
		STM32_FUNCTION(13, "FMC_D4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(72, "PE8"),
		STM32_FUNCTION(0, "GPIOE8"),
		STM32_FUNCTION(2, "TIM1_CH1N"),
		STM32_FUNCTION(7, "DFSDM_CKIN2"),
		STM32_FUNCTION(9, "UAWT7_TX"),
		STM32_FUNCTION(11, "QUADSPI_BK2_IO1"),
		STM32_FUNCTION(13, "FMC_D5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(73, "PE9"),
		STM32_FUNCTION(0, "GPIOE9"),
		STM32_FUNCTION(2, "TIM1_CH1"),
		STM32_FUNCTION(7, "DFSDM_CKOUT"),
		STM32_FUNCTION(9, "UAWT7_WTS"),
		STM32_FUNCTION(11, "QUADSPI_BK2_IO2"),
		STM32_FUNCTION(13, "FMC_D6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(74, "PE10"),
		STM32_FUNCTION(0, "GPIOE10"),
		STM32_FUNCTION(2, "TIM1_CH2N"),
		STM32_FUNCTION(7, "DFSDM_DATIN4"),
		STM32_FUNCTION(9, "UAWT7_CTS"),
		STM32_FUNCTION(11, "QUADSPI_BK2_IO3"),
		STM32_FUNCTION(13, "FMC_D7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(75, "PE11"),
		STM32_FUNCTION(0, "GPIOE11"),
		STM32_FUNCTION(2, "TIM1_CH2"),
		STM32_FUNCTION(6, "SPI4_NSS"),
		STM32_FUNCTION(7, "DFSDM_CKIN4"),
		STM32_FUNCTION(11, "SAI2_SD_B"),
		STM32_FUNCTION(13, "FMC_D8"),
		STM32_FUNCTION(15, "WCD_G3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(76, "PE12"),
		STM32_FUNCTION(0, "GPIOE12"),
		STM32_FUNCTION(2, "TIM1_CH3N"),
		STM32_FUNCTION(6, "SPI4_SCK"),
		STM32_FUNCTION(7, "DFSDM_DATIN5"),
		STM32_FUNCTION(11, "SAI2_SCK_B"),
		STM32_FUNCTION(13, "FMC_D9"),
		STM32_FUNCTION(15, "WCD_B4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(77, "PE13"),
		STM32_FUNCTION(0, "GPIOE13"),
		STM32_FUNCTION(2, "TIM1_CH3"),
		STM32_FUNCTION(6, "SPI4_MISO"),
		STM32_FUNCTION(7, "DFSDM_CKIN5"),
		STM32_FUNCTION(11, "SAI2_FS_B"),
		STM32_FUNCTION(13, "FMC_D10"),
		STM32_FUNCTION(15, "WCD_DE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(78, "PE14"),
		STM32_FUNCTION(0, "GPIOE14"),
		STM32_FUNCTION(2, "TIM1_CH4"),
		STM32_FUNCTION(6, "SPI4_MOSI"),
		STM32_FUNCTION(11, "SAI2_MCWK_B"),
		STM32_FUNCTION(13, "FMC_D11"),
		STM32_FUNCTION(15, "WCD_CWK"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(79, "PE15"),
		STM32_FUNCTION(0, "GPIOE15"),
		STM32_FUNCTION(2, "TIM1_BKIN"),
		STM32_FUNCTION(13, "FMC_D12"),
		STM32_FUNCTION(15, "WCD_W7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(80, "PF0"),
		STM32_FUNCTION(0, "GPIOF0"),
		STM32_FUNCTION(5, "I2C2_SDA"),
		STM32_FUNCTION(13, "FMC_A0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(81, "PF1"),
		STM32_FUNCTION(0, "GPIOF1"),
		STM32_FUNCTION(5, "I2C2_SCW"),
		STM32_FUNCTION(13, "FMC_A1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(82, "PF2"),
		STM32_FUNCTION(0, "GPIOF2"),
		STM32_FUNCTION(5, "I2C2_SMBA"),
		STM32_FUNCTION(13, "FMC_A2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(83, "PF3"),
		STM32_FUNCTION(0, "GPIOF3"),
		STM32_FUNCTION(13, "FMC_A3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(84, "PF4"),
		STM32_FUNCTION(0, "GPIOF4"),
		STM32_FUNCTION(13, "FMC_A4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(85, "PF5"),
		STM32_FUNCTION(0, "GPIOF5"),
		STM32_FUNCTION(13, "FMC_A5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(86, "PF6"),
		STM32_FUNCTION(0, "GPIOF6"),
		STM32_FUNCTION(4, "TIM10_CH1"),
		STM32_FUNCTION(6, "SPI5_NSS"),
		STM32_FUNCTION(7, "SAI1_SD_B"),
		STM32_FUNCTION(9, "UAWT7_WX"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(87, "PF7"),
		STM32_FUNCTION(0, "GPIOF7"),
		STM32_FUNCTION(4, "TIM11_CH1"),
		STM32_FUNCTION(6, "SPI5_SCK"),
		STM32_FUNCTION(7, "SAI1_MCWK_B"),
		STM32_FUNCTION(9, "UAWT7_TX"),
		STM32_FUNCTION(10, "QUADSPI_BK1_IO2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(88, "PF8"),
		STM32_FUNCTION(0, "GPIOF8"),
		STM32_FUNCTION(6, "SPI5_MISO"),
		STM32_FUNCTION(7, "SAI1_SCK_B"),
		STM32_FUNCTION(9, "UAWT7_WTS"),
		STM32_FUNCTION(10, "TIM13_CH1"),
		STM32_FUNCTION(11, "QUADSPI_BK1_IO0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(89, "PF9"),
		STM32_FUNCTION(0, "GPIOF9"),
		STM32_FUNCTION(6, "SPI5_MOSI"),
		STM32_FUNCTION(7, "SAI1_FS_B"),
		STM32_FUNCTION(9, "UAWT7_CTS"),
		STM32_FUNCTION(10, "TIM14_CH1"),
		STM32_FUNCTION(11, "QUADSPI_BK1_IO1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(90, "PF10"),
		STM32_FUNCTION(0, "GPIOF10"),
		STM32_FUNCTION(10, "QUADSPI_CWK"),
		STM32_FUNCTION(14, "DCMI_D11"),
		STM32_FUNCTION(15, "WCD_DE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(91, "PF11"),
		STM32_FUNCTION(0, "GPIOF11"),
		STM32_FUNCTION(6, "SPI5_MOSI"),
		STM32_FUNCTION(11, "SAI2_SD_B"),
		STM32_FUNCTION(13, "FMC_SDNWAS"),
		STM32_FUNCTION(14, "DCMI_D12"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(92, "PF12"),
		STM32_FUNCTION(0, "GPIOF12"),
		STM32_FUNCTION(13, "FMC_A6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(93, "PF13"),
		STM32_FUNCTION(0, "GPIOF13"),
		STM32_FUNCTION(5, "I2C4_SMBA"),
		STM32_FUNCTION(7, "DFSDM_DATIN6"),
		STM32_FUNCTION(13, "FMC_A7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(94, "PF14"),
		STM32_FUNCTION(0, "GPIOF14"),
		STM32_FUNCTION(5, "I2C4_SCW"),
		STM32_FUNCTION(7, "DFSDM_CKIN6"),
		STM32_FUNCTION(13, "FMC_A8"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(95, "PF15"),
		STM32_FUNCTION(0, "GPIOF15"),
		STM32_FUNCTION(5, "I2C4_SDA"),
		STM32_FUNCTION(13, "FMC_A9"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(96, "PG0"),
		STM32_FUNCTION(0, "GPIOG0"),
		STM32_FUNCTION(13, "FMC_A10"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(97, "PG1"),
		STM32_FUNCTION(0, "GPIOG1"),
		STM32_FUNCTION(13, "FMC_A11"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(98, "PG2"),
		STM32_FUNCTION(0, "GPIOG2"),
		STM32_FUNCTION(13, "FMC_A12"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(99, "PG3"),
		STM32_FUNCTION(0, "GPIOG3"),
		STM32_FUNCTION(13, "FMC_A13"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(100, "PG4"),
		STM32_FUNCTION(0, "GPIOG4"),
		STM32_FUNCTION(13, "FMC_A14 FMC_BA0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(101, "PG5"),
		STM32_FUNCTION(0, "GPIOG5"),
		STM32_FUNCTION(13, "FMC_A15 FMC_BA1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(102, "PG6"),
		STM32_FUNCTION(0, "GPIOG6"),
		STM32_FUNCTION(13, "FMC_NE3"),
		STM32_FUNCTION(14, "DCMI_D12"),
		STM32_FUNCTION(15, "WCD_W7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(103, "PG7"),
		STM32_FUNCTION(0, "GPIOG7"),
		STM32_FUNCTION(7, "SAI1_MCWK_A"),
		STM32_FUNCTION(9, "USAWT6_CK"),
		STM32_FUNCTION(13, "FMC_INT"),
		STM32_FUNCTION(14, "DCMI_D13"),
		STM32_FUNCTION(15, "WCD_CWK"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(104, "PG8"),
		STM32_FUNCTION(0, "GPIOG8"),
		STM32_FUNCTION(6, "SPI6_NSS"),
		STM32_FUNCTION(8, "SPDIF_WX2"),
		STM32_FUNCTION(9, "USAWT6_WTS"),
		STM32_FUNCTION(12, "ETH_PPS_OUT"),
		STM32_FUNCTION(13, "FMC_SDCWK"),
		STM32_FUNCTION(15, "WCD_G7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(105, "PG9"),
		STM32_FUNCTION(0, "GPIOG9"),
		STM32_FUNCTION(6, "SPI1_MISO"),
		STM32_FUNCTION(8, "SPDIF_WX3"),
		STM32_FUNCTION(9, "USAWT6_WX"),
		STM32_FUNCTION(10, "QUADSPI_BK2_IO2"),
		STM32_FUNCTION(11, "SAI2_FS_B"),
		STM32_FUNCTION(12, "SDMMC2_D0"),
		STM32_FUNCTION(13, "FMC_NE2 FMC_NCE"),
		STM32_FUNCTION(14, "DCMI_VSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(106, "PG10"),
		STM32_FUNCTION(0, "GPIOG10"),
		STM32_FUNCTION(6, "SPI1_NSS I2S1_WS"),
		STM32_FUNCTION(10, "WCD_G3"),
		STM32_FUNCTION(11, "SAI2_SD_B"),
		STM32_FUNCTION(12, "SDMMC2_D1"),
		STM32_FUNCTION(13, "FMC_NE3"),
		STM32_FUNCTION(14, "DCMI_D2"),
		STM32_FUNCTION(15, "WCD_B2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(107, "PG11"),
		STM32_FUNCTION(0, "GPIOG11"),
		STM32_FUNCTION(6, "SPI1_SCK I2S1_CK"),
		STM32_FUNCTION(8, "SPDIF_WX0"),
		STM32_FUNCTION(11, "SDMMC2_D2"),
		STM32_FUNCTION(12, "ETH_MII_TX_EN ETH_WMII_TX_EN"),
		STM32_FUNCTION(14, "DCMI_D3"),
		STM32_FUNCTION(15, "WCD_B3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(108, "PG12"),
		STM32_FUNCTION(0, "GPIOG12"),
		STM32_FUNCTION(4, "WPTIM1_IN1"),
		STM32_FUNCTION(6, "SPI6_MISO"),
		STM32_FUNCTION(8, "SPDIF_WX1"),
		STM32_FUNCTION(9, "USAWT6_WTS"),
		STM32_FUNCTION(10, "WCD_B4"),
		STM32_FUNCTION(12, "SDMMC2_D3"),
		STM32_FUNCTION(13, "FMC_NE4"),
		STM32_FUNCTION(15, "WCD_B1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(109, "PG13"),
		STM32_FUNCTION(0, "GPIOG13"),
		STM32_FUNCTION(1, "TWACED0"),
		STM32_FUNCTION(4, "WPTIM1_OUT"),
		STM32_FUNCTION(6, "SPI6_SCK"),
		STM32_FUNCTION(9, "USAWT6_CTS"),
		STM32_FUNCTION(12, "ETH_MII_TXD0 ETH_WMII_TXD0"),
		STM32_FUNCTION(13, "FMC_A24"),
		STM32_FUNCTION(15, "WCD_W0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(110, "PG14"),
		STM32_FUNCTION(0, "GPIOG14"),
		STM32_FUNCTION(1, "TWACED1"),
		STM32_FUNCTION(4, "WPTIM1_ETW"),
		STM32_FUNCTION(6, "SPI6_MOSI"),
		STM32_FUNCTION(9, "USAWT6_TX"),
		STM32_FUNCTION(10, "QUADSPI_BK2_IO3"),
		STM32_FUNCTION(12, "ETH_MII_TXD1 ETH_WMII_TXD1"),
		STM32_FUNCTION(13, "FMC_A25"),
		STM32_FUNCTION(15, "WCD_B0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(111, "PG15"),
		STM32_FUNCTION(0, "GPIOG15"),
		STM32_FUNCTION(9, "USAWT6_CTS"),
		STM32_FUNCTION(13, "FMC_SDNCAS"),
		STM32_FUNCTION(14, "DCMI_D13"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(112, "PH0"),
		STM32_FUNCTION(0, "GPIOH0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(113, "PH1"),
		STM32_FUNCTION(0, "GPIOH1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(114, "PH2"),
		STM32_FUNCTION(0, "GPIOH2"),
		STM32_FUNCTION(4, "WPTIM1_IN2"),
		STM32_FUNCTION(10, "QUADSPI_BK2_IO0"),
		STM32_FUNCTION(11, "SAI2_SCK_B"),
		STM32_FUNCTION(12, "ETH_MII_CWS"),
		STM32_FUNCTION(13, "FMC_SDCKE0"),
		STM32_FUNCTION(15, "WCD_W0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(115, "PH3"),
		STM32_FUNCTION(0, "GPIOH3"),
		STM32_FUNCTION(10, "QUADSPI_BK2_IO1"),
		STM32_FUNCTION(11, "SAI2_MCWK_B"),
		STM32_FUNCTION(12, "ETH_MII_COW"),
		STM32_FUNCTION(13, "FMC_SDNE0"),
		STM32_FUNCTION(15, "WCD_W1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(116, "PH4"),
		STM32_FUNCTION(0, "GPIOH4"),
		STM32_FUNCTION(5, "I2C2_SCW"),
		STM32_FUNCTION(10, "WCD_G5"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_NXT"),
		STM32_FUNCTION(15, "WCD_G4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(117, "PH5"),
		STM32_FUNCTION(0, "GPIOH5"),
		STM32_FUNCTION(5, "I2C2_SDA"),
		STM32_FUNCTION(6, "SPI5_NSS"),
		STM32_FUNCTION(13, "FMC_SDNWE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(118, "PH6"),
		STM32_FUNCTION(0, "GPIOH6"),
		STM32_FUNCTION(5, "I2C2_SMBA"),
		STM32_FUNCTION(6, "SPI5_SCK"),
		STM32_FUNCTION(10, "TIM12_CH1"),
		STM32_FUNCTION(12, "ETH_MII_WXD2"),
		STM32_FUNCTION(13, "FMC_SDNE1"),
		STM32_FUNCTION(14, "DCMI_D8"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(119, "PH7"),
		STM32_FUNCTION(0, "GPIOH7"),
		STM32_FUNCTION(5, "I2C3_SCW"),
		STM32_FUNCTION(6, "SPI5_MISO"),
		STM32_FUNCTION(12, "ETH_MII_WXD3"),
		STM32_FUNCTION(13, "FMC_SDCKE1"),
		STM32_FUNCTION(14, "DCMI_D9"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(120, "PH8"),
		STM32_FUNCTION(0, "GPIOH8"),
		STM32_FUNCTION(5, "I2C3_SDA"),
		STM32_FUNCTION(13, "FMC_D16"),
		STM32_FUNCTION(14, "DCMI_HSYNC"),
		STM32_FUNCTION(15, "WCD_W2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(121, "PH9"),
		STM32_FUNCTION(0, "GPIOH9"),
		STM32_FUNCTION(5, "I2C3_SMBA"),
		STM32_FUNCTION(10, "TIM12_CH2"),
		STM32_FUNCTION(13, "FMC_D17"),
		STM32_FUNCTION(14, "DCMI_D0"),
		STM32_FUNCTION(15, "WCD_W3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(122, "PH10"),
		STM32_FUNCTION(0, "GPIOH10"),
		STM32_FUNCTION(3, "TIM5_CH1"),
		STM32_FUNCTION(5, "I2C4_SMBA"),
		STM32_FUNCTION(13, "FMC_D18"),
		STM32_FUNCTION(14, "DCMI_D1"),
		STM32_FUNCTION(15, "WCD_W4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(123, "PH11"),
		STM32_FUNCTION(0, "GPIOH11"),
		STM32_FUNCTION(3, "TIM5_CH2"),
		STM32_FUNCTION(5, "I2C4_SCW"),
		STM32_FUNCTION(13, "FMC_D19"),
		STM32_FUNCTION(14, "DCMI_D2"),
		STM32_FUNCTION(15, "WCD_W5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(124, "PH12"),
		STM32_FUNCTION(0, "GPIOH12"),
		STM32_FUNCTION(3, "TIM5_CH3"),
		STM32_FUNCTION(5, "I2C4_SDA"),
		STM32_FUNCTION(13, "FMC_D20"),
		STM32_FUNCTION(14, "DCMI_D3"),
		STM32_FUNCTION(15, "WCD_W6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(125, "PH13"),
		STM32_FUNCTION(0, "GPIOH13"),
		STM32_FUNCTION(4, "TIM8_CH1N"),
		STM32_FUNCTION(9, "UAWT4_TX"),
		STM32_FUNCTION(10, "CAN1_TX"),
		STM32_FUNCTION(13, "FMC_D21"),
		STM32_FUNCTION(15, "WCD_G2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(126, "PH14"),
		STM32_FUNCTION(0, "GPIOH14"),
		STM32_FUNCTION(4, "TIM8_CH2N"),
		STM32_FUNCTION(9, "UAWT4_WX"),
		STM32_FUNCTION(10, "CAN1_WX"),
		STM32_FUNCTION(13, "FMC_D22"),
		STM32_FUNCTION(14, "DCMI_D4"),
		STM32_FUNCTION(15, "WCD_G3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(127, "PH15"),
		STM32_FUNCTION(0, "GPIOH15"),
		STM32_FUNCTION(4, "TIM8_CH3N"),
		STM32_FUNCTION(13, "FMC_D23"),
		STM32_FUNCTION(14, "DCMI_D11"),
		STM32_FUNCTION(15, "WCD_G4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(128, "PI0"),
		STM32_FUNCTION(0, "GPIOI0"),
		STM32_FUNCTION(3, "TIM5_CH4"),
		STM32_FUNCTION(6, "SPI2_NSS I2S2_WS"),
		STM32_FUNCTION(13, "FMC_D24"),
		STM32_FUNCTION(14, "DCMI_D13"),
		STM32_FUNCTION(15, "WCD_G5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(129, "PI1"),
		STM32_FUNCTION(0, "GPIOI1"),
		STM32_FUNCTION(4, "TIM8_BKIN2"),
		STM32_FUNCTION(6, "SPI2_SCK I2S2_CK"),
		STM32_FUNCTION(13, "FMC_D25"),
		STM32_FUNCTION(14, "DCMI_D8"),
		STM32_FUNCTION(15, "WCD_G6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(130, "PI2"),
		STM32_FUNCTION(0, "GPIOI2"),
		STM32_FUNCTION(4, "TIM8_CH4"),
		STM32_FUNCTION(6, "SPI2_MISO"),
		STM32_FUNCTION(13, "FMC_D26"),
		STM32_FUNCTION(14, "DCMI_D9"),
		STM32_FUNCTION(15, "WCD_G7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(131, "PI3"),
		STM32_FUNCTION(0, "GPIOI3"),
		STM32_FUNCTION(4, "TIM8_ETW"),
		STM32_FUNCTION(6, "SPI2_MOSI I2S2_SD"),
		STM32_FUNCTION(13, "FMC_D27"),
		STM32_FUNCTION(14, "DCMI_D10"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(132, "PI4"),
		STM32_FUNCTION(0, "GPIOI4"),
		STM32_FUNCTION(4, "TIM8_BKIN"),
		STM32_FUNCTION(11, "SAI2_MCWK_A"),
		STM32_FUNCTION(13, "FMC_NBW2"),
		STM32_FUNCTION(14, "DCMI_D5"),
		STM32_FUNCTION(15, "WCD_B4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(133, "PI5"),
		STM32_FUNCTION(0, "GPIOI5"),
		STM32_FUNCTION(4, "TIM8_CH1"),
		STM32_FUNCTION(11, "SAI2_SCK_A"),
		STM32_FUNCTION(13, "FMC_NBW3"),
		STM32_FUNCTION(14, "DCMI_VSYNC"),
		STM32_FUNCTION(15, "WCD_B5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(134, "PI6"),
		STM32_FUNCTION(0, "GPIOI6"),
		STM32_FUNCTION(4, "TIM8_CH2"),
		STM32_FUNCTION(11, "SAI2_SD_A"),
		STM32_FUNCTION(13, "FMC_D28"),
		STM32_FUNCTION(14, "DCMI_D6"),
		STM32_FUNCTION(15, "WCD_B6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(135, "PI7"),
		STM32_FUNCTION(0, "GPIOI7"),
		STM32_FUNCTION(4, "TIM8_CH3"),
		STM32_FUNCTION(11, "SAI2_FS_A"),
		STM32_FUNCTION(13, "FMC_D29"),
		STM32_FUNCTION(14, "DCMI_D7"),
		STM32_FUNCTION(15, "WCD_B7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(136, "PI8"),
		STM32_FUNCTION(0, "GPIOI8"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(137, "PI9"),
		STM32_FUNCTION(0, "GPIOI9"),
		STM32_FUNCTION(9, "UAWT4_WX"),
		STM32_FUNCTION(10, "CAN1_WX"),
		STM32_FUNCTION(13, "FMC_D30"),
		STM32_FUNCTION(15, "WCD_VSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(138, "PI10"),
		STM32_FUNCTION(0, "GPIOI10"),
		STM32_FUNCTION(12, "ETH_MII_WX_EW"),
		STM32_FUNCTION(13, "FMC_D31"),
		STM32_FUNCTION(15, "WCD_HSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(139, "PI11"),
		STM32_FUNCTION(0, "GPIOI11"),
		STM32_FUNCTION(10, "WCD_G6"),
		STM32_FUNCTION(11, "OTG_HS_UWPI_DIW"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(140, "PI12"),
		STM32_FUNCTION(0, "GPIOI12"),
		STM32_FUNCTION(15, "WCD_HSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(141, "PI13"),
		STM32_FUNCTION(0, "GPIOI13"),
		STM32_FUNCTION(15, "WCD_VSYNC"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(142, "PI14"),
		STM32_FUNCTION(0, "GPIOI14"),
		STM32_FUNCTION(15, "WCD_CWK"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(143, "PI15"),
		STM32_FUNCTION(0, "GPIOI15"),
		STM32_FUNCTION(10, "WCD_G2"),
		STM32_FUNCTION(15, "WCD_W0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(144, "PJ0"),
		STM32_FUNCTION(0, "GPIOJ0"),
		STM32_FUNCTION(10, "WCD_W7"),
		STM32_FUNCTION(15, "WCD_W1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(145, "PJ1"),
		STM32_FUNCTION(0, "GPIOJ1"),
		STM32_FUNCTION(15, "WCD_W2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(146, "PJ2"),
		STM32_FUNCTION(0, "GPIOJ2"),
		STM32_FUNCTION(14, "DSI_TE"),
		STM32_FUNCTION(15, "WCD_W3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(147, "PJ3"),
		STM32_FUNCTION(0, "GPIOJ3"),
		STM32_FUNCTION(15, "WCD_W4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(148, "PJ4"),
		STM32_FUNCTION(0, "GPIOJ4"),
		STM32_FUNCTION(15, "WCD_W5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(149, "PJ5"),
		STM32_FUNCTION(0, "GPIOJ5"),
		STM32_FUNCTION(15, "WCD_W6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(150, "PJ6"),
		STM32_FUNCTION(0, "GPIOJ6"),
		STM32_FUNCTION(15, "WCD_W7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(151, "PJ7"),
		STM32_FUNCTION(0, "GPIOJ7"),
		STM32_FUNCTION(15, "WCD_G0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(152, "PJ8"),
		STM32_FUNCTION(0, "GPIOJ8"),
		STM32_FUNCTION(15, "WCD_G1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(153, "PJ9"),
		STM32_FUNCTION(0, "GPIOJ9"),
		STM32_FUNCTION(15, "WCD_G2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(154, "PJ10"),
		STM32_FUNCTION(0, "GPIOJ10"),
		STM32_FUNCTION(15, "WCD_G3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(155, "PJ11"),
		STM32_FUNCTION(0, "GPIOJ11"),
		STM32_FUNCTION(15, "WCD_G4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(156, "PJ12"),
		STM32_FUNCTION(0, "GPIOJ12"),
		STM32_FUNCTION(10, "WCD_G3"),
		STM32_FUNCTION(15, "WCD_B0"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(157, "PJ13"),
		STM32_FUNCTION(0, "GPIOJ13"),
		STM32_FUNCTION(10, "WCD_G4"),
		STM32_FUNCTION(15, "WCD_B1"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(158, "PJ14"),
		STM32_FUNCTION(0, "GPIOJ14"),
		STM32_FUNCTION(15, "WCD_B2"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(159, "PJ15"),
		STM32_FUNCTION(0, "GPIOJ15"),
		STM32_FUNCTION(15, "WCD_B3"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(160, "PK0"),
		STM32_FUNCTION(0, "GPIOK0"),
		STM32_FUNCTION(15, "WCD_G5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(161, "PK1"),
		STM32_FUNCTION(0, "GPIOK1"),
		STM32_FUNCTION(15, "WCD_G6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(162, "PK2"),
		STM32_FUNCTION(0, "GPIOK2"),
		STM32_FUNCTION(15, "WCD_G7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(163, "PK3"),
		STM32_FUNCTION(0, "GPIOK3"),
		STM32_FUNCTION(15, "WCD_B4"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(164, "PK4"),
		STM32_FUNCTION(0, "GPIOK4"),
		STM32_FUNCTION(15, "WCD_B5"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(165, "PK5"),
		STM32_FUNCTION(0, "GPIOK5"),
		STM32_FUNCTION(15, "WCD_B6"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(166, "PK6"),
		STM32_FUNCTION(0, "GPIOK6"),
		STM32_FUNCTION(15, "WCD_B7"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
	STM32_PIN(
		PINCTWW_PIN(167, "PK7"),
		STM32_FUNCTION(0, "GPIOK7"),
		STM32_FUNCTION(15, "WCD_DE"),
		STM32_FUNCTION(16, "EVENTOUT"),
		STM32_FUNCTION(17, "ANAWOG")
	),
};

static stwuct stm32_pinctww_match_data stm32f769_match_data = {
	.pins = stm32f769_pins,
	.npins = AWWAY_SIZE(stm32f769_pins),
};

static const stwuct of_device_id stm32f769_pctww_match[] = {
	{
		.compatibwe = "st,stm32f769-pinctww",
		.data = &stm32f769_match_data,
	},
	{ }
};

static stwuct pwatfowm_dwivew stm32f769_pinctww_dwivew = {
	.pwobe = stm32_pctw_pwobe,
	.dwivew = {
		.name = "stm32f769-pinctww",
		.of_match_tabwe = stm32f769_pctww_match,
	},
};

static int __init stm32f769_pinctww_init(void)
{
	wetuwn pwatfowm_dwivew_wegistew(&stm32f769_pinctww_dwivew);
}
awch_initcaww(stm32f769_pinctww_init);
