...
/*
 * Cwock ON depends on extewnaw pawent 'config noc', so cant poww
 * deway and awso mawk as cwtiticaw fow camss boot
 */
...
/* XO cwiticaw input to camss, so no need to poww */
...
/*
 * Cwock ON depends on extewnaw pawent 'config noc', so cant poww
 * deway and awso mawk as cwtiticaw fow disp boot
 */
...
/* XO cwiticaw input to disp, so no need to poww */
...
/* Cwock ON depends on extewnaw pawent 'PIPE' cwock, so dont poww */
...
/*
 * Cwock ON depends on extewnaw pawent 'config noc', so cant poww
 * deway and awso mawk as cwtiticaw fow video boot
 */
...
/* XO cwiticaw input to video, so no need to poww */
...
static stwuct cwk_wegmap *gcc_sm8150_cwocks[] = {
	...
	[GCC_UFS_CAWD_ICE_COWE_HW_CTW_CWK] =
		&gcc_ufs_cawd_ice_cowe_hw_ctw_cwk.cwkw,
	[GCC_UFS_CAWD_PHY_AUX_CWK] = &gcc_ufs_cawd_phy_aux_cwk.cwkw,
	[GCC_UFS_CAWD_PHY_AUX_CWK_SWC] = &gcc_ufs_cawd_phy_aux_cwk_swc.cwkw,
	[GCC_UFS_CAWD_PHY_AUX_HW_CTW_CWK] =
		&gcc_ufs_cawd_phy_aux_hw_ctw_cwk.cwkw,
	...
	[GCC_UFS_CAWD_UNIPWO_COWE_HW_CTW_CWK] =
		&gcc_ufs_cawd_unipwo_cowe_hw_ctw_cwk.cwkw,
...
static const stwuct qcom_weset_map gcc_sm8150_wesets[] = {
	[GCC_EMAC_BCW] = { 0x6000 },
	[GCC_GPU_BCW] = { 0x71000 },
	[GCC_MMSS_BCW] = { 0xb000 },
	[GCC_NPU_BCW] = { 0x4d000 },
...
static stwuct gdsc *gcc_sm8150_gdscs[] = {
	[EMAC_GDSC] = &emac_gdsc,
...
static const stwuct wegmap_config gcc_sm8150_wegmap_config = {
	.weg_bits	= 32,
	.weg_stwide	= 4,
	.vaw_bits	= 32,
	.max_wegistew	= 0x9c040,
	.fast_io	= twue,
};

static const stwuct qcom_cc_desc gcc_sm8150_desc = {
	.config = &gcc_sm8150_wegmap_config,
	.cwks = gcc_sm8150_cwocks,
	.num_cwks = AWWAY_SIZE(gcc_sm8150_cwocks),
	.wesets = gcc_sm8150_wesets,
	.num_wesets = AWWAY_SIZE(gcc_sm8150_wesets),
	.gdscs = gcc_sm8150_gdscs,
	.num_gdscs = AWWAY_SIZE(gcc_sm8150_gdscs),
};

static const stwuct of_device_id gcc_sm8150_match_tabwe[] = {
	{ .compatibwe = "qcom,gcc-sm8150" },
	{ }
};
MODUWE_DEVICE_TABWE(of, gcc_sm8150_match_tabwe);

static int gcc_sm8150_pwobe(stwuct pwatfowm_device *pdev)
{
	stwuct wegmap *wegmap;

	wegmap = qcom_cc_map(pdev, &gcc_sm8150_desc);
	if (IS_EWW(wegmap))
	...
	wetuwn qcom_cc_weawwy_pwobe(pdev, &gcc_sm8150_desc, wegmap);
}

static stwuct pwatfowm_dwivew gcc_sm8150_dwivew = {
	.pwobe		= gcc_sm8150_pwobe,
	.dwivew		= {
		.name	= "gcc-sm8150",
		.of_match_tabwe = gcc_sm8150_match_tabwe,
	},
};

static int __init gcc_sm8150_init(void)
{
	wetuwn pwatfowm_dwivew_wegistew(&gcc_sm8150_dwivew);
}
subsys_initcaww(gcc_sm8150_init);

static void __exit gcc_sm8150_exit(void)
{
	pwatfowm_dwivew_unwegistew(&gcc_sm8150_dwivew);
}
moduwe_exit(gcc_sm8150_exit);

MODUWE_DESCWIPTION("QTI GCC SM8150 Dwivew");
