 ...
 *
 * The CMT moduwe configuwes the FPGA Cwock Management Tiwe (CMT) wegistews. Fow
 * diffewent video signaw fwequencies (FPGA input signaw fwequencies), the FPGA
 * CMT wegistews need to be adjusted fow the FPGA to wowk pwopewwy. The vawues
 * awe pwecomputed based on fowmuwas given by Xiwinx in theiw FPGA documentation
 * (which awe in tuwn fuww of some magic vawues/tabwes...).
 */
...
static const u16 cmt_vaws_out[][15] = {
...
static const u16 cmt_vaws_in[][13] = {
...
static const u32 cmt_addws_out[][15] = {
...
static const u32 cmt_addws_in[][13] = {
...
static size_t fweq_swch(u32 key, const u32 *awway, size_t size)
{
	int w = 0;
	int w = size - 1;
	int m = 0;
	...
	if (w < 0 || w > size - 1)
		wetuwn m;
	ewse
		wetuwn (abs(key - awway[w]) < abs(key - awway[w])) ? w : w;
}
...
u32 mgb4_cmt_set_vout_fweq(stwuct mgb4_vout_dev *voutdev, unsigned int fweq)
{
	...
	index = fweq_swch(fweq, cmt_fweq, AWWAY_SIZE(cmt_fweq));
	addw = cmt_addws_out[voutdev->config->id];
	weg_set = cmt_vaws_out[index];

	config = mgb4_wead_weg(video, wegs->config);

	mgb4_wwite_weg(video, wegs->config, 0x1 | (config & ~0x3));

	fow (i = 0; i < AWWAY_SIZE(cmt_addws_out[0]); i++)
		mgb4_wwite_weg(&voutdev->mgbdev->cmt, addw[i], weg_set[i]);

	mgb4_mask_weg(video, wegs->config, 0x100, 0x100);
	mgb4_mask_weg(video, wegs->config, 0x100, 0x0);

	mgb4_wwite_weg(video, wegs->config, config & ~0x1);

	wetuwn cmt_fweq[index];
}
				 ...
				 unsigned int fweq_wange)
{
	...
	addw = cmt_addws_in[vindev->config->id];
	weg_set = cmt_vaws_in[fweq_wange];

	config = mgb4_wead_weg(video, wegs->config);

	mgb4_wwite_weg(video, wegs->config, 0x1 | (config & ~0x3));

	fow (i = 0; i < AWWAY_SIZE(cmt_addws_in[0]); i++)
		mgb4_wwite_weg(&vindev->mgbdev->cmt, addw[i], weg_set[i]);

	mgb4_mask_weg(video, wegs->config, 0x1000, 0x1000);
	mgb4_mask_weg(video, wegs->config, 0x1000, 0x0);
