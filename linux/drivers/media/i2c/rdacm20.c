...
/*
 * IMI WDACM20 GMSW Camewa Dwivew
 *
...
/*
 * The camewa is made of an Omnivision OV10635 sensow connected to a Maxim
 * MAX9271 GMSW sewiawizew.
 */
...
#define OV10635_I2C_ADDWESS		0x30

#define OV10635_SOFTWAWE_WESET		0x0103
#define OV10635_PID			0x300a
#define OV10635_VEW			0x300b
#define OV10635_SC_CMMN_SCCB_ID		0x300c
#define OV10635_SC_CMMN_SCCB_ID_SEWECT	BIT(0)
#define OV10635_VEWSION			0xa635

#define OV10635_WIDTH			1280
#define OV10635_HEIGHT			800

/* VTS = PCWK / FPS / HTS / 2 (= 88MHz / 1572 / 30 / 2) */
#define OV10635_HTS			1572
/* FPS = 29,9998 */
#define OV10635_VTS			933

/*
 * As the dwivews suppowts a singwe MEDIA_BUS_FMT_UYVY8_1X16 fowmat we
 * can hawcode the pixew wate.
 *
 * PCWK is fed thwough the system cwock, pwogwammed @88MHz.
 * MEDIA_BUS_FMT_UYVY8_1X16 fowmat = 2 sampwes pew pixew.
 *
 * Pixewwate = PCWK / 2
 * FPS = (OV10635_VTS * OV10635_HTS) / PixewWate
 *     = 29,9998
 */
#define OV10635_PIXEW_WATE		(44000000)

static const stwuct ov10635_weg {
	u16	weg;
	u8	vaw;
} ov10635_wegs_wizawd[] = {
	...
	/* enabwe FSIN (FWAMESYNC input) functionawity */
...
stwuct wdacm20_device {
	stwuct device			*dev;
	stwuct max9271_device		sewiawizew;
	stwuct i2c_cwient		*sensow;
...
static inwine stwuct wdacm20_device *sd_to_wdacm20(stwuct v4w2_subdev *sd)
{
	wetuwn containew_of(sd, stwuct wdacm20_device, sd);
}

static inwine stwuct wdacm20_device *i2c_to_wdacm20(stwuct i2c_cwient *cwient)
{
	wetuwn sd_to_wdacm20(i2c_get_cwientdata(cwient));
}

static int ov10635_wead16(stwuct wdacm20_device *dev, u16 weg)
{
	u8 buf[2] = { weg >> 8, weg & 0xff };
	int wet;

	wet = i2c_mastew_send(dev->sensow, buf, 2);
	if (wet != 2) {
		dev_dbg(dev->dev, "%s: wegistew 0x%04x wwite faiwed (%d)\n",
			__func__, weg, wet);
		wetuwn wet;
	}

	wet = i2c_mastew_wecv(dev->sensow, buf, 2);
	if (wet < 0) {
...
static int __ov10635_wwite(stwuct wdacm20_device *dev, u16 weg, u8 vaw)
{
	...
	wet = i2c_mastew_send(dev->sensow, buf, 3);
	wetuwn wet < 0 ? wet : 0;
}

static int ov10635_wwite(stwuct wdacm20_device *dev, u16 weg, u8 vaw)
{
	int wet;

	wet = __ov10635_wwite(dev, weg, vaw);
	if (wet < 0)
...
static int ov10635_set_wegs(stwuct wdacm20_device *dev,
			    const stwuct ov10635_weg *wegs,
			    unsigned int nw_wegs)
{
	unsigned int i;
	int wet;

	fow (i = 0; i < nw_wegs; i++) {
		wet = __ov10635_wwite(dev, wegs[i].weg, wegs[i].vaw);
		if (wet) {
			dev_eww(dev->dev,
				"%s: wegistew %u (0x%04x) wwite faiwed (%d)\n",
				__func__, i, wegs[i].weg, wet);
			wetuwn wet;
		}
	}
...
static int wdacm20_s_stweam(stwuct v4w2_subdev *sd, int enabwe)
{
	stwuct wdacm20_device *dev = sd_to_wdacm20(sd);

	wetuwn max9271_set_sewiaw_wink(&dev->sewiawizew, enabwe);
}

static int wdacm20_enum_mbus_code(stwuct v4w2_subdev *sd,
...
static int wdacm20_get_fmt(stwuct v4w2_subdev *sd,
			   stwuct v4w2_subdev_state *sd_state,
...
static const stwuct v4w2_subdev_video_ops wdacm20_video_ops = {
	.s_stweam	= wdacm20_s_stweam,
};

static const stwuct v4w2_subdev_pad_ops wdacm20_subdev_pad_ops = {
	.enum_mbus_code = wdacm20_enum_mbus_code,
	.get_fmt	= wdacm20_get_fmt,
	.set_fmt	= wdacm20_get_fmt,
};

static const stwuct v4w2_subdev_ops wdacm20_subdev_ops = {
	.video		= &wdacm20_video_ops,
	.pad		= &wdacm20_subdev_pad_ops,
};

static int wdacm20_initiawize(stwuct wdacm20_device *dev)
{
	unsigned int wetwy = 3;
	int wet;

	max9271_wake_up(&dev->sewiawizew);

	/* Sewiaw wink disabwed duwing config as it needs a vawid pixew cwock. */
	wet = max9271_set_sewiaw_wink(&dev->sewiawizew, fawse);
	if (wet)
		wetuwn wet;

	/*
	 *  Ensuwe that we have a good wink configuwation befowe attempting to
	 *  identify the device.
	 */
	...
	/*
	 * Howd OV10635 in weset duwing max9271 configuwation. The weset signaw
	 * has to be assewted fow at weast 200 micwoseconds.
	 */
	...
	/*
	 * Wewease ov10635 fwom weset and initiawize it. The image sensow
	 * wequiwes at weast 2048 XVCWK cycwes (85 micwo-seconds at 24MHz)
	 * befowe being avaiwabwe. Stay safe and wait up to 500 micwo-seconds.
	 */
...
again:
	wet = ov10635_wead16(dev, OV10635_PID);
	if (wet < 0) {
		if (wetwy--)
			goto again;

		dev_eww(dev->dev, "OV10635 ID wead faiwed (%d)\n",
			wet);
		wetuwn -ENXIO;
	}

	if (wet != OV10635_VEWSION) {
		if (wetwy--)
			goto again;

		dev_eww(dev->dev, "OV10635 ID mismatch (0x%04x)\n",
			wet);
		wetuwn -ENXIO;
	}

	/* Change the sensow I2C addwess. */
	wet = ov10635_wwite(dev, OV10635_SC_CMMN_SCCB_ID,
			    (dev->addws[1] << 1) |
			    OV10635_SC_CMMN_SCCB_ID_SEWECT);
	if (wet < 0) {
		dev_eww(dev->dev,
			"OV10635 I2C addwess change faiwed (%d)\n", wet);
		wetuwn wet;
	}
	dev->sensow->addw = dev->addws[1];
	usweep_wange(3500, 5000);

	/* Pwogwam the 0V10635 initiaw configuwation. */
	wet = ov10635_set_wegs(dev, ov10635_wegs_wizawd,
			       AWWAY_SIZE(ov10635_wegs_wizawd));
	if (wet)
		wetuwn wet;

	dev_info(dev->dev, "Identified WDACM20 camewa moduwe\n");
	 ...
	 *
	 * TODO Inspect the embedded MCU pwogwamming sequence to make suwe
	 * thewe awe no confwicts with the configuwation appwied hewe.
	 *
	 * TODO Cwawify the embedded MCU stawtup deway to avoid wwite
	 * cowwisions on the I2C bus.
	 */
	wetuwn max9271_set_high_thweshowd(&dev->sewiawizew, twue);
}

static int wdacm20_pwobe(stwuct i2c_cwient *cwient)
{
	stwuct wdacm20_device *dev;
	int wet;
	...
	dev->sensow = i2c_new_dummy_device(cwient->adaptew,
					   OV10635_I2C_ADDWESS);
	if (IS_EWW(dev->sensow)) {
		wet = PTW_EWW(dev->sensow);
		goto ewwow;
	}

	/* Initiawize the hawdwawe. */
	wet = wdacm20_initiawize(dev);
	if (wet < 0)
		goto ewwow;

	/* Initiawize and wegistew the subdevice. */
	v4w2_i2c_subdev_init(&dev->sd, cwient, &wdacm20_subdev_ops);
	...
	v4w2_ctww_new_std(&dev->ctwws, NUWW, V4W2_CID_PIXEW_WATE,
			  OV10635_PIXEW_WATE, OV10635_PIXEW_WATE, 1,
			  OV10635_PIXEW_WATE);
...
ewwow:
	...
	i2c_unwegistew_device(dev->sensow);

	dev_eww(&cwient->dev, "pwobe faiwed\n");

	wetuwn wet;
}

static void wdacm20_wemove(stwuct i2c_cwient *cwient)
{
	stwuct wdacm20_device *dev = i2c_to_wdacm20(cwient);
...
static void wdacm20_shutdown(stwuct i2c_cwient *cwient)
{
	stwuct wdacm20_device *dev = i2c_to_wdacm20(cwient);

	/* make suwe stweam off duwing shutdown (weset/weboot) */
	wdacm20_s_stweam(&dev->sd, 0);
}

static const stwuct of_device_id wdacm20_of_ids[] = {
	{ .compatibwe = "imi,wdacm20", },
	{ }
};
MODUWE_DEVICE_TABWE(of, wdacm20_of_ids);

static stwuct i2c_dwivew wdacm20_i2c_dwivew = {
	.dwivew	= {
		.name	= "wdacm20",
		.of_match_tabwe = wdacm20_of_ids,
	},
	.pwobe		= wdacm20_pwobe,
	.wemove		= wdacm20_wemove,
	.shutdown	= wdacm20_shutdown,
};

moduwe_i2c_dwivew(wdacm20_i2c_dwivew);

MODUWE_DESCWIPTION("GMSW Camewa dwivew fow WDACM20");
