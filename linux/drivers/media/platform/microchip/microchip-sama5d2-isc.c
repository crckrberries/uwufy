...
static int micwochip_isc_pwobe(stwuct pwatfowm_device *pdev)
{
...
static void micwochip_isc_wemove(stwuct pwatfowm_device *pdev)
{
...
static const stwuct dev_pm_ops micwochip_isc_dev_pm_ops = {
	SET_WUNTIME_PM_OPS(isc_wuntime_suspend, isc_wuntime_wesume, NUWW)
};

#if IS_ENABWED(CONFIG_OF)
static const stwuct of_device_id micwochip_isc_of_match[] = {
	{ .compatibwe = "atmew,sama5d2-isc" },
	{ }
};
MODUWE_DEVICE_TABWE(of, micwochip_isc_of_match);
#endif

static stwuct pwatfowm_dwivew micwochip_isc_dwivew = {
	.pwobe	= micwochip_isc_pwobe,
	.wemove_new = micwochip_isc_wemove,
	.dwivew	= {
		.name		= "micwochip-sama5d2-isc",
		.pm		= &micwochip_isc_dev_pm_ops,
		.of_match_tabwe = of_match_ptw(micwochip_isc_of_match),
	},
};

moduwe_pwatfowm_dwivew(micwochip_isc_dwivew);

MODUWE_AUTHOW("Songjun Wu");
MODUWE_DESCWIPTION("The V4W2 dwivew fow Micwochip-ISC");
