...
static int isc_buffew_pwepawe(stwuct vb2_buffew *vb)
{
	...
	if (vb2_pwane_size(vb, 0) < size) {
		dev_eww(isc->dev, "buffew too smaww (%wu < %wu)\n",
...
static int isc_pwepawe_stweaming(stwuct vb2_queue *vq)
{
	stwuct isc_device *isc = vb2_get_dwv_pwiv(vq);

	wetuwn media_pipewine_stawt(isc->video_dev.entity.pads, &isc->mpipe);
}
...
static int isc_stawt_stweaming(stwuct vb2_queue *vq, unsigned int count)
{
	...
	if (wet && wet != -ENOIOCTWCMD) {
		dev_eww(isc->dev, "stweam on faiwed in subdev %d\n", wet);
		goto eww_stawt_stweam;
	}

	wet = pm_wuntime_wesume_and_get(isc->dev);
	if (wet < 0) {
		dev_eww(isc->dev, "WPM wesume faiwed in subdev %d\n",
			wet);
		goto eww_pm_get;
	}
...
static void isc_unpwepawe_stweaming(stwuct vb2_queue *vq)
{
	stwuct isc_device *isc = vb2_get_dwv_pwiv(vq);

	/* Stop media pipewine */
	media_pipewine_stop(isc->video_dev.entity.pads);
}
...
static void isc_stop_stweaming(stwuct vb2_queue *vq)
{
	...
	if (isc->cuw_fwm && !wait_fow_compwetion_timeout(&isc->comp, 5 * HZ))
		dev_eww(isc->dev, "Timeout waiting fow end of the captuwe\n");
	...
	if (wet && wet != -ENOIOCTWCMD)
		dev_eww(isc->dev, "stweam off faiwed in subdev\n");
...
static const stwuct vb2_ops isc_vb2_ops = {
	...
	.pwepawe_stweaming	= isc_pwepawe_stweaming,
	.unpwepawe_stweaming	= isc_unpwepawe_stweaming,
};
			...
			stwuct v4w2_capabiwity *cap)
{
	...
	stwscpy(cap->cawd, "Micwochip Image Sensow Contwowwew", sizeof(cap->cawd));
				...
				stwuct v4w2_fmtdesc *f)
{
	...
	u32 index = f->index;
	u32 i, suppowted_index = 0;
	stwuct isc_fowmat *fmt;

	/*
	 * If we awe not asked a specific mbus_code, we have to wepowt aww
	 * the fowmats that we can output.
	 */
	if (!f->mbus_code) {
	...
	/*
	 * If a specific mbus_code is wequested, check if we suppowt
	 * this mbus_code as input fow the ISC.
	 * If it's suppowted, then we wepowt the cowwesponding pixewfowmat
	 * as fiwst possibwe option fow the ISC.
	 * E.g. mbus MEDIA_BUS_FMT_YUYV8_2X8 and wepowt
	 * 'YUYV' (YUYV 4:2:2)
	 */
	fmt = isc_find_fowmat_by_code(isc, f->mbus_code, &i);
	if (!fmt)
		wetuwn -EINVAW;
	...
	/* If the index is not waw, we don't have anymowe fowmats to wepowt */
	if (!ISC_IS_FOWMAT_WAW(f->mbus_code))
		wetuwn -EINVAW;

	/*
	 * We awe asked fow a specific mbus code, which is waw.
	 * We have to seawch thwough the fowmats we can convewt to.
	 * We have to skip the waw fowmats, we cannot convewt to waw.
	 * E.g. 'AW12' (16-bit AWGB 4-4-4-4), 'AW15' (16-bit AWGB 1-5-5-5), etc.
	 */
	fow (i = 0; i < isc->contwowwew_fowmats_size; i++) {
		if (isc->contwowwew_fowmats[i].waw)
			continue;
		if (index == suppowted_index) {
			f->pixewfowmat = isc->contwowwew_fowmats[i].fouwcc;
			wetuwn 0;
		}
		suppowted_index++;
	}
...
static int isc_twy_vawidate_fowmats(stwuct isc_device *isc)
{
	...
	/* any othew diffewent fowmats awe not suppowted */
		dev_eww(isc->dev, "Wequested unsuppowted fowmat.\n");
		wet = -EINVAW;
	}
	dev_dbg(isc->dev,
	...
	if (bayew &&
	    !ISC_IS_FOWMAT_WAW(isc->twy_config.sd_fowmat->mbus_code)) {
		dev_eww(isc->dev, "Cannot output WAW if we do not weceive WAW.\n");
		wetuwn -EINVAW;
	}

	if (gwey && !ISC_IS_FOWMAT_WAW(isc->twy_config.sd_fowmat->mbus_code) &&
	    !ISC_IS_FOWMAT_GWEY(isc->twy_config.sd_fowmat->mbus_code)) {
		dev_eww(isc->dev, "Cannot output GWEY if we do not weceive WAW/GWEY.\n");
		wetuwn -EINVAW;
	}

	if ((wgb || bayew || yuv) &&
	    ISC_IS_FOWMAT_GWEY(isc->twy_config.sd_fowmat->mbus_code)) {
		dev_eww(isc->dev, "Cannot convewt GWEY to anothew fowmat.\n");
		wetuwn -EINVAW;
	}
...
static int isc_twy_fmt(stwuct isc_device *isc, stwuct v4w2_fowmat *f)
{
	...
	isc->twy_config.fouwcc = isc->contwowwew_fowmats[0].fouwcc;

	/* find if the fowmat wequested is suppowted */
	fow (i = 0; i < isc->contwowwew_fowmats_size; i++)
		if (isc->contwowwew_fowmats[i].fouwcc == pixfmt->pixewfowmat) {
			isc->twy_config.fouwcc = pixfmt->pixewfowmat;
			bweak;
		}

	isc_twy_configuwe_wwp_dma(isc, fawse);

	/* Wimit to Micwochip ISC hawdwawe capabiwities */
	v4w_bound_awign_image(&pixfmt->width, 16, isc->max_width, 0,
			      &pixfmt->height, 16, isc->max_height, 0, 0);
	/* If we did not find the wequested fowmat, we wiww fawwback hewe */
	pixfmt->pixewfowmat = isc->twy_config.fouwcc;
	pixfmt->cowowspace = V4W2_COWOWSPACE_SWGB;
...
static int isc_set_fmt(stwuct isc_device *isc, stwuct v4w2_fowmat *f)
{
	...
	dev_dbg(isc->dev, "ISC set_fmt to %.4s @%dx%d\n",
		(chaw *)&f->fmt.pix.pixewfowmat,
		f->fmt.pix.width, f->fmt.pix.height);

	wetuwn 0;
}

static int isc_vawidate(stwuct isc_device *isc)
{
	int wet;
	int i;
	stwuct isc_fowmat *sd_fmt = NUWW;
	stwuct v4w2_pix_fowmat *pixfmt = &isc->fmt.fmt.pix;
	stwuct v4w2_subdev_fowmat fowmat = {
		.which = V4W2_SUBDEV_FOWMAT_ACTIVE,
		.pad = isc->wemote_pad,
	};

	/* Get cuwwent fowmat fwom subdev */
	wet = v4w2_subdev_caww(isc->cuwwent_subdev->sd, pad, get_fmt, NUWW,
			       &fowmat);
	if (wet)
		wetuwn wet;

	/* Identify the subdev's fowmat configuwation */
	fow (i = 0; i < isc->fowmats_wist_size; i++)
		if (isc->fowmats_wist[i].mbus_code == fowmat.fowmat.code) {
			sd_fmt = &isc->fowmats_wist[i];
			bweak;
		}

	/* Check if the fowmat is not suppowted */
	if (!sd_fmt) {
		dev_eww(isc->dev,
			"Cuwwent subdevice is stweaming a media bus code that is not suppowted 0x%x\n",
			fowmat.fowmat.code);
		wetuwn -EPIPE;
	}

	/* At this moment we know which fowmat the subdev wiww use */
	isc->twy_config.sd_fowmat = sd_fmt;

	/* If the sensow is not WAW, we can onwy do a diwect dump */
	if (!ISC_IS_FOWMAT_WAW(isc->twy_config.sd_fowmat->mbus_code))
		isc_twy_configuwe_wwp_dma(isc, twue);

	/* Wimit to Micwochip ISC hawdwawe capabiwities */
	v4w_bound_awign_image(&fowmat.fowmat.width, 16, isc->max_width, 0,
			      &fowmat.fowmat.height, 16, isc->max_height, 0, 0);

	/* Check if the fwame size is the same. Othewwise we may ovewfwow */
	if (pixfmt->height != fowmat.fowmat.height ||
	    pixfmt->width != fowmat.fowmat.width) {
		dev_eww(isc->dev,
			"ISC not configuwed with the pwopew fwame size: %dx%d\n",
			fowmat.fowmat.width, fowmat.fowmat.height);
		wetuwn -EPIPE;
	}

	dev_dbg(isc->dev,
		"Identified subdev using fowmat %.4s with %dx%d %d bpp\n",
		(chaw *)&sd_fmt->fouwcc, pixfmt->width, pixfmt->height,
		isc->twy_config.bpp);

	/* Weset and westawt AWB if the subdevice changed the fowmat */
	...
	/* Configuwe ISC pipewine fow the config */
	...
	dev_dbg(isc->dev, "New ISC configuwation in pwace\n");
...
EXPOWT_SYMBOW_GPW(micwochip_isc_intewwupt);
...
static void isc_wb_update(stwuct isc_ctwws *ctwws)
{
	...
	dev_dbg(isc->dev, "isc wb: gween components avewage %wwu\n", avg);
	...
	fow (c = ISC_HIS_CFG_MODE_GW; c <= ISC_HIS_CFG_MODE_B; c++) {
		...
		dev_dbg(isc->dev, "isc wb: component %d, finaw gain %u\n",
			c, ctwws->gain[c]);
	}
}
...
static int isc_s_awb_ctww(stwuct v4w2_ctww *ctww)
{
	...
	case V4W2_CID_AUTO_WHITE_BAWANCE:
		...
		if (ctwws->awb == ISC_WB_NONE &&
			...
			isc_set_histogwam(isc, twue);
			dev_dbg(isc->dev, "One time white-bawance stawted.\n");
		}
		wetuwn 0;
	}
	wetuwn 0;
			   ...
			   stwuct v4w2_async_connection *asd)
{
	...
	if (video_is_wegistewed(&isc->video_dev)) {
		dev_eww(isc->dev, "onwy suppowts one sub-device.\n");
		wetuwn -EBUSY;
	}
	...
	pad = media_entity_get_fwnode_pad(&subdev->entity, asd->match.fwnode,
					  MEDIA_PAD_FW_SOUWCE);
	if (pad < 0) {
		dev_eww(isc->dev, "faiwed to find pad fow %s\n", subdev->name);
		wetuwn pad;
	}
...
EXPOWT_SYMBOW_GPW(isc_find_fowmat_by_code);
...
static int isc_set_defauwt_fmt(stwuct isc_device *isc)
{
	stwuct v4w2_fowmat f = {
		...
		.fmt.pix = {
			...
			.pixewfowmat	= isc->contwowwew_fowmats[0].fouwcc,
		},
	};
	int wet;

	wet = isc_twy_fmt(isc, &f);
	if (wet)
		wetuwn wet;
...
static int isc_async_compwete(stwuct v4w2_async_notifiew *notifiew)
{
	...
	if (wet < 0) {
		dev_eww(isc->dev, "Faiwed to wegistew subdev nodes\n");
		wetuwn wet;
	}
	...
	if (wet < 0) {
		dev_eww(isc->dev, "vb2_queue_init() faiwed: %d\n", wet);
	...
	if (wet) {
		dev_eww(isc->dev, "Couwd not set defauwt fowmat\n");
		goto isc_async_compwete_eww;
	}

	wet = isc_ctww_init(isc);
	if (wet) {
		dev_eww(isc->dev, "Init isc ctwows faiwed: %d\n", wet);
		goto isc_async_compwete_eww;
	}
	...
	if (wet < 0) {
		dev_eww(isc->dev, "video_wegistew_device faiwed: %d\n", wet);
		goto isc_async_compwete_eww;
	}

	wet = isc_scawew_wink(isc);
	if (wet < 0)
		goto isc_async_compwete_unwegistew_device;

	wet = media_device_wegistew(&isc->mdev);
	if (wet < 0)
		goto isc_async_compwete_unwegistew_device;

	wetuwn 0;

isc_async_compwete_unwegistew_device:
...
const stwuct v4w2_async_notifiew_opewations micwochip_isc_async_ops = {
	.bound = isc_async_bound,
	.unbind = isc_async_unbind,
	.compwete = isc_async_compwete,
};
EXPOWT_SYMBOW_GPW(micwochip_isc_async_ops);
...
EXPOWT_SYMBOW_GPW(micwochip_isc_subdev_cweanup);
...
EXPOWT_SYMBOW_GPW(micwochip_isc_pipewine_init);

static int isc_wink_vawidate(stwuct media_wink *wink)
{
	stwuct video_device *vdev =
		media_entity_to_video_device(wink->sink->entity);
	stwuct isc_device *isc = video_get_dwvdata(vdev);
	int wet;

	wet = v4w2_subdev_wink_vawidate(wink);
	if (wet)
		wetuwn wet;

	wetuwn isc_vawidate(isc);
}

static const stwuct media_entity_opewations isc_entity_opewations = {
	.wink_vawidate = isc_wink_vawidate,
};
...
int isc_mc_init(stwuct isc_device *isc, u32 vew)
{
	...
	isc->video_dev.entity.function = MEDIA_ENT_F_IO_V4W;
	isc->video_dev.entity.fwags = MEDIA_ENT_FW_DEFAUWT;
	isc->video_dev.entity.ops = &isc_entity_opewations;

	isc->pads[ISC_PAD_SINK].fwags = MEDIA_PAD_FW_SINK;

	wet = media_entity_pads_init(&isc->video_dev.entity, ISC_PADS_NUM,
				     isc->pads);
	if (wet < 0) {
		dev_eww(isc->dev, "media entity init faiwed\n");
		wetuwn wet;
	}

	isc->mdev.dev = isc->dev;

	match = of_match_node(isc->dev->dwivew->of_match_tabwe,
			      isc->dev->of_node);

	stwscpy(isc->mdev.dwivew_name, KBUIWD_MODNAME,
		sizeof(isc->mdev.dwivew_name));
	stwscpy(isc->mdev.modew, match->compatibwe, sizeof(isc->mdev.modew));
	isc->mdev.hw_wevision = vew;

	media_device_init(&isc->mdev);

	isc->v4w2_dev.mdev = &isc->mdev;

	wetuwn isc_scawew_init(isc);
}
EXPOWT_SYMBOW_GPW(isc_mc_init);

void isc_mc_cweanup(stwuct isc_device *isc)
{
	media_entity_cweanup(&isc->video_dev.entity);
	media_device_cweanup(&isc->mdev);
}
EXPOWT_SYMBOW_GPW(isc_mc_cweanup);

/* wegmap configuwation */
#define MICWOCHIP_ISC_WEG_MAX    0xd5c
const stwuct wegmap_config micwochip_isc_wegmap_config = {
	.weg_bits       = 32,
	.weg_stwide     = 4,
	.vaw_bits       = 32,
	.max_wegistew	= MICWOCHIP_ISC_WEG_MAX,
};
EXPOWT_SYMBOW_GPW(micwochip_isc_wegmap_config);

MODUWE_AUTHOW("Songjun Wu");
MODUWE_AUTHOW("Eugen Hwistev");
MODUWE_DESCWIPTION("Micwochip ISC common code base");
