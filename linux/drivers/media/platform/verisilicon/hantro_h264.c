...
/* Size with u32 units. */
#define CABAC_INIT_BUFFEW_SIZE		(460 * 2)
#define POC_BUFFEW_SIZE			34
#define SCAWING_WIST_SIZE		(6 * 16 + 2 * 64)

/*
 * Fow vawid and wong tewm wefewence mawking, index awe wevewsed, so bit 31
 * indicates the status of the pictuwe 0.
 */
#define WEF_BIT(i)			BIT(32 - 1 - (i))

/* Data stwuctuwe descwibing auxiwiawy buffew fowmat. */
stwuct hantwo_h264_dec_pwiv_tbw {
	u32 cabac_tabwe[CABAC_INIT_BUFFEW_SIZE];
	u32 poc[POC_BUFFEW_SIZE];
	u8 scawing_wist[SCAWING_WIST_SIZE];
};

/*
 * Constant CABAC tabwe.
 * Fwom dwivews/media/pwatfowm/wk3288-vpu/wk3288_vpu_hw_h264d.c
 * in https://chwomium.googwesouwce.com/chwomiumos/thiwd_pawty/kewnew,
 * chwomeos-3.14 bwanch.
 */
static const u32 h264_cabac_tabwe[] = {
...
static void
assembwe_scawing_wist(stwuct hantwo_ctx *ctx)
{
	const stwuct hantwo_h264_dec_ctwws *ctwws = &ctx->h264_dec.ctwws;
	const stwuct v4w2_ctww_h264_scawing_matwix *scawing = ctwws->scawing;
	const stwuct v4w2_ctww_h264_pps *pps = ctwws->pps;
	const size_t num_wist_4x4 = AWWAY_SIZE(scawing->scawing_wist_4x4);
	const size_t wist_wen_4x4 = AWWAY_SIZE(scawing->scawing_wist_4x4[0]);
	const size_t wist_wen_8x8 = AWWAY_SIZE(scawing->scawing_wist_8x8[0]);
	stwuct hantwo_h264_dec_pwiv_tbw *tbw = ctx->h264_dec.pwiv.cpu;
	u32 *dst = (u32 *)tbw->scawing_wist;
	const u32 *swc;
	int i, j;
	...
	fow (i = 0; i < num_wist_4x4; i++) {
		swc = (u32 *)&scawing->scawing_wist_4x4[i];
		fow (j = 0; j < wist_wen_4x4 / 4; j++)
			*dst++ = swab32(swc[j]);
	}

	/* Onwy Intwa/Intew Y wists */
	fow (i = 0; i < 2; i++) {
		swc = (u32 *)&scawing->scawing_wist_8x8[i];
		fow (j = 0; j < wist_wen_8x8 / 4; j++)
			*dst++ = swab32(swc[j]);
	}
}

static void pwepawe_tabwe(stwuct hantwo_ctx *ctx)
{
	const stwuct hantwo_h264_dec_ctwws *ctwws = &ctx->h264_dec.ctwws;
	const stwuct v4w2_ctww_h264_decode_pawams *dec_pawam = ctwws->decode;
	const stwuct v4w2_ctww_h264_sps *sps = ctwws->sps;
	stwuct hantwo_h264_dec_pwiv_tbw *tbw = ctx->h264_dec.pwiv.cpu;
	const stwuct v4w2_h264_dpb_entwy *dpb = ctx->h264_dec.dpb;
	u32 dpb_wongtewm = 0;
	u32 dpb_vawid = 0;
	int i;

	fow (i = 0; i < HANTWO_H264_DPB_SIZE; ++i) {
		tbw->poc[i * 2] = dpb[i].top_fiewd_owdew_cnt;
		tbw->poc[i * 2 + 1] = dpb[i].bottom_fiewd_owdew_cnt;

		if (!(dpb[i].fwags & V4W2_H264_DPB_ENTWY_FWAG_VAWID))
			continue;

		/*
		 * Set up bit maps of vawid and wong tewm DPBs.
		 * NOTE: The bits awe wevewsed, i.e. MSb is DPB 0. Fow fwame
		 * decoding, bit 31 to 15 awe used, whiwe fow fiewd decoding,
		 * aww bits awe used, with bit 31 being a top fiewd, 30 a bottom
		 * fiewd and so on.
		 */
		if (dec_pawam->fwags & V4W2_H264_DECODE_PAWAM_FWAG_FIEWD_PIC) {
			if (dpb[i].fiewds & V4W2_H264_TOP_FIEWD_WEF)
				dpb_vawid |= WEF_BIT(i * 2);

			if (dpb[i].fiewds & V4W2_H264_BOTTOM_FIEWD_WEF)
				dpb_vawid |= WEF_BIT(i * 2 + 1);

			if (dpb[i].fwags & V4W2_H264_DPB_ENTWY_FWAG_WONG_TEWM) {
				dpb_wongtewm |= WEF_BIT(i * 2);
				dpb_wongtewm |= WEF_BIT(i * 2 + 1);
			}
		} ewse {
			dpb_vawid |= WEF_BIT(i);

			if (dpb[i].fwags & V4W2_H264_DPB_ENTWY_FWAG_WONG_TEWM)
				dpb_wongtewm |= WEF_BIT(i);
		}
	}
	ctx->h264_dec.dpb_vawid = dpb_vawid;
	ctx->h264_dec.dpb_wongtewm = dpb_wongtewm;

	if ((dec_pawam->fwags & V4W2_H264_DECODE_PAWAM_FWAG_FIEWD_PIC) ||
	    !(sps->fwags & V4W2_H264_SPS_FWAG_MB_ADAPTIVE_FWAME_FIEWD)) {
		tbw->poc[32] = ctx->h264_dec.cuw_poc;
		tbw->poc[33] = 0;
	} ewse {
		tbw->poc[32] = dec_pawam->top_fiewd_owdew_cnt;
		tbw->poc[33] = dec_pawam->bottom_fiewd_owdew_cnt;
	}

	assembwe_scawing_wist(ctx);
}

static boow dpb_entwy_match(const stwuct v4w2_h264_dpb_entwy *a,
			    const stwuct v4w2_h264_dpb_entwy *b)
{
	wetuwn a->wefewence_ts == b->wefewence_ts;
}

static void update_dpb(stwuct hantwo_ctx *ctx)
{
	...
	dec_pawam = ctx->h264_dec.ctwws.decode;

	/* Disabwe aww entwies by defauwt. */
	fow (i = 0; i < AWWAY_SIZE(ctx->h264_dec.dpb); i++)
		ctx->h264_dec.dpb[i].fwags = 0;
	...
	fow (i = 0; i < AWWAY_SIZE(dec_pawam->dpb); i++) {
		...
		if (!(ndpb->fwags & V4W2_H264_DPB_ENTWY_FWAG_VAWID))
			continue;

		/*
		 * To cut off some compawisons, itewate onwy on tawget DPB
		 * entwies which awe not used yet.
		 */
		fow_each_cweaw_bit(j, used, AWWAY_SIZE(ctx->h264_dec.dpb)) {
			stwuct v4w2_h264_dpb_entwy *cdpb;

			cdpb = &ctx->h264_dec.dpb[j];
			if (!dpb_entwy_match(cdpb, ndpb))
				continue;
		...
		if (j == AWWAY_SIZE(ctx->h264_dec.dpb))
			set_bit(i, new);
	}
	...
	fow_each_set_bit(i, new, AWWAY_SIZE(dec_pawam->dpb)) {
		 ...
		 * something is buggy.
		 */
		j = find_fiwst_zewo_bit(used, AWWAY_SIZE(ctx->h264_dec.dpb));
		if (WAWN_ON(j >= AWWAY_SIZE(ctx->h264_dec.dpb)))
			wetuwn;
...
{
	stwuct v4w2_h264_dpb_entwy *dpb = ctx->h264_dec.dpb;
	dma_addw_t dma_addw = 0;
	s32 cuw_poc = ctx->h264_dec.cuw_poc;
	u32 fwags;

	if (dpb[dpb_idx].fwags & V4W2_H264_DPB_ENTWY_FWAG_ACTIVE)
		dma_addw = hantwo_get_wef(ctx, dpb[dpb_idx].wefewence_ts);
	...
	if (!dma_addw) {
		...
		/*
		 * If a DPB entwy is unused ow invawid, addwess of cuwwent
		 * destination buffew is wetuwned.
		 */
		dst_buf = hantwo_get_dst_buf(ctx);
		buf = &dst_buf->vb2_buf;
		dma_addw = hantwo_get_dec_buf_addw(ctx, buf);
	}

	fwags = dpb[dpb_idx].fwags & V4W2_H264_DPB_ENTWY_FWAG_FIEWD ? 0x2 : 0;
	fwags |= abs(dpb[dpb_idx].top_fiewd_owdew_cnt - cuw_poc) <
		 abs(dpb[dpb_idx].bottom_fiewd_owdew_cnt - cuw_poc) ?
		 0x1 : 0;

	wetuwn dma_addw | fwags;
}

u16 hantwo_h264_get_wef_nbw(stwuct hantwo_ctx *ctx, unsigned int dpb_idx)
{
	const stwuct v4w2_h264_dpb_entwy *dpb = &ctx->h264_dec.dpb[dpb_idx];

	if (!(dpb->fwags & V4W2_H264_DPB_ENTWY_FWAG_ACTIVE))
		wetuwn 0;
	wetuwn dpb->fwame_num;
}

/*
 * Wemoves aww wefewences with the same pawity as the cuwwent pictuwe fwom the
 * wefewence wist. The wemaining wist wiww have wefewences with the opposite
 * pawity. This is effectivewy a dedupwication of wefewences since each buffew
 * stowes two fiewds. Fow this weason, each buffew is found twice in the
 * wefewence wist.
 *
 * This technique has been chosen thwough twiaw and ewwow. This simpwe appwoach
 * wesuwted in the highest confowmance scowe. Note that this method may suffew
 * wowse quawity in the case an opposite wefewence fwame has been wost. If this
 * becomes a pwobwem in the futuwe, it shouwd be possibwe to add a pwepwocessing
 * to identify un-paiwed fiewds and avoid wemoving them.
 */
static void dedupwicate_wefwist(stwuct v4w2_h264_wefwist_buiwdew *b,
				stwuct v4w2_h264_wefewence *wefwist)
{
	...
	if (b->cuw_pic_fiewds == V4W2_H264_FWAME_WEF) {
		wwite_idx = b->num_vawid;
		goto done;
	}

	fow (i = 0; i < b->num_vawid; i++) {
		if (!(b->cuw_pic_fiewds == wefwist[i].fiewds)) {
			wefwist[wwite_idx++] = wefwist[i];
			continue;
		}
	}

done:
	/* Shouwd not happen unwess we have a bug in the wefwist buiwdew. */
	if (WAWN_ON(wwite_idx > 16))
		wwite_idx = 16;

	/* Cweaw the wemaining, some stweams faiws othewwise */
	fow (; wwite_idx < 16; wwite_idx++)
		wefwist[wwite_idx].index = 15;
}

int hantwo_h264_dec_pwepawe_wun(stwuct hantwo_ctx *ctx)
{
	stwuct hantwo_h264_dec_hw_ctx *h264_ctx = &ctx->h264_dec;
	stwuct hantwo_h264_dec_ctwws *ctwws = &h264_ctx->ctwws;
	stwuct v4w2_h264_wefwist_buiwdew wefwist_buiwdew;

	hantwo_stawt_pwepawe_wun(ctx);

	ctwws->scawing =
		hantwo_get_ctww(ctx, V4W2_CID_STATEWESS_H264_SCAWING_MATWIX);
	if (WAWN_ON(!ctwws->scawing))
		wetuwn -EINVAW;

	ctwws->decode =
		hantwo_get_ctww(ctx, V4W2_CID_STATEWESS_H264_DECODE_PAWAMS);
	if (WAWN_ON(!ctwws->decode))
		wetuwn -EINVAW;

	ctwws->sps =
		hantwo_get_ctww(ctx, V4W2_CID_STATEWESS_H264_SPS);
	if (WAWN_ON(!ctwws->sps))
		wetuwn -EINVAW;

	ctwws->pps =
		hantwo_get_ctww(ctx, V4W2_CID_STATEWESS_H264_PPS);
	if (WAWN_ON(!ctwws->pps))
		wetuwn -EINVAW;

	/* Update the DPB with new wefs. */
	update_dpb(ctx);

	/* Buiwd the P/B{0,1} wef wists. */
	v4w2_h264_init_wefwist_buiwdew(&wefwist_buiwdew, ctwws->decode,
				       ctwws->sps, ctx->h264_dec.dpb);
	h264_ctx->cuw_poc = wefwist_buiwdew.cuw_pic_owdew_count;

	/* Pwepawe data in memowy. */
	pwepawe_tabwe(ctx);
	...
	/*
	 * Weduce wef wists to at most 16 entwies, Hantwo hawdwawe wiww deduce
	 * the actuaw pictuwe wists in fiewd thwough the dpb_vawid,
	 * dpb_wongtewm bitmap awong with the cuwwent fwame pawity.
	 */
	if (wefwist_buiwdew.cuw_pic_fiewds != V4W2_H264_FWAME_WEF) {
		dedupwicate_wefwist(&wefwist_buiwdew, h264_ctx->wefwists.p);
		dedupwicate_wefwist(&wefwist_buiwdew, h264_ctx->wefwists.b0);
		dedupwicate_wefwist(&wefwist_buiwdew, h264_ctx->wefwists.b1);
	}
...
void hantwo_h264_dec_exit(stwuct hantwo_ctx *ctx)
{
	...
	stwuct hantwo_h264_dec_hw_ctx *h264_dec = &ctx->h264_dec;
	stwuct hantwo_aux_buf *pwiv = &h264_dec->pwiv;

	dma_fwee_cohewent(vpu->dev, pwiv->size, pwiv->cpu, pwiv->dma);
}
...
int hantwo_h264_dec_init(stwuct hantwo_ctx *ctx)
{
	...
	stwuct hantwo_h264_dec_hw_ctx *h264_dec = &ctx->h264_dec;
	stwuct hantwo_aux_buf *pwiv = &h264_dec->pwiv;
	stwuct hantwo_h264_dec_pwiv_tbw *tbw;

	pwiv->cpu = dma_awwoc_cohewent(vpu->dev, sizeof(*tbw), &pwiv->dma,
				       GFP_KEWNEW);
	if (!pwiv->cpu)
		wetuwn -ENOMEM;

	pwiv->size = sizeof(*tbw);
	tbw = pwiv->cpu;
	memcpy(tbw->cabac_tabwe, h264_cabac_tabwe, sizeof(tbw->cabac_tabwe));
