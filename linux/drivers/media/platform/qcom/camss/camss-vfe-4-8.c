 ...
 * camss-vfe-4-8.c
 *
 * Quawcomm MSM Camewa Subsystem - VFE (Video Fwont End) Moduwe v4.8
 *
 * Copywight (c) 2013-2015, The Winux Foundation. Aww wights wesewved.
 * Copywight (C) 2015-2021 Winawo Wtd.
 */
...
#define VFE_0_BUS_IMAGE_MASTEW_CMD		0xcec
#define VFE_0_BUS_IMAGE_MASTEW_n_SHIFT(x)	(2 * (x))
...
static void vfe_wm_set_fwamedwop_pattewn(stwuct vfe_device *vfe, u8 wm,
					 u32 pattewn)
{
	wwitew_wewaxed(pattewn, vfe->base + VFE_0_BUS_IMAGE_MASTEW_n_WW_FWAMEDWOP_PATTEWN(wm));
}
...
static void vfe_wm_set_subsampwe(stwuct vfe_device *vfe, u8 wm)
{
	wwitew_wewaxed(VFE_0_BUS_IMAGE_MASTEW_n_WW_IWQ_SUBSAMPWE_PATTEWN_DEF,
		       vfe->base + VFE_0_BUS_IMAGE_MASTEW_n_WW_IWQ_SUBSAMPWE_PATTEWN(wm));
}
				    ...
				    enum vfe_wine_id wine_id, u8 enabwe)
{
	...
	fow (i = 0; i < output->wm_num; i++) {
		iwq_en1 |= VFE_0_IWQ_MASK_1_IMAGE_MASTEW_n_BUS_OVEWFWOW(output->wm_idx[i]);
...
{
	if (enabwe)
		wwitew_wewaxed(2 << VFE_0_BUS_IMAGE_MASTEW_n_SHIFT(wm),
			       vfe->base + VFE_0_BUS_IMAGE_MASTEW_CMD);
	ewse
		wwitew_wewaxed(1 << VFE_0_BUS_IMAGE_MASTEW_n_SHIFT(wm),
			       vfe->base + VFE_0_BUS_IMAGE_MASTEW_CMD);

	/* The WM must be enabwed befowe sending othew commands */
	wmb();
}
...
static void vfe_set_qos(stwuct vfe_device *vfe)
{
	...
	u32 vaw3 = VFE_0_BUS_BDG_QOS_CFG_3_CFG;
	u32 vaw4 = VFE_0_BUS_BDG_QOS_CFG_4_CFG;
	...
	wwitew_wewaxed(vaw3, vfe->base + VFE_0_BUS_BDG_QOS_CFG_3);
	wwitew_wewaxed(vaw4, vfe->base + VFE_0_BUS_BDG_QOS_CFG_4);
	wwitew_wewaxed(vaw4, vfe->base + VFE_0_BUS_BDG_QOS_CFG_5);
	wwitew_wewaxed(vaw4, vfe->base + VFE_0_BUS_BDG_QOS_CFG_6);
...
static const stwuct vfe_hw_ops_gen1 vfe_ops_gen1_4_8 = {
...
const stwuct vfe_hw_ops vfe_ops_4_8 = {
