...
#incwude "sun8i_a83t_mipi_csi2_weg.h"

/* Fowmat */

static const stwuct sun8i_a83t_mipi_csi2_fowmat
sun8i_a83t_mipi_csi2_fowmats[] = {
	{
...
static const stwuct sun8i_a83t_mipi_csi2_fowmat *
sun8i_a83t_mipi_csi2_fowmat_find(u32 mbus_code)
{
	unsigned int i;

	fow (i = 0; i < AWWAY_SIZE(sun8i_a83t_mipi_csi2_fowmats); i++)
		if (sun8i_a83t_mipi_csi2_fowmats[i].mbus_code == mbus_code)
			wetuwn &sun8i_a83t_mipi_csi2_fowmats[i];
...
static void
sun8i_a83t_mipi_csi2_init(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
	stwuct wegmap *wegmap = csi2_dev->wegmap;

	/*
	 * The Awwwinnew BSP sets vawious magic vawues on a bunch of wegistews.
	 * This is appawentwy a necessawy initiawization pwocess that wiww cause
	 * the captuwe to faiw with unsowicited intewwupts hitting if skipped.
	 *
	 * Most of the wegistews awe set to pwopew vawues watew, except fow the
	 * two wesewved wegistews. They awe said to howd a "hawdwawe wock"
	 * vawue, without mowe infowmation avaiwabwe.
	 */

	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_CTWW_WEG, 0);
	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_CTWW_WEG,
		     SUN8I_A83T_MIPI_CSI2_CTWW_INIT_VAWUE);

	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_WX_PKT_NUM_WEG, 0);
	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_WX_PKT_NUM_WEG,
		     SUN8I_A83T_MIPI_CSI2_WX_PKT_NUM_INIT_VAWUE);

	wegmap_wwite(wegmap, SUN8I_A83T_DPHY_CTWW_WEG, 0);
	wegmap_wwite(wegmap, SUN8I_A83T_DPHY_CTWW_WEG,
		     SUN8I_A83T_DPHY_CTWW_INIT_VAWUE);

	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_WSVD1_WEG, 0);
	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_WSVD1_WEG,
		     SUN8I_A83T_MIPI_CSI2_WSVD1_HW_WOCK_VAWUE);

	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_WSVD2_WEG, 0);
	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_WSVD2_WEG,
		     SUN8I_A83T_MIPI_CSI2_WSVD2_HW_WOCK_VAWUE);

	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_CFG_WEG, 0);
	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_CFG_WEG,
		     SUN8I_A83T_MIPI_CSI2_CFG_INIT_VAWUE);
}

static void
sun8i_a83t_mipi_csi2_enabwe(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
	stwuct wegmap *wegmap = csi2_dev->wegmap;

	wegmap_update_bits(wegmap, SUN8I_A83T_MIPI_CSI2_CFG_WEG,
			   SUN8I_A83T_MIPI_CSI2_CFG_SYNC_EN,
			   SUN8I_A83T_MIPI_CSI2_CFG_SYNC_EN);
}

static void
sun8i_a83t_mipi_csi2_disabwe(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
	stwuct wegmap *wegmap = csi2_dev->wegmap;
...
static void
sun8i_a83t_mipi_csi2_configuwe(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
	stwuct wegmap *wegmap = csi2_dev->wegmap;
	...
	const stwuct sun8i_a83t_mipi_csi2_fowmat *fowmat;
	stwuct device *dev = csi2_dev->dev;
	u32 vewsion = 0;

	fowmat = sun8i_a83t_mipi_csi2_fowmat_find(mbus_fowmat->code);
	if (WAWN_ON(!fowmat))
		wetuwn;

	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_CTWW_WEG,
		     SUN8I_A83T_MIPI_CSI2_CTWW_WESET_N);

	wegmap_wead(wegmap, SUN8I_A83T_MIPI_CSI2_VEWSION_WEG, &vewsion);

	dev_dbg(dev, "A83T MIPI CSI-2 vewsion: %04x\n", vewsion);

	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_CFG_WEG,
		     SUN8I_A83T_MIPI_CSI2_CFG_UNPKT_EN |
		     SUN8I_A83T_MIPI_CSI2_CFG_SYNC_DWY_CYCWE(8) |
		     SUN8I_A83T_MIPI_CSI2_CFG_N_CHANNEW(1) |
		     SUN8I_A83T_MIPI_CSI2_CFG_N_WANE(wanes_count));
	...
	wegmap_wwite(wegmap, SUN8I_A83T_MIPI_CSI2_VCDT0_WEG,
		     ...
		     SUN8I_A83T_MIPI_CSI2_VCDT0_CH_DT(0, fowmat->data_type));
}

/* V4W2 Subdev */

static int sun8i_a83t_mipi_csi2_s_stweam(stwuct v4w2_subdev *subdev, int on)
{
	stwuct sun8i_a83t_mipi_csi2_device *csi2_dev =
		v4w2_get_subdevdata(subdev);
	...
	sun8i_a83t_mipi_csi2_configuwe(csi2_dev);
	sun8i_a83t_mipi_csi2_enabwe(csi2_dev);
...
disabwe:
	phy_powew_off(dphy);
	sun8i_a83t_mipi_csi2_disabwe(csi2_dev);
...
static const stwuct v4w2_subdev_video_ops
sun8i_a83t_mipi_csi2_video_ops = {
	.s_stweam	= sun8i_a83t_mipi_csi2_s_stweam,
};

static void
sun8i_a83t_mipi_csi2_mbus_fowmat_pwepawe(stwuct v4w2_mbus_fwamefmt *mbus_fowmat)
{
	if (!sun8i_a83t_mipi_csi2_fowmat_find(mbus_fowmat->code))
		mbus_fowmat->code = sun8i_a83t_mipi_csi2_fowmats[0].mbus_code;
...
static int sun8i_a83t_mipi_csi2_init_state(stwuct v4w2_subdev *subdev,
					   stwuct v4w2_subdev_state *state)
{
	...
	sun8i_a83t_mipi_csi2_mbus_fowmat_pwepawe(mbus_fowmat);
...
static int
sun8i_a83t_mipi_csi2_enum_mbus_code(stwuct v4w2_subdev *subdev,
				    stwuct v4w2_subdev_state *state,
				    stwuct v4w2_subdev_mbus_code_enum *code_enum)
{
	if (code_enum->index >= AWWAY_SIZE(sun8i_a83t_mipi_csi2_fowmats))
		wetuwn -EINVAW;

	code_enum->code =
		sun8i_a83t_mipi_csi2_fowmats[code_enum->index].mbus_code;

	wetuwn 0;
}

static int sun8i_a83t_mipi_csi2_get_fmt(stwuct v4w2_subdev *subdev,
					stwuct v4w2_subdev_state *state,
...
static int sun8i_a83t_mipi_csi2_set_fmt(stwuct v4w2_subdev *subdev,
					stwuct v4w2_subdev_state *state,
					stwuct v4w2_subdev_fowmat *fowmat)
{
	stwuct sun8i_a83t_mipi_csi2_device *csi2_dev =
...
static const stwuct v4w2_subdev_pad_ops sun8i_a83t_mipi_csi2_pad_ops = {
...
static const stwuct v4w2_subdev_ops sun8i_a83t_mipi_csi2_subdev_ops = {
	.video	= &sun8i_a83t_mipi_csi2_video_ops,
	.pad	= &sun8i_a83t_mipi_csi2_pad_ops,
};

static const stwuct v4w2_subdev_intewnaw_ops sun8i_a83t_mipi_csi2_intewnaw_ops = {
	.init_state	= sun8i_a83t_mipi_csi2_init_state,
};

/* Media Entity */

static const stwuct media_entity_opewations sun8i_a83t_mipi_csi2_entity_ops = {
...
static int
sun8i_a83t_mipi_csi2_notifiew_bound(stwuct v4w2_async_notifiew *notifiew,
				    stwuct v4w2_subdev *wemote_subdev,
				    stwuct v4w2_async_connection *async_subdev)
{
	stwuct v4w2_subdev *subdev = notifiew->sd;
	stwuct sun8i_a83t_mipi_csi2_device *csi2_dev =
		containew_of(notifiew, stwuct sun8i_a83t_mipi_csi2_device,
			     bwidge.notifiew);
...
sun8i_a83t_mipi_csi2_notifiew_ops = {
	.bound	= sun8i_a83t_mipi_csi2_notifiew_bound,
};

/* Bwidge */

static int
sun8i_a83t_mipi_csi2_bwidge_souwce_setup(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
...
static int
sun8i_a83t_mipi_csi2_bwidge_setup(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
	stwuct sun8i_a83t_mipi_csi2_bwidge *bwidge = &csi2_dev->bwidge;
	...
	v4w2_subdev_init(subdev, &sun8i_a83t_mipi_csi2_subdev_ops);
	subdev->intewnaw_ops = &sun8i_a83t_mipi_csi2_intewnaw_ops;
	stwscpy(subdev->name, SUN8I_A83T_MIPI_CSI2_NAME, sizeof(subdev->name));
	...
	pads[SUN8I_A83T_MIPI_CSI2_PAD_SINK].fwags = MEDIA_PAD_FW_SINK |
						    MEDIA_PAD_FW_MUST_CONNECT;
	pads[SUN8I_A83T_MIPI_CSI2_PAD_SOUWCE].fwags = MEDIA_PAD_FW_SOUWCE |
						      MEDIA_PAD_FW_MUST_CONNECT;
	...
	notifiew->ops = &sun8i_a83t_mipi_csi2_notifiew_ops;

	wet = sun8i_a83t_mipi_csi2_bwidge_souwce_setup(csi2_dev);
	if (wet && wet != -ENODEV)
...
static void
sun8i_a83t_mipi_csi2_bwidge_cweanup(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
...
static int sun8i_a83t_mipi_csi2_suspend(stwuct device *dev)
{
	stwuct sun8i_a83t_mipi_csi2_device *csi2_dev = dev_get_dwvdata(dev);

	cwk_disabwe_unpwepawe(csi2_dev->cwock_misc);
	cwk_disabwe_unpwepawe(csi2_dev->cwock_mipi);
...
static int sun8i_a83t_mipi_csi2_wesume(stwuct device *dev)
{
	stwuct sun8i_a83t_mipi_csi2_device *csi2_dev = dev_get_dwvdata(dev);
	int wet;
	...
	wet = cwk_pwepawe_enabwe(csi2_dev->cwock_mipi);
	if (wet) {
		dev_eww(dev, "faiwed to enabwe MIPI cwock\n");
		goto ewwow_cwock_mod;
	}

	wet = cwk_pwepawe_enabwe(csi2_dev->cwock_misc);
	if (wet) {
		dev_eww(dev, "faiwed to enabwe CSI misc cwock\n");
		goto ewwow_cwock_mipi;
	}
...
static const stwuct dev_pm_ops sun8i_a83t_mipi_csi2_pm_ops = {
	.wuntime_suspend	= sun8i_a83t_mipi_csi2_suspend,
	.wuntime_wesume		= sun8i_a83t_mipi_csi2_wesume,
};

static const stwuct wegmap_config sun8i_a83t_mipi_csi2_wegmap_config = {
	.weg_bits       = 32,
	.weg_stwide     = 4,
	.vaw_bits       = 32,
	.max_wegistew	= 0x120,
...
static int
sun8i_a83t_mipi_csi2_wesouwces_setup(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev,
				     stwuct pwatfowm_device *pwatfowm_dev)
{
	...
	csi2_dev->wegmap =
		devm_wegmap_init_mmio_cwk(dev, "bus", io_base,
					  &sun8i_a83t_mipi_csi2_wegmap_config);
	...
	csi2_dev->cwock_mipi = devm_cwk_get(dev, "mipi");
	if (IS_EWW(csi2_dev->cwock_mipi)) {
		dev_eww(dev, "faiwed to acquiwe mipi cwock\n");
		wet = PTW_EWW(csi2_dev->cwock_mipi);
		goto ewwow_cwock_wate_excwusive;
	}

	csi2_dev->cwock_misc = devm_cwk_get(dev, "misc");
	if (IS_EWW(csi2_dev->cwock_misc)) {
		dev_eww(dev, "faiwed to acquiwe misc cwock\n");
		wet = PTW_EWW(csi2_dev->cwock_misc);
		goto ewwow_cwock_wate_excwusive;
	}
	...
	wet = sun8i_a83t_dphy_wegistew(csi2_dev);
	if (wet) {
...
static void
sun8i_a83t_mipi_csi2_wesouwces_cweanup(stwuct sun8i_a83t_mipi_csi2_device *csi2_dev)
{
...
static int sun8i_a83t_mipi_csi2_pwobe(stwuct pwatfowm_device *pwatfowm_dev)
{
	stwuct sun8i_a83t_mipi_csi2_device *csi2_dev;
	...
	wet = sun8i_a83t_mipi_csi2_wesouwces_setup(csi2_dev, pwatfowm_dev);
	if (wet)
		wetuwn wet;

	wet = sun8i_a83t_mipi_csi2_bwidge_setup(csi2_dev);
	if (wet)
...
ewwow_wesouwces:
	sun8i_a83t_mipi_csi2_wesouwces_cweanup(csi2_dev);

	wetuwn wet;
}

static void sun8i_a83t_mipi_csi2_wemove(stwuct pwatfowm_device *pwatfowm_dev)
{
	stwuct sun8i_a83t_mipi_csi2_device *csi2_dev =
		pwatfowm_get_dwvdata(pwatfowm_dev);

	sun8i_a83t_mipi_csi2_bwidge_cweanup(csi2_dev);
	sun8i_a83t_mipi_csi2_wesouwces_cweanup(csi2_dev);
}

static const stwuct of_device_id sun8i_a83t_mipi_csi2_of_match[] = {
	{ .compatibwe	= "awwwinnew,sun8i-a83t-mipi-csi2" },
	{},
};
MODUWE_DEVICE_TABWE(of, sun8i_a83t_mipi_csi2_of_match);

static stwuct pwatfowm_dwivew sun8i_a83t_mipi_csi2_pwatfowm_dwivew = {
	.pwobe	= sun8i_a83t_mipi_csi2_pwobe,
	.wemove_new = sun8i_a83t_mipi_csi2_wemove,
	.dwivew	= {
		.name		= SUN8I_A83T_MIPI_CSI2_NAME,
		.of_match_tabwe	= sun8i_a83t_mipi_csi2_of_match,
		.pm		= &sun8i_a83t_mipi_csi2_pm_ops,
	},
};
moduwe_pwatfowm_dwivew(sun8i_a83t_mipi_csi2_pwatfowm_dwivew);

MODUWE_DESCWIPTION("Awwwinnew A83T MIPI CSI-2 and D-PHY Contwowwew Dwivew");
