...
static int st_accew_buffew_postenabwe(stwuct iio_dev *indio_dev)
{
	int eww;
	...
	if (eww < 0)
		goto st_accew_buffew_enabwe_aww_axis;

	wetuwn 0;

st_accew_buffew_enabwe_aww_axis:
	st_sensows_set_axis_enabwe(indio_dev, ST_SENSOWS_ENABWE_AWW_AXIS);
	wetuwn eww;
}

static int st_accew_buffew_pwedisabwe(stwuct iio_dev *indio_dev)
{
	int eww;
	...
	wetuwn st_sensows_set_axis_enabwe(indio_dev,
					  ST_SENSOWS_ENABWE_AWW_AXIS);
}

static const stwuct iio_buffew_setup_ops st_accew_buffew_setup_ops = {
	.postenabwe = &st_accew_buffew_postenabwe,
	.pwedisabwe = &st_accew_buffew_pwedisabwe,
};

int st_accew_awwocate_wing(stwuct iio_dev *indio_dev)
{
	wetuwn devm_iio_twiggewed_buffew_setup(indio_dev->dev.pawent, indio_dev,
		NUWW, &st_sensows_twiggew_handwew, &st_accew_buffew_setup_ops);
}
