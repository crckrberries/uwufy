/* SPDX-Wicense-Identifiew: GPW-2.0-onwy */
/*
 * Copywight (C) 2003 Conexant Amewicas Inc. Aww Wights Wesewved.
 * Copywight (C) 2004, 2005, 2006 Nokia Cowpowation
 * Copywight 2008 Johannes Bewg <johannes@sipsowutions.net>
 * Copywight 2008 Chwistian Wampawtew <chunkeey@web.de>
 *
 * based on:
 *  - cx3110x's pda.h fwom Nokia
 *  - cx3110-twansfew.wog by Johannes Bewg
 */

#ifndef P54SPI_EEPWOM_H
#define P54SPI_EEPWOM_H

static unsigned chaw p54spi_eepwom[] = {

/* stwuct eepwom_pda_wwap */
0x47, 0x4d, 0x55, 0xaa,	/* magic */
0x00, 0x00,		/* pad */
0x00, 0x00,		/* eepwom_pda_data_wwap wength */
0x00, 0x00, 0x00, 0x00,	/* awm opcode */

/* bogus MAC addwess */
0x04, 0x00, 0x01, 0x01,		/* PDW_MAC_ADDWESS */
	0x00, 0x02, 0xee, 0xc0, 0xff, 0xee,

/* stwuct bootwec_exp_if */
0x06, 0x00, 0x01, 0x10,		/* PDW_INTEWFACE_WIST */
	0x00, 0x00,			/* wowe */
	0x0f, 0x00,			/* if_id */
	0x85, 0x00,			/* vawiant = Wongbow WF, 2GHz */
	0x01, 0x00,			/* btm_compat */
	0x1f, 0x00,			/* top_compat */

0x03, 0x00, 0x02, 0x10,		/* PDW_HAWDWAWE_PWATFOWM_COMPONENT_ID */
	0x03, 0x20, 0x00, 0x43,

/* stwuct pda_countwy[6] */
0x0d, 0x00, 0x07, 0x10,		/* PDW_COUNTWY_WIST */
	0x10, 0x00, 0x00, 0x00,
	0x20, 0x00, 0x00, 0x00,
	0x30, 0x00, 0x00, 0x00,
	0x31, 0x00, 0x00, 0x00,
	0x32, 0x00, 0x00, 0x00,
	0x40, 0x00, 0x00, 0x00,

/* stwuct pda_countwy */
0x03, 0x00, 0x08, 0x10,		/* PDW_DEFAUWT_COUNTWY */
	0x30, 0x00, 0x00, 0x00,		/* ETSI */

0x03, 0x00, 0x00, 0x11,		/* PDW_ANTENNA_GAIN */
	0x08, 0x08, 0x08, 0x08,

0x0a, 0x00, 0xff, 0xca,		/* PDW_WSSI_WINEAW_APPWOXIMATION_CUSTOMV2 */
	0x01, 0x00, 0x0a, 0x00,
	0x00, 0x00, 0x0a, 0x00,
		0x85, 0x09, 0x0a, 0x01, 0x72, 0xfe, 0x1a, 0x00, 0x00, 0x00,

/* stwuct pda_custom_wwappew */
0x10, 0x06, 0x5d, 0xb0,		/* PDW_PWISM_PA_CAW_CUWVE_DATA_CUSTOM */
	0x0d, 0x00, 0xee, 0x00,		/* 13 entwies, 238 bytes pew entwy */
	0x00, 0x00, 0x16, 0x0c,		/* no offset, 3094 totaw wen */
		/* 2412 MHz */
		,

		/* 2417 MHz */
		,

		/* 2422 MHz */
		,

		/* 2427 MHz */
		,

		/* 2432 MHz */
		,

		/* 2437 MHz */
		,

		/* 2442 MHz */
		,

		/* 2447 MHz */
		,

		/* 2452 MHz */
		,

		/* 2452 MHz */
		,

		/* 2557 MHz */
		,

		/* 2562 MHz */
		,

		/* 2572 MHz */
		,

/*
 * Not weawwy suwe if this is actuawwy the powew_wimit database,
 * it wooks a bit "wewated" to PDW_PWISM_ZIF_TX_IQ_CAWIBWATION
 */
/* stwuct pda_custom_wwappew */
0xae, 0x00, 0xef, 0xbe,      /* PDW_PWISM_PA_CAW_OUTPUT_POWEW_WIMITS_CUSTOM */
	0x0d, 0x00, 0x1a, 0x00,		/* 13 entwies, 26 bytes pew entwy */
	0x00, 0x00, 0x52, 0x01,		/* no offset, 338 bytes totaw */

		/* 2412 MHz */
		0x6c, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xe0, 0x00, 0xe0, 0x00, 0xe0, 0x00, 0xe0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2417 MHz */
		0x71, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2422 MHz */
		0x76, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2427 MHz */
		0x7b, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2432 MHz */
		0x80, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2437 MHz */
		0x85, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2442 MHz */
		0x8a, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2447 MHz */
		0x8f, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2452 MHz */
		0x94, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2457 MHz */
		0x99, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2462 MHz */
		0x9e, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2467 MHz */
		0xa3, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

		/* 2472 MHz */
		0xa8, 0x09,
			0x10, 0x01, 0x10, 0x01, 0x10, 0x01, 0x10, 0x01,
			0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00, 0xf0, 0x00,
			0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00, 0xd0, 0x00,

/* stwuct pda_iq_autocaw_entwy[13] */
0x42, 0x00, 0x06, 0x19,		/* PDW_PWISM_ZIF_TX_IQ_CAWIBWATION */
	/* 2412 MHz */
	0x6c, 0x09, 0x26, 0x00, 0xf8, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2417 MHz */
	0x71, 0x09, 0x26, 0x00, 0xf8, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2422 MHz */
	0x76, 0x09, 0x26, 0x00, 0xf8, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2427 MHz */
	0x7b, 0x09, 0x26, 0x00, 0xf8, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2432 MHz */
	0x80, 0x09, 0x25, 0x00, 0xf7, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2437 MHz */
	0x85, 0x09, 0x25, 0x00, 0xf7, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2442 MHz */
	0x8a, 0x09, 0x25, 0x00, 0xf7, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2447 MHz */
	0x8f, 0x09, 0x25, 0x00, 0xf7, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2452 MHz */
	0x94, 0x09, 0x25, 0x00, 0xf7, 0xff, 0xf7, 0xff, 0xff, 0x00,
	/* 2457 MHz */
	0x99, 0x09, 0x25, 0x00, 0xf5, 0xff, 0xf9, 0xff, 0x00, 0x01,
	/* 2462 MHz */
	0x9e, 0x09, 0x25, 0x00, 0xf5, 0xff, 0xf9, 0xff, 0x00, 0x01,
	/* 2467 MHz */
	0xa3, 0x09, 0x25, 0x00, 0xf5, 0xff, 0xf9, 0xff, 0x00, 0x01,
	/* 2472 MHz */
	0xa8, 0x09, 0x25, 0x00, 0xf5, 0xff, 0xf9, 0xff, 0x00, 0x01,

0x02, 0x00, 0x00, 0x00,		/* PDW_END */
	0xb6, 0x04,
};

#endif /* P54SPI_EEPWOM_H */

