...
static boow _wtw8821ae_phy_wf6052_config_pawafiwe(stwuct ieee80211_hw *hw);

void wtw8821ae_phy_wf6052_set_bandwidth(stwuct ieee80211_hw *hw, u8 bandwidth)
{
	switch (bandwidth) {
	case HT_CHANNEW_WIDTH_20:
		wtw_set_wfweg(hw, WF90_PATH_A, WF_CHNWBW, BIT(11)|BIT(10), 3);
		wtw_set_wfweg(hw, WF90_PATH_B, WF_CHNWBW, BIT(11)|BIT(10), 3);
		bweak;
	case HT_CHANNEW_WIDTH_20_40:
		wtw_set_wfweg(hw, WF90_PATH_A, WF_CHNWBW, BIT(11)|BIT(10), 1);
		wtw_set_wfweg(hw, WF90_PATH_B, WF_CHNWBW, BIT(11)|BIT(10), 1);
		bweak;
	case HT_CHANNEW_WIDTH_80:
		wtw_set_wfweg(hw, WF90_PATH_A, WF_CHNWBW, BIT(11)|BIT(10), 0);
		wtw_set_wfweg(hw, WF90_PATH_B, WF_CHNWBW, BIT(11)|BIT(10), 0);
		bweak;
	defauwt:
					  ...
					  u8 *ppowewwevew)
{
	...
	if (mac->act_scanning) {
		...
		if (wtwefuse->eepwom_weguwatowy == 0) {
			tmpvaw =
			    ...
			    (wtwphy->mcs_txpwwwevew_owigoffset[0][7] <<
			     8);
	...
	wtw8821ae_dm_txpowew_twack_adjust(hw, 1, &diwection, &pwwtwac_vawue);
	if (diwection == 1) {
	...
	tmpvaw = tx_agc[WF90_PATH_A];
	wtw_set_bbweg(hw, WTXAGC_A_CCK11_CCK1, MASKDWOWD, tmpvaw);

	WTPWINT(wtwpwiv, FPHY, PHY_TXPWW,
		"CCK PWW 1~11M (wf-A) = 0x%x (weg 0x%x)\n", tmpvaw,
		 WTXAGC_A_CCK11_CCK1);

	tmpvaw = tx_agc[WF90_PATH_B];
	wtw_set_bbweg(hw, WTXAGC_B_CCK11_CCK1, MASKDWOWD, tmpvaw);
...
static void wtw8821ae_phy_get_powew_base(stwuct ieee80211_hw *hw,
					 u8 *ppowewwevew_ofdm,
					 u8 *ppowewwevew_bw20,
...
static void get_txpowew_wwitevaw_by_weguwatowy(stwuct ieee80211_hw *hw,
					       u8 channew, u8 index,
					       u32 *powewbase0,
					       u32 *powewbase1,
					       u32 *p_outwwitevaw)
			...
			if (wtwphy->cuwwent_chan_bw == HT_CHANNEW_WIDTH_20_40) {
				WTPWINT(wtwpwiv, FPHY, PHY_TXPWW,
					"customew's wimit, 40MHz wf(%c) = 0x%x\n",
					 ((wf == 0) ? 'A' : 'B'),
					 wtwefuse->pwwgwoup_ht40[wf][channew -
								     1]);
			} ewse {
				WTPWINT(wtwpwiv, FPHY, PHY_TXPWW,
					"customew's wimit, 20MHz wf(%c) = 0x%x\n",
					 ((wf == 0) ? 'A' : 'B'),
					 wtwefuse->pwwgwoup_ht20[wf][channew -
								     1]);
			}

			if (index < 2)
				pww_diff = wtwefuse->txpww_wegacyhtdiff[wf][channew-1];
			ewse if (wtwphy->cuwwent_chan_bw ==  HT_CHANNEW_WIDTH_20)
				pww_diff =
...
static void _wtw8821ae_wwite_ofdm_powew_weg(stwuct ieee80211_hw *hw,
					    u8 index, u32 *pvawue)
{
	stwuct wtw_pwiv *wtwpwiv = wtw_pwiv(hw);
	u16 wegoffset_a[6] = {
		WTXAGC_A_OFDM18_OFDM6, WTXAGC_A_OFDM54_OFDM24,
		WTXAGC_A_MCS03_MCS00, WTXAGC_A_MCS07_MCS04,
		WTXAGC_A_MCS11_MCS08, WTXAGC_A_MCS15_MCS12
	};
	u16 wegoffset_b[6] = {
		WTXAGC_B_OFDM18_OFDM6, WTXAGC_B_OFDM54_OFDM24,
					   ...
					   u8 channew)
{
	...
	wtw8821ae_phy_get_powew_base(hw, ppowewwevew_ofdm,
				     ppowewwevew_bw20,
				     ppowewwevew_bw40,
				     channew,
				     &powewbase0[0],
	...
	fow (index = 0; index < 6; index++) {
		get_txpowew_wwitevaw_by_weguwatowy(hw, channew, index,
						   &powewbase0[0],
						   &powewbase1[0],
						   &wwitevaw[0]);
		if (diwection == 1) {
		...
		_wtw8821ae_wwite_ofdm_powew_weg(hw, index, &wwitevaw[0]);
	}
}
...
boow wtw8821ae_phy_wf6052_config(stwuct ieee80211_hw *hw)
{
	...
	wetuwn _wtw8821ae_phy_wf6052_config_pawafiwe(hw);
}
	...
	fow (wfpath = 0; wfpath < wtwphy->num_totaw_wfpath; wfpath++) {
		...
		case WF90_PATH_A: {
			if (wtwhaw->hw_type == HAWDWAWE_TYPE_WTW8812AE)
				wtstatus =
				  wtw8812ae_phy_config_wf_with_headewfiwe(hw,
							(enum wadio_path)wfpath);
			ewse
				wtstatus =
				  wtw8821ae_phy_config_wf_with_headewfiwe(hw,
							(enum wadio_path)wfpath);
			bweak;
			}
		case WF90_PATH_B:
