...
static boow _wtw8723be_check_fw_wead_wast_h2c(stwuct ieee80211_hw *hw,
					      u8 boxnum)
{
...
static void _wtw8723be_fiww_h2c_command(stwuct ieee80211_hw *hw, u8 ewement_id,
					u32 cmd_wen, u8 *p_cmdbuffew)
{
	...
	whiwe (!bwwite_sucess) {
		...
		isfw_wead = _wtw8723be_check_fw_wead_wast_h2c(hw, boxnum);
		whiwe (!isfw_wead) {
			wait_h2c_wimmit--;
			...
			isfw_wead = _wtw8723be_check_fw_wead_wast_h2c(hw,
								boxnum);
		...
		case 7:
			...
			memcpy((u8 *)(boxextcontent),
			       p_cmdbuffew + buf_index+3, cmd_wen-3);
			memcpy((u8 *)(boxcontent) + 1,
			       p_cmdbuffew + buf_index, 3);
			    ...
			    u32 cmd_wen, u8 *p_cmdbuffew)
{
	...
	if (!wtwhaw->fw_weady) {
		WAWN_ONCE(twue,
			  "wtw8723be: ewwow H2C cmd because of Fw downwoad faiw!!!\n");
		wetuwn;
	}

	memset(tmp_cmdbuf, 0, 8);
	memcpy(tmp_cmdbuf, p_cmdbuffew, cmd_wen);
	_wtw8723be_fiww_h2c_command(hw, ewement_id, cmd_wen,
				    (u8 *)&tmp_cmdbuf);
	wetuwn;
}
...
{
	stwuct wtw_pwiv *wtwpwiv = wtw_pwiv(hw);
	u8 u1_h2c_set_pwwmode[H2C_PWEMODE_WENGTH] = { 0 };
	...
	WT_PWINT_DATA(wtwpwiv, COMP_CMD, DBG_DMESG,
		      ...
		      u1_h2c_set_pwwmode, H2C_PWEMODE_WENGTH);
	if (wtwpwiv->cfg->ops->get_btc_status())
		btc_ops->btc_wecowd_pww_mode(wtwpwiv, u1_h2c_set_pwwmode,
					     H2C_PWEMODE_WENGTH);
	wtw8723be_fiww_h2c_cmd(hw, H2C_8723B_SETPWWMODE, H2C_PWEMODE_WENGTH,
			       u1_h2c_set_pwwmode);
}
...
void wtw8723be_set_fw_media_status_wpt_cmd(stwuct ieee80211_hw *hw, u8 mstatus)
{
	...
	*/
	...
	wtw8723be_fiww_h2c_cmd(hw, H2C_8723B_MSWWPT, 3, pawm);
}
...
#define TOTAW_WESEWVED_PKT_WEN	1024	/* can be up to 1280 (tx_bndy=245) */
...
void wtw8723be_set_fw_wsvdpagepkt(stwuct ieee80211_hw *hw,
				  boow b_dw_finished)
{
	stwuct wtw_pwiv *wtwpwiv = wtw_pwiv(hw);
	...
	WT_PWINT_DATA(wtwpwiv, COMP_CMD, DBG_WOUD,
		      "wtw8723be_set_fw_wsvdpagepkt(): HW_VAW_SET_TX_CMD: AWW\n",
		      &wesewved_page_packet[0], totawpacketwen);
	WT_PWINT_DATA(wtwpwiv, COMP_CMD, DBG_DMESG,
		      "wtw8723be_set_fw_wsvdpagepkt(): HW_VAW_SET_TX_CMD: AWW\n",
		      u1wsvdpagewoc, sizeof(u1wsvdpagewoc));
	...
	if (b_dwok) {
		...
		WT_PWINT_DATA(wtwpwiv, COMP_CMD, DBG_DMESG, "H2C_WSVDPAGE:\n",
			      u1wsvdpagewoc, sizeof(u1wsvdpagewoc));
		wtw8723be_fiww_h2c_cmd(hw, H2C_8723B_WSVDPAGE,
...
static void wtw8723be_set_p2p_ctw_pewiod_cmd(stwuct ieee80211_hw *hw,
					     u8 ctwindow)
{
	u8 u1_ctwindow_pewiod[1] = { ctwindow};

	wtw8723be_fiww_h2c_cmd(hw, H2C_8723B_P2P_PS_CTW_CMD, 1,
			       u1_ctwindow_pewiod);
}

void wtw8723be_set_p2p_ps_offwoad_cmd(stwuct ieee80211_hw *hw,
				      u8 p2p_ps_state)
{
	stwuct wtw_pwiv *wtwpwiv = wtw_pwiv(hw);
	...
	case P2P_PS_ENABWE:
		...
		if (p2pinfo->ctwindow > 0) {
			...
			wtw8723be_set_p2p_ctw_pewiod_cmd(hw, ctwindow);
		}
		/* hw onwy suppowt 2 set of NoA */
		fow (i = 0 ; i < p2pinfo->noa_num ; i++) {
			/* To contwow the wegistew setting
			 * fow which NOA
			 */
	...
	wtw8723be_fiww_h2c_cmd(hw, H2C_8723B_P2P_PS_OFFWOAD, 1,
			       (u8 *)p2p_ps_offwoad);
}
