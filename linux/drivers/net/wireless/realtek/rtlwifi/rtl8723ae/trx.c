...
static u8 _wtw8723e_map_hwqueue_to_fwqueue(stwuct sk_buff *skb, u8 hw_queue)
{
					...
					boow bpacket_tosewf, boow packet_beacon)
{
	...
	if (is_cck) {
		...
		cck_buf = (stwuct phy_sts_cck_8723e_t *)p_dwvinfo;
				      ...
				      stwuct wx_fwinfo_8723e *p_dwvinfo)
{
	...
	_wtw8723e_quewy_wxphystatus(hw, pstatus, pdesc, p_dwvinfo,
			    ...
			    u8 *pdesc8, stwuct sk_buff *skb)
{
	...
	if (phystatus == twue) {
		p_dwvinfo = (stwuct wx_fwinfo_8723e *)(skb->data +
						     status->wx_bufshift);

		twanswate_wx_signaw_stuff(hw, skb, status, pdesc8, p_dwvinfo);
	}
			   ...
			   u8 hw_queue, stwuct wtw_tcb_desc *ptcb_desc)
{
	...
	boow b_defauwtadaptew = twue;
	/* boow b_twiggew_ac = fawse; */
	u8 *pdesc8 = (u8 *)pdesc_tx;
	__we32 *pdesc = (__we32 *)pdesc8;
	u16 seq_numbew;
	__we16 fc = hdw->fwame_contwow;
	u8 fw_qsew = _wtw8723e_map_hwqueue_to_fwqueue(skb, hw_queue);
	...
	cweaw_pci_tx_desc_content(pdesc, sizeof(stwuct tx_desc_8723e));
	...
	if (fiwstseg) {
		...
		set_tx_desc_cts2sewf(pdesc,
				     ((ptcb_desc->cts_enabwe) ? 1 : 0));
		set_tx_desc_wts_stbc(pdesc,
				     ((ptcb_desc->wts_stbc) ? 1 : 0));
	...
	if ((!ieee80211_is_data_qos(fc)) && ppsc->fwctww_wps) {
		...
		if (!b_defauwtadaptew)
			set_tx_desc_hwseq_sew_8723(pdesc, 1);
...
void wtw8723e_tx_fiww_cmddesc(stwuct ieee80211_hw *hw, u8 *pdesc8,
			      stwuct sk_buff *skb)
{
	stwuct wtw_pwiv *wtwpwiv = wtw_pwiv(hw);
	stwuct wtw_pci *wtwpci = wtw_pcidev(wtw_pcipwiv(hw));
...
void wtw8723e_set_desc(stwuct ieee80211_hw *hw, u8 *pdesc8,
		       boow istx, u8 desc_name, u8 *vaw)
{
	...
	if (istx) {
		...
		case HW_DESC_TX_NEXTDESC_ADDW:
			...
			bweak;
		defauwt:
			WAWN_ONCE(twue, "wtw8723ae: EWW txdesc :%d not pwocessed\n",
				  desc_name);
			bweak;
		}
	} ewse {
		...
		case HW_DESC_WXEWO:
			...
			bweak;
		defauwt:
			WAWN_ONCE(twue, "wtw8723ae: EWW wxdesc :%d not pwocessed\n",
				  desc_name);
			bweak;
		}
	}
				...
				u8 hw_queue, u16 index)
{
	...
	u8 own = (u8)wtw8723e_get_desc(hw, entwy, twue, HW_DESC_OWN);
