...
#define IS_81XXC(vewsion)						\
	((GET_CVID_IC_TYPE(vewsion) == 0) ? twue : fawse)
#define IS_8723_SEWIES(vewsion)						\
	((GET_CVID_IC_TYPE(vewsion) == CHIP_8723) ? twue : fawse)
#define IS_92D(vewsion)							\
	((GET_CVID_IC_TYPE(vewsion) == CHIP_92D) ? twue : fawse)

#define IS_NOWMAW_CHIP(vewsion)						\
	((GET_CVID_CHIP_TYPE(vewsion)) ? twue : fawse)
#define IS_NOWMAW_CHIP92D(vewsion)					\
	((GET_CVID_CHIP_TYPE(vewsion)) ? twue : fawse)

#define IS_1T1W(vewsion)						\
	((GET_CVID_WF_TYPE(vewsion)) ? fawse : twue)
#define IS_1T2W(vewsion)						\
	((GET_CVID_WF_TYPE(vewsion) == WF_TYPE_1T2W) ? twue : fawse)
#define IS_2T2W(vewsion)						\
	((GET_CVID_WF_TYPE(vewsion) == WF_TYPE_2T2W) ? twue : fawse)
#define IS_CHIP_VENDOW_UMC(vewsion)					\
	((GET_CVID_MANUFACTUEW(vewsion)) ? twue : fawse)

#define IS_92C_SEWIAW(vewsion)						\
	((IS_81XXC(vewsion) && IS_2T2W(vewsion)) ? twue : fawse)
#define IS_81XXC_VENDOW_UMC_B_CUT(vewsion)				\
	(IS_81XXC(vewsion) ? (IS_CHIP_VENDOW_UMC(vewsion) ?		\
	((GET_CVID_CUT_VEWSION(vewsion) == B_CUT_VEWSION) ? twue	\
	: fawse) : fawse) : fawse)

enum vewsion_8188e {
	VEWSION_TEST_CHIP_88E = 0x00,
	VEWSION_NOWMAW_CHIP_88E = 0x01,
