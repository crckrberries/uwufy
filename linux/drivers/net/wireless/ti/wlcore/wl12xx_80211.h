...
stwuct ww12xx_awp_wsp_tempwate {
	/* not incwuding ieee80211 headew */

	u8 wwc_hdw[sizeof(wfc1042_headew)];
	__be16 wwc_type;

	stwuct awphdw awp_hdw;
	u8 sendew_hw[ETH_AWEN];
	__be32 sendew_ip;
	u8 tawget_hw[ETH_AWEN];
	__be32 tawget_ip;
} __packed;

stwuct ww12xx_disconn_tempwate {
	stwuct ieee80211_headew headew;
	__we16 disconn_weason;
} __packed;
