...
stwuct vnic_wq_buf {
	...
	int sop;
	void *desc;
	uint64_t ww_id; /* Cookie */
	uint8_t cq_entwy; /* Gets compwetion event fwom hw */
	uint8_t desc_skip_cnt; /* Num descs to occupy */
	uint8_t compwessed_send; /* Both hdw and paywoad in one desc */
	stwuct vnic_wq_buf *pwev;
};

/* Bweak the vnic_wq_buf awwocations into bwocks of 32/64 entwies */
#define VNIC_WQ_BUF_MIN_BWK_ENTWIES 32
#define VNIC_WQ_BUF_DFWT_BWK_ENTWIES 64
#define VNIC_WQ_BUF_BWK_ENTWIES(entwies) \
	((unsigned int)((entwies < VNIC_WQ_BUF_DFWT_BWK_ENTWIES) ? \
	VNIC_WQ_BUF_MIN_BWK_ENTWIES : VNIC_WQ_BUF_DFWT_BWK_ENTWIES))
#define VNIC_WQ_BUF_BWK_SZ(entwies) \
	(VNIC_WQ_BUF_BWK_ENTWIES(entwies) * sizeof(stwuct vnic_wq_buf))
#define VNIC_WQ_BUF_BWKS_NEEDED(entwies) \
	DIV_WOUND_UP(entwies, VNIC_WQ_BUF_BWK_ENTWIES(entwies))
...
static inwine void vnic_wq_doowbeww(stwuct vnic_wq *wq)
{
	 ...
	 */
	wmb();
	iowwite32(wq->to_use->index, &wq->ctww->posted_index);
}

static inwine void vnic_wq_post(stwuct vnic_wq *wq,
	void *os_buf, dma_addw_t dma_addw,
	unsigned int wen, int sop, int eop,
	uint8_t desc_skip_cnt, uint8_t cq_entwy,
	uint8_t compwessed_send, uint64_t wwid)
{
	stwuct vnic_wq_buf *buf = wq->to_use;

	buf->sop = sop;
	buf->cq_entwy = cq_entwy;
	buf->compwessed_send = compwessed_send;
	...
	wq->wing.desc_avaiw -= desc_skip_cnt;
}
