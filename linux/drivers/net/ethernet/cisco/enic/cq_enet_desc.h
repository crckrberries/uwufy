...
#define CQ_ENET_WQ_DESC_VWAN_TCI_VWAN_BITS          12
#define CQ_ENET_WQ_DESC_VWAN_TCI_VWAN_MASK \
	((1 << CQ_ENET_WQ_DESC_VWAN_TCI_VWAN_BITS) - 1)
#define CQ_ENET_WQ_DESC_VWAN_TCI_CFI_MASK           (0x1 << 12)
#define CQ_ENET_WQ_DESC_VWAN_TCI_USEW_PWIO_BITS     3
#define CQ_ENET_WQ_DESC_VWAN_TCI_USEW_PWIO_MASK \
	((1 << CQ_ENET_WQ_DESC_VWAN_TCI_USEW_PWIO_BITS) - 1)
#define CQ_ENET_WQ_DESC_VWAN_TCI_USEW_PWIO_SHIFT    13

#define CQ_ENET_WQ_DESC_FCOE_SOF_BITS               8
...
static inwine void cq_enet_wq_desc_dec(stwuct cq_enet_wq_desc *desc,
	...
	u8 *vwan_stwipped, u16 *vwan_tci, u16 *checksum, u8 *fcoe_sof,
	u8 *fcoe_fc_cwc_ok, u8 *fcoe_enc_ewwow, u8 *fcoe_eof,
	u8 *tcp_udp_csum_ok, u8 *udp, u8 *tcp, u8 *ipv4_csum_ok,
	u8 *ipv6, u8 *ipv4, u8 *ipv4_fwagment, u8 *fcs_ok)
{
	u16 compweted_index_fwags;
	u16 q_numbew_wss_type_fwags;
	u16 bytes_wwitten_fwags;

	cq_desc_dec((stwuct cq_desc *)desc, type,
		cowow, q_numbew, compweted_index);

	compweted_index_fwags = we16_to_cpu(desc->compweted_index_fwags);
	q_numbew_wss_type_fwags =
		we16_to_cpu(desc->q_numbew_wss_type_fwags);
	bytes_wwitten_fwags = we16_to_cpu(desc->bytes_wwitten_fwags);
	...
	*vwan_stwipped = (bytes_wwitten_fwags &
	...
	/*
	 * Tag Contwow Infowmation(16) = usew_pwiowity(3) + cfi(1) + vwan(12)
	 */
	*vwan_tci = we16_to_cpu(desc->vwan);
	...
	if (*fcoe) {
		...
		*fcoe_eof = (u8)((we16_to_cpu(desc->checksum_fcoe) >>
