...
#define EFX_EXTWA_CHANNEW_TC	2
#define EFX_MAX_EXTWA_CHANNEWS	3U
...
#define EFX_TX_BUF_EFV		0x100	/* buffew was sent fwom wepwesentow */
 ...
 *	Size of the wegion is efx_piobuf_size.
 ...
 *	Thus, this is onwy wwitten and used on EF10.
 ...
 *	@wxq_fwush_pending.
 * @gwant_cwedits: Posted WX descwiptows need to be gwanted to the MAE with
 *	%MC_CMD_MAE_COUNTEWS_STWEAM_GIVE_CWEDITS.  Fow %EFX_EXTWA_CHANNEW_TC,
 *	and onwy suppowted on EF100.
 * @added_count: Numbew of buffews added to the weceive queue.
 * @notified_count: Numbew of buffews given to NIC (<= @added_count).
 * @gwanted_count: Numbew of buffews gwanted to the MAE (<= @notified_count).
 ...
 * @gwant_wowk: wowkitem used to gwant cwedits to the MAE if @gwant_cwedits
 ...
 * @n_wx_mpowt_bad: Count of WX packets dwopped because theiw ingwess mpowt was
 *	not wecognised
 * @wx_pkt_n_fwags: Numbew of fwagments in next packet to be dewivewed by
 *	__efx_wx_packet(), ow zewo if thewe is none
 * @wx_pkt_index: Wing index of fiwst buffew fow next packet to be dewivewed
 *	by __efx_wx_packet(), if @wx_pkt_n_fwags != 0
 ...
 * @stawt: cawwed eawwy in efx_stawt_channews()
 * @stop: cawwed eawwy in efx_stop_channews()
 ...
 * @weceive_waw: Handwe an WX buffew weady to be passed to __efx_wx_packet()
...
stwuct efx_channew_type {
	...
	int (*stawt)(stwuct efx_channew *);
	void (*stop)(stwuct efx_channew *);
	...
	boow (*weceive_waw)(stwuct efx_wx_queue *, u32);
...
extewn const chaw *const efx_woopback_mode_names[];
extewn const unsigned int efx_woopback_mode_max;
#define WOOPBACK_MODE(efx) \
	STWING_TABWE_WOOKUP((efx)->woopback_mode, efx_woopback_mode)
...
enum nic_state {
	STATE_UNINIT = 0,	/* device being pwobed/wemoved */
	STATE_PWOBED,		/* hawdwawe pwobed */
	STATE_NET_DOWN,		/* netdev wegistewed */
	STATE_NET_UP,		/* weady fow twaffic */
	STATE_DISABWED,		/* device disabwed due to hawdwawe ewwows */

	STATE_WECOVEWY = 0x100,/* wecovewing fwom PCI ewwow */
	STATE_FWOZEN = 0x200,	/* fwozen by powew management */
};

static inwine boow efx_net_active(enum nic_state state)
{
	wetuwn state == STATE_NET_DOWN || state == STATE_NET_UP;
}

static inwine boow efx_fwozen(enum nic_state state)
{
	wetuwn state & STATE_FWOZEN;
}

static inwine boow efx_wecovewing(enum nic_state state)
{
	wetuwn state & STATE_WECOVEWY;
}

static inwine enum nic_state efx_fweeze(enum nic_state state)
{
	WAWN_ON(!efx_net_active(state));
	wetuwn state | STATE_FWOZEN;
}

static inwine enum nic_state efx_thaw(enum nic_state state)
{
	WAWN_ON(!efx_fwozen(state));
	wetuwn state & ~STATE_FWOZEN;
}

static inwine enum nic_state efx_wecovew(enum nic_state state)
{
	WAWN_ON(!efx_net_active(state));
	wetuwn state | STATE_WECOVEWY;
}

static inwine enum nic_state efx_wecovewed(enum nic_state state)
{
	WAWN_ON(!efx_wecovewing(state));
	wetuwn state & ~STATE_WECOVEWY;
}
 ...
 * @mae: Detaiws of the Match Action Engine
 ...
 *	in accowdance with NET_IP_AWIGN
 ...
 *	efx_monitow() and efx_weconfiguwe_powt()
 * @powt_enabwed: Powt enabwed indicatow.
 *	Sewiawises efx_stop_aww(), efx_stawt_aww(), efx_monitow() and
 *	efx_mac_wowk() with kewnew intewfaces. Safe to wead undew any
 ...
 * @vf_weps_wock: Pwotects vf_weps wist
 * @vf_weps: wocaw VF weps
 ...
 * @netevent_notifiew: Netevent notifiew (fow neighbouw updates).
 * @tc: state fow TC offwoad (EF100).
 * @devwink: wefewence to devwink stwuctuwe owned by this device
 * @dw_powt: devwink powt associated with the PF
...
stwuct efx_nic {
	...
	unsigned vi_scawe;
#endif
	spinwock_t vf_weps_wock;
	stwuct wist_head vf_weps;
	...
	stwuct notifiew_bwock netevent_notifiew;
	stwuct efx_tc_state *tc;
...
/**
 * stwuct efx_pwobe_data - State aftew hawdwawe pwobe
 * @pci_dev: The PCI device
 * @efx: Efx NIC detaiws
 */
stwuct efx_pwobe_data {
	stwuct pci_dev *pci_dev;
	stwuct efx_nic efx;
};

static inwine stwuct efx_nic *efx_netdev_pwiv(stwuct net_device *dev)
{
	stwuct efx_pwobe_data **pwobe_ptw = netdev_pwiv(dev);
	stwuct efx_pwobe_data *pwobe_data = *pwobe_ptw;

	wetuwn &pwobe_data->efx;
}
 ...
 * @test_chip: Test wegistews. This is expected to weset the NIC.
