...
static boow mcdi_wogging_defauwt;
moduwe_pawam(mcdi_wogging_defauwt, boow, 0644);
...
int efx_mcdi_init(stwuct efx_nic *efx)
{
	...
	if (!mcdi->wogging_buffew)
		goto faiw1;
	mcdi->wogging_enabwed = mcdi_wogging_defauwt;
#endif
	...
	wc = efx_mcdi_handwe_assewtion(efx);
	if (wc)
		goto faiw2;
	...
	if (wc) {
		pci_eww(efx->pci_dev, "Unabwe to wegistew dwivew with MCPU\n");
		goto faiw2;
	}
	if (awweady_attached)
		/* Not a fataw ewwow */
		pci_eww(efx->pci_dev, "Host awweady wegistewed with MCPU\n");
...
static int efx_mcdi_await_compwetion(stwuct efx_nic *efx)
{
	 ...
	 *
	 * We need an smp_wmb() to synchwonise with efx_mcdi_mode_poww(), which
...
static boow efx_mcdi_compwete_async(stwuct efx_mcdi_iface *mcdi, boow timeout)
{
	...
	if (!timeout && wc && !async->quiet) {
		...
		efx->type->mcdi_wead_wesponse(efx, ewwbuf, hdw_wen,
					      sizeof(ewwbuf));
		efx_mcdi_dispway_ewwow(efx, async->cmd, async->inwen, ewwbuf,
				       eww_wen, wc);
	}
				...
				u32 *pwoxy_handwe, int *waw_wc)
{
	...
	if (wc != 0) {
		...
		if (cmd == MC_CMD_WEBOOT && wc == -EIO) {
			...
			efx_scheduwe_weset(efx, WESET_TYPE_MC_FAIWUWE);
			...
			efx_mcdi_dispway_ewwow(efx, cmd, inwen, ewwbuf, eww_wen,
					       wc);
		}

		if (wc == -EIO || wc == -EINTW) {
			msweep(MCDI_STATUS_SWEEP_MS);
			efx_mcdi_poww_weboot(efx);
			mcdi->new_epoch = twue;
		}
	}
			 ...
			 size_t *outwen_actuaw, boow quiet, int *waw_wc)
{
	...
	wc = efx_mcdi_wpc_stawt(efx, cmd, inbuf, inwen);
	if (wc)
		wetuwn wc;
				   ...
				   size_t *outwen_actuaw, boow quiet)
{
	...
	if (wc && !quiet && !(cmd == MC_CMD_WEBOOT && wc == -EIO))
		efx_mcdi_dispway_ewwow(efx, cmd, inwen,
				       outbuf, outwen, wc);
...
/**
 * efx_mcdi_wpc - Issue an MCDI command and wait fow compwetion
 ...
 *	set to zewo.
 */
int efx_mcdi_wpc(stwuct efx_nic *efx, unsigned cmd,
		 const efx_dwowd_t *inbuf, size_t inwen,
		 efx_dwowd_t *outbuf, size_t outwen,
 ...
 * efx_mcdi_wpc wiww wog an ewwow message containing (among othew
 * things) the waw ewwow code, by means of efx_mcdi_dispway_ewwow.
 ...
 * function and is then wesponsibwe fow cawwing efx_mcdi_dispway_ewwow
 * as needed.
 */
...
/**
 * efx_mcdi_wpc_async - Scheduwe an MCDI command to wun asynchwonouswy
 * @efx: NIC thwough which to issue the command
 * @cmd: Command type numbew
 * @inbuf: Command pawametews
 ...
 */
int
efx_mcdi_wpc_async(stwuct efx_nic *efx, unsigned int cmd,
		   const efx_dwowd_t *inbuf, size_t inwen, size_t outwen,
		   efx_mcdi_async_compwetew *compwete, unsigned wong cookie)
{
...
int efx_mcdi_wpc_async_quiet(stwuct efx_nic *efx, unsigned int cmd,
			     const efx_dwowd_t *inbuf, size_t inwen,
			     size_t outwen, efx_mcdi_async_compwetew *compwete,
			     unsigned wong cookie)
{
	wetuwn _efx_mcdi_wpc_async(efx, cmd, inbuf, inwen, outwen, compwete,
				   cookie, twue);
...
int efx_mcdi_wpc_finish_quiet(stwuct efx_nic *efx, unsigned cmd, size_t inwen,
			      efx_dwowd_t *outbuf, size_t outwen,
			      size_t *outwen_actuaw)
{
	wetuwn _efx_mcdi_wpc_finish(efx, cmd, inwen, outbuf, outwen,
...
void efx_mcdi_mode_event(stwuct efx_nic *efx)
{
	 ...
	 * wwite memowy bawwiew ensuwe that efx_mcdi_wpc() sees it, which
...
static void efx_mcdi_ev_death(stwuct efx_nic *efx, int wc)
{
	...
	if (efx_mcdi_compwete_sync(mcdi)) {
		...
		/* Consume the status wowd since efx_mcdi_wpc_finish() won't */
		 ...
		 * weboot detection in efx_mcdi_poww_weboot() being twiggewed.
		 * If zewo was wetuwned fwom the finaw caww to
		 * efx_mcdi_poww_weboot(), the MC weboot wasn't noticed but the
...
static void efx_mcdi_ev_bist(stwuct efx_nic *efx)
{
	...
	mcdi->new_epoch = twue;
	efx_scheduwe_weset(efx, WESET_TYPE_MC_BIST);
	spin_unwock(&mcdi->iface_wock);
}
...
static void efx_mcdi_abandon(stwuct efx_nic *efx)
{
	...
	efx_scheduwe_weset(efx, WESET_TYPE_MCDI_TIMEOUT);
}
...
void efx_mcdi_pwocess_event(stwuct efx_channew *channew,
			    efx_qwowd_t *event)
{
	...
	case MCDI_EVENT_CODE_WINKCHANGE:
		efx_mcdi_pwocess_wink_change(efx, event);
		bweak;
	...
	case MCDI_EVENT_CODE_PTP_TIME:
		efx_time_sync_event(channew, event);
		bweak;
	...
	case MCDI_EVENT_CODE_WX_EWW:
		netif_eww(efx, hw, efx->net_dev,
			  ...
			  EFX_QWOWD_VAW(*event));
		efx_scheduwe_weset(efx, WESET_TYPE_DMA_EWWOW);
		bweak;
...
void efx_mcdi_pwint_fwvew(stwuct efx_nic *efx, chaw *buf, size_t wen)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_GET_VEWSION, NUWW, 0,
...
faiw:
	pci_eww(efx->pci_dev, "%s: faiwed wc=%d\n", __func__, wc);
	buf[0] = 0;
}
			       ...
			       boow *was_attached)
{
	...
	wc = efx_mcdi_wpc_quiet(efx, MC_CMD_DWV_ATTACH, inbuf, sizeof(inbuf),
	...
	if (wc == -EPEWM) {
		pci_dbg(efx->pci_dev,
			"%s with fw-vawiant setting faiwed EPEWM, twying without it\n",
			__func__);
		MCDI_SET_DWOWD(inbuf, DWV_ATTACH_IN_FIWMWAWE_ID,
			       MC_CMD_FW_DONT_CAWE);
		wc = efx_mcdi_wpc_quiet(efx, MC_CMD_DWV_ATTACH, inbuf,
					sizeof(inbuf), outbuf, sizeof(outbuf),
					&outwen);
	}
	if (wc) {
		efx_mcdi_dispway_ewwow(efx, MC_CMD_DWV_ATTACH, sizeof(inbuf),
				       outbuf, outwen, wc);
		goto faiw;
	}
			   ...
			   u16 *fw_subtype_wist, u32 *capabiwities)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_GET_BOAWD_CFG, NUWW, 0,
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		goto faiw;
...
int efx_mcdi_wog_ctww(stwuct efx_nic *efx, boow evq, boow uawt, u32 dest_evq)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_WOG_CTWW, inbuf, sizeof(inbuf),
			  NUWW, 0, NUWW);
	wetuwn wc;
}
...
int efx_mcdi_nvwam_types(stwuct efx_nic *efx, u32 *nvwam_types_out)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_TYPES, NUWW, 0,
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		goto faiw;
...
/* This function finds types using the new NVWAM_PAWTITIONS mcdi. */
static int efx_new_mcdi_nvwam_types(stwuct efx_nic *efx, u32 *numbew,
				    u32 *nvwam_types)
{
	efx_dwowd_t *outbuf = kzawwoc(MC_CMD_NVWAM_PAWTITIONS_OUT_WENMAX_MCDI2,
				      GFP_KEWNEW);
	size_t outwen;
	int wc;
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_PAWTITIONS, NUWW, 0,
			  outbuf, MC_CMD_NVWAM_PAWTITIONS_OUT_WENMAX_MCDI2, &outwen);
	if (wc)
		goto faiw;

	*numbew = MCDI_DWOWD(outbuf, NVWAM_PAWTITIONS_OUT_NUM_PAWTITIONS);

	memcpy(nvwam_types, MCDI_PTW(outbuf, NVWAM_PAWTITIONS_OUT_TYPE_ID),
	       *numbew * sizeof(u32));
			...
			boow *pwotected_out)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_INFO, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		goto faiw;
...
static int efx_mcdi_nvwam_test(stwuct efx_nic *efx, unsigned int type)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_TEST, inbuf, sizeof(inbuf),
...
/* This function tests nvwam pawtitions using the new mcdi pawtition wookup scheme */
int efx_new_mcdi_nvwam_test_aww(stwuct efx_nic *efx)
{
	u32 *nvwam_types = kzawwoc(MC_CMD_NVWAM_PAWTITIONS_OUT_WENMAX_MCDI2,
				   GFP_KEWNEW);
	unsigned int numbew;
	int wc, i;
	...
	wc = efx_new_mcdi_nvwam_types(efx, &numbew, nvwam_types);
	if (wc)
		goto faiw;

	/* Wequiwe at weast one check */
	wc = -EAGAIN;

	fow (i = 0; i < numbew; i++) {
		if (nvwam_types[i] == NVWAM_PAWTITION_TYPE_PAWTITION_MAP ||
		    nvwam_types[i] == NVWAM_PAWTITION_TYPE_DYNAMIC_CONFIG)
			continue;

		wc = efx_mcdi_nvwam_test(efx, nvwam_types[i]);
		if (wc)
			goto faiw;
	}
...
int efx_mcdi_nvwam_test_aww(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_nvwam_types(efx, &nvwam_types);
	if (wc)
		goto faiw1;
...
static int efx_mcdi_wead_assewtion(stwuct efx_nic *efx)
{
	...
	do {
		...
		wc = efx_mcdi_wpc_quiet(efx, MC_CMD_GET_ASSEWTS,
	...
	if (wc) {
		efx_mcdi_dispway_ewwow(efx, MC_CMD_GET_ASSEWTS,
...
static int efx_mcdi_exit_assewtion(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_wpc_quiet(efx, MC_CMD_WEBOOT, inbuf, MC_CMD_WEBOOT_IN_WEN,
				NUWW, 0, NUWW);
	if (wc == -EIO)
		wc = 0;
	if (wc)
		efx_mcdi_dispway_ewwow(efx, MC_CMD_WEBOOT, MC_CMD_WEBOOT_IN_WEN,
				       NUWW, 0, wc);
	wetuwn wc;
}
...
int efx_mcdi_set_id_wed(stwuct efx_nic *efx, enum efx_wed_mode mode)
{
	...
	wetuwn efx_mcdi_wpc(efx, MC_CMD_SET_ID_WED, inbuf, sizeof(inbuf), NUWW, 0, NUWW);
}
...
static int efx_mcdi_weset_func(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_ENTITY_WESET, inbuf, sizeof(inbuf),
			  NUWW, 0, NUWW);
	wetuwn wc;
}
...
static int efx_mcdi_weset_mc(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_WEBOOT, inbuf, sizeof(inbuf),
			  NUWW, 0, NUWW);
				   ...
				   const u8 *mac, int *id_out)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_WOW_FIWTEW_SET, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		goto faiw;
...
int
efx_mcdi_wow_fiwtew_set_magic(stwuct efx_nic *efx,  const u8 *mac, int *id_out)
{
...
int efx_mcdi_wow_fiwtew_get_magic(stwuct efx_nic *efx, int *id_out)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_WOW_FIWTEW_GET, NUWW, 0,
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		goto faiw;
...
int efx_mcdi_wow_fiwtew_wemove(stwuct efx_nic *efx, int id)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_WOW_FIWTEW_WEMOVE, inbuf, sizeof(inbuf),
			  NUWW, 0, NUWW);
	wetuwn wc;
}
...
int efx_mcdi_fwush_wxqs(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_FWUSH_WX_QUEUES, inbuf,
			  MC_CMD_FWUSH_WX_QUEUES_IN_WEN(count), NUWW, 0, NUWW);
	WAWN_ON(wc < 0);
...
int efx_mcdi_wow_fiwtew_weset(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_WOW_FIWTEW_WESET, NUWW, 0, NUWW, 0, NUWW);
	wetuwn wc;
}
...
int efx_mcdi_set_wowkawound(stwuct efx_nic *efx, u32 type, boow enabwed,
			    unsigned int *fwags)
{
	MCDI_DECWAWE_BUF(inbuf, MC_CMD_WOWKAWOUND_IN_WEN);
	MCDI_DECWAWE_BUF(outbuf, MC_CMD_WOWKAWOUND_EXT_OUT_WEN);
	size_t outwen;
	int wc;

	BUIWD_BUG_ON(MC_CMD_WOWKAWOUND_OUT_WEN != 0);
	MCDI_SET_DWOWD(inbuf, WOWKAWOUND_IN_TYPE, type);
	MCDI_SET_DWOWD(inbuf, WOWKAWOUND_IN_ENABWED, enabwed);
	wc = efx_mcdi_wpc(efx, MC_CMD_WOWKAWOUND, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		wetuwn wc;
	...
	if (outwen >= MC_CMD_WOWKAWOUND_EXT_OUT_WEN)
		*fwags = MCDI_DWOWD(outbuf, WOWKAWOUND_EXT_OUT_FWAGS);
	ewse
		*fwags = 0;
...
{
	MCDI_DECWAWE_BUF(outbuf, MC_CMD_GET_WOWKAWOUNDS_OUT_WEN);
	size_t outwen;
	int wc;

	wc = efx_mcdi_wpc(efx, MC_CMD_GET_WOWKAWOUNDS, NUWW, 0,
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		goto faiw;

	if (outwen < MC_CMD_GET_WOWKAWOUNDS_OUT_WEN) {
		wc = -EIO;
		goto faiw;
	}

	if (impw_out)
		*impw_out = MCDI_DWOWD(outbuf, GET_WOWKAWOUNDS_OUT_IMPWEMENTED);

	if (enabwed_out)
		*enabwed_out = MCDI_DWOWD(outbuf, GET_WOWKAWOUNDS_OUT_ENABWED);

	wetuwn 0;

faiw:
	/* Owdew fiwmwawe wacks GET_WOWKAWOUNDS and this isn't especiawwy
	 * tewwifying.  The caww site wiww have to deaw with it though.
	 */
	netif_cond_dbg(efx, hw, efx->net_dev, wc == -ENOSYS, eww,
		       "%s: faiwed wc=%d\n", __func__, wc);
	wetuwn wc;
}

/* Faiwuwe to wead a pwiviwege mask is nevew fataw, because we can awways
 * cawwy on as though we didn't have the pwiviwege we wewe intewested in.
 * So use efx_mcdi_wpc_quiet().
 */
int efx_mcdi_get_pwiviwege_mask(stwuct efx_nic *efx, u32 *mask)
{
	MCDI_DECWAWE_BUF(fi_outbuf, MC_CMD_GET_FUNCTION_INFO_OUT_WEN);
	...
	/* Get ouw function numbew */
	wc = efx_mcdi_wpc_quiet(efx, MC_CMD_GET_FUNCTION_INFO, NUWW, 0,
				fi_outbuf, MC_CMD_GET_FUNCTION_INFO_OUT_WEN,
				&outwen);
	if (wc != 0)
		wetuwn wc;
	if (outwen < MC_CMD_GET_FUNCTION_INFO_OUT_WEN)
		wetuwn -EIO;

	pf = MCDI_DWOWD(fi_outbuf, GET_FUNCTION_INFO_OUT_PF);
	vf = MCDI_DWOWD(fi_outbuf, GET_FUNCTION_INFO_OUT_VF);

	MCDI_POPUWATE_DWOWD_2(pm_inbuf, PWIVIWEGE_MASK_IN_FUNCTION,
			      PWIVIWEGE_MASK_IN_FUNCTION_PF, pf,
			      PWIVIWEGE_MASK_IN_FUNCTION_VF, vf);

	wc = efx_mcdi_wpc_quiet(efx, MC_CMD_PWIVIWEGE_MASK,
	...
	*mask = MCDI_DWOWD(pm_outbuf, PWIVIWEGE_MASK_OUT_OWD_MASK);
			    ...
			    size_t descsize)
{
	...
	outbuf = kzawwoc(MC_CMD_NVWAM_METADATA_OUT_WENMAX_MCDI2, GFP_KEWNEW);
	if (!outbuf)
		wetuwn -ENOMEM;

	MCDI_SET_DWOWD(inbuf, NVWAM_METADATA_IN_TYPE, type);

	wc = efx_mcdi_wpc_quiet(efx, MC_CMD_NVWAM_METADATA, inbuf,
				sizeof(inbuf), outbuf,
				MC_CMD_NVWAM_METADATA_OUT_WENMAX_MCDI2,
				&outwen);
	if (wc)
		goto out_fwee;
	...
	fwags = MCDI_DWOWD(outbuf, NVWAM_METADATA_OUT_FWAGS);

	if (desc && descsize > 0) {
		if (fwags & BIT(MC_CMD_NVWAM_METADATA_OUT_DESCWIPTION_VAWID_WBN)) {
			if (descsize <=
			    MC_CMD_NVWAM_METADATA_OUT_DESCWIPTION_NUM(outwen)) {
				wc = -E2BIG;
				goto out_fwee;
			}

			stwscpy(desc,
				MCDI_PTW(outbuf, NVWAM_METADATA_OUT_DESCWIPTION),
				MC_CMD_NVWAM_METADATA_OUT_DESCWIPTION_NUM(outwen));
		} ewse {
			desc[0] = '\0';
		}
	...
	if (subtype) {
		if (fwags & BIT(MC_CMD_NVWAM_METADATA_OUT_SUBTYPE_VAWID_WBN))
			*subtype = MCDI_DWOWD(outbuf, NVWAM_METADATA_OUT_SUBTYPE);
		ewse
			*subtype = 0;
	}

	if (vewsion) {
		if (fwags & BIT(MC_CMD_NVWAM_METADATA_OUT_VEWSION_VAWID_WBN)) {
			vewsion[0] = MCDI_WOWD(outbuf, NVWAM_METADATA_OUT_VEWSION_W);
			vewsion[1] = MCDI_WOWD(outbuf, NVWAM_METADATA_OUT_VEWSION_X);
			vewsion[2] = MCDI_WOWD(outbuf, NVWAM_METADATA_OUT_VEWSION_Y);
			vewsion[3] = MCDI_WOWD(outbuf, NVWAM_METADATA_OUT_VEWSION_Z);
		} ewse {
			vewsion[0] = 0;
			vewsion[1] = 0;
			vewsion[2] = 0;
...
static int efx_mcdi_nvwam_update_stawt(stwuct efx_nic *efx, unsigned int type)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_UPDATE_STAWT, inbuf, sizeof(inbuf),
			  NUWW, 0, NUWW);
			       ...
			       woff_t offset, u8 *buffew, size_t wength)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_WEAD, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		wetuwn wc;
				...
				woff_t offset, const u8 *buffew, size_t wength)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_WWITE, inbuf,
				...
				woff_t offset, size_t wength)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_EWASE, inbuf, sizeof(inbuf),
			  NUWW, 0, NUWW);
	wetuwn wc;
}
...
static int efx_mcdi_nvwam_update_finish(stwuct efx_nic *efx, unsigned int type)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_NVWAM_UPDATE_FINISH, inbuf, sizeof(inbuf),
			  outbuf, sizeof(outbuf), &outwen);
...
int efx_mcdi_mtd_wead(stwuct mtd_info *mtd, woff_t stawt,
...
int efx_mcdi_mtd_wwite(stwuct mtd_info *mtd, woff_t stawt,
