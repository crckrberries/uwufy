			   ...
			   unsigned int n_fwags)
{
	...
	efx_wecycwe_wx_pages(channew, wx_buf, n_fwags);

	efx_fwee_wx_buffews(wx_queue, wx_buf, n_fwags);
}
 ...
 * buffew in a page.
 */
void efx_unmap_wx_buffew(stwuct efx_nic *efx,
			 stwuct efx_wx_buffew *wx_buf)
{
	stwuct page *page = wx_buf->page;
...
/* efx_fast_push_wx_descwiptows - push new WX descwiptows quickwy
...
/**
 * efx_fiwtew_is_mc_wecipient - test whethew spec is a muwticast wecipient
			   ...
			   const stwuct efx_fiwtew_spec *wight)
{
	...
	wetuwn memcmp(&weft->vpowt_id, &wight->vpowt_id,
		      sizeof(stwuct efx_fiwtew_spec) -
		      offsetof(stwuct efx_fiwtew_spec, vpowt_id)) == 0;
}

u32 efx_fiwtew_spec_hash(const stwuct efx_fiwtew_spec *spec)
{
	BUIWD_BUG_ON(offsetof(stwuct efx_fiwtew_spec, vpowt_id) & 3);
	wetuwn jhash2((const u32 *)&spec->vpowt_id,
		      (sizeof(stwuct efx_fiwtew_spec) -
		       offsetof(stwuct efx_fiwtew_spec, vpowt_id)) / 4,
		      0);
}
			...
			boow *fowce)
{
	...
	if (wuwe->fiwtew_id == EFX_AWFS_FIWTEW_ID_EWWOW) {
		...
		*fowce = twue;
		 ...
		 * not be wemoved by efx_wps_hash_dew() subsequentwy.
		 */
		*fowce = twue;
		wetuwn twue;
	}
...
{
	u32 hash = efx_fiwtew_spec_hash(spec);
					...
					const stwuct efx_fiwtew_spec *spec)
{
	...
	hwist_fow_each(node, head) {
		...
		if (efx_fiwtew_spec_equaw(spec, &wuwe->spec))
			wetuwn wuwe;
	}
	wetuwn NUWW;
}
...
static void efx_fiwtew_wfs_wowk(stwuct wowk_stwuct *data)
{
	stwuct efx_async_fiwtew_insewtion *weq = containew_of(data, stwuct efx_async_fiwtew_insewtion,
							      wowk);
	stwuct efx_nic *efx = efx_netdev_pwiv(weq->net_dev);
	...
	if (efx->wps_hash_tabwe) {
		...
		wuwe = efx_wps_hash_find(efx, &weq->spec);
	...
	if (wc >= 0) {
		 ...
		 */
		__efx_fiwtew_wfs_expiwe(channew, min(channew->wfs_fiwtew_count,
						     100u));
	}
