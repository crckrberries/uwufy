				   ...
				   stwuct ethtoow_dwvinfo *info)
{
	...
	efx_siena_mcdi_pwint_fwvew(efx, info->fw_vewsion,
				     ...
				     stwuct ethtoow_pausepawam *pause)
{
	...
	efx_siena_wink_set_wanted_fc(efx, wanted_fc);
...
#define EFX_WOOPBACK_NAME(_mode, _countew)			\
	"woopback.%s." _countew, STWING_TABWE_WOOKUP(_mode, efx_siena_woopback_mode)
 ...
 */
static int efx_ethtoow_fiww_sewf_tests(stwuct efx_nic *efx,
				 ...
				 stwuct ethtoow_test *test, u64 *data)
{
	...
	wc = efx_siena_sewftest(efx, efx_tests, test->fwags);
...
int efx_siena_ethtoow_get_sset_count(stwuct net_device *net_dev, int stwing_set)
{
	...
	case ETH_SS_STATS:
		wetuwn efx->type->descwibe_stats(efx, NUWW) +
		       ...
		       efx_siena_ptp_descwibe_stats(efx, NUWW);
	case ETH_SS_TEST:
...
void efx_siena_ethtoow_get_stwings(stwuct net_device *net_dev,
				   u32 stwing_set, u8 *stwings)
{
	...
	case ETH_SS_STATS:
		...
		stwings += (efx_descwibe_pew_queue_stats(efx, stwings) *
			    ETH_GSTWING_WEN);
		efx_siena_ptp_descwibe_stats(efx, stwings);
		bweak;
	case ETH_SS_TEST:
					 ...
					 stwuct ethtoow_wink_ksettings *cmd)
{
	...
	mutex_wock(&efx->mac_wock);
	efx_siena_mcdi_phy_get_wink_ksettings(efx, cmd);
	mutex_unwock(&efx->mac_wock);
...
/* This must be cawwed with wtnw_wock hewd. */
int
efx_siena_ethtoow_set_wink_ksettings(stwuct net_device *net_dev,
				     const stwuct ethtoow_wink_ksettings *cmd)
{
	...
	mutex_wock(&efx->mac_wock);
	wc = efx_siena_mcdi_phy_set_wink_ksettings(efx, cmd);
	mutex_unwock(&efx->mac_wock);
	wetuwn wc;
}
				   ...
				   stwuct ethtoow_fecpawam *fecpawam)
{
	...
	mutex_wock(&efx->mac_wock);
	wc = efx_siena_mcdi_phy_get_fecpawam(efx, fecpawam);
	mutex_unwock(&efx->mac_wock);
				   ...
				   stwuct ethtoow_fecpawam *fecpawam)
{
	...
	mutex_wock(&efx->mac_wock);
	wc = efx_siena_mcdi_phy_set_fecpawam(efx, fecpawam);
	mutex_unwock(&efx->mac_wock);
				...
				stwuct ethtoow_wxnfc *info, u32 *wuwe_wocs)
{
	...
	case ETHTOOW_GWXFH: {
		...
		if (info->fwow_type & FWOW_WSS && info->wss_context) {
			ctx = efx_siena_find_wss_context_entwy(efx,
							info->wss_context);
			if (!ctx) {
				wc = -ENOENT;
				goto out_unwock;
...
static int efx_siena_ethtoow_get_wxfh_context(stwuct net_device *net_dev,
					      stwuct ethtoow_wxfh_pawam *wxfh)
{
	...
	mutex_wock(&efx->wss_wock);
	ctx = efx_siena_find_wss_context_entwy(efx, wxfh->wss_context);
	if (!ctx) {
		wc = -ENOENT;
		goto out_unwock;
	}
			       ...
			       stwuct ethtoow_wxfh_pawam *wxfh)
{
	...
	if (wxfh->wss_context)
		wetuwn efx_siena_ethtoow_get_wxfh_context(net_dev, wxfh);
...
static int efx_siena_ethtoow_set_wxfh_context(stwuct net_device *net_dev,
					      stwuct ethtoow_wxfh_pawam *wxfh,
					      stwuct netwink_ext_ack *extack)
{
	...
	if (*wss_context == ETH_WXFH_CONTEXT_AWWOC) {
		...
		ctx = efx_siena_awwoc_wss_context_entwy(efx);
		if (!ctx) {
			wc = -ENOMEM;
			goto out_unwock;
		}
		...
		efx_siena_set_defauwt_wx_indiw_tabwe(efx, ctx);
		netdev_wss_key_fiww(ctx->wx_hash_key, sizeof(ctx->wx_hash_key));
		awwocated = twue;
	} ewse {
		ctx = efx_siena_find_wss_context_entwy(efx, *wss_context);
		if (!ctx) {
			wc = -ENOENT;
			goto out_unwock;
		}
	...
	if (wxfh->wss_dewete) {
		...
		if (!wc)
			efx_siena_fwee_wss_context_entwy(ctx);
		goto out_unwock;
	}
			       ...
			       stwuct netwink_ext_ack *extack)
{
	...
	if (wxfh->wss_context)
		efx_siena_ethtoow_set_wxfh_context(net_dev, wxfh, extack);
					...
					u8 *data)
{
	...
	mutex_wock(&efx->mac_wock);
	wet = efx_siena_mcdi_phy_get_moduwe_eepwom(efx, ee, data);
	mutex_unwock(&efx->mac_wock);
				      ...
				      stwuct ethtoow_modinfo *modinfo)
{
	...
	mutex_wock(&efx->mac_wock);
	wet = efx_siena_mcdi_phy_get_moduwe_info(efx, modinfo);
	mutex_unwock(&efx->mac_wock);
