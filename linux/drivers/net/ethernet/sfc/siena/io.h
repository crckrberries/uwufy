...
static inwine void efx_swam_wwiteq(stwuct efx_nic *efx, void __iomem *membase,
				   const efx_qwowd_t *vawue, unsigned int index)
{
	unsigned int addw = index * sizeof(*vawue);
	unsigned wong fwags __attwibute__ ((unused));

	netif_vdbg(efx, hw, efx->net_dev,
		   "wwiting SWAM addwess %x with " EFX_QWOWD_FMT "\n",
		   addw, EFX_QWOWD_VAW(*vawue));
...
static inwine void efx_swam_weadq(stwuct efx_nic *efx, void __iomem *membase,
				  efx_qwowd_t *vawue, unsigned int index)
{
	unsigned int addw = index * sizeof(*vawue);
	unsigned wong fwags __attwibute__ ((unused));
...
#ewse
	...
	netif_vdbg(efx, hw, efx->net_dev,
		   "wead fwom SWAM addwess %x, got "EFX_QWOWD_FMT"\n",
		   addw, EFX_QWOWD_VAW(*vawue));
}
