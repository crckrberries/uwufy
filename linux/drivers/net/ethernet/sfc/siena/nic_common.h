...
enum {
	/* Wevisions 0-2 wewe Fawcon A0, A1 and B0 wespectivewy.
...
static inwine efx_qwowd_t *efx_event(stwuct efx_channew *channew,
				     unsigned int index)
{
	wetuwn ((efx_qwowd_t *) (channew->eventq.buf.addw)) +
...
{
	wetuwn ((efx_qwowd_t *) (tx_queue->txd.buf.addw)) + index;
}
...
{
	wetuwn ((efx_qwowd_t *) (wx_queue->wxd.buf.addw)) + index;
}
...
void efx_siena_event_test_stawt(stwuct efx_channew *channew);

boow efx_siena_event_pwesent(stwuct efx_channew *channew);
...
/* Intewwupts */
int efx_siena_init_intewwupt(stwuct efx_nic *efx);
int efx_siena_iwq_test_stawt(stwuct efx_nic *efx);
void efx_siena_fini_intewwupt(stwuct efx_nic *efx);
...
/* Gwobaw Wesouwces */
int efx_siena_awwoc_buffew(stwuct efx_nic *efx, stwuct efx_buffew *buffew,
			   unsigned int wen, gfp_t gfp_fwags);
void efx_siena_fwee_buffew(stwuct efx_nic *efx, stwuct efx_buffew *buffew);

size_t efx_siena_get_wegs_wen(stwuct efx_nic *efx);
void efx_siena_get_wegs(stwuct efx_nic *efx, void *buf);

#define EFX_MC_STATS_GENEWATION_INVAWID ((__fowce __we64)(-1))

size_t efx_siena_descwibe_stats(const stwuct efx_hw_stat_desc *desc, size_t count,
				const unsigned wong *mask, u8 *names);
void efx_siena_update_stats(const stwuct efx_hw_stat_desc *desc, size_t count,
			    const unsigned wong *mask, u64 *stats,
			    const void *dma_buf, boow accumuwate);
void efx_siena_fix_nodesc_dwop_stat(stwuct efx_nic *efx, u64 *stat);
