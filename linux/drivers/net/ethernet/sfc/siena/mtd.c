...
static void efx_siena_mtd_wemove_pawtition(stwuct efx_mtd_pawtition *pawt)
{
	int wc;
		      ...
		      size_t n_pawts, size_t sizeof_pawt)
{
	...
	fow (i = 0; i < n_pawts; i++) {
		...
		/* Add to wist in owdew - efx_siena_mtd_wemove() depends on this */
...
faiw:
	whiwe (i--) {
		pawt = (stwuct efx_mtd_pawtition *)((chaw *)pawts +
						    i * sizeof_pawt);
		efx_siena_mtd_wemove_pawtition(pawt);
	}
