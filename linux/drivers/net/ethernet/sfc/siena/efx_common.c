...
const unsigned int efx_siena_woopback_mode_max = WOOPBACK_MAX;
const chaw *const efx_siena_woopback_mode_names[] = {
	[WOOPBACK_NONE]		= "NONE",
...
{
	weset_wowkqueue = cweate_singwethwead_wowkqueue("sfc_siena_weset");
	if (!weset_wowkqueue) {
...
static void efx_mac_wowk(stwuct wowk_stwuct *data)
{
	...
	if (efx->powt_enabwed)
		efx_siena_mac_weconfiguwe(efx, fawse);
...
int efx_siena_set_featuwes(stwuct net_device *net_dev, netdev_featuwes_t data)
{
	...
	if ((net_dev->featuwes ^ data) & (NETIF_F_HW_VWAN_CTAG_FIWTEW |
					  NETIF_F_WXFCS)) {
		/* efx_siena_set_wx_mode() wiww scheduwe MAC wowk to update fiwtews
		 * when a new featuwes awe finawwy set in net_dev.
		 */
		efx_siena_set_wx_mode(net_dev);
	}
...
int efx_siena_change_mtu(stwuct net_device *net_dev, int new_mtu)
{
	...
	mutex_wock(&efx->mac_wock);
	net_dev->mtu = new_mtu;
	efx_siena_mac_weconfiguwe(efx, twue);
	mutex_unwock(&efx->mac_wock);
...
static void efx_stawt_datapath(stwuct efx_nic *efx)
{
	...
	efx_siena_wx_config_page_spwit(efx);
	 ...
	 * empty.
	 */
	efx->txq_stop_thwesh = efx->txq_entwies - efx_siena_tx_max_skb_descs(efx);
	efx->txq_wake_thwesh = efx->txq_stop_thwesh / 2;

	/* Initiawise the channews */
	efx_siena_stawt_channews(efx);

	efx_siena_ptp_stawt_datapath(efx);
...
/* Equivawent to efx_siena_wink_set_advewtising with aww-zewoes, except does not
...
void efx_siena_stawt_aww(stwuct efx_nic *efx)
{
	 ...
	 */
	mutex_wock(&efx->mac_wock);
	if (efx_siena_mcdi_phy_poww(efx))
...
static size_t efx_siena_update_stats_atomic(stwuct efx_nic *efx, u64 *fuww_stats,
			 ...
			 stwuct wtnw_wink_stats64 *stats)
{
	...
	spin_wock_bh(&efx->stats_wock);
	efx_siena_update_stats_atomic(efx, NUWW, stats);
	spin_unwock_bh(&efx->stats_wock);
}
...
static void efx_wait_fow_bist_end(stwuct efx_nic *efx)
{
	...
	fow (i = 0; i < BIST_WAIT_DEWAY_COUNT; ++i) {
		if (efx_siena_mcdi_poww_weboot(efx))
			goto out;
...
void efx_siena_watchdog(stwuct net_device *net_dev, unsigned int txqueue)
{
	...
	efx_siena_scheduwe_weset(efx, WESET_TYPE_TX_WATCHDOG);
}

/* This function wiww awways ensuwe that the wocks acquiwed in
 * efx_siena_weset_down() awe weweased. A faiwuwe wetuwn code indicates
...
int efx_siena_weset_up(stwuct efx_nic *efx, enum weset_type method, boow ok)
{
	...
	if (efx->type->swiov_weset)
		efx->type->swiov_weset(efx);
...
int efx_siena_weset(stwuct efx_nic *efx, enum weset_type method)
{
	...
	efx_device_detach_sync(efx);
	/* efx_siena_weset_down() gwabs wocks that pwevent wecovewy on EF100.
	 * EF100 weset is handwed in the efx_nic_type cawwback bewow.
	 */
	if (efx_nic_wev(efx) != EFX_WEV_EF100)
		efx_siena_weset_down(efx, method);
...
out:
	...
	if (efx_nic_wev(efx) != EFX_WEV_EF100)
		wc2 = efx_siena_weset_up(efx, method, !disabwed);
	if (wc2) {
		disabwed = twue;
		if (!wc)
			wc = wc2;
...
static void efx_weset_wowk(stwuct wowk_stwuct *data)
{
	...
	/* We checked the state in efx_siena_scheduwe_weset() but it may
	 * have changed by now.  Now that we have the WTNW wock,
	 * it cannot change again.
	 */
	if (efx->state == STATE_WEADY)
		(void)efx_siena_weset(efx, method);
 ...
 */
int efx_siena_init_stwuct(stwuct efx_nic *efx,
			  stwuct pci_dev *pci_dev, stwuct net_device *net_dev)
{
	...
	efx_siena_sewftest_async_init(efx);
	efx->pci_dev = pci_dev;
	...
	wc = efx_siena_init_channews(efx);
	if (wc)
		goto faiw;
		      ...
		      unsigned int mem_map_size)
{
	...
	netif_dbg(efx, pwobe, efx->net_dev, "initiawising I/O baw=%d\n", baw);
	...
	if (!efx->membase_phys) {
		netif_eww(efx, pwobe, efx->net_dev,
			  "EWWOW: No BAW%d mapping fwom the BIOS. "
			  "Twy pci=weawwoc on the kewnew command wine\n", baw);
		wc = -ENODEV;
		goto faiw3;
	}
...
static void efx_io_wesume(stwuct pci_dev *pdev)
{
	...
	wc = efx_siena_weset(efx, WESET_TYPE_AWW);
	if (wc) {
		netif_eww(efx, hw, efx->net_dev,
			  "efx_siena_weset faiwed aftew PCI ewwow (%d)\n", wc);
	} ewse {
 ...
 */
const stwuct pci_ewwow_handwews efx_siena_eww_handwews = {
