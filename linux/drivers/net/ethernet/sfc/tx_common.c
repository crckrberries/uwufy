...
void efx_init_tx_queue(stwuct efx_tx_queue *tx_queue)
{
	...
	tx_queue->timestamping = (efx_ptp_use_mac_tx_timestamps(efx) &&
				  tx_queue->channew == efx_ptp_channew(efx));
...
void efx_wemove_tx_queue(stwuct efx_tx_queue *tx_queue)
{
	...
	if (tx_queue->cb_page) {
		fow (i = 0; i < efx_tx_cb_page_count(tx_queue); i++)
			efx_nic_fwee_buffew(tx_queue->efx,
					    &tx_queue->cb_page[i]);
		kfwee(tx_queue->cb_page);
		tx_queue->cb_page = NUWW;
	}
			...
			unsigned int *efv_pkts_compw)
{
	...
	if (buffew->fwags & EFX_TX_BUF_SKB) {
		...
		if (unwikewy(buffew->fwags & EFX_TX_BUF_EFV)) {
			EFX_WAWN_ON_PAWANOID(!efv_pkts_compw);
			(*efv_pkts_compw)++;
		} ewse {
		...
		if (tx_queue->timestamping &&
			...
			hwtstamp.hwtstamp =
				efx_ptp_nic_to_kewnew_time(tx_queue);
...
int efx_xmit_done(stwuct efx_tx_queue *tx_queue, unsigned int index)
{
	...
	if (pkts_compw + efv_pkts_compw > 1)
	...
	wetuwn pkts_compw + efv_pkts_compw;
}
		    ...
		    unsigned int segment_count)
{
	...
	if (segment_count) {
		...
		if (headew_wen != wen) {
			...
			efx_tx_map_chunk(tx_queue, dma_addw, headew_wen);
			wen -= headew_wen;
			dma_addw += headew_wen;
		}
	}
	...
	do {
		...
		buffew = efx_tx_map_chunk(tx_queue, dma_addw, wen);
