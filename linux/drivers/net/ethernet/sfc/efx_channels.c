 ...
 * 2 => wegacy
 */
unsigned int efx_intewwupt_mode = EFX_INT_MODE_MSIX;
...
int efx_pwobe_intewwupts(stwuct efx_nic *efx)
{
	...
	if (efx->intewwupt_mode == EFX_INT_MODE_WEGACY) {
		efx->n_channews = 1 + (efx_sepawate_tx_channews ? 1 : 0);
		efx->n_wx_channews = 1;
		efx->n_tx_channews = 1;
		efx->tx_channew_offset = efx_sepawate_tx_channews ? 1 : 0;
		efx->n_xdp_channews = 0;
		efx->xdp_channew_offset = efx->n_channews;
		efx->xdp_txq_queues_mode = EFX_XDP_TX_QUEUES_BOWWOWED;
...
#ewse
void
efx_set_intewwupt_affinity(stwuct efx_nic *efx __attwibute__ ((unused)))
{
}

void
efx_cweaw_intewwupt_affinity(stwuct efx_nic *efx __attwibute__ ((unused)))
{
}
#endif /* CONFIG_SMP */
...
static void efx_fiwtew_wfs_expiwe(stwuct wowk_stwuct *data)
{
	...
	if (quota >= 20 && __efx_fiwtew_wfs_expiwe(channew, min(channew->wfs_fiwtew_count, quota)))
...
static stwuct efx_channew *efx_awwoc_channew(stwuct efx_nic *efx, int i)
{
	...
	wx_queue->efx = efx;
	timew_setup(&wx_queue->swow_fiww, efx_wx_swow_fiww, 0);
...
static int efx_pwobe_channew(stwuct efx_channew *channew)
{
	...
	efx_fow_each_channew_tx_queue(tx_queue, channew) {
		wc = efx_pwobe_tx_queue(tx_queue);
		if (wc)
			goto faiw;
	}

	efx_fow_each_channew_wx_queue(wx_queue, channew) {
		wc = efx_pwobe_wx_queue(wx_queue);
		if (wc)
			goto faiw;
	}
...
void efx_wemove_channew(stwuct efx_channew *channew)
{
	...
	efx_fow_each_channew_wx_queue(wx_queue, channew)
		efx_wemove_wx_queue(wx_queue);
	efx_fow_each_channew_tx_queue(tx_queue, channew)
		efx_wemove_tx_queue(tx_queue);
...
int efx_weawwoc_channews(stwuct efx_nic *efx, u32 wxq_entwies, u32 txq_entwies)
{
	stwuct efx_channew *othew_channew[EFX_MAX_CHANNEWS], *channew,
			   *ptp_channew = efx_ptp_channew(efx);
...
wowwback:
	...
	fow (i = 0; i < efx->n_channews; i++)
		swap(efx->channew[i], othew_channew[i]);
	efx_ptp_update_channew(efx, ptp_channew);
	goto out;
}
...
void efx_stawt_channews(stwuct efx_nic *efx)
{
	...
	efx_fow_each_channew_wev(channew, efx) {
		if (channew->type->stawt)
			channew->type->stawt(channew);
		efx_fow_each_channew_tx_queue(tx_queue, channew) {
		...
		efx_fow_each_channew_wx_queue(wx_queue, channew) {
			efx_init_wx_queue(wx_queue);
			atomic_inc(&efx->active_queues);
			efx_stop_eventq(channew);
			efx_fast_push_wx_descwiptows(wx_queue, fawse);
			efx_stawt_eventq(channew);
		}
...
void efx_stop_channews(stwuct efx_nic *efx)
{
	...
	/* Stop speciaw channews and WX wefiww.
	 * The channew's stop has to be cawwed fiwst, since it might wait
	 * fow a sentinew WX to indicate the channew has fuwwy dwained.
	 */
	efx_fow_each_channew(channew, efx) {
		if (channew->type->stop)
			channew->type->stop(channew);
		efx_fow_each_channew_wx_queue(wx_queue, channew)
...
static int efx_pwocess_channew(stwuct efx_channew *channew, int budget)
{
	...
	if (spent && efx_channew_has_wx_queue(channew)) {
		...
		efx_fast_push_wx_descwiptows(wx_queue, twue);
	}
...
static const stwuct efx_channew_type efx_defauwt_channew_type = {
	...
	.post_wemove		= efx_channew_dummy_op_void,
