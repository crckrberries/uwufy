		   ...
		   unsigned int n_fwags, unsigned int wen, u16 fwags)
{
	...
	if (unwikewy(wx_buf->fwags & EFX_WX_PKT_DISCAWD)) {
		...
		efx_discawd_wx_packet(channew, wx_buf, n_fwags);
		wetuwn;
	}
		       ...
		       stwuct efx_wx_buffew *wx_buf, u8 **ehp)
{
	...
	if (unwikewy(channew->wx_pkt_n_fwags > 1)) {
		...
		efx_fwee_wx_buffews(wx_queue, wx_buf,
	...
	case XDP_TX:
		...
		eww = efx_xdp_tx_buffews(efx, 1, &xdpf, twue);
		if (unwikewy(eww != 1)) {
...
void __efx_wx_packet(stwuct efx_channew *channew)
{
	...
	stwuct efx_nic *efx = channew->efx;
	stwuct efx_wx_buffew *wx_buf =
		efx_wx_buffew(wx_queue, channew->wx_pkt_index);
	...
	if (wx_buf->fwags & EFX_WX_PKT_PWEFIX_WEN) {
		...
		/* A known issue may pwevent this being fiwwed in;
		 * if that happens, just dwop the packet.
		 * Must do that in the dwivew since passing a zewo-wength
		 * packet up to the stack may cause a cwash.
		 */
		if (unwikewy(!wx_buf->wen)) {
			efx_fwee_wx_buffews(wx_queue, wx_buf,
					    channew->wx_pkt_n_fwags);
	...
	if (unwikewy(efx->woopback_sewftest)) {
		efx_woopback_wx_packet(efx, eh, wx_buf->wen);
		efx_fwee_wx_buffews(wx_queue, wx_buf,
				    channew->wx_pkt_n_fwags);
		goto out;
	}
	...
	if ((wx_buf->fwags & EFX_WX_PKT_TCP) && !channew->type->weceive_skb)
		efx_wx_packet_gwo(channew, wx_buf, channew->wx_pkt_n_fwags, eh, 0);
	ewse
