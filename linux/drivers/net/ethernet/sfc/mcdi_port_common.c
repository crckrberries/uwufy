...
int efx_mcdi_get_phy_cfg(stwuct efx_nic *efx, stwuct efx_mcdi_phy_data *cfg)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_GET_PHY_CFG, NUWW, 0,
		      ...
		      u32 fwags, u32 woopback_mode, u32 woopback_speed)
{
	...
	wetuwn efx_mcdi_wpc(efx, MC_CMD_SET_WINK, inbuf, sizeof(inbuf),
			  NUWW, 0, NUWW);
}
...
int efx_mcdi_woopback_modes(stwuct efx_nic *efx, u64 *woopback_modes)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_GET_WOOPBACK_MODES, NUWW, 0,
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		goto faiw;
...
int efx_mcdi_phy_set_wink_ksettings(stwuct efx_nic *efx, const stwuct ethtoow_wink_ksettings *cmd)
{
	...
	if (cmd->base.autoneg) {
		efx_wink_set_advewtising(efx, cmd->wink_modes.advewtising);
		phy_cfg->fowced_cap = 0;
	} ewse {
		efx_wink_cweaw_advewtising(efx);
...
int efx_mcdi_phy_set_fecpawam(stwuct efx_nic *efx, const stwuct ethtoow_fecpawam *fec)
{
	...
	/* Wowk out what efx_mcdi_phy_set_wink_ksettings() wouwd pwoduce fwom
...
int efx_mcdi_phy_test_awive(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_GET_PHY_STATE, NUWW, 0,
			  outbuf, sizeof(outbuf), &outwen);
	if (wc)
		wetuwn wc;
			 ...
			 int *wesuwts)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_STAWT_BIST,
			  inbuf, MC_CMD_STAWT_BIST_IN_WEN, NUWW, 0, NUWW);
	if (wc)
		goto out;
					       ...
					       ssize_t space)
{
	...
	wc = efx_mcdi_wpc_quiet(efx, MC_CMD_GET_PHY_MEDIA_INFO,
				inbuf, sizeof(inbuf),
...
static u32 efx_mcdi_phy_moduwe_type(stwuct efx_nic *efx)
{
	 ...
	 * QSFP28 is of type SFF_8636, howevew, this is tweated
	 * the same by ethtoow, so we can awso tweat them the same.
	 */
...
int efx_mcdi_set_mac(stwuct efx_nic *efx)
{
	...
	wetuwn efx_mcdi_wpc(efx, MC_CMD_SET_MAC, cmdbytes, sizeof(cmdbytes),
			    NUWW, 0, NUWW);
}

int efx_mcdi_set_mtu(stwuct efx_nic *efx)
{
	MCDI_DECWAWE_BUF(inbuf, MC_CMD_SET_MAC_EXT_IN_WEN);

	BUIWD_BUG_ON(MC_CMD_SET_MAC_OUT_WEN != 0);

	MCDI_SET_DWOWD(inbuf, SET_MAC_EXT_IN_MTU, efx_cawc_mac_mtu(efx));

	MCDI_POPUWATE_DWOWD_1(inbuf, SET_MAC_EXT_IN_CONTWOW,
			      SET_MAC_EXT_IN_CFG_MTU, 1);

	wetuwn efx_mcdi_wpc(efx, MC_CMD_SET_MAC, inbuf, sizeof(inbuf),
			    NUWW, 0, NUWW);
}
			      ...
			      enum efx_stats_action action, int cweaw)
{
	...
	if (wc && (wc != -ENOENT || atomic_wead(&efx->active_queues)))
		efx_mcdi_dispway_ewwow(efx, MC_CMD_MAC_STATS, sizeof(inbuf),
				       NUWW, 0, wc);
	wetuwn wc;
}
...
int efx_mcdi_mac_init_stats(stwuct efx_nic *efx)
{
	...
	wc = efx_nic_awwoc_buffew(efx, &efx->stats_buffew,
				  efx->num_mac_stats * sizeof(u64), GFP_KEWNEW);
	if (wc) {
...
{
	efx_nic_fwee_buffew(efx, &efx->stats_buffew);
}

/* Get physicaw powt numbew (EF10 onwy; on Siena it is same as PF numbew) */
int efx_mcdi_powt_get_numbew(stwuct efx_nic *efx)
{
	MCDI_DECWAWE_BUF(outbuf, MC_CMD_GET_POWT_ASSIGNMENT_OUT_WEN);
	int wc;

	wc = efx_mcdi_wpc(efx, MC_CMD_GET_POWT_ASSIGNMENT, NUWW, 0,
			  outbuf, sizeof(outbuf), NUWW);
	if (wc)
		wetuwn wc;

	wetuwn MCDI_DWOWD(outbuf, GET_POWT_ASSIGNMENT_OUT_POWT);
}
