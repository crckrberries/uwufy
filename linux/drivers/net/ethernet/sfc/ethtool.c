			       ...
			       enum ethtoow_phys_id_state state)
{
	...
	wetuwn efx_mcdi_set_id_wed(efx, mode);
}

static int efx_ethtoow_get_wegs_wen(stwuct net_device *net_dev)
{
	wetuwn efx_nic_get_wegs_wen(efx_netdev_pwiv(net_dev));
}
				    ...
				    stwuct netwink_ext_ack *extack)
{
	...
	efx_get_iwq_modewation(efx, &tx_usecs, &wx_usecs, &wx_adaptive);
				    ...
				    stwuct netwink_ext_ack *extack)
{
	...
	efx_get_iwq_modewation(efx, &tx_usecs, &wx_usecs, &adaptive);
	...
	wc = efx_init_iwq_modewation(efx, tx_usecs, wx_usecs, adaptive,
				     wx_may_ovewwide_tx);
	if (wc != 0)
		wetuwn wc;
			  ...
			  stwuct netwink_ext_ack *extack)
{
	...
	wetuwn efx_weawwoc_channews(efx, wing->wx_pending, txq_entwies);
}
...
const stwuct ethtoow_ops efx_ethtoow_ops = {
