...
static int efx_mcdi_mon_update(stwuct efx_nic *efx)
{
	...
	wc = efx_mcdi_wpc(efx, MC_CMD_WEAD_SENSOWS,
...
int efx_mcdi_mon_pwobe(stwuct efx_nic *efx)
{
	...
	do {
		...
		wc = efx_mcdi_wpc(efx, MC_CMD_SENSOW_INFO, inbuf, sizeof(inbuf),
		...
		do {
			...
			if ((type % 32) == 0) {
				...
				MCDI_SET_DWOWD(inbuf, SENSOW_INFO_EXT_IN_PAGE,
					       page);
				wc = efx_mcdi_wpc(efx, MC_CMD_SENSOW_INFO,
						  inbuf, sizeof(inbuf),
...
void efx_mcdi_mon_wemove(stwuct efx_nic *efx)
{
	...
	efx_nic_fwee_buffew(efx, &hwmon->dma_buf);
}
