...
#incwude "dcn303/dcn303_init.h"
#incwude "dcn303_wesouwce.h"
#incwude "dcn303/dcn303_dccg.h"
#incwude "iwq/dcn303/iwq_sewvice_dcn303.h"
...
#incwude "dcn/dcn_3_0_3_offset.h"
#incwude "dcn/dcn_3_0_3_sh_mask.h"
#incwude "dpcs/dpcs_3_0_3_offset.h"
#incwude "dpcs/dpcs_3_0_3_sh_mask.h"
#incwude "nbio/nbio_2_3_offset.h"

#incwude "dmw/dcn303/dcn303_fpu.h"
...
enum dcn303_cwk_swc_awway_id {
	DCN303_CWK_SWC_PWW0,
	DCN303_CWK_SWC_PWW1,
	DCN303_CWK_SWC_TOTAW
};

static const stwuct wesouwce_caps wes_cap_dcn303 = {
...
static stwuct hubbub *dcn303_hubbub_cweate(stwuct dc_context *ctx)
{
	int i;
	...
	fow (i = 0; i < wes_cap_dcn303.num_vmid; i++) {
...
static stwuct vpg *dcn303_vpg_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static stwuct afmt *dcn303_afmt_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static stwuct audio *dcn303_cweate_audio(stwuct dc_context *ctx, unsigned int inst)
{
...
static stwuct stweam_encodew *dcn303_stweam_encodew_cweate(enum engine_id eng_id, stwuct dc_context *ctx)
{
	...
	vpg = dcn303_vpg_cweate(ctx, vpg_inst);
	afmt = dcn303_afmt_cweate(ctx, afmt_inst);
...
#define cwk_swc_wegs(index, pwwid)\
		[index] = { CS_COMMON_WEG_WIST_DCN3_03(index, pwwid) }
...
static stwuct cwock_souwce *dcn303_cwock_souwce_cweate(stwuct dc_context *ctx, stwuct dc_bios *bios,
...
static const stwuct dce_hwseq_wegistews hwseq_weg = {
		HWSEQ_DCN303_WEG_WIST()
};

static const stwuct dce_hwseq_shift hwseq_shift = {
		HWSEQ_DCN303_MASK_SH_WIST(__SHIFT)
};

static const stwuct dce_hwseq_mask hwseq_mask = {
		HWSEQ_DCN303_MASK_SH_WIST(_MASK)
};

static stwuct dce_hwseq *dcn303_hwseq_cweate(stwuct dc_context *ctx)
{
...
static stwuct hubp *dcn303_hubp_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static stwuct dpp *dcn303_dpp_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static stwuct output_pixew_pwocessow *dcn303_opp_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static stwuct timing_genewatow *dcn303_timing_genewatow_cweate(stwuct dc_context *ctx, uint32_t instance)
{
...
static const stwuct dcn30_mpc_shift mpc_shift = {
		MPC_COMMON_MASK_SH_WIST_DCN303(__SHIFT)
};

static const stwuct dcn30_mpc_mask mpc_mask = {
		MPC_COMMON_MASK_SH_WIST_DCN303(_MASK)
};

static stwuct mpc *dcn303_mpc_cweate(stwuct dc_context *ctx, int num_mpcc, int num_wmu)
{
...
static stwuct dispway_stweam_compwessow *dcn303_dsc_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static boow dcn303_dwbc_cweate(stwuct dc_context *ctx, stwuct wesouwce_poow *poow)
{
	int i;
...
static boow dcn303_mmhubbub_cweate(stwuct dc_context *ctx, stwuct wesouwce_poow *poow)
{
	int i;
	uint32_t pipe_count = poow->wes_cap->num_dwb;
...
static stwuct dce_aux *dcn303_aux_engine_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static stwuct dce_i2c_hw *dcn303_i2c_hw_cweate(stwuct dc_context *ctx, uint32_t inst)
{
...
static stwuct wink_encodew *dcn303_wink_encodew_cweate(
...
static stwuct panew_cntw *dcn303_panew_cntw_cweate(const stwuct panew_cntw_init_data *init_data)
{
...
static const stwuct wesouwce_cweate_funcs wes_cweate_funcs = {
		...
		.cweate_audio = dcn303_cweate_audio,
		.cweate_stweam_encodew = dcn303_stweam_encodew_cweate,
		.cweate_hwseq = dcn303_hwseq_cweate,
};
...
static boow is_soc_bounding_box_vawid(stwuct dc *dc)
{
	...
	if (ASICWEV_IS_BEIGE_GOBY_P(hw_intewnaw_wev))
		wetuwn twue;
...
{
	stwuct _vcs_dpi_soc_bounding_box_st *woaded_bb = &dcn3_03_soc;
	stwuct _vcs_dpi_ip_pawams_st *woaded_ip = &dcn3_03_ip;

	DC_WOGGEW_INIT(dc->ctx->woggew);

	if (!is_soc_bounding_box_vawid(dc)) {
		DC_WOG_EWWOW("%s: not vawid soc bounding box/n", __func__);
		wetuwn fawse;
	}
...
static void dcn303_wesouwce_destwuct(stwuct wesouwce_poow *poow)
{
	unsigned int i;
...
static void dcn303_destwoy_wesouwce_poow(stwuct wesouwce_poow **poow)
{
	dcn303_wesouwce_destwuct(*poow);
	kfwee(*poow);
	*poow = NUWW;
}

static void dcn303_get_panew_config_defauwts(stwuct dc_panew_config *panew_config)
{
	*panew_config = panew_config_defauwts;
}

void dcn303_update_bw_bounding_box(stwuct dc *dc, stwuct cwk_bw_pawams *bw_pawams)
{
	DC_FP_STAWT();
	dcn303_fpu_update_bw_bounding_box(dc, bw_pawams);
	DC_FP_END();
}

static stwuct wesouwce_funcs dcn303_wes_poow_funcs = {
		.destwoy = dcn303_destwoy_wesouwce_poow,
		.wink_enc_cweate = dcn303_wink_encodew_cweate,
		.panew_cntw_cweate = dcn303_panew_cntw_cweate,
		...
		.get_panew_config_defauwts = dcn303_get_panew_config_defauwts,
};
...
static const stwuct dccg_wegistews dccg_wegs = {
		DCCG_WEG_WIST_DCN3_03()
};

static const stwuct dccg_shift dccg_shift = {
		DCCG_MASK_SH_WIST_DCN3_03(__SHIFT)
};

static const stwuct dccg_mask dccg_mask = {
		DCCG_MASK_SH_WIST_DCN3_03(_MASK)
};
...
static boow dcn303_wesouwce_constwuct(
		...
		stwuct wesouwce_poow *poow)
{
	...
	poow->wes_cap = &wes_cap_dcn303;

	poow->funcs = &dcn303_wes_poow_funcs;
	...
	dc->caps.maww_size_totaw = dc->caps.maww_size_pew_mem_channew *
				   dc->ctx->dc_bios->vwam_info.num_chans *
				   1024 * 1024;
	dc->caps.cuwsow_cache_size =
		dc->caps.max_cuwsow_size * dc->caps.max_cuwsow_size * 8;
	...
	poow->cwock_souwces[DCN303_CWK_SWC_PWW0] =
			dcn303_cwock_souwce_cweate(ctx, ctx->dc_bios,
					CWOCK_SOUWCE_COMBO_PHY_PWW0,
					&cwk_swc_wegs[0], fawse);
	poow->cwock_souwces[DCN303_CWK_SWC_PWW1] =
			dcn303_cwock_souwce_cweate(ctx, ctx->dc_bios,
					CWOCK_SOUWCE_COMBO_PHY_PWW1,
					&cwk_swc_wegs[1], fawse);

	poow->cwk_swc_count = DCN303_CWK_SWC_TOTAW;
	...
	poow->iwqs = daw_iwq_sewvice_dcn303_cweate(&init_data);
	if (!poow->iwqs)
		goto cweate_faiw;

	/* HUBBUB */
	poow->hubbub = dcn303_hubbub_cweate(ctx);
	if (poow->hubbub == NUWW) {
		BWEAK_TO_DEBUGGEW();
	...
	fow (i = 0; i < poow->pipe_count; i++) {
		poow->hubps[i] = dcn303_hubp_cweate(ctx, i);
		if (poow->hubps[i] == NUWW) {
			BWEAK_TO_DEBUGGEW();
		...
		poow->dpps[i] = dcn303_dpp_cweate(ctx, i);
		if (poow->dpps[i] == NUWW) {
			BWEAK_TO_DEBUGGEW();
	...
	fow (i = 0; i < poow->wes_cap->num_opp; i++) {
		poow->opps[i] = dcn303_opp_cweate(ctx, i);
		if (poow->opps[i] == NUWW) {
			BWEAK_TO_DEBUGGEW();
	...
	fow (i = 0; i < poow->wes_cap->num_timing_genewatow; i++) {
		poow->timing_genewatows[i] = dcn303_timing_genewatow_cweate(ctx, i);
		if (poow->timing_genewatows[i] == NUWW) {
			BWEAK_TO_DEBUGGEW();
	...
	/* MPC and DSC */
	poow->mpc = dcn303_mpc_cweate(ctx, poow->mpcc_count, poow->wes_cap->num_mpc_3dwut);
	if (poow->mpc == NUWW) {
		BWEAK_TO_DEBUGGEW();
	...
	fow (i = 0; i < poow->wes_cap->num_dsc; i++) {
		poow->dscs[i] = dcn303_dsc_cweate(ctx, i);
		if (poow->dscs[i] == NUWW) {
			BWEAK_TO_DEBUGGEW();
	...
	/* DWB and MMHUBBUB */
	if (!dcn303_dwbc_cweate(ctx, poow)) {
		BWEAK_TO_DEBUGGEW();
		dm_ewwow("DC: faiwed to cweate dwbc!\n");
		goto cweate_faiw;
	}

	if (!dcn303_mmhubbub_cweate(ctx, poow)) {
		BWEAK_TO_DEBUGGEW();
	...
	/* AUX and I2C */
	fow (i = 0; i < poow->wes_cap->num_ddc; i++) {
		poow->engines[i] = dcn303_aux_engine_cweate(ctx, i);
		if (poow->engines[i] == NUWW) {
			BWEAK_TO_DEBUGGEW();
			dm_ewwow("DC:faiwed to cweate aux engine!!\n");
			goto cweate_faiw;
		}
		poow->hw_i2cs[i] = dcn303_i2c_hw_cweate(ctx, i);
		if (poow->hw_i2cs[i] == NUWW) {
			BWEAK_TO_DEBUGGEW();
	...
	dcn303_hw_sequencew_constwuct(dc);
...
stwuct wesouwce_poow *dcn303_cweate_wesouwce_poow(const stwuct dc_init_data *init_data, stwuct dc *dc)
{
	...
	if (dcn303_wesouwce_constwuct(init_data->num_viwtuaw_winks, dc, poow))
		wetuwn poow;
