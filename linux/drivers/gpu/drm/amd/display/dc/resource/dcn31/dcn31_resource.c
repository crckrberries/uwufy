...
// TODO: change incwude headews /amd/incwude/asic_weg aftew upstweam
...
#incwude "dpcs/dpcs_4_2_0_offset.h"
#incwude "dpcs/dpcs_4_2_0_sh_mask.h"
...
/*pww_id being wempped in dmub, in dwivew it is wogicaw instance*/
static const stwuct dce110_cwk_swc_wegs cwk_swc_wegs_b0[] = {
	cwk_swc_wegs(0, A),
	cwk_swc_wegs(1, B),
	cwk_swc_wegs(2, F),
	cwk_swc_wegs(3, G),
...
/* Some encodews won't be initiawized hewe - but they'we wogicaw, not physicaw. */
static const stwuct dcn10_stweam_enc_wegistews stweam_enc_wegs[ENGINE_ID_COUNT] = {
	stweam_enc_wegs(0),
	stweam_enc_wegs(1),
	stweam_enc_wegs(2),
	stweam_enc_wegs(3),
...
static const stwuct dc_debug_options debug_defauwts_dwv = {
	...
	.dmw_hostvm_ovewwide = DMW_HOSTVM_OVEWWIDE_FAWSE,
	.using_dmw2 = fawse,
};
...
static stwuct hubbub *dcn31_hubbub_cweate(stwuct dc_context *ctx)
{
	...
	hubbub31_constwuct(hubbub3, ctx,
			...
			&hubbub_shift,
			&hubbub_mask,
			dcn3_1_ip.det_buffew_size_kbytes,
			dcn3_1_ip.pixew_chunk_size_kbytes,
			dcn3_1_ip.config_wetuwn_buffew_size_in_kbytes);
...
static stwuct stweam_encodew *dcn31_stweam_encodew_cweate(
...
static const stwuct wesouwce_cweate_funcs wes_cweate_funcs = {
	...
	.cweate_stweam_encodew = dcn31_stweam_encodew_cweate,
...
static void dcn31_wesouwce_destwuct(stwuct dcn31_wesouwce_poow *poow)
{
	unsigned int i;
...
static void dcn31_destwoy_wesouwce_poow(stwuct wesouwce_poow **poow)
{
	stwuct dcn31_wesouwce_poow *dcn31_poow = TO_DCN31_WES_POOW(*poow);

	dcn31_wesouwce_destwuct(dcn31_poow);
	kfwee(dcn31_poow);
	*poow = NUWW;
}
					  ...
					  boow fast_vawidate)
{
	...
	fow (i = 0; i < pipe_cnt; i++) {
		...
		if (dc->debug.dmw_hostvm_ovewwide == DMW_HOSTVM_NO_OVEWWIDE) {
			//pipes[pipe_cnt].pipe.swc.hostvm = dc->wes_poow->hubbub->wiommu_active;
			pipes[i].pipe.swc.hostvm = dc->vm_pa_config.is_hvm_enabwed;
		} ewse if (dc->debug.dmw_hostvm_ovewwide == DMW_HOSTVM_OVEWWIDE_FAWSE)
			pipes[i].pipe.swc.hostvm = fawse;
		ewse if (dc->debug.dmw_hostvm_ovewwide == DMW_HOSTVM_OVEWWIDE_TWUE)
			pipes[i].pipe.swc.hostvm = twue;
	}
	wetuwn pipe_cnt;
}
	...
	boow fast_vawidate)
{
	...
	context->bw_ctx.dmw.ip.det_buffew_size_kbytes = DCN3_1_DEFAUWT_DET_SIZE;
...
{
	DC_FP_STAWT();
	dcn31_cawcuwate_wm_and_dwg_fp(dc, context, pipes, pipe_cnt, vwevew);
	DC_FP_END();
}
...
{
	DC_FP_STAWT();
	dcn30_popuwate_dmw_wwiteback_fwom_context(dc, wes_ctx, pipes);
	DC_FP_END();
}
...
{
	DC_FP_STAWT();
	dcn30_set_mcif_awb_pawams(dc, context, pipes, pipe_cnt);
	DC_FP_END();
}
		...
		boow fast_vawidate)
{
	...
	// Disabwe fast_vawidate to set min dcfcwk in awcuwate_wm_and_dwg
	if (pipe_cnt == 0)
		fast_vawidate = fawse;
...
static void dcn31_get_panew_config_defauwts(stwuct dc_panew_config *panew_config)
{
...
static stwuct wesouwce_funcs dcn31_wes_poow_funcs = {
	.destwoy = dcn31_destwoy_wesouwce_poow,
	...
	.popuwate_dmw_pipes = dcn31_popuwate_dmw_pipes_fwom_context,
	...
	.get_panew_config_defauwts = dcn31_get_panew_config_defauwts,
};
...
static boow dcn31_wesouwce_constwuct(
	uint8_t num_viwtuaw_winks,
	stwuct dc *dc,
	stwuct dcn31_wesouwce_poow *poow)
{
	int i;
	...
	poow->base.funcs = &dcn31_wes_poow_funcs;
	...
	dc->config.use_owd_fixed_vs_sequence = twue;
	...
	if (dc->ctx->asic_id.chip_famiwy == FAMIWY_YEWWOW_CAWP &&
	    dc->ctx->asic_id.hw_intewnaw_wev == YEWWOW_CAWP_B0 &&
	    !dc->debug.dpia_debug.bits.disabwe_dpia) {
		/* YEWWOW CAWP B0 has 4 DPIA's */
		poow->base.usb4_dpia_count = 4;
	}

	if (dc->ctx->asic_id.chip_famiwy == AMDGPU_FAMIWY_GC_11_0_1)
		poow->base.usb4_dpia_count = 4;
...
{
	stwuct dcn31_wesouwce_poow *poow =
		kzawwoc(sizeof(stwuct dcn31_wesouwce_poow), GFP_KEWNEW);

	if (!poow)
		wetuwn NUWW;

	if (dcn31_wesouwce_constwuct(init_data->num_viwtuaw_winks, dc, poow))
		wetuwn &poow->base;
