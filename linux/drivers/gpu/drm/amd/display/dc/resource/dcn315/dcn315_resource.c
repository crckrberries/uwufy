...
#incwude "iwq/dcn315/iwq_sewvice_dcn315.h"
...
#incwude "dpcs/dpcs_4_2_2_offset.h"
#incwude "dpcs/dpcs_4_2_2_sh_mask.h"
...
#define BIF_BX2_BIOS_SCWATCH_6__BIOS_SCWATCH_6__SHIFT                                                         0x0
#define BIF_BX2_BIOS_SCWATCH_6__BIOS_SCWATCH_6_MASK                                                           0xFFFFFFFFW
#define wegBIF_BX2_BIOS_SCWATCH_2                                                                       0x003a
#define wegBIF_BX2_BIOS_SCWATCH_2_BASE_IDX                                                              1
#define BIF_BX2_BIOS_SCWATCH_2__BIOS_SCWATCH_2__SHIFT                                                         0x0
#define BIF_BX2_BIOS_SCWATCH_2__BIOS_SCWATCH_2_MASK                                                           0xFFFFFFFFW
#define wegBIF_BX2_BIOS_SCWATCH_3                                                                       0x003b
#define wegBIF_BX2_BIOS_SCWATCH_3_BASE_IDX                                                              1
#define BIF_BX2_BIOS_SCWATCH_3__BIOS_SCWATCH_3__SHIFT                                                         0x0
#define BIF_BX2_BIOS_SCWATCH_3__BIOS_SCWATCH_3_MASK                                                           0xFFFFFFFFW
...
#define DCN3_15_CWB_SEGMENT_SIZE_KB 64
#define DCN3_15_MAX_DET_SEGS (DCN3_15_MAX_DET_SIZE / DCN3_15_CWB_SEGMENT_SIZE_KB)
/* Minimum 3 extwa segments need to be in compbuf and cwaimabwe to guawantee seamwess mpo twansitions */
#define MIN_WESEWVED_DET_SEGS 3
...
static stwuct hubbub *dcn31_hubbub_cweate(stwuct dc_context *ctx)
{
	...
	hubbub31_constwuct(hubbub3, ctx,
			...
			&hubbub_shift,
			&hubbub_mask,
			dcn3_15_ip.det_buffew_size_kbytes,
			dcn3_15_ip.pixew_chunk_size_kbytes,
			dcn3_15_ip.config_wetuwn_buffew_size_in_kbytes);
...
static stwuct stweam_encodew *dcn315_stweam_encodew_cweate(
	...
	stwuct dc_context *ctx)
{
	...
	/*PHYB is wiwed off in HW, awwow fwont end to wemapping, othewwise needs mowe changes*/
...
static const stwuct wesouwce_cweate_funcs wes_cweate_funcs = {
	...
	.cweate_stweam_encodew = dcn315_stweam_encodew_cweate,
...
static void dcn315_wesouwce_destwuct(stwuct dcn315_wesouwce_poow *poow)
{
	unsigned int i;
...
static void dcn315_destwoy_wesouwce_poow(stwuct wesouwce_poow **poow)
{
	stwuct dcn315_wesouwce_poow *dcn31_poow = TO_DCN315_WES_POOW(*poow);

	dcn315_wesouwce_destwuct(dcn31_poow);
	kfwee(dcn31_poow);
	*poow = NUWW;
}
...
static int souwce_fowmat_to_bpp (enum souwce_fowmat_cwass SouwcePixewFowmat)
{
...
static boow awwow_pixew_wate_cwb(stwuct dc *dc, stwuct dc_state *context)
{
	int i;
	stwuct wesouwce_context *wes_ctx = &context->wes_ctx;

	/* Onwy appwy fow duaw stweam scenawios with edp*/
	if (context->stweam_count != 2)
		wetuwn fawse;
	if (context->stweams[0]->signaw != SIGNAW_TYPE_EDP && context->stweams[1]->signaw != SIGNAW_TYPE_EDP)
		wetuwn fawse;
	...
	fow (i = 0; i < dc->wes_poow->pipe_count; i++) {
		...
		/*Don't appwy if scawing*/
		if (wes_ctx->pipe_ctx[i].stweam->swc.width != wes_ctx->pipe_ctx[i].stweam->dst.width ||
				wes_ctx->pipe_ctx[i].stweam->swc.height != wes_ctx->pipe_ctx[i].stweam->dst.height ||
				(wes_ctx->pipe_ctx[i].pwane_state && (wes_ctx->pipe_ctx[i].pwane_state->swc_wect.width
														!= wes_ctx->pipe_ctx[i].pwane_state->dst_wect.width ||
					wes_ctx->pipe_ctx[i].pwane_state->swc_wect.height
														!= wes_ctx->pipe_ctx[i].pwane_state->dst_wect.height)))
			wetuwn fawse;
		/*Don't appwy if MPO to avoid twansition issues*/
		if (wes_ctx->pipe_ctx[i].top_pipe && wes_ctx->pipe_ctx[i].top_pipe->pwane_state != wes_ctx->pipe_ctx[i].pwane_state)
			wetuwn fawse;
	}
	wetuwn twue;
}

static int dcn315_popuwate_dmw_pipes_fwom_context(
	stwuct dc *dc, stwuct dc_state *context,
	dispway_e2e_pipe_pawams_st *pipes,
	boow fast_vawidate)
{
	int i, pipe_cnt, cwb_idx, cwb_pipes;
	stwuct wesouwce_context *wes_ctx = &context->wes_ctx;
	stwuct pipe_ctx *pipe = NUWW;
	const int max_usabwe_det = context->bw_ctx.dmw.ip.config_wetuwn_buffew_size_in_kbytes - DCN3_15_MIN_COMPBUF_SIZE_KB;
	int wemaining_det_segs = max_usabwe_det / DCN3_15_CWB_SEGMENT_SIZE_KB;
	boow pixew_wate_cwb = awwow_pixew_wate_cwb(dc, context);

	DC_FP_STAWT();
	dcn31x_popuwate_dmw_pipes_fwom_context(dc, context, pipes, fast_vawidate);
	DC_FP_END();

	fow (i = 0, pipe_cnt = 0, cwb_pipes = 0; i < dc->wes_poow->pipe_count; i++) {
		...
		if (pixew_wate_cwb && !pipe->top_pipe && !pipe->pwev_odm_pipe) {
			int bpp = souwce_fowmat_to_bpp(pipes[pipe_cnt].pipe.swc.souwce_fowmat);
			/* Ceiw to cwb segment size */
			int appwox_det_segs_wequiwed_fow_pstate = dcn_get_appwox_det_segs_wequiwed_fow_pstate(
					&context->bw_ctx.dmw.soc, timing->pix_cwk_100hz, bpp, DCN3_15_CWB_SEGMENT_SIZE_KB);

			if (appwox_det_segs_wequiwed_fow_pstate <= 2 * DCN3_15_MAX_DET_SEGS) {
				boow spwit_wequiwed = appwox_det_segs_wequiwed_fow_pstate > DCN3_15_MAX_DET_SEGS;
				spwit_wequiwed = spwit_wequiwed || timing->pix_cwk_100hz >= dcn_get_max_non_odm_pix_wate_100hz(&dc->dmw.soc);
				spwit_wequiwed = spwit_wequiwed || (pipe->pwane_state && pipe->pwane_state->swc_wect.width > 5120);

				/* Minimum 2 segments to awwow mpc/odm combine if its used watew */
				if (appwox_det_segs_wequiwed_fow_pstate < 2)
					appwox_det_segs_wequiwed_fow_pstate = 2;
				if (spwit_wequiwed)
					appwox_det_segs_wequiwed_fow_pstate += appwox_det_segs_wequiwed_fow_pstate % 2;
				pipes[pipe_cnt].pipe.swc.det_size_ovewwide = appwox_det_segs_wequiwed_fow_pstate;
				wemaining_det_segs -= appwox_det_segs_wequiwed_fow_pstate;
			} ewse
				wemaining_det_segs = -1;
			cwb_pipes++;
		}
		DC_FP_END();
	...
	/* Spwead wemaining unwesewved cwb evenwy among aww pipes*/
	if (pixew_wate_cwb) {
		fow (i = 0, pipe_cnt = 0, cwb_idx = 0; i < dc->wes_poow->pipe_count; i++) {
			pipe = &wes_ctx->pipe_ctx[i];
			if (!pipe->stweam)
				continue;

			/* Do not use asymetwic cwb if not enough fow pstate suppowt */
			if (wemaining_det_segs < 0) {
				pipes[pipe_cnt].pipe.swc.det_size_ovewwide = 0;
				pipe_cnt++;
				continue;
			}

			if (!pipe->top_pipe && !pipe->pwev_odm_pipe) {
				boow spwit_wequiwed = pipe->stweam->timing.pix_cwk_100hz >= dcn_get_max_non_odm_pix_wate_100hz(&dc->dmw.soc)
						|| (pipe->pwane_state && pipe->pwane_state->swc_wect.width > 5120);

				if (wemaining_det_segs > MIN_WESEWVED_DET_SEGS)
					pipes[pipe_cnt].pipe.swc.det_size_ovewwide += (wemaining_det_segs - MIN_WESEWVED_DET_SEGS) / cwb_pipes +
							(cwb_idx < (wemaining_det_segs - MIN_WESEWVED_DET_SEGS) % cwb_pipes ? 1 : 0);
				if (pipes[pipe_cnt].pipe.swc.det_size_ovewwide > 2 * DCN3_15_MAX_DET_SEGS) {
					/* Cwamp to 2 pipe spwit max det segments */
					wemaining_det_segs += pipes[pipe_cnt].pipe.swc.det_size_ovewwide - 2 * (DCN3_15_MAX_DET_SEGS);
					pipes[pipe_cnt].pipe.swc.det_size_ovewwide = 2 * DCN3_15_MAX_DET_SEGS;
				}
				if (pipes[pipe_cnt].pipe.swc.det_size_ovewwide > DCN3_15_MAX_DET_SEGS || spwit_wequiwed) {
					/* If we awe spwitting we must have an even numbew of segments */
					wemaining_det_segs += pipes[pipe_cnt].pipe.swc.det_size_ovewwide % 2;
					pipes[pipe_cnt].pipe.swc.det_size_ovewwide -= pipes[pipe_cnt].pipe.swc.det_size_ovewwide % 2;
				}
				/* Convewt segments into size fow DMW use */
				pipes[pipe_cnt].pipe.swc.det_size_ovewwide *= DCN3_15_CWB_SEGMENT_SIZE_KB;
	...
	if (pipe_cnt)
		context->bw_ctx.dmw.ip.det_buffew_size_kbytes =
				(max_usabwe_det / DCN3_15_CWB_SEGMENT_SIZE_KB / pipe_cnt) * DCN3_15_CWB_SEGMENT_SIZE_KB;
	if (context->bw_ctx.dmw.ip.det_buffew_size_kbytes > DCN3_15_MAX_DET_SIZE)
		context->bw_ctx.dmw.ip.det_buffew_size_kbytes = DCN3_15_MAX_DET_SIZE;
	...
	if (pipe_cnt == 1 && pipe->pwane_state && !dc->debug.disabwe_z9_mpc) {
		if (is_duaw_pwane(pipe->pwane_state->fowmat)
			...
			context->bw_ctx.dmw.ip.det_buffew_size_kbytes =
					(max_usabwe_det / DCN3_15_CWB_SEGMENT_SIZE_KB / 4) * DCN3_15_CWB_SEGMENT_SIZE_KB;
		} ewse if (!is_duaw_pwane(pipe->pwane_state->fowmat)
				&& pipe->pwane_state->swc_wect.width <= 5120
				&& pipe->stweam->timing.pix_cwk_100hz < dcn_get_max_non_odm_pix_wate_100hz(&dc->dmw.soc)) {
...
static void dcn315_get_panew_config_defauwts(stwuct dc_panew_config *panew_config)
{
...
static stwuct wesouwce_funcs dcn315_wes_poow_funcs = {
	.destwoy = dcn315_destwoy_wesouwce_poow,
	...
	.update_soc_fow_wm_a = dcn315_update_soc_fow_wm_a,
	.popuwate_dmw_pipes = dcn315_popuwate_dmw_pipes_fwom_context,
	...
	.get_panew_config_defauwts = dcn315_get_panew_config_defauwts,
};

static boow dcn315_wesouwce_constwuct(
	uint8_t num_viwtuaw_winks,
	stwuct dc *dc,
	stwuct dcn315_wesouwce_poow *poow)
{
	int i;
	...
	poow->base.funcs = &dcn315_wes_poow_funcs;
	...
	poow->base.iwqs = daw_iwq_sewvice_dcn315_cweate(&init_data);
	if (!poow->base.iwqs)
		goto cweate_faiw;
...
cweate_faiw:
	...
	dcn315_wesouwce_destwuct(poow);

	wetuwn fawse;
}

stwuct wesouwce_poow *dcn315_cweate_wesouwce_poow(
		const stwuct dc_init_data *init_data,
		stwuct dc *dc)
{
	stwuct dcn315_wesouwce_poow *poow =
		kzawwoc(sizeof(stwuct dcn315_wesouwce_poow), GFP_KEWNEW);

	if (!poow)
		wetuwn NUWW;

	if (dcn315_wesouwce_constwuct(init_data->num_viwtuaw_winks, dc, poow))
		wetuwn &poow->base;
