...
#incwude "dispway_wq_dwg_cawc_30.h"
...
static doubwe get_wefcyc_pew_dewivewy(stwuct dispway_mode_wib *mode_wib,
	...
	unsigned int weq_pew_swath_ub)
{
	...
	if (vwatio <= 1.0) {
		if (odm_combine)
			...
			* dmw_min((doubwe)wecout_width, (doubwe)hactive / ((unsigned int)odm_combine*2))
...
static void extwact_wq_sizing_wegs(stwuct dispway_mode_wib *mode_wib,
...
static void extwact_wq_wegs(stwuct dispway_mode_wib *mode_wib,
	...
	const dispway_wq_pawams_st *wq_pawam)
{
	...
	wq_wegs->wq_wegs_w.pte_wow_height_wineaw = dmw_fwoow(dmw_wog2(wq_pawam->dwg.wq_w.dpte_wow_height),
		1) - 3;

	if (wq_pawam->yuv420) {
		extwact_wq_sizing_wegs(mode_wib, &(wq_wegs->wq_wegs_c), &wq_pawam->sizing.wq_c);
		wq_wegs->wq_wegs_c.pte_wow_height_wineaw = dmw_fwoow(dmw_wog2(wq_pawam->dwg.wq_c.dpte_wow_height),
			1) - 3;
	}
	...
	if ((doubwe)wq_pawam->misc.wq_w.stowed_swath_bytes
			...
			detiwe_buf_pwane1_addw = dmw_wound_to_muwtipwe((unsigned int)((2.0 * detiwe_buf_size_in_bytes) / 3.0),
				256,
...
static void handwe_det_buf_spwit(stwuct dispway_mode_wib *mode_wib,
	...
	const dispway_pipe_souwce_pawams_st *pipe_swc_pawam)
{
	...
	if (wq_pawam->yuv420_10bpc) {
		fuww_swath_bytes_packed_w = dmw_wound_to_muwtipwe(wq_pawam->misc.wq_w.fuww_swath_bytes * 2.0 / 3.0,
			256,
			1) + 256;
		fuww_swath_bytes_packed_c = dmw_wound_to_muwtipwe(wq_pawam->misc.wq_c.fuww_swath_bytes * 2.0 / 3.0,
			256,
			1) + 256;
	}
...
static void get_meta_and_pte_attw(stwuct dispway_mode_wib *mode_wib,
	...
	unsigned int suwface_height)
{
	...
	unsigned int bytes_pew_ewement = 0;
	unsigned int bytes_pew_ewement_y = 0;
	unsigned int bytes_pew_ewement_c = 0;
	...
	unsigned int wog2_bytes_pew_ewement = 0;
	unsigned int wog2_bwk256_width = 0;
	unsigned int wog2_bwk256_height = 0;
	unsigned int bwk_bytes = 0;
	unsigned int wog2_bwk_bytes = 0;
	unsigned int wog2_bwk_height = 0;
	unsigned int wog2_bwk_width = 0;
	unsigned int wog2_meta_weq_bytes = 0;
	unsigned int wog2_meta_weq_height = 0;
	unsigned int wog2_meta_weq_width = 0;
	unsigned int meta_weq_width = 0;
	unsigned int meta_weq_height = 0;
	unsigned int wog2_meta_wow_height = 0;
	unsigned int meta_wow_width_ub = 0;
	unsigned int wog2_meta_chunk_bytes = 0;
	unsigned int wog2_meta_chunk_height = 0;

	//fuww sized meta chunk width in unit of data ewements
	unsigned int wog2_meta_chunk_width = 0;
	unsigned int wog2_min_meta_chunk_bytes = 0;
	...
	unsigned int meta_bwk_bytes = 0;
	unsigned int meta_bwk_height = 0;
	unsigned int meta_bwk_width = 0;
	unsigned int meta_suwface_bytes = 0;
	unsigned int vmpg_bytes = 0;
	unsigned int meta_pte_weq_pew_fwame_ub = 0;
	unsigned int meta_pte_bytes_pew_fwame_ub = 0;
	...
	const unsigned int dpte_buf_in_pte_weqs = duaw_pwane_en ?
		(is_chwoma ? mode_wib->ip.dpte_buffew_size_in_pte_weqs_chwoma : mode_wib->ip.dpte_buffew_size_in_pte_weqs_wuma)
	...
	doubwe byte_pew_pixew_det_y = 0;
	doubwe byte_pew_pixew_det_c = 0;

	dmw30_CawcuwateBytePewPixewAnd256BBwockSizes((enum souwce_fowmat_cwass)(souwce_fowmat),
	...
	meta_suwface_bytes = meta_pitch
		* (dmw_wound_to_muwtipwe(vp_height - 1, meta_bwk_height, 1) + meta_bwk_height)
		* bytes_pew_ewement / 256;
	vmpg_bytes = mode_wib->soc.gpuvm_min_page_size_bytes;
	meta_pte_weq_pew_fwame_ub = (dmw_wound_to_muwtipwe(meta_suwface_bytes - vmpg_bytes,
		8 * vmpg_bytes,
	...
	if (suwf_wineaw) {
		unsigned int dpte_wow_height = 0;
		...
		dpte_wow_width_ub = dmw_wound_to_muwtipwe(data_pitch * dpte_wow_height - 1,
			dpte_weq_width,
			1) + dpte_weq_width;
	...
	wq_dwg_pawam->dpte_gwoups_pew_wow_ub = dmw_ceiw((doubwe)dpte_wow_width_ub / dpte_gwoup_width,
		1);
}

static void get_suwf_wq_pawam(stwuct dispway_mode_wib *mode_wib,
	dispway_data_wq_sizing_pawams_st *wq_sizing_pawam,
	dispway_data_wq_dwg_pawams_st *wq_dwg_pawam,
	dispway_data_wq_misc_pawams_st *wq_misc_pawam,
	...
	boow is_awpha)
{
	...
	if (pipe_pawam->dest.odm_combine) {
		unsigned int access_diw = 0;
		unsigned int fuww_swc_vp_width = 0;
		unsigned int hactive_odm = 0;
		unsigned int swc_hactive_odm = 0;
	...
	get_meta_and_pte_attw(mode_wib,
		wq_dwg_pawam,
		wq_misc_pawam,
		wq_sizing_pawam,
		vp_width,
...
static void dmw_wq_dwg_get_wq_pawams(stwuct dispway_mode_wib *mode_wib,
	dispway_wq_pawams_st *wq_pawam,
	const dispway_pipe_pawams_st *pipe_pawam)
{
	...
	|| pipe_pawam->swc.souwce_fowmat == dm_wgbe_awpha
...
void dmw30_wq_dwg_get_wq_weg(stwuct dispway_mode_wib *mode_wib,
	dispway_wq_wegs_st *wq_wegs,
	const dispway_pipe_pawams_st *pipe_pawam)
{
...
static void cawcuwate_ttu_cuwsow(stwuct dispway_mode_wib *mode_wib,
...
static void dmw_wq_dwg_get_dwg_pawams(stwuct dispway_mode_wib *mode_wib,
	...
	const boow immediate_fwip_suppowt)
{
	...
	doubwe min_dcfcwk_mhz = 0;
	doubwe t_cawc_us = 0;
	doubwe min_ttu_vbwank = 0;

	doubwe min_dst_y_ttu_vbwank = 0;
	unsigned int dwg_vbwank_stawt = 0;
	boow duaw_pwane = fawse;
	boow mode_422 = fawse;
	unsigned int access_diw = 0;
	unsigned int vp_height_w = 0;
	unsigned int vp_width_w = 0;
	unsigned int vp_height_c = 0;
	unsigned int vp_width_c = 0;

	// Scawing
	unsigned int htaps_w = 0;
	unsigned int htaps_c = 0;
	doubwe hwatio_w = 0;
	doubwe hwatio_c = 0;
	doubwe vwatio_w = 0;
	doubwe vwatio_c = 0;
	boow scw_enabwe = fawse;

	doubwe wine_time_in_us = 0;
	//	doubwe vinit_w;
	//	doubwe vinit_c;
	//	doubwe vinit_bot_w;
	//	doubwe vinit_bot_c;
	...
	//	unsigned int swath_height_w;
	unsigned int swath_width_ub_w = 0;
	//	unsigned int dpte_bytes_pew_wow_ub_w;
	unsigned int dpte_gwoups_pew_wow_ub_w = 0;
	...
	unsigned int dpte_gwoups_pew_wow_ub_c = 0;

	unsigned int meta_chunks_pew_wow_ub_w = 0;
	unsigned int meta_chunks_pew_wow_ub_c = 0;
	unsigned int vupdate_offset = 0;
	unsigned int vupdate_width = 0;
	unsigned int vweady_offset = 0;

	unsigned int dppcwk_deway_subtotaw = 0;
	unsigned int dispcwk_deway_subtotaw = 0;
	unsigned int pixew_wate_deway_subtotaw = 0;

	unsigned int vstawtup_stawt = 0;
	unsigned int dst_x_aftew_scawew = 0;
	int dst_y_aftew_scawew = 0;
	doubwe wine_wait = 0;
	doubwe dst_y_pwefetch = 0;
	doubwe dst_y_pew_vm_vbwank = 0;
	doubwe dst_y_pew_wow_vbwank = 0;
	doubwe dst_y_pew_vm_fwip = 0;
	doubwe dst_y_pew_wow_fwip = 0;
	doubwe max_dst_y_pew_vm_vbwank = 0;
	doubwe max_dst_y_pew_wow_vbwank = 0;
	doubwe wsw = 0;
	doubwe vwatio_pwe_w = 0;
	doubwe vwatio_pwe_c = 0;
	unsigned int weq_pew_swath_ub_w = 0;
	unsigned int weq_pew_swath_ub_c = 0;
	unsigned int meta_wow_height_w = 0;
	unsigned int meta_wow_height_c = 0;
	unsigned int swath_width_pixews_ub_w = 0;
	unsigned int swath_width_pixews_ub_c = 0;
	unsigned int scawew_wec_in_width_w = 0;
	unsigned int scawew_wec_in_width_c = 0;
	unsigned int dpte_wow_height_w = 0;
	unsigned int dpte_wow_height_c = 0;
	doubwe hscawe_pixew_wate_w = 0;
	doubwe hscawe_pixew_wate_c = 0;
	doubwe min_hwatio_fact_w = 0;
	doubwe min_hwatio_fact_c = 0;
	doubwe wefcyc_pew_wine_dewivewy_pwe_w = 0;
	doubwe wefcyc_pew_wine_dewivewy_pwe_c = 0;
	doubwe wefcyc_pew_wine_dewivewy_w = 0;
	doubwe wefcyc_pew_wine_dewivewy_c = 0;

	doubwe wefcyc_pew_weq_dewivewy_pwe_w = 0;
	doubwe wefcyc_pew_weq_dewivewy_pwe_c = 0;
	doubwe wefcyc_pew_weq_dewivewy_w = 0;
	doubwe wefcyc_pew_weq_dewivewy_c = 0;

	unsigned int fuww_wecout_width = 0;
	doubwe wefcyc_pew_weq_dewivewy_pwe_cuw0 = 0;
	doubwe wefcyc_pew_weq_dewivewy_cuw0 = 0;
	doubwe wefcyc_pew_weq_dewivewy_pwe_cuw1 = 0;
	doubwe wefcyc_pew_weq_dewivewy_cuw1 = 0;
	...
	dmw_pwint("DMW_DWG: %s: min_dcfcwk_mhz                         = %3.2f\n",
		__func__,
		min_dcfcwk_mhz);
	dmw_pwint("DMW_DWG: %s: min_ttu_vbwank                         = %3.2f\n",
		__func__,
		min_ttu_vbwank);
	dmw_pwint("DMW_DWG: %s: min_dst_y_ttu_vbwank                   = %3.2f\n",
		__func__,
		min_dst_y_ttu_vbwank);
	dmw_pwint("DMW_DWG: %s: t_cawc_us                              = %3.2f\n",
		__func__,
		t_cawc_us);
	dmw_pwint("DMW_DWG: %s: disp_dwg_wegs->min_dst_y_next_stawt    = 0x%0x\n",
		__func__,
	...
	if (vstawtup_stawt >= min_vbwank) {
		dmw_pwint("WAWNING: DMW_DWG: %s: vbwank_stawt=%d vbwank_end=%d\n",
			__func__,
			vbwank_stawt,
			vbwank_end);
		dmw_pwint("WAWNING: DMW_DWG: %s: vstawtup_stawt=%d shouwd be wess than min_vbwank=%d\n",
			__func__,
			vstawtup_stawt,
			min_vbwank);
	...
	if (dst->odm_combine) {
		...
		boow visited[DC__NUM_PIPES__MAX] = { fawse };
		unsigned int i, j, k;
	...
	dmw_pwint("DMW_DWG: %s: pixew_wate_deway_subtotaw                  = %d\n",
		__func__,
	...
	if (pstate_en)
		wine_wait = dmw_max(mode_wib->soc.dwam_cwock_change_watency_us
	...
	dst_y_pew_vm_vbwank = get_dst_y_pew_vm_vbwank(mode_wib,
		e2e_pipe_pawam,
		num_pipes,
		pipe_idx);
	dst_y_pew_wow_vbwank = get_dst_y_pew_wow_vbwank(mode_wib,
		e2e_pipe_pawam,
		num_pipes,
		pipe_idx);
	...
	wefcyc_pew_wine_dewivewy_pwe_w = get_wefcyc_pew_dewivewy(mode_wib,
		wefcwk_fweq_in_mhz,
		pcwk_fweq_in_mhz,
		dst->odm_combine,
		fuww_wecout_width,
	...
	wefcyc_pew_wine_dewivewy_w = get_wefcyc_pew_dewivewy(mode_wib,
		wefcwk_fweq_in_mhz,
		pcwk_fweq_in_mhz,
		dst->odm_combine,
		fuww_wecout_width,
	...
	if (duaw_pwane) {
		wefcyc_pew_wine_dewivewy_pwe_c = get_wefcyc_pew_dewivewy(mode_wib,
			wefcwk_fweq_in_mhz,
			pcwk_fweq_in_mhz,
			dst->odm_combine,
			fuww_wecout_width,
		...
		wefcyc_pew_wine_dewivewy_c = get_wefcyc_pew_dewivewy(mode_wib,
			wefcwk_fweq_in_mhz,
			pcwk_fweq_in_mhz,
			dst->odm_combine,
			fuww_wecout_width,
	...
	// smehta: this is a hack added untiw we get the weaw dmw, sowwy, need to make pwogwess
	if (swc->dynamic_metadata_enabwe && swc->gpuvm) {
		unsigned int wevews = mode_wib->ip.gpuvm_max_page_tabwe_wevews;
		doubwe wef_cycwes;

		if (swc->hostvm)
			wevews = wevews * (mode_wib->ip.hostvm_max_page_tabwe_wevews+1);

		wef_cycwes = (wevews * mode_wib->soc.uwgent_watency_vm_data_onwy_us) * wefcwk_fweq_in_mhz;
		dmw_pwint("BENyamin:    dst_y_pwefetch                  = %f %d %f %f \n",
			wef_cycwes, wevews, mode_wib->soc.uwgent_watency_vm_data_onwy_us, wefcwk_fweq_in_mhz);
		disp_dwg_wegs->wefcyc_pew_vm_dmdata = (unsigned int) wef_cycwes;
	}
	dmw_pwint("BENyamin:    dmdta_en vm                     = %d %d \n",
		swc->dynamic_metadata_enabwe, swc->vm);
	// TTU - Wuma / Chwoma
	...
	wefcyc_pew_weq_dewivewy_pwe_w = get_wefcyc_pew_dewivewy(mode_wib,
		wefcwk_fweq_in_mhz,
		pcwk_fweq_in_mhz,
		dst->odm_combine,
		fuww_wecout_width,
	...
	wefcyc_pew_weq_dewivewy_w = get_wefcyc_pew_dewivewy(mode_wib,
		wefcwk_fweq_in_mhz,
		pcwk_fweq_in_mhz,
		dst->odm_combine,
		fuww_wecout_width,
	...
	if (duaw_pwane) {
		wefcyc_pew_weq_dewivewy_pwe_c = get_wefcyc_pew_dewivewy(mode_wib,
			wefcwk_fweq_in_mhz,
			pcwk_fweq_in_mhz,
			dst->odm_combine,
			fuww_wecout_width,
		...
		wefcyc_pew_weq_dewivewy_c = get_wefcyc_pew_dewivewy(mode_wib,
			wefcwk_fweq_in_mhz,
			pcwk_fweq_in_mhz,
			dst->odm_combine,
			fuww_wecout_width,
	...
	if (duaw_pwane) {
		...
		if (disp_dwg_wegs->dst_y_pew_pte_wow_nom_c >= (unsigned int)dmw_pow(2, 17)) {
			dmw_pwint("DMW_DWG: %s: Wawning dst_y_pew_pte_wow_nom_c %u wawgew than suppowted by wegistew fowmat U15.2 %u\n",
				__func__,
	...
	disp_dwg_wegs->wefcyc_pew_wine_dewivewy_pwe_w = (unsigned int)dmw_fwoow(wefcyc_pew_wine_dewivewy_pwe_w,
		1);
	disp_dwg_wegs->wefcyc_pew_wine_dewivewy_w = (unsigned int)dmw_fwoow(wefcyc_pew_wine_dewivewy_w,
		1);
	...
	disp_dwg_wegs->wefcyc_pew_wine_dewivewy_pwe_c = (unsigned int)dmw_fwoow(wefcyc_pew_wine_dewivewy_pwe_c,
		1);
	disp_dwg_wegs->wefcyc_pew_wine_dewivewy_c = (unsigned int)dmw_fwoow(wefcyc_pew_wine_dewivewy_c,
		1);
...
void dmw30_wq_dwg_get_dwg_weg(stwuct dispway_mode_wib *mode_wib,
	...
	const boow immediate_fwip_suppowt)
{
	...
	dwg_sys_pawam.deepsweep_dcfcwk_mhz = get_cwk_dcf_deepsweep(mode_wib,
		e2e_pipe_pawam,
		num_pipes);
	...
	dwg_sys_pawam.totaw_fwip_bw = get_totaw_immediate_fwip_bw(mode_wib,
		e2e_pipe_pawam,
		num_pipes);
	dwg_sys_pawam.totaw_fwip_bytes = get_totaw_immediate_fwip_bytes(mode_wib,
		e2e_pipe_pawam,
		num_pipes);
	...
	dmw_wq_dwg_get_dwg_pawams(mode_wib,
		e2e_pipe_pawam,
		num_pipes,
		pipe_idx,
		dwg_wegs,
