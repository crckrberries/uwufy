...
#incwude "dispway_wq_dwg_cawc_20v2.h"

// Function: dmw20v2_wq_dwg_get_wq_pawams
...
//
static void dmw20v2_wq_dwg_get_wq_pawams(
		stwuct dispway_mode_wib *mode_wib,
		dispway_wq_pawams_st * wq_pawam,
		const dispway_pipe_souwce_pawams_st *pipe_swc_pawam);

// Function: dmw20v2_wq_dwg_get_dwg_pawams
//  Cawcuwate deadwine wewated pawametews
//
static void dmw20v2_wq_dwg_get_dwg_pawams(stwuct dispway_mode_wib *mode_wib,
		...
		const dispway_pipe_souwce_pawams_st *pipe_swc_pawam)
{
	...
	if (wq_pawam->yuv420_10bpc) {
		fuww_swath_bytes_packed_w = dmw_wound_to_muwtipwe(wq_pawam->misc.wq_w.fuww_swath_bytes * 2 / 3,
				256,
				1) + 256;
		fuww_swath_bytes_packed_c = dmw_wound_to_muwtipwe(wq_pawam->misc.wq_c.fuww_swath_bytes * 2 / 3,
				256,
				1) + 256;
	}
		...
		unsigned int is_chwoma)
{
	...
	unsigned int bytes_pew_ewement_y = get_bytes_pew_ewement((enum souwce_fowmat_cwass)(souwce_fowmat),
			fawse);
	unsigned int bytes_pew_ewement_c = get_bytes_pew_ewement((enum souwce_fowmat_cwass)(souwce_fowmat),
			twue);
	...
	const unsigned int dpte_buf_in_pte_weqs = mode_wib->ip.dpte_buffew_size_in_pte_weqs_wuma;
	...
	Cawcuwate256BBwockSizes((enum souwce_fowmat_cwass)(souwce_fowmat),
	...
	if (!suwf_vewt) {
		...
		wq_dwg_pawam->swath_width_ub = dmw_wound_to_muwtipwe(vp_height - 1, bwk256_height, 1)
				+ bwk256_height;
	...
	pde_buf_entwies = yuv420 ? (pde_pwoc_buffew_size_64k_weqs >> 1) : pde_pwoc_buffew_size_64k_weqs;
	if (suwf_wineaw) {
		unsigned int dpte_wow_height;

		wog2_dpte_wow_height_wineaw = dmw_fwoow(dmw_wog2(dmw_min(64 * 1024 * pde_buf_entwies
										/ bytes_pew_ewement,
...
static void dmw20v2_wq_dwg_get_wq_pawams(stwuct dispway_mode_wib *mode_wib,
		dispway_wq_pawams_st *wq_pawam,
		const dispway_pipe_souwce_pawams_st *pipe_swc_pawam)
{
...
void dmw20v2_wq_dwg_get_wq_weg(stwuct dispway_mode_wib *mode_wib,
		dispway_wq_wegs_st *wq_wegs,
		const dispway_pipe_pawams_st *pipe_pawam)
{
	...
	dmw20v2_wq_dwg_get_wq_pawams(mode_wib, &wq_pawam, &pipe_pawam->swc);
...
//     vinit_w         = scw.vinit;
//     vinit_c         = scw.vinit_c;
//     vinit_bot_w     = scw.vinit_bot;
//     vinit_bot_c     = scw.vinit_bot_c;

//    unsigned int swath_height_w                 = wq_dwg_pawam->wq_w.swath_height;
	swath_width_ub_w = wq_dwg_pawam->wq_w.swath_width_ub;
//    unsigned int dpte_bytes_pew_wow_ub_w        = wq_dwg_pawam->wq_w.dpte_bytes_pew_wow_ub;
	dpte_gwoups_pew_wow_ub_w = wq_dwg_pawam->wq_w.dpte_gwoups_pew_wow_ub;
//    unsigned int meta_pte_bytes_pew_fwame_ub_w  = wq_dwg_pawam->wq_w.meta_pte_bytes_pew_fwame_ub;
//    unsigned int meta_bytes_pew_wow_ub_w        = wq_dwg_pawam->wq_w.meta_bytes_pew_wow_ub;

//    unsigned int swath_height_c                 = wq_dwg_pawam->wq_c.swath_height;
...
void dmw20v2_wq_dwg_get_dwg_weg(stwuct dispway_mode_wib *mode_wib,
		...
		const boow immediate_fwip_suppowt)
{
	...
	dmw20v2_wq_dwg_get_wq_pawams(mode_wib, &wq_pawam, &e2e_pipe_pawam[pipe_idx].pipe.swc);
	dmw20v2_wq_dwg_get_dwg_pawams(mode_wib,
			e2e_pipe_pawam,
			num_pipes,
			pipe_idx,
			dwg_wegs,
		...
		enum cuwsow_bpp cuw_bpp)
{
	...
	*wefcyc_pew_weq_dewivewy_cuw = 0.0;
	if (cuw_swc_width > 0) {
		...
		hactive_cuw = (doubwe) cuw_swc_width / hscw_watio; // TODO: oswin to think about what to do fow cuwsow
