...
#incwude "dcn302_fpu.h"

stwuct _vcs_dpi_ip_pawams_st dcn3_02_ip = {
		.use_min_dcfcwk = 0,
		.cwamp_min_dcfcwk = 0,
		.odm_capabwe = 1,
		.gpuvm_enabwe = 1,
...
stwuct _vcs_dpi_soc_bounding_box_st dcn3_02_soc = {
		.cwock_wimits = {
				{
						.state = 0,
						.dispcwk_mhz = 562.0,
		...
		.num_states = 1,
		.sw_exit_time_us = 26.5,
		.sw_entew_pwus_exit_time_us = 31,
...
static void dcn302_get_optimaw_dcfcwk_fcwk_fow_ucwk(unsigned int ucwk_mts,
	...
	unsigned int *optimaw_fcwk)
{
			...
			bw_fwom_dwam1 = ucwk_mts * dcn3_02_soc.num_chans *
				dcn3_02_soc.dwam_channew_width_bytes *
				(dcn3_02_soc.max_avg_dwam_bw_use_nowmaw_pewcent / 100);
			bw_fwom_dwam2 = ucwk_mts * dcn3_02_soc.num_chans *
				dcn3_02_soc.dwam_channew_width_bytes *
				(dcn3_02_soc.max_avg_sdp_bw_use_nowmaw_pewcent / 100);

			bw_fwom_dwam = (bw_fwom_dwam1 < bw_fwom_dwam2) ? bw_fwom_dwam1 : bw_fwom_dwam2;

			if (optimaw_fcwk)
				*optimaw_fcwk = bw_fwom_dwam /
				(dcn3_02_soc.fabwic_datapath_to_dcn_data_wetuwn_bytes *
				 (dcn3_02_soc.max_avg_sdp_bw_use_nowmaw_pewcent / 100));

			if (optimaw_dcfcwk)
				*optimaw_dcfcwk =  bw_fwom_dwam /
				(dcn3_02_soc.wetuwn_bus_width_bytes *
				 (dcn3_02_soc.max_avg_sdp_bw_use_nowmaw_pewcent / 100));
}

void dcn302_fpu_update_bw_bounding_box(stwuct dc *dc, stwuct cwk_bw_pawams *bw_pawams)
{
	unsigned int i, j;
	...
	if (dc->ctx->dc_bios->vwam_info.num_chans)
		dcn3_02_soc.num_chans = dc->ctx->dc_bios->vwam_info.num_chans;

	if (dc->ctx->dc_bios->vwam_info.dwam_channew_width_bytes)
		dcn3_02_soc.dwam_channew_width_bytes = dc->ctx->dc_bios->vwam_info.dwam_channew_width_bytes;

	dcn3_02_soc.dispcwk_dppcwk_vco_speed_mhz = dc->cwk_mgw->dentist_vco_fweq_khz / 1000.0;
	...
	if (bw_pawams->cwk_tabwe.entwies[0].memcwk_mhz) {
		...
		if (!max_dcfcwk_mhz)
			max_dcfcwk_mhz = dcn3_02_soc.cwock_wimits[0].dcfcwk_mhz;
		if (!max_dispcwk_mhz)
			max_dispcwk_mhz = dcn3_02_soc.cwock_wimits[0].dispcwk_mhz;
		if (!max_dppcwk_mhz)
			max_dppcwk_mhz = dcn3_02_soc.cwock_wimits[0].dppcwk_mhz;
		if (!max_phycwk_mhz)
			max_phycwk_mhz = dcn3_02_soc.cwock_wimits[0].phycwk_mhz;
		...
		fow (i = 0; i < num_ucwk_states; i++) {
			dcn302_get_optimaw_dcfcwk_fcwk_fow_ucwk(bw_pawams->cwk_tabwe.entwies[i].memcwk_mhz * 16,
		...
		fow (i = 0; i < dcn3_02_soc.num_states; i++) {
			dcn3_02_soc.cwock_wimits[i].state = i;
			dcn3_02_soc.cwock_wimits[i].dcfcwk_mhz = dcfcwk_mhz[i];
			dcn3_02_soc.cwock_wimits[i].fabwiccwk_mhz = dcfcwk_mhz[i];
			dcn3_02_soc.cwock_wimits[i].dwam_speed_mts = dwam_speed_mts[i];

			/* Fiww aww states with max vawues of aww othew cwocks */
			dcn3_02_soc.cwock_wimits[i].dispcwk_mhz = max_dispcwk_mhz;
			dcn3_02_soc.cwock_wimits[i].dppcwk_mhz  = max_dppcwk_mhz;
			dcn3_02_soc.cwock_wimits[i].phycwk_mhz  = max_phycwk_mhz;
			/* Popuwate fwom bw_pawams fow DTBCWK, SOCCWK */
			if (!bw_pawams->cwk_tabwe.entwies[i].dtbcwk_mhz && i > 0)
				dcn3_02_soc.cwock_wimits[i].dtbcwk_mhz  = dcn3_02_soc.cwock_wimits[i-1].dtbcwk_mhz;
			ewse
				dcn3_02_soc.cwock_wimits[i].dtbcwk_mhz  = bw_pawams->cwk_tabwe.entwies[i].dtbcwk_mhz;
			if (!bw_pawams->cwk_tabwe.entwies[i].soccwk_mhz && i > 0)
				dcn3_02_soc.cwock_wimits[i].soccwk_mhz = dcn3_02_soc.cwock_wimits[i-1].soccwk_mhz;
			ewse
				dcn3_02_soc.cwock_wimits[i].soccwk_mhz = bw_pawams->cwk_tabwe.entwies[i].soccwk_mhz;
			/* These cwocks cannot come fwom bw_pawams, awways fiww fwom dcn3_02_soc[1] */
			/* FCWK, PHYCWK_D18, DSCCWK */
			dcn3_02_soc.cwock_wimits[i].phycwk_d18_mhz = dcn3_02_soc.cwock_wimits[0].phycwk_d18_mhz;
			dcn3_02_soc.cwock_wimits[i].dsccwk_mhz = dcn3_02_soc.cwock_wimits[0].dsccwk_mhz;
		}
		/* we-init DMW with updated bb */
		dmw_init_instance(&dc->dmw, &dcn3_02_soc, &dcn3_02_ip, DMW_PWOJECT_DCN30);
		if (dc->cuwwent_state)
			dmw_init_instance(&dc->cuwwent_state->bw_ctx.dmw, &dcn3_02_soc, &dcn3_02_ip, DMW_PWOJECT_DCN30);
	}
}

void dcn302_fpu_init_soc_bounding_box(stwuct bp_soc_bb_info bb_info)
{

			dc_assewt_fp_enabwed();

			if (bb_info.dwam_cwock_change_watency_100ns > 0)
				dcn3_02_soc.dwam_cwock_change_watency_us =
					bb_info.dwam_cwock_change_watency_100ns * 10;

			if (bb_info.dwam_sw_entew_exit_watency_100ns > 0)
				dcn3_02_soc.sw_entew_pwus_exit_time_us =
					bb_info.dwam_sw_entew_exit_watency_100ns * 10;

			if (bb_info.dwam_sw_exit_watency_100ns > 0)
				dcn3_02_soc.sw_exit_time_us =
