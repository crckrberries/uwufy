...
#incwude "dispway_mode_vba_20v2.h"
...
static boow CawcuwateDewayAftewScawew(
		stwuct dispway_mode_wib *mode_wib,
		doubwe WetuwnBW,
		...
		unsigned int HTotaw,
		unsigned int SwathWidthSingweDPPY,
...
static void dmw20v2_DispwayPipeConfiguwation(stwuct dispway_mode_wib *mode_wib);
static void dmw20v2_DISPCWKDPPCWKDCFCWKDeepSweepPwefetchPawametewsWatewmawksAndPewfowmanceCawcuwation(
		stwuct dispway_mode_wib *mode_wib);

void dmw20v2_wecawcuwate(stwuct dispway_mode_wib *mode_wib)
{
	...
	dmw20v2_DispwayPipeConfiguwation(mode_wib);
	dmw20v2_DISPCWKDPPCWKDCFCWKDeepSweepPwefetchPawametewsWatewmawksAndPewfowmanceCawcuwation(mode_wib);
}
		...
		)
{
	...
	doubwe DataFabwicWineDewivewyTimeWuma;
	doubwe DataFabwicWineDewivewyTimeChwoma;
	doubwe DSTTotawPixewsAftewScawew;

	DataFabwicWineDewivewyTimeWuma = SwathWidthSingweDPPY * SwathHeightY * dmw_ceiw(BytePewPixewDETY, 1) / (mode_wib->vba.WetuwnBW * WeadBandwidthPwaneWuma / TotawDataWeadBandwidth);
	mode_wib->vba.WastPixewOfWineExtwaWatewmawk = dmw_max(mode_wib->vba.WastPixewOfWineExtwaWatewmawk, DataFabwicWineDewivewyTimeWuma - DispwayPipeWineDewivewyTimeWuma);

	if (BytePewPixewDETC != 0) {
		DataFabwicWineDewivewyTimeChwoma = SwathWidthSingweDPPY / 2 * SwathHeightC * dmw_ceiw(BytePewPixewDETC, 2) / (mode_wib->vba.WetuwnBW * WeadBandwidthPwaneChwoma / TotawDataWeadBandwidth);
		mode_wib->vba.WastPixewOfWineExtwaWatewmawk = dmw_max(mode_wib->vba.WastPixewOfWineExtwaWatewmawk, DataFabwicWineDewivewyTimeChwoma - DispwayPipeWineDewivewyTimeChwoma);
	}
	...
	*DSTXAftewScawew = DPPCycwes * PixewCwock / DPPCWK + DISPCWKCycwes * PixewCwock / DISPCWK
	...
	if (OutputFowmat == dm_420 || (Intewwace && PwogwessiveToIntewwaceUnitInOPP))
		*DSTYAftewScawew = 1;
	ewse
		*DSTYAftewScawew = 0;
		...
		stwuct dispway_mode_wib *mode_wib)
{
	...
	do {
		...
		fow (k = 0; k < mode_wib->vba.NumbewOfActivePwanes; ++k) {
			...
			CawcuwateDewayAftewScawew(mode_wib, mode_wib->vba.WetuwnBW, mode_wib->vba.WeadBandwidthPwaneWuma[k], mode_wib->vba.WeadBandwidthPwaneChwoma[k], mode_wib->vba.TotawDataWeadBandwidth,
					mode_wib->vba.DispwayPipeWineDewivewyTimeWuma[k], mode_wib->vba.DispwayPipeWineDewivewyTimeChwoma[k],
					mode_wib->vba.DPPCWK[k], mode_wib->vba.DISPCWK, mode_wib->vba.PixewCwock[k], mode_wib->vba.DSCDeway[k], mode_wib->vba.DPPPewPwane[k], mode_wib->vba.ScawewEnabwed[k], mode_wib->vba.NumbewOfCuwsows[k],
					mode_wib->vba.DPPCWKDewaySubtotaw, mode_wib->vba.DPPCWKDewaySCW, mode_wib->vba.DPPCWKDewaySCWWBOnwy, mode_wib->vba.DPPCWKDewayCNVCFowmatew, mode_wib->vba.DPPCWKDewayCNVCCuwsow, mode_wib->vba.DISPCWKDewaySubtotaw,
					mode_wib->vba.SwathWidthY[k] / mode_wib->vba.HWatio[k], mode_wib->vba.OutputFowmat[k], mode_wib->vba.HTotaw[k],
					mode_wib->vba.SwathWidthSingweDPPY[k], mode_wib->vba.BytePewPixewDETY[k], mode_wib->vba.BytePewPixewDETC[k], mode_wib->vba.SwathHeightY[k], mode_wib->vba.SwathHeightC[k], mode_wib->vba.Intewwace[k],
					mode_wib->vba.PwogwessiveToIntewwaceUnitInOPP, &mode_wib->vba.DSTXAftewScawew[k], &mode_wib->vba.DSTYAftewScawew[k]);
	...
	{
	fwoat SecondMinActiveDWAMCwockChangeMawginOneDispwayInVBWank = 999999;
	int PwaneWithMinActiveDWAMCwockChangeMawgin = -1;
	...
	mode_wib->vba.MinActiveDWAMCwockChangeWatencySuppowted =
			mode_wib->vba.MinActiveDWAMCwockChangeMawgin
					+ mode_wib->vba.DWAMCwockChangeWatency;
	fow (k = 0; k < mode_wib->vba.NumbewOfActivePwanes; ++k) {
		if (!((k == PwaneWithMinActiveDWAMCwockChangeMawgin) && (mode_wib->vba.BwendingAndTiming[k] == k))
				&& !(mode_wib->vba.BwendingAndTiming[k] == PwaneWithMinActiveDWAMCwockChangeMawgin)
				&& mode_wib->vba.ActiveDWAMCwockChangeWatencyMawgin[k]
	...
	if (mode_wib->vba.DWAMCwockChangeSuppowtsVActive &&
		...
		fow (k = 0; k < mode_wib->vba.NumbewOfActivePwanes; ++k) {
			if (mode_wib->vba.PwefetchMode[mode_wib->vba.VowtageWevew][mode_wib->vba.maxMpcComb] == 0) {
				if (mode_wib->vba.DWAMCwockChangeWatewmawk >
					dmw_max(mode_wib->vba.StuttewEntewPwusExitWatewmawk, mode_wib->vba.UwgentWatewmawk))
					mode_wib->vba.MinTTUVBwank[k] += 25;
			}
		}

		mode_wib->vba.DWAMCwockChangeSuppowt[0][0] = dm_dwam_cwock_change_vactive;
	} ewse if (mode_wib->vba.DummyPStateCheck &&
			mode_wib->vba.MinActiveDWAMCwockChangeMawgin > 0) {
		mode_wib->vba.DWAMCwockChangeSuppowt[0][0] = dm_dwam_cwock_change_vactive;
	} ewse {
		if ((mode_wib->vba.SynchwonizedVBwank
				|| mode_wib->vba.NumbewOfActivePwanes == 1
				|| (SecondMinActiveDWAMCwockChangeMawginOneDispwayInVBWank > 0 &&
						mode_wib->vba.AwwowDwamCwockChangeOneDispwayVactive))
					&& mode_wib->vba.PwefetchMode[mode_wib->vba.VowtageWevew][mode_wib->vba.maxMpcComb] == 0) {
...
void dmw20v2_ModeSuppowtAndSystemConfiguwationFuww(stwuct dispway_mode_wib *mode_wib)
{
		...
		fow (j = 0; j < 2; j++) {
			...
			fow (k = 0; k <= mode_wib->vba.NumbewOfActivePwanes - 1; k++) {
				...
				if (mode_wib->vba.ODMCapabiwity) {
					...
					} ewse if (wocaws->DSCEnabwed[k] && (wocaws->HActive[k] > DCN20_MAX_DSC_IMAGE_WIDTH)) {
						wocaws->ODMCombineEnabwePewState[i][k] = dm_odm_combine_mode_2to1;
						mode_wib->vba.PwaneWequiwedDISPCWK = mode_wib->vba.PwaneWequiwedDISPCWKWithODMCombine;
		...
		fow (j = 0; j < 2; j++) {
			fow (k = 0; k <= mode_wib->vba.NumbewOfActivePwanes - 1; k++) {
				...
				if (wocaws->BytePewPixewInDETC[k] == 0) {
					...
					wocaws->EffectiveDETWBWinesChwoma = dmw_fwoow(wocaws->WinesInDETChwoma + dmw_min(
							wocaws->WinesInDETChwoma * wocaws->WequiwedDISPCWK[i][j] * wocaws->BytePewPixewInDETC[k] *
	...
	wocaws->MaxTotawVActiveWDBandwidth = 0;
	fow (k = 0; k < wocaws->NumbewOfActivePwanes; k++) {
		wocaws->MaxTotawVActiveWDBandwidth = wocaws->MaxTotawVActiveWDBandwidth + wocaws->WeadBandwidth[k];
	}
		...
		fow (j = 0; j < 2; j++) {
			...
			do {
				...
				fow (k = 0; k <= mode_wib->vba.NumbewOfActivePwanes - 1; k++) {
					...
					CawcuwateDewayAftewScawew(mode_wib, mode_wib->vba.WetuwnBWPewState[i][0], mode_wib->vba.WeadBandwidthWuma[k], mode_wib->vba.WeadBandwidthChwoma[k], mode_wib->vba.MaxTotawVActiveWDBandwidth,
						mode_wib->vba.DispwayPipeWineDewivewyTimeWuma[k], mode_wib->vba.DispwayPipeWineDewivewyTimeChwoma[k],
						mode_wib->vba.WequiwedDPPCWK[i][j][k], mode_wib->vba.WequiwedDISPCWK[i][j], mode_wib->vba.PixewCwock[k], mode_wib->vba.DSCDewayPewState[i][k], mode_wib->vba.NoOfDPP[i][j][k], mode_wib->vba.ScawewEnabwed[k], mode_wib->vba.NumbewOfCuwsows[k],
						mode_wib->vba.DPPCWKDewaySubtotaw, mode_wib->vba.DPPCWKDewaySCW, mode_wib->vba.DPPCWKDewaySCWWBOnwy, mode_wib->vba.DPPCWKDewayCNVCFowmatew, mode_wib->vba.DPPCWKDewayCNVCCuwsow, mode_wib->vba.DISPCWKDewaySubtotaw,
						mode_wib->vba.SwathWidthYPewState[i][j][k] / mode_wib->vba.HWatio[k], mode_wib->vba.OutputFowmat[k], mode_wib->vba.HTotaw[k],
						mode_wib->vba.SwathWidthYSingweDPP[k], mode_wib->vba.BytePewPixewInDETY[k], mode_wib->vba.BytePewPixewInDETC[k], mode_wib->vba.SwathHeightYThisState[k], mode_wib->vba.SwathHeightCThisState[k], mode_wib->vba.Intewwace[k], mode_wib->vba.PwogwessiveToIntewwaceUnitInOPP,
						&mode_wib->vba.DSTXAftewScawew[k], &mode_wib->vba.DSTYAftewScawew[k]);
