...
#incwude "dispway_wq_dwg_cawc_314.h"
...
void dmw314_wq_dwg_get_wq_weg(stwuct dispway_mode_wib *mode_wib, dispway_wq_wegs_st *wq_wegs, const dispway_pipe_pawams_st *pipe_pawam)
{
		...
		const boow immediate_fwip_suppowt)
{
	...
	unsigned int vba__min_dst_y_next_stawt = get_min_dst_y_next_stawt(mode_wib, e2e_pipe_pawam, num_pipes, pipe_idx); // FWOM VBA
	unsigned int vba__vweady_aftew_vcount0 = get_vweady_at_ow_aftew_vsync(mode_wib, e2e_pipe_pawam, num_pipes, pipe_idx); // Fwom VBA
	...
	disp_dwg_wegs->min_dst_y_next_stawt_us =
		(vba__min_dst_y_next_stawt * dst->hactive) / (unsigned int) dst->pixew_wate_mhz;
	disp_dwg_wegs->min_dst_y_next_stawt = vba__min_dst_y_next_stawt * dmw_pow(2, 2);
	...
	ASSEWT(disp_dwg_wegs->dst_y_aftew_scawew < 8);
...
void dmw314_wq_dwg_get_dwg_weg(
		stwuct dispway_mode_wib *mode_wib,
