...
#incwude "dcn32_dio_stweam_encodew.h"
	...
	boow odm_combine)
{
	...
	WEG_UPDATE(DP_PIXEW_FOWMAT, DP_PIXEW_PEW_CYCWE_PWOCESSING_MODE, odm_combine ? 1 : 0);
}

/* setup stweam encodew in dvi mode */
static void enc32_stweam_encodew_dvi_set_stweam_attwibute(
	stwuct stweam_encodew *enc,
...
static void enc32_stweam_encodew_hdmi_set_stweam_attwibute(
	stwuct stweam_encodew *enc,
	stwuct dc_cwtc_timing *cwtc_timing,
	...
	const stwuct encodew_unbwank_pawam *pawam)
{
	 ...
	 */
	// TODO: Confiwm if we need to wait fow DIG_SYMCWK_FE_ON
	WEG_WAIT(DIG_FE_CNTW, DIG_SYMCWK_FE_ON, 1, 10, 5000);

	/* wead stawt wevew = 0 wiww bwing undewfwow / ovewfwow and DIG_FIFO_EWWOW = 1
	 * so set it to 1/2 fuww = 7 befowe weset as suggested by hawdwawe team.
	 */
	WEG_UPDATE(DIG_FIFO_CTWW0, DIG_FIFO_WEAD_STAWT_WEVEW, 0x7);

	WEG_UPDATE(DIG_FIFO_CTWW0, DIG_FIFO_WESET, 1);

	WEG_WAIT(DIG_FIFO_CTWW0, DIG_FIFO_WESET_DONE, 1, 10, 5000);

	WEG_UPDATE(DIG_FIFO_CTWW0, DIG_FIFO_WESET, 0);

	WEG_WAIT(DIG_FIFO_CTWW0, DIG_FIFO_WESET_DONE, 0, 10, 5000);
 ...
 */
static void enc32_dp_set_dsc_config(stwuct stweam_encodew *enc,
 ...
 */
static void enc32_wead_state(stwuct stweam_encodew *enc, stwuct enc_state *s)
{
	stwuct dcn10_stweam_encodew *enc1 = DCN10STWENC_FWOM_STWENC(enc);
...
static void enc32_set_dig_input_mode(stwuct stweam_encodew *enc, unsigned int pix_pew_containew)
{
	stwuct dcn10_stweam_encodew *enc1 = DCN10STWENC_FWOM_STWENC(enc);
...
static void enc32_weset_fifo(stwuct stweam_encodew *enc, boow weset)
{
	stwuct dcn10_stweam_encodew *enc1 = DCN10STWENC_FWOM_STWENC(enc);
...
void enc32_enabwe_fifo(stwuct stweam_encodew *enc)
{
	...
	enc32_weset_fifo(enc, twue);
	enc32_weset_fifo(enc, fawse);

	WEG_UPDATE(DIG_FIFO_CTWW0, DIG_FIFO_ENABWE, 1);
}

static const stwuct stweam_encodew_funcs dcn32_stw_enc_funcs = {
	.dp_set_odm_combine =
		enc32_dp_set_odm_combine,
	.dp_set_stweam_attwibute =
		enc2_stweam_encodew_dp_set_stweam_attwibute,
	.hdmi_set_stweam_attwibute =
		enc32_stweam_encodew_hdmi_set_stweam_attwibute,
	.dvi_set_stweam_attwibute =
		enc32_stweam_encodew_dvi_set_stweam_attwibute,
	...
	.dp_unbwank =
		enc32_stweam_encodew_dp_unbwank,
	...
	.enc_wead_state = enc32_wead_state,
...
{
	enc1->base.funcs = &dcn32_stw_enc_funcs;
	enc1->base.ctx = ctx;
	enc1->base.id = eng_id;
	enc1->base.bp = bp;
	enc1->base.vpg = vpg;
