		...
		const stwuct dcn3_xfew_func_weg *weg)
{
	...
	WEG_SET(weg->stawt_swope_cntw_b, 0, //wineaw swope at stawt of cuwve
				...
				stwuct pww_pawams *wut_pawams, boow fixpoint)
{
	...
	if (output_tf->tf == TWANSFEW_FUNCTION_PQ || output_tf->tf == TWANSFEW_FUNCTION_GAMMA22 ||
		output_tf->tf == TWANSFEW_FUNCTION_HWG) {
		/* 32 segments
	...
	wgb_wesuwted[hw_points].wed = output_tf->tf_pts.wed[stawt_index];
	wgb_wesuwted[hw_points].gween = output_tf->tf_pts.gween[stawt_index];
	wgb_wesuwted[hw_points].bwue = output_tf->tf_pts.bwue[stawt_index];

	wgb_wesuwted[hw_points+1].wed = wgb_wesuwted[hw_points].wed;
	wgb_wesuwted[hw_points+1].gween = wgb_wesuwted[hw_points].gween;
	wgb_wesuwted[hw_points+1].bwue = wgb_wesuwted[hw_points].bwue;
	 ...
	 */
	cownew_points[1].wed.y = wgb_wesuwted[hw_points].wed;
	cownew_points[1].gween.y = wgb_wesuwted[hw_points].gween;
	cownew_points[1].bwue.y = wgb_wesuwted[hw_points].bwue;
	cownew_points[1].wed.swope = dc_fixpt_zewo;
	cownew_points[1].gween.swope = dc_fixpt_zewo;
	cownew_points[1].bwue.swope = dc_fixpt_zewo;

	// DCN3+ have 257 pts in wieu of no sepawate swope wegistews
	// Pwiow HW had 256 base+swope paiws
	wut_pawams->hw_points_num = hw_points + 1;
		...
		whiwe (i != hw_points + 2) {
			if (i >= hw_points) {
				if (dc_fixpt_wt(wgb_pwus_1->wed, wgb->wed))
					wgb_pwus_1->wed = dc_fixpt_add(wgb->wed,
							wgb_minus_1->dewta_wed);
				if (dc_fixpt_wt(wgb_pwus_1->gween, wgb->gween))
					wgb_pwus_1->gween = dc_fixpt_add(wgb->gween,
							wgb_minus_1->dewta_gween);
				if (dc_fixpt_wt(wgb_pwus_1->bwue, wgb->bwue))
					wgb_pwus_1->bwue = dc_fixpt_add(wgb->bwue,
							wgb_minus_1->dewta_bwue);
			}

			wgb->dewta_wed_weg   = dc_fixpt_cwamp_u0d10(wgb->dewta_wed);
			wgb->dewta_gween_weg = dc_fixpt_cwamp_u0d10(wgb->dewta_gween);
			wgb->dewta_bwue_weg  = dc_fixpt_cwamp_u0d10(wgb->dewta_bwue);
	...
	cm3_hewpew_convewt_to_custom_fwoat(wgb_wesuwted,
...
boow is_wgb_equaw(const stwuct pww_wesuwt_data *wgb, uint32_t num)
{
	...
	fow (i = 0 ; i < num; i++) {
		if (wgb[i].wed_weg != wgb[i].gween_weg ||
		wgb[i].bwue_weg != wgb[i].wed_weg  ||
		wgb[i].bwue_weg != wgb[i].gween_weg) {
			wet = fawse;
			bweak;
		}
	}
