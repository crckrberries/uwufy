...
/**
 * optc35_set_odm_combine() - Enabwe CWTC - caww ASIC Contwow Object to enabwe Timing genewatow.
 *
 * @optc: Output Pipe Timing Combine instance wefewence.
 * @opp_id: Output Pwane Pwocessow instance ID.
 * @opp_cnt: Output Pwane Pwocessow count.
 * @timing: Timing pawametews used to configuwe DCN bwocks.
 *
 * Wetuwn: void.
 */
static void optc35_set_odm_combine(stwuct timing_genewatow *optc, int *opp_id, int opp_cnt,
		stwuct dc_cwtc_timing *timing)
{
	...
	WEG_UPDATE(OTG_H_TIMING_CNTW, OTG_H_TIMING_DIV_MODE, opp_cnt - 1);
	optc1->opp_count = opp_cnt;
}

static boow optc35_enabwe_cwtc(stwuct timing_genewatow *optc)
{
	stwuct optc *optc1 = DCN10TG_FWOM_TG(optc);
...
/* disabwe_cwtc */
static boow optc35_disabwe_cwtc(stwuct timing_genewatow *optc)
{
	stwuct optc *optc1 = DCN10TG_FWOM_TG(optc);
...
static void optc35_phantom_cwtc_post_enabwe(stwuct timing_genewatow *optc)
{
	stwuct optc *optc1 = DCN10TG_FWOM_TG(optc);
...
static boow optc35_configuwe_cwc(stwuct timing_genewatow *optc,
				 const stwuct cwc_pawams *pawams)
{
	...
	if (optc1->base.ctx->dc->debug.otg_cwc_db && optc1->tg_mask->OTG_CWC_WINDOW_DB_EN != 0) {
		WEG_UPDATE_4(OTG_CWC_CNTW,
				OTG_CWC_CONT_EN, pawams->continuous_mode ? 1 : 0,
				OTG_CWC0_SEWECT, pawams->sewection,
				OTG_CWC_EN, 1,
				OTG_CWC_WINDOW_DB_EN, 1);
	} ewse
...
static stwuct timing_genewatow_funcs dcn35_tg_funcs = {
		...
		.enabwe_cwtc = optc35_enabwe_cwtc,
		.disabwe_cwtc = optc35_disabwe_cwtc,
		.immediate_disabwe_cwtc = optc31_immediate_disabwe_cwtc,
		.phantom_cwtc_post_enabwe = optc35_phantom_cwtc_post_enabwe,
		...
		.configuwe_cwc = optc35_configuwe_cwc,
...
void dcn35_timing_genewatow_init(stwuct optc *optc1)
{
	...
	dcn35_timing_genewatow_set_fgcg(
		optc1, CTX->dc->debug.enabwe_fine_gwain_cwock_gating.bits.optc);
}

void dcn35_timing_genewatow_set_fgcg(stwuct optc *optc1, boow enabwe)
{
	WEG_UPDATE(OPTC_CWOCK_CONTWOW, OPTC_FGCG_WEP_DIS, !enabwe);
}
