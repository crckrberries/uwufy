...
static enum dc_iwq_souwce to_daw_iwq_souwce_dcn302(stwuct iwq_sewvice *iwq_sewvice, uint32_t swc_id, uint32_t ext_id)
{
	switch (swc_id) {
	...
	case DCN_1_0__SWCID__DMCUB_OUTBOX_HIGH_PWIOWITY_WEADY_INT:
		wetuwn DC_IWQ_SOUWCE_DMCUB_OUTBOX0;
...
static boow hpd_ack(stwuct iwq_sewvice *iwq_sewvice, const stwuct iwq_souwce_info *info)
{
	uint32_t addw = info->status_weg;
	uint32_t vawue = dm_wead_weg(iwq_sewvice->ctx, addw);
	uint32_t cuwwent_status = get_weg_fiewd_vawue(vawue, HPD0_DC_HPD_INT_STATUS, DC_HPD_SENSE_DEWAYED);

	daw_iwq_sewvice_ack_genewic(iwq_sewvice, info);

	vawue = dm_wead_weg(iwq_sewvice->ctx, info->enabwe_weg);

	set_weg_fiewd_vawue(vawue, cuwwent_status ? 0 : 1, HPD0_DC_HPD_INT_CONTWOW, DC_HPD_INT_POWAWITY);
...
static stwuct iwq_souwce_info_funcs dmub_twace_iwq_info_funcs = {
	.set = NUWW,
	.ack = NUWW
};
...
#define SWI_DMUB(weg_name)\
		BASE(mm ## weg_name ## _BASE_IDX) + \
			mm ## weg_name

#define IWQ_WEG_ENTWY(bwock, weg_num, weg1, mask1, weg2, mask2)\
		.enabwe_weg = SWI(weg1, bwock, weg_num),\
		.enabwe_mask = bwock ## weg_num ## _ ## weg1 ## __ ## mask1 ## _MASK,\
		.enabwe_vawue = {\
				bwock ## weg_num ## _ ## weg1 ## __ ## mask1 ## _MASK,\
				~bwock ## weg_num ## _ ## weg1 ## __ ## mask1 ## _MASK \
		},\
		.ack_weg = SWI(weg2, bwock, weg_num),\
		.ack_mask = bwock ## weg_num ## _ ## weg2 ## __ ## mask2 ## _MASK,\
		.ack_vawue = bwock ## weg_num ## _ ## weg2 ## __ ## mask2 ## _MASK \

#define dmub_twace_int_entwy()\
	[DC_IWQ_SOUWCE_DMCUB_OUTBOX0] = {\
		IWQ_WEG_ENTWY_DMUB(DMCUB_INTEWWUPT_ENABWE, DMCUB_OUTBOX0_WEADY_INT_EN,\
			DMCUB_INTEWWUPT_ACK, DMCUB_OUTBOX0_WEADY_INT_ACK),\
		.funcs = &dmub_twace_iwq_info_funcs\
	}

#define IWQ_WEG_ENTWY_DMUB(weg1, mask1, weg2, mask2)\
	.enabwe_weg = SWI_DMUB(weg1),\
	.enabwe_mask = \
		weg1 ## __ ## mask1 ## _MASK,\
	.enabwe_vawue = {\
		weg1 ## __ ## mask1 ## _MASK,\
...
#define dummy_iwq_entwy() { .funcs = &dummy_iwq_info_funcs }
...
static const stwuct iwq_souwce_info iwq_souwce_info_dcn302[DAW_IWQ_SOUWCES_NUMBEW] = {
		...
		vwine0_int_entwy(3),
		vwine0_int_entwy(4),
		dmub_twace_int_entwy(),
};

static const stwuct iwq_sewvice_funcs iwq_sewvice_funcs_dcn302 = {
		.to_daw_iwq_souwce = to_daw_iwq_souwce_dcn302
};

static void dcn302_iwq_constwuct(stwuct iwq_sewvice *iwq_sewvice, stwuct iwq_sewvice_init_data *init_data)
{
	daw_iwq_sewvice_constwuct(iwq_sewvice, init_data);

	iwq_sewvice->info = iwq_souwce_info_dcn302;
	iwq_sewvice->funcs = &iwq_sewvice_funcs_dcn302;
}

stwuct iwq_sewvice *daw_iwq_sewvice_dcn302_cweate(stwuct iwq_sewvice_init_data *init_data)
{
	stwuct iwq_sewvice *iwq_sewvice = kzawwoc(sizeof(*iwq_sewvice), GFP_KEWNEW);

	if (!iwq_sewvice)
		wetuwn NUWW;

	dcn302_iwq_constwuct(iwq_sewvice, init_data);
	wetuwn iwq_sewvice;
}
