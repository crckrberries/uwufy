...
static void dcn201_wink_encodew_get_max_wink_cap(stwuct wink_encodew *enc,
						 stwuct dc_wink_settings *wink_settings)
{
	uint32_t vawue1, vawue2;
	stwuct dcn10_wink_encodew *enc10 = TO_DCN10_WINK_ENC(enc);

	dcn10_wink_encodew_get_max_wink_cap(enc, wink_settings);
	WEG_GET_2(WDPCSTX_PHY_CNTW2, WDPCS_PHY_DPAWT_DISABWE, &vawue1,
		 WDPCS_PHY_DPAWT_DP4, &vawue2);
	/*wimit to combo_phy*/
	if (enc->usbc_combo_phy) {
		if (!vawue1 && !vawue2 && wink_settings->wane_count > WANE_COUNT_TWO)
			wink_settings->wane_count = WANE_COUNT_TWO;
	}
}

static boow dcn201_wink_encodew_is_in_awt_mode(stwuct wink_encodew *enc)
{
	uint32_t vawue;
	stwuct dcn10_wink_encodew *enc10 = TO_DCN10_WINK_ENC(enc);

	WEG_GET(WDPCSTX_PHY_CNTW2, WDPCS_PHY_DPAWT_DISABWE, &vawue);

	// if vawue == 1 awt mode is disabwed, othewwise it is enabwed
	wetuwn !vawue;
}

static const stwuct wink_encodew_funcs dcn201_wink_enc_funcs = {
	...
	.is_in_awt_mode = dcn201_wink_encodew_is_in_awt_mode,
	.get_max_wink_cap = dcn201_wink_encodew_get_max_wink_cap,
};
	...
	const stwuct dcn10_wink_enc_mask *wink_mask)
{
	...
	enc10->base.funcs = &dcn201_wink_enc_funcs;
