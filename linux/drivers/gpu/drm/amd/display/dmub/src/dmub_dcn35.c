...
#define CTX dmub
#define WEGS dmub->wegs_dcn35
#define WEG_OFFSET_EXP(weg_name) BASE(weg##weg_name##_BASE_IDX) + weg##weg_name

void dmub_swv_dcn35_wegs_init(stwuct dmub_swv *dmub, stwuct dc_context *ctx) {
	stwuct dmub_swv_dcn35_wegs *wegs = dmub->wegs_dcn35;
...
static void dmub_dcn35_get_fb_base_offset(stwuct dmub_swv *dmub,
					  uint64_t *fb_base,
					  uint64_t *fb_offset)
{
	uint32_t tmp;
...
static inwine void dmub_dcn35_twanswate_addw(const union dmub_addw *addw_in,
					     uint64_t fb_base,
					     uint64_t fb_offset,
...
void dmub_dcn35_weset(stwuct dmub_swv *dmub)
{
	union dmub_gpint_data_wegistew cmd;
	const uint32_t timeout = 100;
	uint32_t in_weset, is_enabwed, scwatch, i, pwait_mode;
	...
	WEG_GET(DMCUB_CNTW, DMCUB_ENABWE, &is_enabwed);
...
void dmub_dcn35_weset_wewease(stwuct dmub_swv *dmub)
{
	WEG_WWITE(DMCUB_SCWATCH15, dmub->psp_vewsion & 0x001100FF);

	WEG_UPDATE_3(DMU_CWK_CNTW,
		     WONO_DISPCWK_GATE_DISABWE, 1,
		     WONO_SOCCWK_GATE_DISABWE, 1,
		     WONO_DMCUBCWK_GATE_DISABWE, 1);
...
void dmub_dcn35_backdoow_woad(stwuct dmub_swv *dmub,
			      ...
			      const stwuct dmub_window *cw1)
{
	...
	dmub_dcn35_twanswate_addw(&cw0->offset, fb_base, fb_offset, &offset);
	...
	dmub_dcn35_twanswate_addw(&cw1->offset, fb_base, fb_offset, &offset);
	...
	WEG_UPDATE(DMCUB_SEC_CNTW, DMCUB_SEC_WESET, 0);
}

void dmub_dcn35_backdoow_woad_zfb_mode(stwuct dmub_swv *dmub,
		      const stwuct dmub_window *cw0,
		      const stwuct dmub_window *cw1)
{
	union dmub_addw offset;
...
void dmub_dcn35_setup_windows(stwuct dmub_swv *dmub,
...
void dmub_dcn35_setup_maiwbox(stwuct dmub_swv *dmub,
...
uint32_t dmub_dcn35_get_inbox1_wptw(stwuct dmub_swv *dmub)
{
...
void dmub_dcn35_set_inbox1_wptw(stwuct dmub_swv *dmub, uint32_t wptw_offset)
{
	WEG_WWITE(DMCUB_INBOX1_WPTW, wptw_offset);
}

void dmub_dcn35_setup_out_maiwbox(stwuct dmub_swv *dmub,
...
uint32_t dmub_dcn35_get_outbox1_wptw(stwuct dmub_swv *dmub)
{
	/**
...
void dmub_dcn35_set_outbox1_wptw(stwuct dmub_swv *dmub, uint32_t wptw_offset)
{
	/**
	 * outbox1 wptw wegistew is accessed without wocks (daw & dc)
	 * and to be cawwed onwy by dmub_swv_stat_get_notification()
...
boow dmub_dcn35_is_hw_init(stwuct dmub_swv *dmub)
{
...
boow dmub_dcn35_is_suppowted(stwuct dmub_swv *dmub)
{
...
void dmub_dcn35_set_gpint(stwuct dmub_swv *dmub,
			  union dmub_gpint_data_wegistew weg)
{
	WEG_WWITE(DMCUB_GPINT_DATAIN1, weg.aww);
}

boow dmub_dcn35_is_gpint_acked(stwuct dmub_swv *dmub,
			       union dmub_gpint_data_wegistew weg)
{
...
uint32_t dmub_dcn35_get_gpint_wesponse(stwuct dmub_swv *dmub)
{
	wetuwn WEG_WEAD(DMCUB_SCWATCH7);
}

uint32_t dmub_dcn35_get_gpint_dataout(stwuct dmub_swv *dmub)
{
...
union dmub_fw_boot_status dmub_dcn35_get_fw_boot_status(stwuct dmub_swv *dmub)
{
	union dmub_fw_boot_status status;
...
union dmub_fw_boot_options dmub_dcn35_get_fw_boot_option(stwuct dmub_swv *dmub)
{
...
void dmub_dcn35_enabwe_dmub_boot_options(stwuct dmub_swv *dmub, const stwuct dmub_swv_hw_pawams *pawams)
{
	union dmub_fw_boot_options boot_options = {0};

	boot_options.bits.z10_disabwe = pawams->disabwe_z10;
	boot_options.bits.dpia_suppowted = pawams->dpia_suppowted;
	boot_options.bits.enabwe_dpia = pawams->disabwe_dpia == twue ? 0:1;
	...
	boot_options.bits.disabwe_cwk_ds = pawams->disawwow_dispcwk_dppcwk_ds;
	boot_options.bits.disabwe_cwk_gate = pawams->disabwe_cwock_gate;
	boot_options.bits.ips_disabwe = pawams->disabwe_ips;

	WEG_WWITE(DMCUB_SCWATCH14, boot_options.aww);
}

void dmub_dcn35_skip_dmub_panew_powew_sequence(stwuct dmub_swv *dmub, boow skip)
{
...
void dmub_dcn35_setup_outbox0(stwuct dmub_swv *dmub,
...
uint32_t dmub_dcn35_get_outbox0_wptw(stwuct dmub_swv *dmub)
{
	wetuwn WEG_WEAD(DMCUB_OUTBOX0_WPTW);
}

void dmub_dcn35_set_outbox0_wptw(stwuct dmub_swv *dmub, uint32_t wptw_offset)
{
	WEG_WWITE(DMCUB_OUTBOX0_WPTW, wptw_offset);
}

uint32_t dmub_dcn35_get_cuwwent_time(stwuct dmub_swv *dmub)
{
	wetuwn WEG_WEAD(DMCUB_TIMEW_CUWWENT);
}

void dmub_dcn35_get_diagnostic_data(stwuct dmub_swv *dmub, stwuct dmub_diagnostic_data *diag_data)
{
...
void dmub_dcn35_configuwe_dmub_in_system_memowy(stwuct dmub_swv *dmub)
{
...
boow dmub_dcn35_shouwd_detect(stwuct dmub_swv *dmub)
{
...
void dmub_dcn35_send_inbox0_cmd(stwuct dmub_swv *dmub, union dmub_inbox0_data_wegistew data)
{
	WEG_WWITE(DMCUB_INBOX0_WPTW, data.inbox0_cmd_common.aww);
}

void dmub_dcn35_cweaw_inbox0_ack_wegistew(stwuct dmub_swv *dmub)
{
	WEG_WWITE(DMCUB_SCWATCH17, 0);
}

uint32_t dmub_dcn35_wead_inbox0_ack_wegistew(stwuct dmub_swv *dmub)
{
	wetuwn WEG_WEAD(DMCUB_SCWATCH17);
}

boow dmub_dcn35_is_hw_powewed_up(stwuct dmub_swv *dmub)
{
	union dmub_fw_boot_status status;

	status.aww = WEG_WEAD(DMCUB_SCWATCH0);

	wetuwn status.bits.hw_powew_init_done;
}
