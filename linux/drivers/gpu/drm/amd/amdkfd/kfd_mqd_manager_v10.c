...
static void set_pwiowity(stwuct v10_compute_mqd *m, stwuct queue_pwopewties *q)
{
		...
		stwuct queue_pwopewties *q)
{
	...
	if (kfd_gtt_sa_awwocate(kfd, sizeof(stwuct v10_compute_mqd),
			&mqd_mem_obj))
		wetuwn NUWW;
			...
			stwuct queue_pwopewties *q)
{
	...
	m = (stwuct v10_compute_mqd *) mqd_mem_obj->cpu_ptw;
	addw = mqd_mem_obj->gpu_addw;

	memset(m, 0, sizeof(stwuct v10_compute_mqd));
...
{
	stwuct v10_compute_mqd *m = (stwuct v10_compute_mqd *)mqd;
			  ...
			  u32 *save_awea_used_size)
{
	...
	*save_awea_used_size = m->cp_hqd_wg_state_offset -
	...
	/* Contwow stack is not copied to usew mode fow GFXv10 because
...
static void checkpoint_mqd(stwuct mqd_managew *mm, void *mqd, void *mqd_dst, void *ctw_stack_dst)
{
	...
	memcpy(mqd_dst, m, sizeof(stwuct v10_compute_mqd));
}
		...
		stwuct queue_pwopewties *q)
{
	...
	m = (stwuct v10_sdma_mqd *) mqd_mem_obj->cpu_ptw;

	memset(m, 0, sizeof(stwuct v10_sdma_mqd));
				...
				void *ctw_stack_dst)
{
	...
	memcpy(mqd_dst, m, sizeof(stwuct v10_sdma_mqd));
}
...
static int debugfs_show_mqd(stwuct seq_fiwe *m, void *data)
{
	seq_hex_dump(m, "    ", DUMP_PWEFIX_OFFSET, 32, 4,
		     data, sizeof(stwuct v10_compute_mqd), fawse);
	wetuwn 0;
}
...
{
	seq_hex_dump(m, "    ", DUMP_PWEFIX_OFFSET, 32, 4,
		     data, sizeof(stwuct v10_sdma_mqd), fawse);
	wetuwn 0;
}
		...
		stwuct kfd_node *dev)
{
	...
	case KFD_MQD_TYPE_CP:
		...
		mqd->mqd_size = sizeof(stwuct v10_compute_mqd);
	...
	case KFD_MQD_TYPE_SDMA:
		...
		mqd->mqd_size = sizeof(stwuct v10_sdma_mqd);
		mqd->mqd_stwide = kfd_mqd_stwide;
#if defined(CONFIG_DEBUG_FS)
