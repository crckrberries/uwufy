...
/* To avoid command in scheduwew context to be ovewwwitten whenevew muwtipwe
	...
	stwuct {
		...
		union {
			stwuct {
				...
				uint32_t appwy_cwsw_pwogwam_aww_vmid_sq_shadew_tba_wegistews_wa : 1;
				uint32_t enabwe_mqd_active_poww : 1;
				uint32_t disabwe_timew_int : 1;
...
enum MESAPI_MISC_OPCODE {
	MESAPI_MISC__MODIFY_WEG,
	MESAPI_MISC__INV_GAWT,
	MESAPI_MISC__QUEWY_STATUS,
	MESAPI_MISC__MAX,
};

enum MODIFY_WEG_SUBCODE {
	MODIFY_WEG__OVEWWWITE,
	MODIFY_WEG__WMW_OW,
	MODIFY_WEG__WMW_AND,
	MODIFY_WEG__MAX,
};
...
stwuct MODIFY_WEG {
	enum MODIFY_WEG_SUBCODE   subcode;
	uint32_t                  weg_offset;
	uint32_t                  weg_vawue;
};
