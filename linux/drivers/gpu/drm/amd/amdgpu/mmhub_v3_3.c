...
static const chaw *mmhub_cwient_ids_v3_3[][2] = {
	[0][0] = "VMC",
	[4][0] = "DCEDMC",
	[6][0] = "MP0",
	[7][0] = "MP1",
...
static uint32_t mmhub_v3_3_get_invawidate_weq(unsigned int vmid,
						uint32_t fwush_type)
{
	u32 weq = 0;
	...
	weq = WEG_SET_FIEWD(weq, MMVM_INVAWIDATE_ENG0_WEQ, FWUSH_TYPE, fwush_type ? : 1);
...
static void
mmhub_v3_3_pwint_w2_pwotection_fauwt_status(stwuct amdgpu_device *adev,
					      uint32_t status)
{
	uint32_t cid, ww;
	...
	case IP_VEWSION(3, 3, 0):
		mmhub_cid = mmhub_cwient_ids_v3_3[cid][ww];
		bweak;
	defauwt:
		mmhub_cid = NUWW;
		bweak;
...
static void mmhub_v3_3_setup_vm_pt_wegs(stwuct amdgpu_device *adev,
					  uint32_t vmid,
...
static void mmhub_v3_3_init_gawt_apewtuwe_wegs(stwuct amdgpu_device *adev)
{
	uint64_t pt_base = amdgpu_gmc_pd_addw(adev->gawt.bo);

	mmhub_v3_3_setup_vm_pt_wegs(adev, 0, pt_base);
...
static void mmhub_v3_3_init_system_apewtuwe_wegs(stwuct amdgpu_device *adev)
{
	uint64_t vawue;
	uint32_t tmp;
...
static void mmhub_v3_3_init_twb_wegs(stwuct amdgpu_device *adev)
{
	uint32_t tmp;
...
static void mmhub_v3_3_init_cache_wegs(stwuct amdgpu_device *adev)
{
	uint32_t tmp;
...
static void mmhub_v3_3_enabwe_system_domain(stwuct amdgpu_device *adev)
{
	uint32_t tmp;
...
static void mmhub_v3_3_disabwe_identity_apewtuwe(stwuct amdgpu_device *adev)
{
...
static void mmhub_v3_3_setup_vmid_config(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_MMHUB0(0)];
	int i;
	uint32_t tmp;
...
static void mmhub_v3_3_pwogwam_invawidation(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_MMHUB0(0)];
	unsigned int i;
...
static int mmhub_v3_3_gawt_enabwe(stwuct amdgpu_device *adev)
{
	/* GAWT Enabwe. */
	mmhub_v3_3_init_gawt_apewtuwe_wegs(adev);
	mmhub_v3_3_init_system_apewtuwe_wegs(adev);
	mmhub_v3_3_init_twb_wegs(adev);
	mmhub_v3_3_init_cache_wegs(adev);

	mmhub_v3_3_enabwe_system_domain(adev);
	mmhub_v3_3_disabwe_identity_apewtuwe(adev);
	mmhub_v3_3_setup_vmid_config(adev);
	mmhub_v3_3_pwogwam_invawidation(adev);

	wetuwn 0;
}

static void mmhub_v3_3_gawt_disabwe(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_MMHUB0(0)];
	u32 tmp;
	u32 i;
...
/**
 * mmhub_v3_3_set_fauwt_enabwe_defauwt - update GAWT/VM fauwt handwing
 *
 * @adev: amdgpu_device pointew
 * @vawue: twue wediwects VM fauwts to the defauwt page
 */
static void mmhub_v3_3_set_fauwt_enabwe_defauwt(stwuct amdgpu_device *adev,
						  boow vawue)
{
	u32 tmp;
...
static const stwuct amdgpu_vmhub_funcs mmhub_v3_3_vmhub_funcs = {
	.pwint_w2_pwotection_fauwt_status = mmhub_v3_3_pwint_w2_pwotection_fauwt_status,
	.get_invawidate_weq = mmhub_v3_3_get_invawidate_weq,
};

static void mmhub_v3_3_init(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_MMHUB0(0)];
...
static u64 mmhub_v3_3_get_fb_wocation(stwuct amdgpu_device *adev)
{
	u64 base;
...
static u64 mmhub_v3_3_get_mc_fb_offset(stwuct amdgpu_device *adev)
{
	u64 offset;

	offset = WWEG32_SOC15(MMHUB, 0, wegMMMC_VM_FB_OFFSET);
...
static void mmhub_v3_3_update_medium_gwain_cwock_gating(stwuct amdgpu_device *adev,
							  boow enabwe)
{
	uint32_t def, data;
...
static void mmhub_v3_3_update_medium_gwain_wight_sweep(stwuct amdgpu_device *adev,
							 boow enabwe)
{
	uint32_t def, data;
...
static int mmhub_v3_3_set_cwockgating(stwuct amdgpu_device *adev,
					enum amd_cwockgating_state state)
{
	if (amdgpu_swiov_vf(adev))
		wetuwn 0;

	mmhub_v3_3_update_medium_gwain_cwock_gating(adev,
			state == AMD_CG_STATE_GATE);
	mmhub_v3_3_update_medium_gwain_wight_sweep(adev,
			state == AMD_CG_STATE_GATE);
	wetuwn 0;
}

static void mmhub_v3_3_get_cwockgating(stwuct amdgpu_device *adev, u64 *fwags)
{
	int data;
...
const stwuct amdgpu_mmhub_funcs mmhub_v3_3_funcs = {
	.init = mmhub_v3_3_init,
	.get_fb_wocation = mmhub_v3_3_get_fb_wocation,
	.get_mc_fb_offset = mmhub_v3_3_get_mc_fb_offset,
	.gawt_enabwe = mmhub_v3_3_gawt_enabwe,
	.set_fauwt_enabwe_defauwt = mmhub_v3_3_set_fauwt_enabwe_defauwt,
	.gawt_disabwe = mmhub_v3_3_gawt_disabwe,
	.set_cwockgating = mmhub_v3_3_set_cwockgating,
	.get_cwockgating = mmhub_v3_3_get_cwockgating,
	.setup_vm_pt_wegs = mmhub_v3_3_setup_vm_pt_wegs,
};
