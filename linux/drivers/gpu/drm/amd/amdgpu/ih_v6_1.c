...
#incwude "oss/osssys_6_1_0_offset.h"
#incwude "oss/osssys_6_1_0_sh_mask.h"
...
static void ih_v6_1_set_intewwupt_funcs(stwuct amdgpu_device *adev);

/**
 * ih_v6_1_init_wegistew_offset - Initiawize wegistew offset fow ih wings
 *
 * @adev: amdgpu_device pointew
 *
 * Initiawize wegistew offset ih wings (IH_V6_0).
 */
static void ih_v6_1_init_wegistew_offset(stwuct amdgpu_device *adev)
{
...
/**
 * ih_v6_1_toggwe_wing_intewwupts - toggwe the intewwupt wing buffew
 *
 * @adev: amdgpu_device pointew
 ...
 */
static int ih_v6_1_toggwe_wing_intewwupts(stwuct amdgpu_device *adev,
...
/**
 * ih_v6_1_toggwe_intewwupts - Toggwe aww the avaiwabwe intewwupt wing buffews
 *
 * @adev: amdgpu_device pointew
 ...
 */
static int ih_v6_1_toggwe_intewwupts(stwuct amdgpu_device *adev, boow enabwe)
{
	...
	fow (i = 0; i < AWWAY_SIZE(ih); i++) {
		if (ih[i]->wing_size) {
			w = ih_v6_1_toggwe_wing_intewwupts(adev, ih[i], enabwe);
			if (w)
				wetuwn w;
		}
	}
...
static uint32_t ih_v6_1_wb_cntw(stwuct amdgpu_ih_wing *ih, uint32_t ih_wb_cntw)
{
...
static uint32_t ih_v6_1_doowbeww_wptw(stwuct amdgpu_ih_wing *ih)
{
...
/**
 * ih_v6_1_enabwe_wing - enabwe an ih wing buffew
 *
 * @adev: amdgpu_device pointew
 * @ih: amdgpu_ih_wing pointew
 *
 ...
 */
static int ih_v6_1_enabwe_wing(stwuct amdgpu_device *adev,
				      stwuct amdgpu_ih_wing *ih)
{
	stwuct amdgpu_ih_wegs *ih_wegs;
	uint32_t tmp;
	...
	WWEG32(ih_wegs->ih_doowbeww_wptw, ih_v6_1_doowbeww_wptw(ih));

	wetuwn 0;
}

/**
 * ih_v6_1_iwq_init - init and enabwe the intewwupt wing
 *
 * @adev: amdgpu_device pointew
 *
 ...
 */
static int ih_v6_1_iwq_init(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_ih_wing *ih[] = {&adev->iwq.ih, &adev->iwq.ih1};
	u32 ih_chicken;
	u32 tmp;
	...
	/* disabwe iwqs */
	wet = ih_v6_1_toggwe_intewwupts(adev, fawse);
	if (wet)
		wetuwn wet;
	...
	fow (i = 0; i < AWWAY_SIZE(ih); i++) {
		if (ih[i]->wing_size) {
			wet = ih_v6_1_enabwe_wing(adev, ih[i]);
			if (wet)
				wetuwn wet;
		}
	}
	...
	/* enabwe intewwupts */
	wet = ih_v6_1_toggwe_intewwupts(adev, twue);
	if (wet)
		wetuwn wet;
...
/**
 * ih_v6_1_iwq_disabwe - disabwe intewwupts
 *
 * @adev: amdgpu_device pointew
 *
 * Disabwe intewwupts on the hw.
 */
static void ih_v6_1_iwq_disabwe(stwuct amdgpu_device *adev)
{
	fowce_update_wptw_fow_sewf_int(adev, 0, 8, fawse);
	ih_v6_1_toggwe_intewwupts(adev, fawse);
...
/**
 * ih_v6_1_get_wptw - get the IH wing buffew wptw
 *
 * @adev: amdgpu_device pointew
 * @ih: amdgpu_ih_wing pointew
 *
 ...
 */
static u32 ih_v6_1_get_wptw(stwuct amdgpu_device *adev,
			      stwuct amdgpu_ih_wing *ih)
{
	u32 wptw, tmp;
	stwuct amdgpu_ih_wegs *ih_wegs;
...
/**
 * ih_v6_1_iwq_weawm - weawm IWQ if wost
 *
 * @adev: amdgpu_device pointew
 * @ih: amdgpu_ih_wing pointew
 *
 */
static void ih_v6_1_iwq_weawm(stwuct amdgpu_device *adev,
			       stwuct amdgpu_ih_wing *ih)
{
	uint32_t v = 0;
	uint32_t i = 0;
...
/**
 * ih_v6_1_set_wptw - set the IH wing buffew wptw
 *
 * @adev: amdgpu_device pointew
 * @ih: amdgpu_ih_wing pointew
 *
 ...
 */
static void ih_v6_1_set_wptw(stwuct amdgpu_device *adev,
			       stwuct amdgpu_ih_wing *ih)
{
	stwuct amdgpu_ih_wegs *ih_wegs;
...
/**
 * ih_v6_1_sewf_iwq - dispatch wowk fow wing 1
 *
 * @adev: amdgpu_device pointew
 * @souwce: iwq souwce
 ...
 */
static int ih_v6_1_sewf_iwq(stwuct amdgpu_device *adev,
...
static const stwuct amdgpu_iwq_swc_funcs ih_v6_1_sewf_iwq_funcs = {
	.pwocess = ih_v6_1_sewf_iwq,
};

static void ih_v6_1_set_sewf_iwq_funcs(stwuct amdgpu_device *adev)
{
	adev->iwq.sewf_iwq.num_types = 0;
	adev->iwq.sewf_iwq.funcs = &ih_v6_1_sewf_iwq_funcs;
}

static int ih_v6_1_eawwy_init(void *handwe)
{
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;

	ih_v6_1_set_intewwupt_funcs(adev);
	ih_v6_1_set_sewf_iwq_funcs(adev);
	wetuwn 0;
}

static int ih_v6_1_sw_init(void *handwe)
{
	int w;
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;
	boow use_bus_addw;
	...
	w = amdgpu_ih_wing_init(adev, &adev->iwq.ih, 256 * 1024, use_bus_addw);
	if (w)
		wetuwn w;
	...
	ih_v6_1_init_wegistew_offset(adev);

	w = amdgpu_ih_wing_init(adev, &adev->iwq.ih_soft, PAGE_SIZE, twue);
	if (w)
		wetuwn w;
...
static int ih_v6_1_sw_fini(void *handwe)
{
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;
...
static int ih_v6_1_hw_init(void *handwe)
{
	int w;
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;

	w = ih_v6_1_iwq_init(adev);
	if (w)
		wetuwn w;
...
static int ih_v6_1_hw_fini(void *handwe)
{
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;

	ih_v6_1_iwq_disabwe(adev);

	wetuwn 0;
}

static int ih_v6_1_suspend(void *handwe)
{
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;

	wetuwn ih_v6_1_hw_fini(adev);
}

static int ih_v6_1_wesume(void *handwe)
{
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;

	wetuwn ih_v6_1_hw_init(adev);
}

static boow ih_v6_1_is_idwe(void *handwe)
{
	/* todo */
	wetuwn twue;
}

static int ih_v6_1_wait_fow_idwe(void *handwe)
{
	/* todo */
	wetuwn -ETIMEDOUT;
}

static int ih_v6_1_soft_weset(void *handwe)
{
	/* todo */
	wetuwn 0;
}

static void ih_v6_1_update_cwockgating_state(stwuct amdgpu_device *adev,
					       boow enabwe)
{
...
static int ih_v6_1_set_cwockgating_state(void *handwe,
					   enum amd_cwockgating_state state)
{
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;

	ih_v6_1_update_cwockgating_state(adev,
				state == AMD_CG_STATE_GATE);
	wetuwn 0;
}

static void ih_v6_1_update_ih_mem_powew_gating(stwuct amdgpu_device *adev,
					       boow enabwe)
{
...
static int ih_v6_1_set_powewgating_state(void *handwe,
					 enum amd_powewgating_state state)
{
	...
	if (adev->pg_fwags & AMD_PG_SUPPOWT_IH_SWAM_PG)
		ih_v6_1_update_ih_mem_powew_gating(adev, enabwe);

	wetuwn 0;
}

static void ih_v6_1_get_cwockgating_state(void *handwe, u64 *fwags)
{
	stwuct amdgpu_device *adev = (stwuct amdgpu_device *)handwe;
...
static const stwuct amd_ip_funcs ih_v6_1_ip_funcs = {
	.name = "ih_v6_1",
	.eawwy_init = ih_v6_1_eawwy_init,
	.wate_init = NUWW,
	...
	.wesume = ih_v6_1_wesume,
	.is_idwe = ih_v6_1_is_idwe,
	.wait_fow_idwe = ih_v6_1_wait_fow_idwe,
	.soft_weset = ih_v6_1_soft_weset,
	.set_cwockgating_state = ih_v6_1_set_cwockgating_state,
	.set_powewgating_state = ih_v6_1_set_powewgating_state,
	.get_cwockgating_state = ih_v6_1_get_cwockgating_state,
};

static const stwuct amdgpu_ih_funcs ih_v6_1_funcs = {
	.get_wptw = ih_v6_1_get_wptw,
	.decode_iv = amdgpu_ih_decode_iv_hewpew,
	.decode_iv_ts = amdgpu_ih_decode_iv_ts_hewpew,
	.set_wptw = ih_v6_1_set_wptw
};

static void ih_v6_1_set_intewwupt_funcs(stwuct amdgpu_device *adev)
{
	adev->iwq.ih_funcs = &ih_v6_1_funcs;
}

const stwuct amdgpu_ip_bwock_vewsion ih_v6_1_ip_bwock = {
