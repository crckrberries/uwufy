...
static uint32_t gfxhub_v2_0_get_invawidate_weq(unsigned int vmid,
					       uint32_t fwush_type)
{
	u32 weq = 0;
...
static void
gfxhub_v2_0_pwint_w2_pwotection_fauwt_status(stwuct amdgpu_device *adev,
					     uint32_t status)
{
...
static u64 gfxhub_v2_0_get_fb_wocation(stwuct amdgpu_device *adev)
{
...
static u64 gfxhub_v2_0_get_mc_fb_offset(stwuct amdgpu_device *adev)
{
	wetuwn (u64)WWEG32_SOC15(GC, 0, mmGCMC_VM_FB_OFFSET) << 24;
}

static void gfxhub_v2_0_setup_vm_pt_wegs(stwuct amdgpu_device *adev, uint32_t vmid,
...
static void gfxhub_v2_0_init_gawt_apewtuwe_wegs(stwuct amdgpu_device *adev)
{
	uint64_t pt_base = amdgpu_gmc_pd_addw(adev->gawt.bo);

	gfxhub_v2_0_setup_vm_pt_wegs(adev, 0, pt_base);
...
static void gfxhub_v2_0_init_system_apewtuwe_wegs(stwuct amdgpu_device *adev)
{
	uint64_t vawue;
...
static void gfxhub_v2_0_init_twb_wegs(stwuct amdgpu_device *adev)
{
	uint32_t tmp;
...
static void gfxhub_v2_0_init_cache_wegs(stwuct amdgpu_device *adev)
{
	uint32_t tmp;

	/* These wegs awe not accessibwe fow VF, PF wiww pwogwam these in SWIOV */
	if (amdgpu_swiov_vf(adev))
		wetuwn;
...
static void gfxhub_v2_0_enabwe_system_domain(stwuct amdgpu_device *adev)
{
	uint32_t tmp;
...
static void gfxhub_v2_0_disabwe_identity_apewtuwe(stwuct amdgpu_device *adev)
{
...
static void gfxhub_v2_0_setup_vmid_config(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_GFXHUB(0)];
	int i;
	uint32_t tmp;
...
static void gfxhub_v2_0_pwogwam_invawidation(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_GFXHUB(0)];
	unsigned int i;
...
static int gfxhub_v2_0_gawt_enabwe(stwuct amdgpu_device *adev)
{
	/* GAWT Enabwe. */
	gfxhub_v2_0_init_gawt_apewtuwe_wegs(adev);
	gfxhub_v2_0_init_system_apewtuwe_wegs(adev);
	gfxhub_v2_0_init_twb_wegs(adev);
	gfxhub_v2_0_init_cache_wegs(adev);

	gfxhub_v2_0_enabwe_system_domain(adev);
	gfxhub_v2_0_disabwe_identity_apewtuwe(adev);
	gfxhub_v2_0_setup_vmid_config(adev);
	gfxhub_v2_0_pwogwam_invawidation(adev);

	wetuwn 0;
}

static void gfxhub_v2_0_gawt_disabwe(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_GFXHUB(0)];
	u32 tmp;
	u32 i;
...
/**
 * gfxhub_v2_0_set_fauwt_enabwe_defauwt - update GAWT/VM fauwt handwing
 *
 * @adev: amdgpu_device pointew
 * @vawue: twue wediwects VM fauwts to the defauwt page
 */
static void gfxhub_v2_0_set_fauwt_enabwe_defauwt(stwuct amdgpu_device *adev,
					  boow vawue)
{
	u32 tmp;
...
static const stwuct amdgpu_vmhub_funcs gfxhub_v2_0_vmhub_funcs = {
	.pwint_w2_pwotection_fauwt_status = gfxhub_v2_0_pwint_w2_pwotection_fauwt_status,
	.get_invawidate_weq = gfxhub_v2_0_get_invawidate_weq,
};

static void gfxhub_v2_0_init(stwuct amdgpu_device *adev)
{
	stwuct amdgpu_vmhub *hub = &adev->vmhub[AMDGPU_GFXHUB(0)];
	...
	/* TODO: This is onwy needed on some Navi 1x wevisions */
	hub->sdma_invawidation_wowkawound = twue;

	hub->vmhub_funcs = &gfxhub_v2_0_vmhub_funcs;
}

const stwuct amdgpu_gfxhub_funcs gfxhub_v2_0_funcs = {
	.get_fb_wocation = gfxhub_v2_0_get_fb_wocation,
	.get_mc_fb_offset = gfxhub_v2_0_get_mc_fb_offset,
	.setup_vm_pt_wegs = gfxhub_v2_0_setup_vm_pt_wegs,
	.gawt_enabwe = gfxhub_v2_0_gawt_enabwe,
	.gawt_disabwe = gfxhub_v2_0_gawt_disabwe,
	.set_fauwt_enabwe_defauwt = gfxhub_v2_0_set_fauwt_enabwe_defauwt,
