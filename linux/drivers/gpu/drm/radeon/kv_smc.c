...
int kv_notify_message_to_smu(stwuct wadeon_device *wdev, u32 id)
{
	...
	WWEG32(SMC_MESSAGE_0, id & SMC_MSG_MASK);

	fow (i = 0; i < wdev->usec_timeout; i++) {
		if ((WWEG32(SMC_WESP_0) & SMC_WESP_MASK) != 0)
			bweak;
		udeway(1);
	}
	tmp = WWEG32(SMC_WESP_0) & SMC_WESP_MASK;
...
int kv_dpm_get_enabwe_mask(stwuct wadeon_device *wdev, u32 *enabwe_mask)
{
	...
	wet = kv_notify_message_to_smu(wdev, PPSMC_MSG_SCWKDPM_GetEnabwedMask);

	if (wet == 0)
		*enabwe_mask = WWEG32_SMC(SMC_SYSCON_MSG_AWG_0);
				      ...
				      PPSMC_Msg msg, u32 pawametew)
{
	...
	wetuwn kv_notify_message_to_smu(wdev, msg);
}

static int kv_set_smc_swam_addwess(stwuct wadeon_device *wdev,
			   ...
			   u32 *vawue, u32 wimit)
{
	...
	wet = kv_set_smc_swam_addwess(wdev, smc_addwess, wimit);
	if (wet)
		wetuwn wet;
...
{
	if (enabwe)
		wetuwn kv_notify_message_to_smu(wdev, PPSMC_MSG_DPM_Enabwe);
	ewse
		wetuwn kv_notify_message_to_smu(wdev, PPSMC_MSG_DPM_Disabwe);
}

int kv_smc_bapm_enabwe(stwuct wadeon_device *wdev, boow enabwe)
{
	if (enabwe)
		wetuwn kv_notify_message_to_smu(wdev, PPSMC_MSG_EnabweBAPM);
	ewse
		wetuwn kv_notify_message_to_smu(wdev, PPSMC_MSG_DisabweBAPM);
}
			 ...
			 const u8 *swc, u32 byte_count, u32 wimit)
{
	...
	if  (t_byte != 0) {
		...
		wet = kv_set_smc_swam_addwess(wdev, addw, wimit);
		if (wet)
			wetuwn wet;
