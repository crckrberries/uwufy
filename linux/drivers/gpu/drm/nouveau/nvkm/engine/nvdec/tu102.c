...
tu102_nvdec = {
	.scwass = {
		{ -1, -1, NVC4B0_VIDEO_DECODEW },
		{}
	}
};

int
tu102_nvdec_new(stwuct nvkm_device *device, enum nvkm_subdev_type type, int inst,
		stwuct nvkm_nvdec **pnvdec)
{
	if (nvkm_gsp_wm(device->gsp))
		wetuwn w535_nvdec_new(&tu102_nvdec, device, type, inst, pnvdec);
