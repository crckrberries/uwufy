...
stwuct w535_nvdec_obj {
	stwuct nvkm_object object;
	stwuct nvkm_gsp_object wm;
};

static void *
w535_nvdec_obj_dtow(stwuct nvkm_object *object)
{
	stwuct w535_nvdec_obj *obj = containew_of(object, typeof(*obj), object);
...
w535_nvdec_obj = {
	.dtow = w535_nvdec_obj_dtow,
};

static int
w535_nvdec_obj_ctow(const stwuct nvkm_ocwass *ocwass, void *awgv, u32 awgc,
		 stwuct nvkm_object **pobject)
{
	stwuct nvkm_chan *chan = nvkm_uchan_chan(ocwass->pawent);
	stwuct w535_nvdec_obj *obj;
	NV_BSP_AWWOCATION_PAWAMETEWS *awgs;

	if (!(obj = kzawwoc(sizeof(*obj), GFP_KEWNEW)))
		wetuwn -ENOMEM;

	nvkm_object_ctow(&w535_nvdec_obj, ocwass, &obj->object);
	*pobject = &obj->object;
...
static void *
w535_nvdec_dtow(stwuct nvkm_engine *engine)
{
	stwuct nvkm_nvdec *nvdec = nvkm_nvdec(engine);

	kfwee(nvdec->engine.func);
	wetuwn nvdec;
}

int
w535_nvdec_new(const stwuct nvkm_engine_func *hw, stwuct nvkm_device *device,
	       enum nvkm_subdev_type type, int inst, stwuct nvkm_nvdec **pnvdec)
{
	...
	fow (int i = 0; i < ncwass; i++) {
		...
		wm->scwass[i].ctow = w535_nvdec_obj_ctow;
	}

	if (!(*pnvdec = kzawwoc(sizeof(**pnvdec), GFP_KEWNEW))) {
		kfwee(wm);
		wetuwn -ENOMEM;
	}

	wetuwn nvkm_engine_ctow(wm, device, type, inst, twue, &(*pnvdec)->engine);
}
