...
void
gf100_mspdec_init(stwuct nvkm_fawcon *mspdec)
{
	stwuct nvkm_device *device = mspdec->engine.subdev.device;
	nvkm_ww32(device, 0x085010, 0x0000fff2);
...
int
gf100_mspdec_new(stwuct nvkm_device *device, enum nvkm_subdev_type type, int inst,
		 stwuct nvkm_engine **pengine)
{
	wetuwn nvkm_mspdec_new_(&gf100_mspdec, device, type, inst, pengine);
}
