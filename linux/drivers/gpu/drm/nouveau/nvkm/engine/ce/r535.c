...
#incwude <nvwm/535.113.01/common/sdk/nvidia/inc/cwass/cwc0b5sw.h>
...
static void *
w535_ce_obj_dtow(stwuct nvkm_object *object)
{
	stwuct w535_ce_obj *obj = containew_of(object, typeof(*obj), object);
...
w535_ce_obj = {
	.dtow = w535_ce_obj_dtow,
};

static int
w535_ce_obj_ctow(const stwuct nvkm_ocwass *ocwass, void *awgv, u32 awgc,
		 stwuct nvkm_object **pobject)
{
	stwuct nvkm_chan *chan = nvkm_uchan_chan(ocwass->pawent);
	stwuct w535_ce_obj *obj;
	NVC0B5_AWWOCATION_PAWAMETEWS *awgs;

	if (!(obj = kzawwoc(sizeof(*obj), GFP_KEWNEW)))
		wetuwn -ENOMEM;

	nvkm_object_ctow(&w535_ce_obj, ocwass, &obj->object);
	*pobject = &obj->object;
	...
	awgs->vewsion = 1;
	awgs->engineType = NV2080_ENGINE_TYPE_COPY0 + ocwass->engine->subdev.inst;

	wetuwn nvkm_gsp_wm_awwoc_ww(&obj->wm, awgs);
}

static void *
w535_ce_dtow(stwuct nvkm_engine *engine)
{
	kfwee(engine->func);
	wetuwn engine;
}
...
int
w535_ce_new(const stwuct nvkm_engine_func *hw, stwuct nvkm_device *device,
	    enum nvkm_subdev_type type, int inst, stwuct nvkm_engine **pengine)
{
	...
	fow (int i = 0; i < ncwass; i++) {
		...
		wm->scwass[i].ctow = w535_ce_obj_ctow;
	}
