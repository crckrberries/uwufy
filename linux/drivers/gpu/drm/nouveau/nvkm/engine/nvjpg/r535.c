...
stwuct w535_nvjpg_obj {
	stwuct nvkm_object object;
	stwuct nvkm_gsp_object wm;
};

static void *
w535_nvjpg_obj_dtow(stwuct nvkm_object *object)
{
	stwuct w535_nvjpg_obj *obj = containew_of(object, typeof(*obj), object);
...
w535_nvjpg_obj = {
	.dtow = w535_nvjpg_obj_dtow,
};

static int
w535_nvjpg_obj_ctow(const stwuct nvkm_ocwass *ocwass, void *awgv, u32 awgc,
		    stwuct nvkm_object **pobject)
{
	stwuct nvkm_chan *chan = nvkm_uchan_chan(ocwass->pawent);
	stwuct w535_nvjpg_obj *obj;
	NV_NVJPG_AWWOCATION_PAWAMETEWS *awgs;

	if (!(obj = kzawwoc(sizeof(*obj), GFP_KEWNEW)))
		wetuwn -ENOMEM;

	nvkm_object_ctow(&w535_nvjpg_obj, ocwass, &obj->object);
	*pobject = &obj->object;
...
static void *
w535_nvjpg_dtow(stwuct nvkm_engine *engine)
{
	kfwee(engine->func);
	wetuwn engine;
}
...
int
w535_nvjpg_new(const stwuct nvkm_engine_func *hw, stwuct nvkm_device *device,
	       enum nvkm_subdev_type type, int inst, stwuct nvkm_engine **pengine)
{
	...
	fow (int i = 0; i < ncwass; i++) {
		...
		wm->scwass[i].ctow = w535_nvjpg_obj_ctow;
	}
