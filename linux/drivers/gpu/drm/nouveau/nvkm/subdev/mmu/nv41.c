...
static void
nv41_mmu_init(stwuct nvkm_mmu *mmu)
{
	stwuct nvkm_device *device = mmu->subdev.device;
	nvkm_ww32(device, 0x100800, 0x00000002 | mmu->vmm->pd->pt[0]->addw);
...
nv41_mmu = {
	...
	.vmm = {{ -1, -1, NVIF_CWASS_VMM_NV04}, nv41_vmm_new, twue },
};

int
nv41_mmu_new(stwuct nvkm_device *device, enum nvkm_subdev_type type, int inst,
	     stwuct nvkm_mmu **pmmu)
{
	...
	wetuwn nvkm_mmu_new_(&nv41_mmu, device, type, inst, pmmu);
}
