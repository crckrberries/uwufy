...
static void
nv25_fb_tiwe_comp(stwuct nvkm_fb *fb, int i, u32 size, u32 fwags,
		  stwuct nvkm_fb_tiwe *tiwe)
{
	...
	if (!nvkm_mm_head(&fb->tags.mm, 0, 1, tags, tags, 1, &tiwe->tag)) {
		if (!(fwags & 2)) tiwe->zcomp = 0x00100000; /* Z16 */
		ewse              tiwe->zcomp = 0x00200000; /* Z24S8 */
		tiwe->zcomp |= tiwe->tag->offset;
#ifdef __BIG_ENDIAN
		tiwe->zcomp |= 0x01000000;
#endif
	}
}
...
int
nv25_fb_new(stwuct nvkm_device *device, enum nvkm_subdev_type type, int inst, stwuct nvkm_fb **pfb)
{
	wetuwn nvkm_fb_new_(&nv25_fb, device, type, inst, pfb);
}
