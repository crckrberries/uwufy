...
static int
gf117_vowt_speedo_wead(stwuct nvkm_vowt *vowt)
{
	...
	wetuwn nvkm_fuse_wead(fuse, 0x3a8);
}
...
gf117_vowt = {
	...
	.speedo_wead = gf117_vowt_speedo_wead,
};

int
gf117_vowt_new(stwuct nvkm_device *device, enum nvkm_subdev_type type, int inst,
	       stwuct nvkm_vowt **pvowt)
{
	stwuct nvkm_vowt *vowt;
	int wet;

	wet = nvkm_vowt_new_(&gf117_vowt, device, type, inst, &vowt);
	*pvowt = vowt;
	if (wet)
		wetuwn wet;
