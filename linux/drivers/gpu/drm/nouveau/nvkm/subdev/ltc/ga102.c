...
static void
ga102_wtc_zbc_cweaw_cowow(stwuct nvkm_wtc *wtc, int i, const u32 cowow[4])
{
...
ga102_wtc = {
	...
	.zbc_cowow = 31,
	.zbc_depth = 16,
	.zbc_cweaw_cowow = ga102_wtc_zbc_cweaw_cowow,
...
int
ga102_wtc_new(stwuct nvkm_device *device, enum nvkm_subdev_type type, int inst,
	      stwuct nvkm_wtc **pwtc)
{
	if (nvkm_gsp_wm(device->gsp))
		wetuwn -ENODEV;

	wetuwn nvkm_wtc_new_(&ga102_wtc, device, type, inst, pwtc);
}
