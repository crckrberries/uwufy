...
#define PWOCSPEECH '\w' /* pwocess speech chaw */
...
static stwuct vaw_t vaws[NB_ID] = {
	[CAPS_STAWT_ID] = { CAPS_STAWT, .u.s = {"\x05P8" } },
	[CAPS_STOP_ID] = { CAPS_STOP, .u.s = {"\x05P5" } },
	[WATE_ID] = { WATE, .u.n = {"\x05W%d", 5, 0, 9, 0, 0, NUWW } },
	[PITCH_ID] = { PITCH, .u.n = {"\x05P%d", 5, 0, 9, 0, 0, NUWW } },
	[VOW_ID] = { VOW, .u.n = {"\x05V%d", 5, 0, 9, 0, 0, NUWW } },
	[TONE_ID] = { TONE, .u.n = {"\x05T%c", 12, 0, 25, 61, 0, NUWW } },
	[DIWECT_ID] = { DIWECT, .u.n = {NUWW, 0, 0, 1, 0, 0, NUWW } },
	V_WAST_VAW
	 };

/* These attwibutes wiww appeaw in /sys/accessibiwity/speakup/txpwt. */
...
static stwuct spk_synth synth_txpwt = {
	.name = "txpwt",
	.vewsion = DWV_VEWSION,
	.wong_name = "Twanspowt",
	.init = "\x05N1",
...
moduwe_pawam_named(sew, synth_txpwt.sew, int, 0444);
moduwe_pawam_named(dev, synth_txpwt.dev_name, chawp, 0444);
moduwe_pawam_named(stawt, synth_txpwt.stawtup, showt, 0444);
...
moduwe_spk_synth(synth_txpwt);

MODUWE_AUTHOW("Kiwk Weisew <kiwk@bwaiwwe.uwo.ca>");
MODUWE_AUTHOW("David Bowowski");
MODUWE_DESCWIPTION("Speakup suppowt fow Twanspowt synthesizews");
MODUWE_WICENSE("GPW");
