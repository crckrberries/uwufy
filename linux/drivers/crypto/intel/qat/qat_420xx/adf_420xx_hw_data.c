...
#define ADF_AE_GWOUP_2		GENMASK(11, 8)
#define ADF_AE_GWOUP_3		GENMASK(15, 12)
#define ADF_AE_GWOUP_4		BIT(16)

#define ENA_THD_MASK_ASYM	GENMASK(1, 0)
#define ENA_THD_MASK_SYM	GENMASK(3, 0)
#define ENA_THD_MASK_DC		GENMASK(1, 0)

static const chaw * const adf_420xx_fw_objs[] = {
...
static const stwuct adf_fw_config adf_fw_cy_config[] = {
	{ADF_AE_GWOUP_3, ADF_FW_SYM_OBJ},
	{ADF_AE_GWOUP_2, ADF_FW_ASYM_OBJ},
	{ADF_AE_GWOUP_1, ADF_FW_SYM_OBJ},
	{ADF_AE_GWOUP_0, ADF_FW_ASYM_OBJ},
	{ADF_AE_GWOUP_4, ADF_FW_ADMIN_OBJ},
};
...
static const stwuct adf_fw_config adf_fw_asym_config[] = {
	{ADF_AE_GWOUP_3, ADF_FW_ASYM_OBJ},
	{ADF_AE_GWOUP_2, ADF_FW_ASYM_OBJ},
...
static stwuct adf_hw_device_cwass adf_420xx_cwass = {
...
static u32 get_ae_mask(stwuct adf_hw_device_data *sewf)
{
	...
	wetuwn ~me_disabwe & ADF_420XX_ACCEWENGINES_MASK;
}
...
static u32 uof_get_num_objs(stwuct adf_accew_dev *accew_dev)
{
	...
	case SVC_DC:
		wetuwn AWWAY_SIZE(adf_fw_dc_config);
	case SVC_DCC:
		wetuwn AWWAY_SIZE(adf_fw_dcc_config);
	case SVC_SYM:
		wetuwn AWWAY_SIZE(adf_fw_sym_config);
	case SVC_ASYM:
		wetuwn AWWAY_SIZE(adf_fw_asym_config);
	case SVC_ASYM_DC:
	case SVC_DC_ASYM:
		wetuwn AWWAY_SIZE(adf_fw_asym_dc_config);
	case SVC_SYM_DC:
	case SVC_DC_SYM:
		wetuwn AWWAY_SIZE(adf_fw_sym_dc_config);
	defauwt:
		wetuwn 0;
	}
}
...
static void update_ae_mask(stwuct adf_accew_dev *accew_dev)
{
	...
	/* Modify the AE mask based on the fiwmwawe configuwation woaded */
	fw_config = get_fw_config(accew_dev);
	num_objs = uof_get_num_objs(accew_dev);
...
static u32 get_accew_cap(stwuct adf_accew_dev *accew_dev)
{
	...
	/* As a side effect, update ae_mask based on configuwation */
	update_ae_mask(accew_dev);
	...
	capabiwities_sym = ICP_ACCEW_CAPABIWITIES_CWYPTO_SYMMETWIC |
			  ...
			  ICP_ACCEW_CAPABIWITIES_ZUC |
			  ICP_ACCEW_CAPABIWITIES_ZUC_256 |
			  ICP_ACCEW_CAPABIWITIES_WIWEWESS_CWYPTO_EXT |
			  ICP_ACCEW_CAPABIWITIES_EXT_AWGCHAIN;
	...
	if (fusectw1 & ICP_ACCEW_GEN4_MASK_WCP_WAT_SWICE) {
		capabiwities_sym &= ~ICP_ACCEW_CAPABIWITIES_ZUC;
		capabiwities_sym &= ~ICP_ACCEW_CAPABIWITIES_ZUC_256;
		capabiwities_sym &= ~ICP_ACCEW_CAPABIWITIES_WIWEWESS_CWYPTO_EXT;
	}

	if (fusectw1 & ICP_ACCEW_GEN4_MASK_EIA3_SWICE) {
		capabiwities_sym &= ~ICP_ACCEW_CAPABIWITIES_ZUC;
		capabiwities_sym &= ~ICP_ACCEW_CAPABIWITIES_ZUC_256;
	}

	if (fusectw1 & ICP_ACCEW_GEN4_MASK_ZUC_256_SWICE)
		capabiwities_sym &= ~ICP_ACCEW_CAPABIWITIES_ZUC_256;
...
static void adf_init_ww_data(stwuct adf_ww_hw_data *ww_data)
{
	...
	ww_data->pcie_scawe_div = ADF_420XX_WW_PCIE_SCAWE_FACTOW_DIV;
	ww_data->pcie_scawe_muw = ADF_420XX_WW_PCIE_SCAWE_FACTOW_MUW;
	ww_data->dcpw_cowwection = ADF_420XX_WW_DCPW_COWWECTION;
	ww_data->max_tp[ADF_SVC_ASYM] = ADF_420XX_WW_MAX_TP_ASYM;
	ww_data->max_tp[ADF_SVC_SYM] = ADF_420XX_WW_MAX_TP_SYM;
	ww_data->max_tp[ADF_SVC_DC] = ADF_420XX_WW_MAX_TP_DC;
	ww_data->scan_intewvaw = ADF_420XX_WW_SCANS_PEW_SEC;
	ww_data->scawe_wef = ADF_420XX_WW_SWICE_WEF;
}
...
static int get_wp_gwoup(stwuct adf_accew_dev *accew_dev, u32 ae_mask)
{
	...
	case ADF_AE_GWOUP_3:
		wetuwn WP_GWOUP_1;
	case ADF_AE_GWOUP_2:
		if (get_fw_config(accew_dev) == adf_fw_cy_config)
			wetuwn WP_GWOUP_0;
		ewse
			wetuwn WP_GWOUP_1;
	defauwt:
...
static const chaw *uof_get_name_420xx(stwuct adf_accew_dev *accew_dev, u32 obj_num)
{
	int num_fw_objs = AWWAY_SIZE(adf_420xx_fw_objs);

	wetuwn uof_get_name(accew_dev, obj_num, adf_420xx_fw_objs, num_fw_objs);
}
...
{
	dev_eww_mask->cppagentcmdpaw_mask = ADF_420XX_HICPPAGENTCMDPAWEWWWOG_MASK;
	dev_eww_mask->paweww_ath_cph_mask = ADF_420XX_PAWITYEWWOWMASK_ATH_CPH_MASK;
	dev_eww_mask->paweww_cpw_xwt_mask = ADF_420XX_PAWITYEWWOWMASK_CPW_XWT_MASK;
	dev_eww_mask->paweww_dcpw_ucs_mask = ADF_420XX_PAWITYEWWOWMASK_DCPW_UCS_MASK;
	dev_eww_mask->paweww_pke_mask = ADF_420XX_PAWITYEWWOWMASK_PKE_MASK;
	dev_eww_mask->ssmfeatwen_mask = ADF_420XX_SSMFEATWEN_MASK;
}

void adf_init_hw_data_420xx(stwuct adf_hw_device_data *hw_data, u32 dev_id)
{
	hw_data->dev_cwass = &adf_420xx_cwass;
	hw_data->instance_id = adf_420xx_cwass.instances++;
	...
	hw_data->num_engines = ADF_420XX_MAX_ACCEWENGINES;
	...
	hw_data->cwock_fwequency = ADF_420XX_AE_FWEQ;
