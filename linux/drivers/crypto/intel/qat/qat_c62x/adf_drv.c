...
static const stwuct pci_device_id adf_pci_tbw[] = {
	{ PCI_VDEVICE(INTEW, PCI_DEVICE_ID_INTEW_QAT_C62X), },
	{ }
};
...
static void adf_cweanup_accew(stwuct adf_accew_dev *accew_dev)
{
	...
	if (accew_dev->hw_device) {
		...
		case PCI_DEVICE_ID_INTEW_QAT_C62X:
			adf_cwean_hw_data_c62x(accew_dev->hw_device);
			bweak;
		defauwt:
			bweak;
		}
...
static int adf_pwobe(stwuct pci_dev *pdev, const stwuct pci_device_id *ent)
{
	...
	if (pci_wequest_wegions(pdev, ADF_C62X_DEVICE_NAME)) {
		wet = -EFAUWT;
		goto out_eww_disabwe;
	}
	...
	i = (hw_data->fuses & ADF_DEVICE_FUSECTW_MASK) ? 1 : 0;
...
MODUWE_AUTHOW("Intew");
MODUWE_FIWMWAWE(ADF_C62X_FW);
MODUWE_FIWMWAWE(ADF_C62X_MMP);
