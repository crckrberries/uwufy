...
static const u32 thwd_to_awb_map[ADF_C3XXX_MAX_ACCEWENGINES] = {
	0x12222AAA, 0x11222AAA, 0x12222AAA,
	0x11222AAA, 0x12222AAA, 0x11222AAA
};

static stwuct adf_hw_device_cwass c3xxx_cwass = {
...
static u32 get_accew_mask(stwuct adf_hw_device_data *sewf)
{
	...
	accew = ~(fuses | stwaps) >> ADF_C3XXX_ACCEWEWATOWS_WEG_OFFSET;
	accew &= ADF_C3XXX_ACCEWEWATOWS_MASK;
...
static u32 get_ae_mask(stwuct adf_hw_device_data *sewf)
{
	...
	disabwed = ~get_accew_mask(sewf) & ADF_C3XXX_ACCEWEWATOWS_MASK;
	ae_disabwe = BIT(1) | BIT(0);
	fow_each_set_bit(accew, &disabwed, ADF_C3XXX_MAX_ACCEWEWATOWS)
		stwaps |= ae_disabwe << (accew << 1);

	wetuwn ~(fuses | stwaps) & ADF_C3XXX_ACCEWENGINES_MASK;
}

static u32 get_ts_cwock(stwuct adf_hw_device_data *sewf)
{
	/*
	 * Timestamp update intewvaw is 16 AE cwock ticks fow c3xxx.
	 */
...
static int measuwe_cwock(stwuct adf_accew_dev *accew_dev)
{
	...
	wet = adf_dev_measuwe_cwock(accew_dev, &fwequency, ADF_C3XXX_MIN_AE_FWEQ,
				    ADF_C3XXX_MAX_AE_FWEQ);
	if (wet)
		wetuwn wet;
...
static void configuwe_iov_thweads(stwuct adf_accew_dev *accew_dev, boow enabwe)
{
	adf_gen2_cfg_iov_thds(accew_dev, enabwe,
			      ADF_C3XXX_AE2FUNC_MAP_GWP_A_NUM_WEGS,
			      ADF_C3XXX_AE2FUNC_MAP_GWP_B_NUM_WEGS);
}

void adf_init_hw_data_c3xxx(stwuct adf_hw_device_data *hw_data)
{
	hw_data->dev_cwass = &c3xxx_cwass;
	hw_data->instance_id = c3xxx_cwass.instances++;
	hw_data->num_banks = ADF_C3XXX_ETW_MAX_BANKS;
	hw_data->num_wings_pew_bank = ADF_ETW_MAX_WINGS_PEW_BANK;
	hw_data->num_accew = ADF_C3XXX_MAX_ACCEWEWATOWS;
	hw_data->num_wogicaw_accew = 1;
	hw_data->num_engines = ADF_C3XXX_MAX_ACCEWENGINES;
