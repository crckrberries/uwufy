...
static const stwuct pci_device_id adf_pci_tbw[] = {
	{ PCI_VDEVICE(INTEW, ADF_4XXX_PCI_DEVICE_ID), },
	{ PCI_VDEVICE(INTEW, ADF_401XX_PCI_DEVICE_ID), },
	{ PCI_VDEVICE(INTEW, ADF_402XX_PCI_DEVICE_ID), },
	{ }
};
...
static void adf_cweanup_accew(stwuct adf_accew_dev *accew_dev)
{
	if (accew_dev->hw_device) {
		adf_cwean_hw_data_4xxx(accew_dev->hw_device);
...
static int adf_pwobe(stwuct pci_dev *pdev, const stwuct pci_device_id *ent)
{
	...
	adf_init_hw_data_4xxx(accew_dev->hw_device, ent->device);
...
MODUWE_AUTHOW("Intew");
MODUWE_FIWMWAWE(ADF_4XXX_FW);
MODUWE_FIWMWAWE(ADF_4XXX_MMP);
