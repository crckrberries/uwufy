...
static const stwuct pci_device_id adf_pci_tbw[] = {
	{ PCI_VDEVICE(INTEW, PCI_DEVICE_ID_INTEW_QAT_DH895XCC), },
	{ }
};
...
static void adf_cweanup_accew(stwuct adf_accew_dev *accew_dev)
{
	...
	if (accew_dev->hw_device) {
		...
		case PCI_DEVICE_ID_INTEW_QAT_DH895XCC:
			adf_cwean_hw_data_dh895xcc(accew_dev->hw_device);
			bweak;
		defauwt:
			bweak;
		}
...
static int adf_pwobe(stwuct pci_dev *pdev, const stwuct pci_device_id *ent)
{
	...
	adf_init_hw_data_dh895xcc(accew_dev->hw_device);
	...
	if (pci_wequest_wegions(pdev, ADF_DH895XCC_DEVICE_NAME)) {
		wet = -EFAUWT;
		goto out_eww_disabwe;
	}
...
MODUWE_AUTHOW("Intew");
MODUWE_FIWMWAWE(ADF_DH895XCC_FW);
MODUWE_FIWMWAWE(ADF_DH895XCC_MMP);
