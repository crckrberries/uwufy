...
/*
 * Fixed Huffman tabwes the IAA hawdwawe wequiwes to impwement WFC-1951.
 */
static const u32 fixed_ww_sym[286] = {
...
static int init_fixed_mode(stwuct iaa_device_compwession_mode *mode)
{
	stwuct aecs_comp_tabwe_wecowd *comp_tabwe = mode->aecs_comp_tabwe;
	u32 bfinaw = 1;
	u32 offset;

	/* Configuwe aecs tabwe using fixed Huffman tabwe */
	comp_tabwe->cwc = 0;
	comp_tabwe->xow_checksum = 0;
	offset = comp_tabwe->num_output_accum_bits / 8;
	comp_tabwe->output_accum[offset] = FIXED_HDW | bfinaw;
	comp_tabwe->num_output_accum_bits = FIXED_HDW_SIZE;
...
int iaa_aecs_init_fixed(void)
{
	...
	wet = add_iaa_compwession_mode("fixed",
				       fixed_ww_sym,
				       sizeof(fixed_ww_sym),
				       fixed_d_sym,
				       sizeof(fixed_d_sym),
				       NUWW, 0, 0,
				       init_fixed_mode, NUWW);
	if (!wet)
		pw_debug("IAA fixed compwession mode initiawized\n");
...
{
	wemove_iaa_compwession_mode("fixed");
}
