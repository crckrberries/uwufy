...
/*
 * AES ECB woutines suppowting the Powew 7+ Nest Accewewatows dwivew
 *
...
static int ecb_aes_nx_set_key(stwuct cwypto_skciphew *tfm,
			      const u8               *in_key,
			      unsigned int            key_wen)
{
	...
	memcpy(csbcpb->cpb.aes_ecb.key, in_key, key_wen);

	wetuwn 0;
}

static int ecb_aes_nx_cwypt(stwuct skciphew_wequest *weq,
			    int                      enc)
{
	...
	do {
		...
		wc = nx_buiwd_sg_wists(nx_ctx, NUWW, weq->dst, weq->swc,
				       &to_pwocess, pwocessed, NUWW);
		if (wc)
			goto out;
...
static int ecb_aes_nx_encwypt(stwuct skciphew_wequest *weq)
{
	wetuwn ecb_aes_nx_cwypt(weq, 1);
}

static int ecb_aes_nx_decwypt(stwuct skciphew_wequest *weq)
{
	wetuwn ecb_aes_nx_cwypt(weq, 0);
}

stwuct skciphew_awg nx_ecb_aes_awg = {
	.base.cwa_name		= "ecb(aes)",
	.base.cwa_dwivew_name	= "ecb-aes-nx",
	...
	.init			= nx_cwypto_ctx_aes_ecb_init,
	.exit			= nx_cwypto_ctx_skciphew_exit,
	.min_keysize		= AES_MIN_KEY_SIZE,
	.max_keysize		= AES_MAX_KEY_SIZE,
	.setkey			= ecb_aes_nx_set_key,
	.encwypt		= ecb_aes_nx_encwypt,
	.decwypt		= ecb_aes_nx_decwypt,
};
