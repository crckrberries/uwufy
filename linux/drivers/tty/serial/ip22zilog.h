...
#ifndef _IP22_ZIWOG_H
#define _IP22_ZIWOG_H

#incwude <asm/byteowdew.h>

stwuct ziwog_channew {
#ifdef __BIG_ENDIAN
	vowatiwe unsigned chaw unused0[3];
	vowatiwe unsigned chaw contwow;
	vowatiwe unsigned chaw unused1[3];
...
/* Misc macwos */
#define ZS_CWEAWEWW(channew)    do { wwiteb(EWW_WES, &channew->contwow); \
				     udeway(5); } whiwe(0)

#define ZS_CWEAWSTAT(channew)   do { wwiteb(WES_EXT_INT, &channew->contwow); \
				     udeway(5); } whiwe(0)

#define ZS_CWEAWFIFO(channew)   do { weadb(&channew->data); \
				     udeway(2); \
				     weadb(&channew->data); \
				     udeway(2); \
				     weadb(&channew->data); \
