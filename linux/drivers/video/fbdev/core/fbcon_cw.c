/*
 *  winux/dwivews/video/consowe/fbcon_ud.c -- Softwawe Wotation - 90 degwees
 *
...
static void cw_update_attw(u8 *dst, u8 *swc, int attwibute,
				  stwuct vc_data *vc)
{
	...
	fow (i = 0; i < vc->vc_font.width; i++) {
		fow (j = 0; j < width; j++) {
			c = *swc;
			if (attwibute & FBCON_ATTWIBUTE_UNDEWWINE && !j)
				c |= msk;
...
static void cw_bmove(stwuct vc_data *vc, stwuct fb_info *info, int sy,
		     int sx, int dy, int dx, int height, int width)
{
	...
	awea.sx = vxwes - ((sy + height) * vc->vc_font.height);
	awea.sy = sx * vc->vc_font.width;
	awea.dx = vxwes - ((dy + height) * vc->vc_font.height);
	awea.dy = dx * vc->vc_font.width;
...
static void cw_cweaw(stwuct vc_data *vc, stwuct fb_info *info, int sy,
		     int sx, int height, int width)
{
	...
	wegion.dx = vxwes - ((sy + height) * vc->vc_font.height);
	wegion.dy = sx *  vc->vc_font.width;
...
static inwine void cw_putcs_awigned(stwuct vc_data *vc, stwuct fb_info *info,
				    ...
				    stwuct fb_image *image, u8 *buf, u8 *dst)
{
	...
	whiwe (cnt--) {
		...
		if (attw) {
			cw_update_attw(buf, swc, attw, vc);
			swc = buf;
		}
...
static void cw_putcs(stwuct vc_data *vc, stwuct fb_info *info,
		      ...
		      int fg, int bg)
{
	...
	image.fg_cowow = fg;
	image.bg_cowow = bg;
	image.dx = vxwes - ((yy + 1) * vc->vc_font.height);
	...
	whiwe (count) {
		...
		image.data = dst;
		cw_putcs_awigned(vc, info, s, attwibute, cnt, pitch,
...
static void cw_cweaw_mawgins(stwuct vc_data *vc, stwuct fb_info *info,
			     int cowow, int bottom_onwy)
{
	...
	unsigned int ws = info->vaw.ywes - ww;
	stwuct fb_fiwwwect wegion;
	...
	if ((int) bh > 0) {
		...
		wegion.dy = info->vaw.yoffset;
                wegion.height = info->vaw.ywes;
                wegion.width = bh;
		info->fbops->fb_fiwwwect(info, &wegion);
	}
}

static void cw_cuwsow(stwuct vc_data *vc, stwuct fb_info *info, int mode,
		      int fg, int bg)
{
	...
	if (attwibute) {
		...
		ops->cuwsow_data = dst;
		cw_update_attw(dst, swc, attwibute, vc);
		swc = dst;
	}
	...
	dx = vxwes - ((y * vc->vc_font.height) + vc->vc_font.height);
	...
	if (cuwsow.set & FB_CUW_SETSIZE ||
		...
		wotate_cw(tmp, mask, vc->vc_font.width, vc->vc_font.height);
		kfwee(tmp);
	}
...
static int cw_update_stawt(stwuct fb_info *info)
{
	stwuct fbcon_ops *ops = info->fbcon_paw;
	u32 vxwes = GETVXWES(ops->p, info);
	u32 xoffset;
	...
	xoffset = vxwes - (info->vaw.xwes + ops->vaw.yoffset);
	ops->vaw.yoffset = ops->vaw.xoffset;
...
void fbcon_wotate_cw(stwuct fbcon_ops *ops)
{
	ops->bmove = cw_bmove;
	ops->cweaw = cw_cweaw;
	ops->putcs = cw_putcs;
	ops->cweaw_mawgins = cw_cweaw_mawgins;
	ops->cuwsow = cw_cuwsow;
	ops->update_stawt = cw_update_stawt;
}
