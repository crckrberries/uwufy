...
static const stwuct hdmi_phy_featuwes *phy_feat;
	...
	unsigned wong wfbitcwk)
{
	...
	if (hfbitcwk != wfbitcwk)
		fweqout = 0;
	ewse if (hfbitcwk / 10 < phy_feat->max_phy)
		fweqout = 1;
	ewse
		fweqout = 2;
...
{
	phy_feat = hdmi_phy_get_featuwes();
	if (!phy_feat)
		wetuwn -ENODEV;

	phy->base = devm_pwatfowm_iowemap_wesouwce_byname(pdev, "phy");
	if (IS_EWW(phy->base)) {
		DSSEWW("can't iowemap TX PHY\n");
