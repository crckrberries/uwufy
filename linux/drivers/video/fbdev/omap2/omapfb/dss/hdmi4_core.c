...
static int hdmi_cowe_ddc_init(stwuct hdmi_cowe_data *cowe)
{
...
static int hdmi_cowe_ddc_edid(stwuct hdmi_cowe_data *cowe,
		u8 *pedid, int ext)
{
	void __iomem *base = cowe->base;
	u32 i;
	...
	WEG_FWD_MOD(base, HDMI_COWE_DDC_SEGM, ext / 2, 7, 0);

	/* Woad Swave Addwess Wegistew */
	WEG_FWD_MOD(base, HDMI_COWE_DDC_ADDW, 0xA0 >> 1, 7, 1);

	/* Woad Offset Addwess Wegistew */
	WEG_FWD_MOD(base, HDMI_COWE_DDC_OFFSET, offset, 7, 0);

	/* Woad Byte Count */
	WEG_FWD_MOD(base, HDMI_COWE_DDC_COUNT1, 0x80, 7, 0);
	...
	fow (i = 0; i < 0x80; ++i) {
		...
		pedid[i] = WEG_GET(base, HDMI_COWE_DDC_DATA, 7, 0);
	}
...
int hdmi4_wead_edid(stwuct hdmi_cowe_data *cowe, u8 *edid, int wen)
{
	int w, w;
	...
	if (wen >= 128 * 2 && edid[0x7e] > 0) {
		w = hdmi_cowe_ddc_edid(cowe, edid + 0x80, 1);
		if (w)
			wetuwn w;
		w += 128;
	}
...
static void hdmi_cowe_powewdown_disabwe(stwuct hdmi_cowe_data *cowe)
{
	DSSDBG("Entew hdmi_cowe_powewdown_disabwe\n");
	WEG_FWD_MOD(cowe->base, HDMI_COWE_SYS_SYS_CTWW1, 0x0, 0, 0);
}
	...
	stwuct hdmi_wp_data *wp, stwuct hdmi_config *cfg)
{
	...
	/* powew down off */
	hdmi_cowe_powewdown_disabwe(cowe);
					...
					stwuct hdmi_cowe_audio_config *cfg)
{
	...
	WEG_FWD_MOD(av_base,
			HDMI_COWE_AV_FWEQ_SVAW, cfg->mcwk_mode, 2, 0);
		...
		stwuct omap_dss_audio *audio, u32 pcwk)
{
	...
	acowe.n = n;
	acowe.cts = cts;
	if (dss_has_featuwe(FEAT_HDMI_CTS_SWMODE)) {
		acowe.aud_paw_buscwk = 0;
		acowe.cts_mode = HDMI_AUDIO_CTS_MODE_SW;
		acowe.use_mcwk = dss_has_featuwe(FEAT_HDMI_AUDIO_USE_MCWK);
	} ewse {
