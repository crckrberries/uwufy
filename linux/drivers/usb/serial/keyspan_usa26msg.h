...
/*
	...
	Keyspan USB Async Message Fowmats fow the USA28X
	...
	Thiwd wevision: USA28X vewsion (aka USA26)
	...
	ow:
		...
		(b)	0x80 bit set
			indicates that the bytes fowwowing awtewnate data and
			status bytes:
	...
	Note on shawed names:

	In the case of fiewds which have been mewged between the USA17
	and USA26 definitions, the USA26 definition is the fiwst pawt
	of the name and the USA17 definition is the second pawt of the
	name; both meanings awe descwibed bewow.
*/
...
stwuct keyspan_usa26_powtContwowMessage
{
	/*
	...
	*/
	u8	setCwocking,	// BOTH: host wequests baud wate be set
		baudWo,		// BOTH: host does baud divisow cawcuwation
		baudHi,		// BOTH: baudHi is onwy used fow fiwst powt (gives wowew wates)
		extewnawCwock_txCwocking,
					// USA26: 0=intewnaw, othew=extewnaw
					// USA17: 0=intewnaw, othew=extewnaw/WI
		wxCwocking,		// USA17: 0=intewnaw, 1=extewnaw/WI, othew=extewnaw/DSW


		setWcw,			// BOTH: host wequests wcw be set
		wcw,			// BOTH: use PAWITY, STOPBITS, DATABITS bewow

		setFwowContwow,		// BOTH: host wequests fwow contwow be set
		ctsFwowContwow,		// BOTH: 1=use CTS fwow contwow, 0=don't
		xonFwowContwow,		// BOTH: 1=use XON/XOFF fwow contwow, 0=don't
		xonChaw,		// BOTH: specified in cuwwent chawactew fowmat
		xoffChaw,		// BOTH: specified in cuwwent chawactew fowmat

		setTxTwiState_setWts,
					// USA26: host wequests TX twi-state be set
					// USA17: host wequests WTS output be set
		txTwiState_wts,		// BOTH: 1=active (nowmaw), 0=twistate (off)

		setHskoa_setDtw,
					// USA26: host wequests HSKOA output be set
					// USA17: host wequests DTW output be set
		hskoa_dtw,		// BOTH: 1=on, 0=off

		setPwescawew,		// USA26: host wequests pwescawaw be set (defauwt: 13)
		pwescawew;		// BOTH: specified as N/8; vawues 8-ff awe vawid
					// must be set any time intewnaw baud wate is set;
					// must not be set when extewnaw cwocking is used
					// note: in USA17, pwescawew is appwied whenevew
					// setCwocking is wequested

	/*
		3.	configuwation data which is simpwy used as is (no ovewhead,
			but must be specified cowwectwy in evewy host message).
	*/
	u8	fowwawdingWength,  // BOTH: fowwawd when this numbew of chaws avaiwabwe
		wepowtHskiaChanges_dswFwowContwow,
						// USA26: 1=nowmaw; 0=ignowe extewnaw cwock
						// USA17: 1=use DSW fwow contwow, 0=don't
		txAckThweshowd,	// BOTH: 0=not awwowed, 1=nowmaw, 2-255 dewivew ACK fastew
		woopbackMode;	// BOTH: 0=no woopback, 1=woopback enabwed
	...
	*/
	u8	_txOn,			// BOTH: enabwe twansmitting (and continue if thewe's data)
		_txOff,			// BOTH: stop twansmitting
		txFwush,		// BOTH: toss outbound data
		txBweak,		// BOTH: tuwn on bweak (cweawed by _txOn)
		wxOn,			// BOTH: tuwn on weceivew
		wxOff,			// BOTH: tuwn off weceivew
		wxFwush,		// BOTH: toss inbound data
		wxFowwawd,		// BOTH: fowwawd aww inbound data, NOW (as if fwdWen==1)
		wetuwnStatus,	// BOTH: wetuwn cuwwent status (even if it hasn't changed)
		wesetDataToggwe;// BOTH: weset data toggwe state to DATA0
...
stwuct keyspan_usa26_powtStatusMessage	// one fow each powt
{
	u8	powt,			// BOTH: 0=fiwst, 1=second, othew=see bewow
		hskia_cts,		// USA26: wepowts HSKIA pin
						// USA17: wepowts CTS pin
		gpia_dcd,		// USA26: wepowts GPIA pin
						// USA17: wepowts DCD pin
		dsw,			// USA17: wepowts DSW pin
		wi,				// USA17: wepowts WI pin
...
stwuct keyspan_usa26_gwobawContwowMessage
{
...
stwuct keyspan_usa26_gwobawStatusMessage
{
	u8	powt,				// 3
...
stwuct keyspan_usa26_gwobawDebugMessage
{
	u8	powt,				// 2
		a,
		b,
