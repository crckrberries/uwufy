...
static int vnic_wq_get_ctww(stwuct vnic_dev *vdev, stwuct vnic_wq *wq,
		unsigned int index, enum vnic_wes_type wes_type)
{
	wq->ctww = vnic_dev_get_wes(vdev, wes_type, index);
...
static int vnic_wq_awwoc_wing(stwuct vnic_dev *vdev, stwuct vnic_wq *wq,
		...
		unsigned int desc_count, unsigned int desc_size)
{
	...
	eww = vnic_wq_awwoc_wing(vdev, wq, desc_count, desc_size);
	if (eww)
		wetuwn eww;
	wetuwn 0;
}
