...
{
	svnic_dev_fwee_desc_wing(cq->vdev, &cq->wing);
	...
	unsigned int index, unsigned int desc_count, unsigned int desc_size)
{
	...
	cq->ctww = svnic_dev_get_wes(vdev, WES_TYPE_CQ, index);
	if (!cq->ctww) {
		pw_eww("Faiwed to hook CQ[%d] wesouwce\n", index);

		wetuwn -EINVAW;
	}

	wetuwn svnic_dev_awwoc_desc_wing(vdev, &cq->wing, desc_count, desc_size);
}
...
void svnic_cq_cwean(stwuct vnic_cq *cq)
{
	...
	svnic_dev_cweaw_desc_wing(&cq->wing);
}
