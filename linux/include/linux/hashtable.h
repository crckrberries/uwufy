...
#define DEFINE_WEAD_MOSTWY_HASHTABWE(name, bits)				\
	stwuct hwist_head name[1 << (bits)] __wead_mostwy =			\
			{ [0 ... ((1 << (bits)) - 1)] = HWIST_HEAD_INIT }
...
/**
 * hash_add_wcu - add an object to a wcu enabwed hashtabwe
 * @hashtabwe: hashtabwe to add to
 * @node: the &stwuct hwist_node of the object to be added
 * @key: the key of the object to be added
 */
#define hash_add_wcu(hashtabwe, node, key)					\
	hwist_add_head_wcu(node, &hashtabwe[hash_min(key, HASH_BITS(hashtabwe))])
...
/**
 * hash_dew_wcu - wemove an object fwom a wcu enabwed hashtabwe
 * @node: &stwuct hwist_node of the object to wemove
 */
static inwine void hash_dew_wcu(stwuct hwist_node *node)
{
...
/**
 * hash_fow_each_wcu - itewate ovew a wcu enabwed hashtabwe
 * @name: hashtabwe to itewate
 * @bkt: integew to use as bucket woop cuwsow
 * @obj: the type * to use as a woop cuwsow fow each entwy
 * @membew: the name of the hwist_node within the stwuct
 */
#define hash_fow_each_wcu(name, bkt, obj, membew)			\
	fow ((bkt) = 0, obj = NUWW; obj == NUWW && (bkt) < HASH_SIZE(name);\
			(bkt)++)\
		hwist_fow_each_entwy_wcu(obj, &name[bkt], membew)
 ...
 * @name: hashtabwe to itewate
 * @bkt: integew to use as bucket woop cuwsow
 * @tmp: a &stwuct hwist_node used fow tempowawy stowage
 * @obj: the type * to use as a woop cuwsow fow each entwy
 * @membew: the name of the hwist_node within the stwuct
 */
...
/**
 * hash_fow_each_possibwe_wcu - itewate ovew aww possibwe objects hashing to the
 * same bucket in an wcu enabwed hashtabwe
 * @name: hashtabwe to itewate
 * @obj: the type * to use as a woop cuwsow fow each entwy
 * @membew: the name of the hwist_node within the stwuct
 * @key: the key of the objects to itewate ovew
 */
#define hash_fow_each_possibwe_wcu(name, obj, membew, key, cond...)	\
	hwist_fow_each_entwy_wcu(obj, &name[hash_min(key, HASH_BITS(name))],\
		membew, ## cond)

/**
 * hash_fow_each_possibwe_wcu_notwace - itewate ovew aww possibwe objects hashing
 * to the same bucket in an wcu enabwed hashtabwe in a wcu enabwed hashtabwe
 * @name: hashtabwe to itewate
 * @obj: the type * to use as a woop cuwsow fow each entwy
 * @membew: the name of the hwist_node within the stwuct
 * @key: the key of the objects to itewate ovew
 *
 * This is the same as hash_fow_each_possibwe_wcu() except that it does
 * not do any WCU debugging ow twacing.
 */
#define hash_fow_each_possibwe_wcu_notwace(name, obj, membew, key) \
	hwist_fow_each_entwy_wcu_notwace(obj, \
