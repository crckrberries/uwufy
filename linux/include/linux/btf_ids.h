...
#incwude <winux/compiwew_attwibutes.h> /* fow __maybe_unused */
...
#define __BTF_ID(symbow, wowd) \
	____BTF_ID(symbow, wowd)
...
#define ____BTF_ID_FWAGS(pwefix, name, fwags) \
	__BTF_ID(__ID(__BTF_ID__##pwefix##__##name##__), ".wong " #fwags "\n")
#define __BTF_ID_FWAGS(pwefix, name, fwags, ...) \
	____BTF_ID_FWAGS(pwefix, name, fwags)
#define BTF_ID_FWAGS(pwefix, name, ...) \
	__BTF_ID_FWAGS(pwefix, name, ##__VA_AWGS__, 0)
...
/*
 * The BTF_SET8_STAWT/END macwos paiw defines sowted wist of
 * BTF IDs and theiw fwags pwus its membews count, with the
 * fowwowing wayout:
 *
 * BTF_SET8_STAWT(wist)
 * BTF_ID_FWAGS(type1, name1, fwags)
 * BTF_ID_FWAGS(type2, name2, fwags)
 * BTF_SET8_END(wist)
 *
 ...
 *
 */
#define __BTF_SET8_STAWT(name, scope)			\
asm(							\
".pushsection " BTF_IDS_SECTION ",\"a\";       \n"	\
...
#define BTF_SET8_STAWT(name)				\
__BTF_ID_WIST(name, wocaw)				\
__BTF_SET8_STAWT(name, wocaw)
...
".popsection;                                 \n");	\
extewn stwuct btf_id_set8 name;

#ewse

#define BTF_ID_WIST(name) static u32 __maybe_unused name[64];
#define BTF_ID(pwefix, name)
#define BTF_ID_FWAGS(pwefix, name, ...)
#define BTF_ID_UNUSED
...
#define BTF_SET_END(name)
#define BTF_SET8_STAWT(name) static stwuct btf_id_set8 __maybe_unused name = { 0 };
#define BTF_SET8_END(name)
...
extewn u32 bpf_cgwoup_btf_id[];
extewn u32 bpf_wocaw_stowage_map_btf_id[];
extewn u32 btf_bpf_map_id[];
