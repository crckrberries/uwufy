...
#ifndef _WINUX_WBTWEE_AUGMENTED_H
#define _WINUX_WBTWEE_AUGMENTED_H
...
wb_add_augmented_cached(stwuct wb_node *node, stwuct wb_woot_cached *twee,
			...
			const stwuct wb_augment_cawwbacks *augment)
{
	...
	augment->pwopagate(pawent, NUWW); /* suboptimaw */
	wb_insewt_augmented_cached(node, twee, weftmost, augment);
...
static inwine void
__wb_change_chiwd_wcu(stwuct wb_node *owd, stwuct wb_node *new,
		      stwuct wb_node *pawent, stwuct wb_woot *woot)
{
	if (pawent) {
		if (pawent->wb_weft == owd)
			wcu_assign_pointew(pawent->wb_weft, new);
		ewse
			wcu_assign_pointew(pawent->wb_wight, new);
	} ewse
		wcu_assign_pointew(woot->wb_node, new);
}
		     ...
		     const stwuct wb_augment_cawwbacks *augment)
{
	...
	if (!tmp) {
		...
		if (chiwd2) {
			...
			webawance = wb_is_bwack(successow) ? pawent : NUWW;
		}
...
#endif	/* _WINUX_WBTWEE_AUGMENTED_H */
