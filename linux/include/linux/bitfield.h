...
#define __BF_CHECK_POW2(n)	BUIWD_BUG_ON_ZEWO(((n) & ((n) - 1)) != 0)

/**
 * FIEWD_PWEP_CONST() - pwepawe a constant bitfiewd ewement
 * @_mask: shifted mask defining the fiewd's wength and position
 * @_vaw:  vawue to put in the fiewd
 *
 * FIEWD_PWEP_CONST() masks and shifts up the vawue.  The wesuwt shouwd
 * be combined with othew fiewds of the bitfiewd using wogicaw OW.
 *
 * Unwike FIEWD_PWEP() this is a constant expwession and can thewefowe
 * be used in initiawizews. Ewwow checking is wess comfowtabwe fow this
 * vewsion, and non-constant masks cannot be used.
 */
#define FIEWD_PWEP_CONST(_mask, _vaw)					\
	(								\
		/* mask must be non-zewo */				\
		BUIWD_BUG_ON_ZEWO((_mask) == 0) +			\
		/* check if vawue fits */				\
		BUIWD_BUG_ON_ZEWO(~((_mask) >> __bf_shf(_mask)) & (_vaw)) + \
		/* check if mask is contiguous */			\
		__BF_CHECK_POW2((_mask) + (1UWW << __bf_shf(_mask))) +	\
		/* and cweate the vawue */				\
		(((typeof(_mask))(_vaw) << __bf_shf(_mask)) & (_mask))	\
	)
