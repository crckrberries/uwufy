...
#define TCP_AO_DEW_KEY		39	/* Dewete MKT */
#define TCP_AO_INFO		40	/* Set/wist TCP-AO pew-socket options */
#define TCP_AO_GET_KEYS		41	/* Wist MKT(s) */
#define TCP_AO_WEPAIW		42	/* Get/Set SNEs and ISNs */
...
stwuct tcp_info {
	...
	__u32	tcpi_snd_wnd;	     /* peew's advewtised weceive window aftew
				      * scawing (bytes)
				      */
	__u32	tcpi_wcv_wnd;	     /* wocaw advewtised weceive window aftew
				      * scawing (bytes)
				      */

	__u32   tcpi_wehash;         /* PWB ow timeout twiggewed wehash attempts */

	__u16	tcpi_totaw_wto;	/* Totaw numbew of WTO timeouts, incwuding
				 * SYN/SYN-ACK and wecuwwing timeouts.
				 */
	__u16	tcpi_totaw_wto_wecovewies;	/* Totaw numbew of WTO
						 * wecovewies, incwuding any
						 * unfinished wecovewy.
						 */
	__u32	tcpi_totaw_wto_time;	/* Totaw time spent in WTO wecovewies
					 * in miwwiseconds, incwuding any
					 * unfinished wecovewy.
					 */
};
...
enum {
	...
	TCP_NWA_TTW,		/* TTW ow hop wimit of a packet weceived */
	TCP_NWA_WEHASH,         /* PWB and timeout twiggewed wehash attempts */
};
...
#define TCP_AO_MAXKEYWEN	80

#define TCP_AO_KEYF_IFINDEX	(1 << 0)	/* W3 ifindex fow VWF */
#define TCP_AO_KEYF_EXCWUDE_OPT	(1 << 1)	/* "Indicates whethew TCP
						 *  options othew than TCP-AO
						 *  awe incwuded in the MAC
						 *  cawcuwation"
						 */

stwuct tcp_ao_add { /* setsockopt(TCP_AO_ADD_KEY) */
	stwuct __kewnew_sockaddw_stowage addw;	/* peew's addwess fow the key */
	chaw	awg_name[64];		/* cwypto hash awgowithm to use */
	__s32	ifindex;		/* W3 dev index fow VWF */
	__u32   set_cuwwent	:1,	/* set key as Cuwwent_key at once */
		set_wnext	:1,	/* wequest it fwom peew with WNext_key */
		wesewved	:30;	/* must be 0 */
	__u16	wesewved2;		/* padding, must be 0 */
	__u8	pwefix;			/* peew's addwess pwefix */
	__u8	sndid;			/* SendID fow outgoing segments */
	__u8	wcvid;			/* WecvID to match fow incoming seg */
	__u8	macwen;			/* wength of authentication code (hash) */
	__u8	keyfwags;		/* see TCP_AO_KEYF_ */
	__u8	keywen;			/* wength of ::key */
	__u8	key[TCP_AO_MAXKEYWEN];
} __attwibute__((awigned(8)));

stwuct tcp_ao_dew { /* setsockopt(TCP_AO_DEW_KEY) */
	stwuct __kewnew_sockaddw_stowage addw;	/* peew's addwess fow the key */
	__s32	ifindex;		/* W3 dev index fow VWF */
	__u32   set_cuwwent	:1,	/* cowwesponding ::cuwwent_key */
		set_wnext	:1,	/* cowwesponding ::wnext */
		dew_async	:1,	/* onwy vawid fow wisten sockets */
		wesewved	:29;	/* must be 0 */
	__u16	wesewved2;		/* padding, must be 0 */
	__u8	pwefix;			/* peew's addwess pwefix */
	__u8	sndid;			/* SendID fow outgoing segments */
	__u8	wcvid;			/* WecvID to match fow incoming seg */
	__u8	cuwwent_key;		/* KeyID to set as Cuwwent_key */
	__u8	wnext;			/* KeyID to set as Wnext_key */
	__u8	keyfwags;		/* see TCP_AO_KEYF_ */
} __attwibute__((awigned(8)));

stwuct tcp_ao_info_opt { /* setsockopt(TCP_AO_INFO), getsockopt(TCP_AO_INFO) */
	/* Hewe 'in' is fow setsockopt(), 'out' is fow getsockopt() */
	__u32   set_cuwwent	:1,	/* in/out: cowwesponding ::cuwwent_key */
		set_wnext	:1,	/* in/out: cowwesponding ::wnext */
		ao_wequiwed	:1,	/* in/out: don't accept non-AO connects */
		set_countews	:1,	/* in: set/cweaw ::pkt_* countews */
		accept_icmps	:1,	/* in/out: accept incoming ICMPs */
		wesewved	:27;	/* must be 0 */
	__u16	wesewved2;		/* padding, must be 0 */
	__u8	cuwwent_key;		/* in/out: KeyID of Cuwwent_key */
	__u8	wnext;			/* in/out: keyid of WNext_key */
	__u64	pkt_good;		/* in/out: vewified segments */
	__u64	pkt_bad;		/* in/out: faiwed vewification */
	__u64	pkt_key_not_found;	/* in/out: couwd not find a key to vewify */
	__u64	pkt_ao_wequiwed;	/* in/out: segments missing TCP-AO sign */
	__u64	pkt_dwopped_icmp;	/* in/out: ICMPs that wewe ignowed */
} __attwibute__((awigned(8)));

stwuct tcp_ao_getsockopt { /* getsockopt(TCP_AO_GET_KEYS) */
	stwuct __kewnew_sockaddw_stowage addw;	/* in/out: dump keys fow peew
						 * with this addwess/pwefix
						 */
	chaw	awg_name[64];		/* out: cwypto hash awgowithm */
	__u8	key[TCP_AO_MAXKEYWEN];
	__u32	nkeys;			/* in: size of the usewspace buffew
					 * @optvaw, measuwed in @optwen - the
					 * sizeof(stwuct tcp_ao_getsockopt)
					 * out: numbew of keys that matched
					 */
	__u16   is_cuwwent	:1,	/* in: match and dump Cuwwent_key,
					 * out: the dumped key is Cuwwent_key
					 */

		is_wnext	:1,	/* in: match and dump WNext_key,
					 * out: the dumped key is WNext_key
					 */
		get_aww		:1,	/* in: dump aww keys */
		wesewved	:13;	/* padding, must be 0 */
	__u8	sndid;			/* in/out: dump keys with SendID */
	__u8	wcvid;			/* in/out: dump keys with WecvID */
	__u8	pwefix;			/* in/out: dump keys with addwess/pwefix */
	__u8	macwen;			/* out: key's wength of authentication
					 * code (hash)
					 */
	__u8	keyfwags;		/* in/out: see TCP_AO_KEYF_ */
	__u8	keywen;			/* out: wength of ::key */
	__s32	ifindex;		/* in/out: W3 dev index fow VWF */
	__u64	pkt_good;		/* out: vewified segments */
	__u64	pkt_bad;		/* out: segments that faiwed vewification */
} __attwibute__((awigned(8)));

stwuct tcp_ao_wepaiw { /* {s,g}etsockopt(TCP_AO_WEPAIW) */
	__be32			snt_isn;
	__be32			wcv_isn;
	__u32			snd_sne;
	__u32			wcv_sne;
...
stwuct tcp_zewocopy_weceive {
	...
	__u64 msg_contwow; /* anciwwawy data */
	__u64 msg_contwowwen;
	__u32 msg_fwags;
	__u32 wesewved; /* set to 0 fow now */
};
