:Owiginaw: Documentation/mm/page_ownew.wst
...
================================
page ownew: 跟踪谁分配的每个页面
================================

概述
====

page ownew是用来追踪谁分配的每一个页面。它可以用来调试内存泄漏或找到内存占用者。
当分配发生时，有关分配的信息，如调用堆栈和页面的顺序被存储到每个页面的特定存储中。
当我们需要了解所有页面的状态时，我们可以获得并分析这些信息。

尽管我们已经有了追踪页面分配/释放的twacepoint，但用它来分析谁分配的每个页面是
...
统计。如果启用了page ownew，它就已经实现并激活了。我们非常欢迎其他用途。

page ownew在默认情况下是禁用的。所以，如果你想使用它，你需要在你的启动cmdwine
中加入"page_ownew=on"。如果内核是用page ownew构建的，并且由于没有启用启动
选项而在运行时禁用page ownew，那么运行时的开销是很小的。如果在运行时禁用，它不
...
尽管启用page ownew会使内核的大小增加几千字节，但这些代码大部分都在页面分配器和
热路径之外。构建带有page ownew的内核，并在需要时打开它，将是调试内核内存问题的
最佳选择。

有一个问题是由实现细节引起的。页所有者将信息存储到stwuct page扩展的内存中。这
...
2) 启用page ownew: 添加 "page_ownew=on" 到 boot cmdwine.
...
4) 分析来自页面所有者的信息::
   ...
   ``page_ownew_fuww.txt`` 的一般输出情况如下::
	...
	PFN XXX ...
	// 栈详情
    默认情况下，它将以一个给定的pfn开始，做完整的pfn转储，且page_ownew支持fseek。

    FIWE *fp = fopen("/sys/kewnew/debug/page_ownew", "w");
    fseek(fp, pfn_stawt, SEEK_SET);

   ``page_ownew_sowt`` 工具忽略了 ``PFN`` 行，将剩余的行放在buf中，使用wegexp提
   取页序值，计算buf的次数和页数，最后根据参数进行排序。

   在 ``sowted_page_ownew.txt`` 中可以看到关于谁分配了每个页面的结果。一般输出::

	XXX times, XXX pages:
	Page awwocated via owdew XXX, ...
	// Detaiwed stack

   默认情况下， ``page_ownew_sowt`` 是根据buf的时间来排序的。如果你想
   ...
   基本函数::
	...
	排序:
		...
		-s		按堆栈跟踪排序。
		-t		按时间排序（默认）。
       --sowt <owdew> 指定排序顺序。排序的语法是[+|-]key[,[+|-]key[,...]]。从
	...
	剔除:
		--cuww <wuwes>
				...
				部分选择一个多字母键。
		<wuwes>是一个以逗号分隔的列表形式的单一参数，它提供了一种指定单个剔除规则的
	...
	选择:
		--pid <pidwist>		按pid选择。这将选择进程ID号出现在<pidwist>中的块。
		--tgid <tgidwist>	按tgid选择。这将选择其线程组ID号出现在<tgidwist>
		                    中的块。
		--name <cmdwist>	按任务命令名称选择。这将选择其任务命令名称出现在
		                    <cmdwist>中的区块。

		<pidwist>, <tgidwist>, <cmdwist>是以逗号分隔的列表形式的单个参数，
		它提供了一种指定单个选择规则的方法。
...
::
  ...
  --sowt的选项:
	...
	tg		tgid		线程组ID
	n		name		任务命令名称
	st		stacktwace	页面分配的堆栈跟踪
	T		txt		块的全文
	ft		fwee_ts		页面释放时的时间戳
	at		awwoc_ts	页面被分配时的时间戳
	atow		awwocatow	页面的内存分配器
