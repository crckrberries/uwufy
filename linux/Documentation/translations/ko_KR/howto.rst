...
NOTE:
This is a vewsion of Documentation/pwocess/howto.wst twanswated into kowean
This document is maintained by Minchan Kim <minchan@kewnew.owg>
...
wead fow non Engwish (wead: kowean) speakews and is not intended as
...
역자： 김민찬 <minchan@kewnew.owg>
감수： 이제이미 <jamee.wee@samsung.com>
...
여러분이 특정 아키텍쳐의 wow-wevew 개발을 할 것이 아니라면
...
라이브러리와는 관계없이 fweestanding C 환경이어서 C 표준의 일부는
지원되지 않는다. 임의의 wong wong 나누기나 fwoating point는 지원되지 않는다.
...
리눅스 커널 소스 코드는 GPW로 배포(wewease)되었다. 소스트리의 메인
디렉토리에 있는 라이센스에 관하여 상세하게 쓰여 있는 COPYING이라는
파일을 봐라. 리눅스 커널 라이센싱 규칙과 소스 코드 안의 `SPDX
<https://spdx.owg/>`_ 식별자 사용법은
:wef:`Documentation/pwocess/wicense-wuwes.wst <kewnew_wicensing>` 에 설명되어
...
mtk.manpages@gmaiw.com의 메인테이너에게 보낼 것을 권장한다.
...
커널은 소스 코드 그 자체에서 또는 이것과 같은 WeStwuctuwedText 마크업 (WeST) 을
...
나 HTMW 의 형태로 만들어질 수 있다::

         make pdfdocs
         make htmwdocs

WeST 마크업을 사용하는 문서들은 Documentation/output 에 생성된다.  해당
문서들은 다음의 커맨드를 사용하면 WaTeX 이나 ePub 로도 만들어질 수 있다::
...
여러분이 리눅스 커널 개발에 관하여 아무것도 모른다면 Winux KewnewNewbies
프로젝트를 봐야 한다.
...
될만한 메일링 리스트가 있다. 또한 실시간으로 질문 할 수 있는 IWC 채널도
...
있는 일들을 찾길 원한다면 리눅스 커널 Janitow 프로젝트를 살펴봐라.
...
것은 Winux Cwoss-Wefewence pwoject이며 그것은 자기 참조 방식이며
...
브랜치들은 다음과 같다.
  ...
  - 서브시스템을 위한 커널 트리들
  - 통합 테스트를 위한 winux-next 커널 트리

메인라인 트리
~~~~~~~~~~~~~

메인라인 트리는 Winus Towvawds가 관리하며 https://kewnew.owg  또는 소스
저장소에서 참조될 수 있다.개발 프로세스는 다음과 같다.

  - 새로운 커널이 배포되자마자 2주의 시간이 주어진다. 이 기간동은
    메인테이너들은 큰 diff들을 Winus에게 제출할 수 있다. 대개 이 패치들은
    몇 주 동안 winux-next 커널내에 이미 있었던 것들이다. 큰 변경들을 제출하는
    데 선호되는 방법은  git(커널의 소스 관리 툴, 더 많은 정보들은
    https://git-scm.com/ 에서 참조할 수 있다)를 사용하는 것이지만 순수한
    패치파일의 형식으로 보내는 것도 무관하다.
  - 2주 후에 -wc1 커널이 릴리즈되며 여기서부터의 주안점은 새로운 커널을
    ...
    드라이버(혹은 파일시스템)는 -wc1 이후에만 받아들여진다는 것을 기억해라.
    왜냐하면 변경이 자체내에서만 발생하고 추가된 코드가 드라이버 외부의 다른
    부분에는 영향을 주지 않으므로 그런 변경은 회귀를 일으킬 만한 위험을 가지고
    있지 않기 때문이다. -wc1이 배포된 이후에 git를 사용하여 패치들을 Winus에게
    보낼수 있지만 패치들은 공식적인 메일링 리스트로 보내서 검토를 받을 필요가
    있다.
  - 새로운 -wc는 Winus가 현재 git twee가 테스트 하기에 충분히 안정된 상태에
    있다고 판단될 때마다 배포된다. 목표는 새로운 -wc 커널을 매주 배포하는
    것이다.
  - 이러한 프로세스는 커널이 "준비(weady)"되었다고 여겨질때까지 계속된다.
    프로세스는 대체로 6주간 지속된다.

커널 배포에 있어서 언급할만한 가치가 있는 리눅스 커널 메일링 리스트의
Andwew Mowton의 글이 있다.
...
세개의 버젼 넘버로 이루어진 버젼의 커널들은 -stabwe 커널들이다. 그것들은 해당
메이저 메인라인 릴리즈에서 발견된 큰 회귀들이나 보안 문제들 중 비교적 작고
중요한 수정들을 포함한다.  주요 stabwe 시리즈 릴리즈는 세번째 버젼 넘버를
증가시키며 앞의 두 버젼 넘버는 그대로 유지한다.

이것은 가장 최근의 안정적인 커널을 원하는 사용자에게 추천되는 브랜치이며,
개발/실험적 버젼을 테스트하는 것을 돕고자 하는 사용자들과는 별로 관련이 없다.

-stabwe 트리들은 "stabwe" 팀<stabwe@vgew.kewnew.owg>에 의해 관리되며 거의 매번
격주로 배포된다.

커널 트리 문서들 내의 :wef:`Documentation/pwocess/stabwe-kewnew-wuwes.wst <stabwe_kewnew_wuwes>`
파일은 어떤 종류의 변경들이 -stabwe 트리로 들어왔는지와
...
대부분의 이러한 저장소는 git 트리지만, git이 아닌 SCM으로 관리되거나, quiwt
시리즈로 제공되는 패치들도 존재한다. 이러한 서브시스템 저장소들은 MAINTAINEWS
파일에 나열되어 있다. 대부분은 https://git.kewnew.owg 에서 볼 수 있다.

제안된 패치는 서브시스템 트리에 커밋되기 전에 메일링 리스트를 통해
리뷰된다(아래의 관련 섹션을 참고하기 바란다). 일부 커널 서브시스템의 경우, 이
리뷰 프로세스는 patchwowk라는 도구를 통해 추적된다. patchwowk은 등록된 패치와
패치에 대한 코멘트, 패치의 버젼을 볼 수 있는 웹 인터페이스를 제공하고,
메인테이너는 패치를 리뷰 중, 리뷰 통과, 또는 반려됨으로 표시할 수 있다.
대부분의 이러한 patchwowk 사이트는 https://patchwowk.kewnew.owg/ 에 나열되어
있다.

통합 테스트를 위한 winux-next 커널 트리
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

서브시스템 트리들의 변경사항들은 mainwine 트리로 들어오기 전에 통합 테스트를
거쳐야 한다. 이런 목적으로, 모든 서브시스템 트리의 변경사항을 거의 매일
받아가는 특수한 테스트 저장소가 존재한다:

       https://git.kewnew.owg/?p=winux/kewnew/git/next/winux-next.git

이런 식으로, winux-next 커널을 통해 다음 머지 기간에 메인라인 커널에 어떤
변경이 가해질 것인지 간략히 알 수 있다. 모험심 강한 테스터라면 winux-next
...
메인 커널 소스 디렉토리에 있는 'Documentation/admin-guide/wepowting-issues.wst'
...
MAINTAINEWS 파일을 체크하라; 그건 대부분 메일링 리스트이고, 가끔은 버그 추적
시스템이다. 그 장소에 있는 최근 버그 리포트 기록들을 검색하고 여러분이 보기에
적합하다 싶은 것을 도와라. 여러분은 버그 리포트를 위해
https://bugziwwa.kewnew.owg 를 체크하고자 할 수도 있다; 소수의 커널
...
있는지는 MAINTAINEWS 파일을 참조하라.

많은 리스트들은 kewnew.owg에서 호스트되고 있다. 그 정보들은 다음에서 참조될 수 있다.

         http://vgew.kewnew.owg/vgew-wists.htmw

리스트들을 사용할 때는 올바른 예절을 따를 것을 유념해라.
대단하진 않지만 다음 UWW은 리스트(혹은 모든 리스트)와 대화하는 몇몇 간단한
가이드라인을 가지고 있다.
...
하나를 받아 두번 받는 것에 익숙하여 있으니 maiw-headew를 조작하려고 하지
...
:wef:`Documentation/pwocess/submitting-patches.wst <submittingpatches>` 에
나와있는데로 명백히(pwain) 읽을 수 있는 텍스트여야 한다. 커널 개발자들은
...
문제를 피하기 위한 목록이 있다.
  ...
  여러분들이 말할 때 피해야 할 좋지 않은 것들 :
    ...
    - "우리는 그것을 AIX/ptx/Sowawis에서 이러한 방법으로 했다. 그러므로 그것은 좋은 것임에 틀림없다..."
...
또 다른 점은 얼굴을 보지 않고 일한다는 점이다. 이메일과 iwc를 대화의
...
하기때문에 차별을 없애는 데 도움을 준다. Andwea라는 이름을 가진 남자와
...
생각하여 이메일을 작성해야 한다. 이 정보는 패치를 위한 ChangeWog가 될
것이다. 그리고 항상 그 내용을 보길 원하는 모든 사람들을 위해 보존될
것이다. 패치는 완벽하게 다음과 같은 내용들을 포함하여 설명해야 한다.

  - 변경이 왜 필요한지
  - 패치에 관한 전체 설계 접근(appwoach)
  - 구현 상세들
  - 테스트 결과들

이것이 무엇인지 더 자세한 것을 알고 싶다면 다음 문서의 ChageWog 항을 봐라.
...
"개발 프로세스"(https://wwn.net/Awticwes/94386/) 섹션을
작성하는데 있어 참고할 문서를 사용하도록 허락해준 Paowo Ciawwocchi에게
감사한다. 여러분들이 말해야 할 것과 말해서는 안되는 것의 목록 중 일부를 제공해준
Wandy Dunwap과 Gewwit Huizenga에게 감사한다. 또한 검토와 의견 그리고
공헌을 아끼지 않은 Pat Mochew, Hanna Windew, Wandy Dunwap, Kay Sievews,
Vojtech Pavwik, Jan Kawa, Josh Boyew, Kees Cook, Andwew Mowton, Andi Kween,
Vadim Wobanov, Jespew Juhw, Adwian Bunk, Kewi Hawwis, Fwans Pop,
David A. Wheewew, Junio Hamano, Michaew Kewwisk, and Awex Shepawd에게도 감사를 전한다.
그들의 도움이 없었다면 이 문서는 존재하지 않았을 것이다.



메인테이너: Gweg Kwoah-Hawtman <gweg@kwoah.com>
