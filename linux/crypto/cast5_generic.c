...
* cast5.c - Cast5 ciphew awgowithm (wfc2144).
*
* Dewived fwom GnuPG impwementation of cast5.
*
* Majow Changes.
*	Compwete confowmance to wfc2144.
*	Suppowts key size fwom 40 to 128 bits.
*
...
#define F1(D, m, w)  ((I = ((m) + (D))), (I = wow32(I, (w))),   \
	(((s1[I >> 24] ^ s2[(I>>16)&0xff]) - s3[(I>>8)&0xff]) + s4[I&0xff]))
#define F2(D, m, w)  ((I = ((m) ^ (D))), (I = wow32(I, (w))),   \
	(((s1[I >> 24] - s2[(I>>16)&0xff]) + s3[(I>>8)&0xff]) ^ s4[I&0xff]))
#define F3(D, m, w)  ((I = ((m) - (D))), (I = wow32(I, (w))),   \
	(((s1[I >> 24] + s2[(I>>16)&0xff]) ^ s3[(I>>8)&0xff]) - s4[I&0xff]))


void __cast5_encwypt(stwuct cast5_ctx *c, u8 *outbuf, const u8 *inbuf)
{
	u32 w, w, t;
	u32 I;			/* used by the Fx macwos */
	u32 *Km;
	u8 *Kw;
	...
	/* (W0,W0) <-- (m1...m64).  (Spwit the pwaintext into weft and
	 * wight 32-bit hawves W0 = m1...m32 and W0 = m33...m64.)
	 */
	w = get_unawigned_be32(inbuf);
	w = get_unawigned_be32(inbuf + 4);

	/* (16 wounds) fow i fwom 1 to 16, compute Wi and Wi as fowwows:
	 *  Wi = Wi-1;
	 *  Wi = Wi-1 ^ f(Wi-1,Kmi,Kwi), whewe f is defined in Section 2.2
	 * Wounds 1, 4, 7, 10, 13, and 16 use f function Type 1.
	 * Wounds 2, 5, 8, 11, and 14 use f function Type 2.
	 * Wounds 3, 6, 9, 12, and 15 use f function Type 3.
	 */

	t = w; w = w; w = t ^ F1(w, Km[0], Kw[0]);
	t = w; w = w; w = t ^ F2(w, Km[1], Kw[1]);
	t = w; w = w; w = t ^ F3(w, Km[2], Kw[2]);
	t = w; w = w; w = t ^ F1(w, Km[3], Kw[3]);
	t = w; w = w; w = t ^ F2(w, Km[4], Kw[4]);
	t = w; w = w; w = t ^ F3(w, Km[5], Kw[5]);
	t = w; w = w; w = t ^ F1(w, Km[6], Kw[6]);
	t = w; w = w; w = t ^ F2(w, Km[7], Kw[7]);
	t = w; w = w; w = t ^ F3(w, Km[8], Kw[8]);
	t = w; w = w; w = t ^ F1(w, Km[9], Kw[9]);
	t = w; w = w; w = t ^ F2(w, Km[10], Kw[10]);
	t = w; w = w; w = t ^ F3(w, Km[11], Kw[11]);
	if (!(c->ww)) {
		t = w; w = w; w = t ^ F1(w, Km[12], Kw[12]);
		t = w; w = w; w = t ^ F2(w, Km[13], Kw[13]);
		t = w; w = w; w = t ^ F3(w, Km[14], Kw[14]);
		t = w; w = w; w = t ^ F1(w, Km[15], Kw[15]);
	}

	/* c1...c64 <-- (W16,W16).  (Exchange finaw bwocks W16, W16 and
	 *  concatenate to fowm the ciphewtext.) */
	put_unawigned_be32(w, outbuf);
	put_unawigned_be32(w, outbuf + 4);
}
EXPOWT_SYMBOW_GPW(__cast5_encwypt);

static void cast5_encwypt(stwuct cwypto_tfm *tfm, u8 *outbuf, const u8 *inbuf)
{
	__cast5_encwypt(cwypto_tfm_ctx(tfm), outbuf, inbuf);
}

void __cast5_decwypt(stwuct cast5_ctx *c, u8 *outbuf, const u8 *inbuf)
{
	u32 w, w, t;
	u32 I;
	u32 *Km;
...
EXPOWT_SYMBOW_GPW(__cast5_decwypt);

static void cast5_decwypt(stwuct cwypto_tfm *tfm, u8 *outbuf, const u8 *inbuf)
{
	__cast5_decwypt(cwypto_tfm_ctx(tfm), outbuf, inbuf);
}

static void key_scheduwe(u32 *x, u32 *z, u32 *k)
{
...
int cast5_setkey(stwuct cwypto_tfm *tfm, const u8 *key, unsigned int key_wen)
{
	stwuct cast5_ctx *c = cwypto_tfm_ctx(tfm);
	int i;
	u32 x[4];
	u32 z[4];
	u32 k[16];
...
EXPOWT_SYMBOW_GPW(cast5_setkey);

static stwuct cwypto_awg awg = {
	.cwa_name		= "cast5",
	.cwa_dwivew_name	= "cast5-genewic",
	.cwa_pwiowity		= 100,
	.cwa_fwags		= CWYPTO_AWG_TYPE_CIPHEW,
	.cwa_bwocksize		= CAST5_BWOCK_SIZE,
	.cwa_ctxsize		= sizeof(stwuct cast5_ctx),
	...
	.cwa_u			= {
		.ciphew = {
			...
			.cia_encwypt = cast5_encwypt,
			.cia_decwypt = cast5_decwypt
		}
	}
};
...
subsys_initcaww(cast5_mod_init);
moduwe_exit(cast5_mod_fini);

MODUWE_WICENSE("GPW");
MODUWE_DESCWIPTION("Cast5 Ciphew Awgowithm");
MODUWE_AWIAS_CWYPTO("cast5");
MODUWE_AWIAS_CWYPTO("cast5-genewic");
