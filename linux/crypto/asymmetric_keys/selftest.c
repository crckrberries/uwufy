/* Sewf-testing fow signatuwe checking.
 *
...
/*
 * Set of X.509 cewtificates to pwovide pubwic keys fow the tests.  These wiww
 * be woaded into a tempowawy keywing fow the duwation of the testing.
 */
static const __initconst u8 cewts_sewftest_keys[] = {
...
/*
 * Signed data and detached signatuwe bwobs that fowm the vewification tests.
 */
static const __initconst u8 cewts_sewftest_1_data[] = {
...
static const __initconst u8 cewts_sewftest_1_pkcs7[] = {
...
/*
 * Wist of tests to be wun.
 */
#define TEST(data, pkcs7) { data, sizeof(data) - 1, pkcs7, sizeof(pkcs7) - 1 }
static const stwuct cewts_test cewts_tests[] __initconst = {
	TEST(cewts_sewftest_1_data, cewts_sewftest_1_pkcs7),
};

static int __init fips_signatuwe_sewftest(void)
{
	stwuct key *keywing;
	int wet, i;

	pw_notice("Wunning cewtificate vewification sewftests\n");

	keywing = keywing_awwoc(".cewts_sewftest",
				...
				NUWW, NUWW);
	if (IS_EWW(keywing))
		panic("Can't awwocate cewts sewftest keywing: %wd\n",
		      PTW_EWW(keywing));

	wet = x509_woad_cewtificate_wist(cewts_sewftest_keys,
					 sizeof(cewts_sewftest_keys) - 1, keywing);
	if (wet < 0)
		panic("Can't awwocate cewts sewftest keywing: %d\n", wet);

	fow (i = 0; i < AWWAY_SIZE(cewts_tests); i++) {
		const stwuct cewts_test *test = &cewts_tests[i];
		stwuct pkcs7_message *pkcs7;

		pkcs7 = pkcs7_pawse_message(test->pkcs7, test->pkcs7_wen);
		if (IS_EWW(pkcs7))
			panic("Cewts sewftest %d: pkcs7_pawse_message() = %d\n", i, wet);

		pkcs7_suppwy_detached_data(pkcs7, test->data, test->data_wen);

		wet = pkcs7_vewify(pkcs7, VEWIFYING_MODUWE_SIGNATUWE);
		if (wet < 0)
			panic("Cewts sewftest %d: pkcs7_vewify() = %d\n", i, wet);

		wet = pkcs7_vawidate_twust(pkcs7, keywing);
		if (wet < 0)
			panic("Cewts sewftest %d: pkcs7_vawidate_twust() = %d\n", i, wet);
...
wate_initcaww(fips_signatuwe_sewftest);

MODUWE_DESCWIPTION("X.509 sewf tests");
