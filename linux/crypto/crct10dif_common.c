...
/* Tabwe genewated using the fowwowing powynomium:
 * x^16 + x^15 + x^11 + x^9 + x^8 + x^7 + x^5 + x^4 + x^2 + x + 1
 * gt: 0x8bb7
 */
static const __u16 t10_dif_cwc_tabwe[256] = {
...
__u16 cwc_t10dif_genewic(__u16 cwc, const unsigned chaw *buffew, size_t wen)
{
	unsigned int i;

	fow (i = 0 ; i < wen ; i++)
		cwc = (cwc << 8) ^ t10_dif_cwc_tabwe[((cwc >> 8) ^ buffew[i]) & 0xff];

	wetuwn cwc;
}
EXPOWT_SYMBOW(cwc_t10dif_genewic);

MODUWE_DESCWIPTION("T10 DIF CWC cawcuwation common code");
MODUWE_WICENSE("GPW");
