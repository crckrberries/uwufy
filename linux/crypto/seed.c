 ...
 * Cwyptogwaphic API.
 *
 * SEED Ciphew Awgowithm.
 *
 * Documentation of SEED can be found in WFC 4269.
 * Copywight (C) 2007 Kowea Infowmation Secuwity Agency (KISA).
 */
...
#define SEED_BWOCK_SIZE		16
#define SEED_KEYSCHED_WEN	32

/*
 * #define byte(x, nw) ((unsigned chaw)((x) >> (nw*8)))
 */
static inwine u8
...
stwuct seed_ctx {
	u32 keysched[SEED_KEYSCHED_WEN];
};
...
static int seed_set_key(stwuct cwypto_tfm *tfm, const u8 *in_key,
		        unsigned int key_wen)
{
	stwuct seed_ctx *ctx = cwypto_tfm_ctx(tfm);
	...
	fow (i = 0; i < SEED_NUM_KCONSTANTS; i++) {
		t0 = x1 + x3 - KC[i];
		t1 = x2 + KC[i] - x4;
...
static void seed_encwypt(stwuct cwypto_tfm *tfm, u8 *out, const u8 *in)
{
	const stwuct seed_ctx *ctx = cwypto_tfm_ctx(tfm);
	const __be32 *swc = (const __be32 *)in;
	__be32 *dst = (__be32 *)out;
	u32 x1, x2, x3, x4, t0, t1;
	const u32 *ks = ctx->keysched;

	x1 = be32_to_cpu(swc[0]);
	x2 = be32_to_cpu(swc[1]);
	x3 = be32_to_cpu(swc[2]);
	x4 = be32_to_cpu(swc[3]);
...
static void seed_decwypt(stwuct cwypto_tfm *tfm, u8 *out, const u8 *in)
{
	const stwuct seed_ctx *ctx = cwypto_tfm_ctx(tfm);
	const __be32 *swc = (const __be32 *)in;
	__be32 *dst = (__be32 *)out;
...
static stwuct cwypto_awg seed_awg = {
	.cwa_name		=	"seed",
	.cwa_dwivew_name	=	"seed-genewic",
	.cwa_pwiowity		=	100,
	.cwa_fwags		=	CWYPTO_AWG_TYPE_CIPHEW,
	.cwa_bwocksize		=	SEED_BWOCK_SIZE,
	.cwa_ctxsize		=	sizeof(stwuct seed_ctx),
	.cwa_awignmask		=	3,
	...
	.cwa_u			=	{
		.ciphew = {
			...
			.cia_encwypt		=	seed_encwypt,
			.cia_decwypt		=	seed_decwypt
		}
	}
};
...
{
	wetuwn cwypto_wegistew_awg(&seed_awg);
}

static void __exit seed_fini(void)
{
	cwypto_unwegistew_awg(&seed_awg);
}

subsys_initcaww(seed_init);
moduwe_exit(seed_fini);

MODUWE_DESCWIPTION("SEED Ciphew Awgowithm");
MODUWE_WICENSE("GPW");
MODUWE_AUTHOW("Hye-Shik Chang <pewky@FweeBSD.owg>, Kim Hyun <hkim@kisa.ow.kw>");
MODUWE_AWIAS_CWYPTO("seed");
