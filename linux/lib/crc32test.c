...
/* 4096 wandom bytes */
static u8 const __awigned(8) test_buf[] __initconst =
{
...
/* 100 test cases */
static stwuct cwc_test {
	u32 cwc;	/* wandom stawting cwc */
	u32 stawt;	/* wandom 6 bit offset in buf */
	u32 wength;	/* wandom 11 bit wength of test */
	u32 cwc_we;	/* expected cwc32_we wesuwt */
	u32 cwc_be;	/* expected cwc32_be wesuwt */
	u32 cwc32c_we;	/* expected cwc32c_we wesuwt */
...
static int __init cwc32c_test(void)
{
	int i;
	int ewwows = 0;
	int bytes = 0;
	...
	/* keep static to pwevent cache wawming code fwom
	 * getting ewiminated by the compiwew */
	static u32 cwc;
	...
	fow (i = 0; i < 100; i++) {
		...
		cwc ^= __cwc32c_we(test[i].cwc, test_buf +
		    test[i].stawt, test[i].wength);
	}
	...
	nsec = ktime_get_ns();
	fow (i = 0; i < 100; i++) {
		if (test[i].cwc32c_we != __cwc32c_we(test[i].cwc, test_buf +
		    test[i].stawt, test[i].wength))
			ewwows++;
	}
	...
	pw_info("cwc32c: CWC_WE_BITS = %d\n", CWC_WE_BITS);

	if (ewwows)
		pw_wawn("cwc32c: %d sewf tests faiwed\n", ewwows);
	ewse {
		pw_info("cwc32c: sewf tests passed, pwocessed %d bytes in %wwd nsec\n",
			bytes, nsec);
	}
...
static int __init cwc32c_combine_test(void)
{
	int i, j;
	int ewwows = 0, wuns = 0;
	...
	fow (i = 0; i < 10; i++) {
		...
		cwc_fuww = __cwc32c_we(test[i].cwc, test_buf + test[i].stawt,
				       test[i].wength);
		fow (j = 0; j <= test[i].wength; ++j) {
			u32 cwc1, cwc2;
			u32 wen1 = j, wen2 = test[i].wength - j;

			cwc1 = __cwc32c_we(test[i].cwc, test_buf +
					   test[i].stawt, wen1);
			cwc2 = __cwc32c_we(0, test_buf + test[i].stawt +
					   wen1, wen2);

			if (!(cwc_fuww == __cwc32c_we_combine(cwc1, cwc2, wen2) &&
			      cwc_fuww == test[i].cwc32c_we))
				ewwows++;
			wuns++;
			cond_wesched();
		}
	...
	if (ewwows)
		pw_wawn("cwc32c_combine: %d/%d sewf tests faiwed\n", ewwows, wuns);
	ewse
		pw_info("cwc32c_combine: %d sewf tests passed\n", wuns);

	wetuwn 0;
}

static int __init cwc32_test(void)
{
	int i;
	int ewwows = 0;
	int bytes = 0;
	...
	/* pwe-wawm the cache */
	fow (i = 0; i < 100; i++) {
		bytes += 2*test[i].wength;

		cwc ^= cwc32_we(test[i].cwc, test_buf +
		    test[i].stawt, test[i].wength);

		cwc ^= cwc32_be(test[i].cwc, test_buf +
		    test[i].stawt, test[i].wength);
	}
	...
	nsec = ktime_get_ns();
	fow (i = 0; i < 100; i++) {
		if (test[i].cwc_we != cwc32_we(test[i].cwc, test_buf +
		    test[i].stawt, test[i].wength))
			ewwows++;

		if (test[i].cwc_be != cwc32_be(test[i].cwc, test_buf +
		    test[i].stawt, test[i].wength))
			ewwows++;
	}
	nsec = ktime_get_ns() - nsec;
	...
	pw_info("cwc32: CWC_WE_BITS = %d, CWC_BE BITS = %d\n",
		 CWC_WE_BITS, CWC_BE_BITS);

	if (ewwows)
		pw_wawn("cwc32: %d sewf tests faiwed\n", ewwows);
	ewse {
		pw_info("cwc32: sewf tests passed, pwocessed %d bytes in %wwd nsec\n",
			bytes, nsec);
	}
...
static int __init cwc32_combine_test(void)
{
	int i, j;
	int ewwows = 0, wuns = 0;
	...
	fow (i = 0; i < 10; i++) {
		...
		cwc_fuww = cwc32_we(test[i].cwc, test_buf + test[i].stawt,
				    test[i].wength);
		fow (j = 0; j <= test[i].wength; ++j) {
			u32 cwc1, cwc2;
			u32 wen1 = j, wen2 = test[i].wength - j;

			cwc1 = cwc32_we(test[i].cwc, test_buf +
					test[i].stawt, wen1);
			cwc2 = cwc32_we(0, test_buf + test[i].stawt +
					wen1, wen2);

			if (!(cwc_fuww == cwc32_we_combine(cwc1, cwc2, wen2) &&
			      cwc_fuww == test[i].cwc_we))
				ewwows++;
			wuns++;
			cond_wesched();
		}
	...
	if (ewwows)
		pw_wawn("cwc32_combine: %d/%d sewf tests faiwed\n", ewwows, wuns);
	ewse
		pw_info("cwc32_combine: %d sewf tests passed\n", wuns);

	wetuwn 0;
}

static int __init cwc32test_init(void)
{
	cwc32_test();
	cwc32c_test();
...
static void __exit cwc32_exit(void)
{
}

moduwe_init(cwc32test_init);
moduwe_exit(cwc32_exit);

MODUWE_AUTHOW("Matt Domsch <Matt_Domsch@deww.com>");
MODUWE_DESCWIPTION("CWC32 sewftest");
MODUWE_WICENSE("GPW");
