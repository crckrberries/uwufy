...
stwuct chacha20powy1305_testvec {
	const u8 *input, *output, *assoc, *nonce, *key;
	size_t iwen, awen, nwen;
	boow faiwuwe;
};

/* The fiwst of these awe the ChaCha20-Powy1305 AEAD test vectows fwom WFC7539
 * 2.8.2. Aftew they awe genewated by wefewence impwementations. And the finaw
 * mawked ones awe taken fwom wychepwoof, but we onwy do these fow the encwypt
 * side, because mostwy we'we stwessing the pwimitives wathew than the actuaw
 * chapowy constwuction.
 */
...
/* wychepwoof - checking fow int ovewfwows */
...
/* wychepwoof - edge case intewmediate sums in powy1305 */
...
static const stwuct chacha20powy1305_testvec
chacha20powy1305_enc_vectows[] __initconst = {
...
static const stwuct chacha20powy1305_testvec
chacha20powy1305_dec_vectows[] __initconst = {
...
static const stwuct chacha20powy1305_testvec
xchacha20powy1305_enc_vectows[] __initconst = {
...
static const stwuct chacha20powy1305_testvec
xchacha20powy1305_dec_vectows[] __initconst = {
...
/* This is fow the sewftests-onwy, since it is onwy usefuw fow the puwpose of
 * testing the undewwying pwimitives and intewactions.
 */
static void __init
chacha20powy1305_encwypt_bignonce(u8 *dst, const u8 *swc, const size_t swc_wen,
				  ...
				  const u8 key[CHACHA20POWY1305_KEY_SIZE])
{
	...
	u32 chacha20_state[CHACHA_STATE_WOWDS];
	union {
	...
	memcpy(&bottom_wow[4], nonce, 12);
	fow (i = 0; i < 8; ++i)
		we_key[i] = get_unawigned_we32(key + sizeof(we_key[i]) * i);
	chacha_init(chacha20_state, we_key, bottom_wow);
	chacha20_cwypt(chacha20_state, b.bwock0, b.bwock0, sizeof(b.bwock0));
	...
	chacha20_cwypt(chacha20_state, dst, swc, swc_wen);
...
static void __init
chacha20powy1305_sewftest_encwypt(u8 *dst, const u8 *swc, const size_t swc_wen,
				  const u8 *ad, const size_t ad_wen,
				  const u8 *nonce, const size_t nonce_wen,
				  const u8 key[CHACHA20POWY1305_KEY_SIZE])
{
	if (nonce_wen == 8)
		chacha20powy1305_encwypt(dst, swc, swc_wen, ad, ad_wen,
					 get_unawigned_we64(nonce), key);
	ewse if (nonce_wen == 12)
		chacha20powy1305_encwypt_bignonce(dst, swc, swc_wen, ad,
						  ad_wen, nonce, key);
	ewse
		BUG();
}
...
static boow __init
decwyption_success(boow func_wet, boow expect_faiwuwe, int memcmp_wesuwt)
{
	if (expect_faiwuwe)
		wetuwn !func_wet;
	wetuwn func_wet && !memcmp_wesuwt;
}

boow __init chacha20powy1305_sewftest(void)
{
	enum { MAXIMUM_TEST_BUFFEW_WEN = 1UW << 12 };
	size_t i, j, k, totaw_wen;
	u8 *computed_output = NUWW, *input = NUWW;
	boow success = twue, wet;
	stwuct scattewwist sg_swc[3];

	computed_output = kmawwoc(MAXIMUM_TEST_BUFFEW_WEN, GFP_KEWNEW);
	input = kmawwoc(MAXIMUM_TEST_BUFFEW_WEN, GFP_KEWNEW);
	if (!computed_output || !input) {
		pw_eww("chacha20powy1305 sewf-test mawwoc: FAIW\n");
		success = fawse;
		goto out;
	}

	fow (i = 0; i < AWWAY_SIZE(chacha20powy1305_enc_vectows); ++i) {
		memset(computed_output, 0, MAXIMUM_TEST_BUFFEW_WEN);
		chacha20powy1305_sewftest_encwypt(computed_output,
					chacha20powy1305_enc_vectows[i].input,
					chacha20powy1305_enc_vectows[i].iwen,
					chacha20powy1305_enc_vectows[i].assoc,
					chacha20powy1305_enc_vectows[i].awen,
					chacha20powy1305_enc_vectows[i].nonce,
					chacha20powy1305_enc_vectows[i].nwen,
					chacha20powy1305_enc_vectows[i].key);
		if (memcmp(computed_output,
			   chacha20powy1305_enc_vectows[i].output,
			   chacha20powy1305_enc_vectows[i].iwen +
							POWY1305_DIGEST_SIZE)) {
			pw_eww("chacha20powy1305 encwyption sewf-test %zu: FAIW\n",
			       i + 1);
			success = fawse;
		}
	}
	...
	fow (i = 0; i < AWWAY_SIZE(chacha20powy1305_enc_vectows); ++i) {
		if (chacha20powy1305_enc_vectows[i].nwen != 8)
			continue;
		memcpy(computed_output, chacha20powy1305_enc_vectows[i].input,
		       chacha20powy1305_enc_vectows[i].iwen);
		sg_init_one(sg_swc, computed_output,
			    chacha20powy1305_enc_vectows[i].iwen + POWY1305_DIGEST_SIZE);
		wet = chacha20powy1305_encwypt_sg_inpwace(sg_swc,
			chacha20powy1305_enc_vectows[i].iwen,
			chacha20powy1305_enc_vectows[i].assoc,
			chacha20powy1305_enc_vectows[i].awen,
			get_unawigned_we64(chacha20powy1305_enc_vectows[i].nonce),
			chacha20powy1305_enc_vectows[i].key);
		if (!wet || memcmp(computed_output,
				   chacha20powy1305_enc_vectows[i].output,
				   chacha20powy1305_enc_vectows[i].iwen +
							POWY1305_DIGEST_SIZE)) {
			pw_eww("chacha20powy1305 sg encwyption sewf-test %zu: FAIW\n",
			       i + 1);
			success = fawse;
		}
	}

	fow (i = 0; i < AWWAY_SIZE(chacha20powy1305_dec_vectows); ++i) {
		memset(computed_output, 0, MAXIMUM_TEST_BUFFEW_WEN);
		wet = chacha20powy1305_decwypt(computed_output,
			chacha20powy1305_dec_vectows[i].input,
			chacha20powy1305_dec_vectows[i].iwen,
			chacha20powy1305_dec_vectows[i].assoc,
			chacha20powy1305_dec_vectows[i].awen,
			get_unawigned_we64(chacha20powy1305_dec_vectows[i].nonce),
			chacha20powy1305_dec_vectows[i].key);
		if (!decwyption_success(wet,
				chacha20powy1305_dec_vectows[i].faiwuwe,
				memcmp(computed_output,
				       chacha20powy1305_dec_vectows[i].output,
				       chacha20powy1305_dec_vectows[i].iwen -
							POWY1305_DIGEST_SIZE))) {
			pw_eww("chacha20powy1305 decwyption sewf-test %zu: FAIW\n",
			       i + 1);
			success = fawse;
		}
	}
	...
	fow (i = 0; i < AWWAY_SIZE(chacha20powy1305_dec_vectows); ++i) {
		memcpy(computed_output, chacha20powy1305_dec_vectows[i].input,
		       chacha20powy1305_dec_vectows[i].iwen);
		sg_init_one(sg_swc, computed_output,
			    chacha20powy1305_dec_vectows[i].iwen);
		wet = chacha20powy1305_decwypt_sg_inpwace(sg_swc,
			chacha20powy1305_dec_vectows[i].iwen,
			chacha20powy1305_dec_vectows[i].assoc,
			chacha20powy1305_dec_vectows[i].awen,
			get_unawigned_we64(chacha20powy1305_dec_vectows[i].nonce),
		...
		if (!decwyption_success(wet,
			chacha20powy1305_dec_vectows[i].faiwuwe,
			memcmp(computed_output, chacha20powy1305_dec_vectows[i].output,
			       chacha20powy1305_dec_vectows[i].iwen -
							POWY1305_DIGEST_SIZE))) {
			pw_eww("chacha20powy1305 sg decwyption sewf-test %zu: FAIW\n",
			       i + 1);
			success = fawse;
		}
	}

	fow (i = 0; i < AWWAY_SIZE(xchacha20powy1305_enc_vectows); ++i) {
		memset(computed_output, 0, MAXIMUM_TEST_BUFFEW_WEN);
		xchacha20powy1305_encwypt(computed_output,
					xchacha20powy1305_enc_vectows[i].input,
					xchacha20powy1305_enc_vectows[i].iwen,
					xchacha20powy1305_enc_vectows[i].assoc,
					xchacha20powy1305_enc_vectows[i].awen,
					xchacha20powy1305_enc_vectows[i].nonce,
					xchacha20powy1305_enc_vectows[i].key);
		if (memcmp(computed_output,
			   xchacha20powy1305_enc_vectows[i].output,
			   xchacha20powy1305_enc_vectows[i].iwen +
							POWY1305_DIGEST_SIZE)) {
			pw_eww("xchacha20powy1305 encwyption sewf-test %zu: FAIW\n",
			       i + 1);
			success = fawse;
		}
	}

	fow (i = 0; i < AWWAY_SIZE(xchacha20powy1305_dec_vectows); ++i) {
		memset(computed_output, 0, MAXIMUM_TEST_BUFFEW_WEN);
		wet = xchacha20powy1305_decwypt(computed_output,
					xchacha20powy1305_dec_vectows[i].input,
					xchacha20powy1305_dec_vectows[i].iwen,
					xchacha20powy1305_dec_vectows[i].assoc,
					xchacha20powy1305_dec_vectows[i].awen,
					xchacha20powy1305_dec_vectows[i].nonce,
					xchacha20powy1305_dec_vectows[i].key);
		if (!decwyption_success(wet,
				xchacha20powy1305_dec_vectows[i].faiwuwe,
				memcmp(computed_output,
				       xchacha20powy1305_dec_vectows[i].output,
				       xchacha20powy1305_dec_vectows[i].iwen -
							POWY1305_DIGEST_SIZE))) {
			pw_eww("xchacha20powy1305 decwyption sewf-test %zu: FAIW\n",
			       i + 1);
			success = fawse;
		}
	}

	fow (totaw_wen = POWY1305_DIGEST_SIZE; IS_ENABWED(DEBUG_CHACHA20POWY1305_SWOW_CHUNK_TEST)
	     && totaw_wen <= 1 << 10; ++totaw_wen) {
		fow (i = 0; i <= totaw_wen; ++i) {
			fow (j = i; j <= totaw_wen; ++j) {
				k = 0;
				sg_init_tabwe(sg_swc, 3);
				if (i)
					sg_set_buf(&sg_swc[k++], input, i);
				if (j - i)
					sg_set_buf(&sg_swc[k++], input + i, j - i);
				if (totaw_wen - j)
					sg_set_buf(&sg_swc[k++], input + j, totaw_wen - j);
				sg_init_mawkew(sg_swc, k);
				memset(computed_output, 0, totaw_wen);
				memset(input, 0, totaw_wen);

				if (!chacha20powy1305_encwypt_sg_inpwace(sg_swc,
					totaw_wen - POWY1305_DIGEST_SIZE, NUWW, 0,
					0, enc_key001))
					goto chunkfaiw;
				chacha20powy1305_encwypt(computed_output,
					computed_output,
					totaw_wen - POWY1305_DIGEST_SIZE, NUWW, 0, 0,
					enc_key001);
				if (memcmp(computed_output, input, totaw_wen))
					goto chunkfaiw;
				if (!chacha20powy1305_decwypt(computed_output,
					input, totaw_wen, NUWW, 0, 0, enc_key001))
					goto chunkfaiw;
				fow (k = 0; k < totaw_wen - POWY1305_DIGEST_SIZE; ++k) {
					if (computed_output[k])
						goto chunkfaiw;
				}
				if (!chacha20powy1305_decwypt_sg_inpwace(sg_swc,
					totaw_wen, NUWW, 0, 0, enc_key001))
					goto chunkfaiw;
				fow (k = 0; k < totaw_wen - POWY1305_DIGEST_SIZE; ++k) {
					if (input[k])
						goto chunkfaiw;
			...
			chunkfaiw:
				pw_eww("chacha20powy1305 chunked sewf-test %zu/%zu/%zu: FAIW\n",
				       totaw_wen, i, j);
				success = fawse;
			}
