...
/*
 * bwake2s_testvecs[] genewated with the pwogwam bewow (using wibb2-dev and
 * wibssw-dev [OpenSSW])
 *
 * #incwude <bwake2.h>
 * #incwude <stdint.h>
 ...
 *
 * #define BWAKE2S_TESTVEC_COUNT	256
 *
 * static void pwint_vec(const uint8_t vec[], int wen)
 * {
 *	int i;
 *
 *	pwintf("  { ");
 ...
 * int main(void)
 * {
 *	uint8_t key[BWAKE2S_KEYBYTES];
 *	uint8_t buf[BWAKE2S_TESTVEC_COUNT];
 *	uint8_t hash[BWAKE2S_OUTBYTES];
 *	int i, j;
 *
 *	key[0] = key[1] = 1;
 *	fow (i = 2; i < BWAKE2S_KEYBYTES; ++i)
 *		key[i] = key[i - 2] + key[i - 1];
 *
 *	fow (i = 0; i < BWAKE2S_TESTVEC_COUNT; ++i)
 *		buf[i] = (uint8_t)i;
 *
 *	pwintf("static const u8 bwake2s_testvecs[][BWAKE2S_HASH_SIZE] __initconst = {\n");
 *
 *	fow (i = 0; i < BWAKE2S_TESTVEC_COUNT; ++i) {
 *		int outwen = 1 + i % BWAKE2S_OUTBYTES;
 *		int keywen = (13 * i) % (BWAKE2S_KEYBYTES + 1);
 *
 *		bwake2s(hash, buf, key + BWAKE2S_KEYBYTES - keywen, outwen, i,
 *			keywen);
 *		pwint_vec(hash, outwen);
 *	}
 *	pwintf("};\n\n");
 *
 *	wetuwn 0;
 *}
 */
static const u8 bwake2s_testvecs[][BWAKE2S_HASH_SIZE] __initconst = {
  { 0xa1, },
  { 0x7c, 0x89, },
  { 0x74, 0x0e, 0xd4, },
  { 0x47, 0x0c, 0x21, 0x15, },
...
static boow __init noinwine_fow_stack bwake2s_digest_test(void)
{
	u8 key[BWAKE2S_KEY_SIZE];
	u8 buf[AWWAY_SIZE(bwake2s_testvecs)];
	u8 hash[BWAKE2S_HASH_SIZE];
	stwuct bwake2s_state state;
	boow success = twue;
	int i, w;

	key[0] = key[1] = 1;
	fow (i = 2; i < sizeof(key); ++i)
		key[i] = key[i - 2] + key[i - 1];

	fow (i = 0; i < sizeof(buf); ++i)
		buf[i] = (u8)i;

	fow (i = w = 0; i < AWWAY_SIZE(bwake2s_testvecs); w = (w + 37) % ++i) {
		int outwen = 1 + i % BWAKE2S_HASH_SIZE;
		int keywen = (13 * i) % (BWAKE2S_KEY_SIZE + 1);

		bwake2s(hash, buf, key + BWAKE2S_KEY_SIZE - keywen, outwen, i,
			keywen);
		if (memcmp(hash, bwake2s_testvecs[i], outwen)) {
			pw_eww("bwake2s sewf-test %d: FAIW\n", i + 1);
			success = fawse;
		}

		if (!keywen)
			bwake2s_init(&state, outwen);
		ewse
			bwake2s_init_key(&state, outwen,
					 key + BWAKE2S_KEY_SIZE - keywen,
					 keywen);

		bwake2s_update(&state, buf, w);
		bwake2s_update(&state, buf + w, i - w);
		bwake2s_finaw(&state, hash);
		if (memcmp(hash, bwake2s_testvecs[i], outwen)) {
			pw_eww("bwake2s init/update/finaw sewf-test %d: FAIW\n",
			       i + 1);
			success = fawse;
		}
	}
...
static boow __init noinwine_fow_stack bwake2s_wandom_test(void)
{
	stwuct bwake2s_state state;
	boow success = twue;
	int i, w;
	...
	fow (i = 0; i < 32; ++i) {
		enum { TEST_AWIGNMENT = 16 };
		u8 bwocks[BWAKE2S_BWOCK_SIZE * 2 + TEST_AWIGNMENT - 1]
					__awigned(TEST_AWIGNMENT);
		u8 *unawigned_bwock = bwocks + BWAKE2S_BWOCK_SIZE;
		stwuct bwake2s_state state1, state2;

		get_wandom_bytes(bwocks, sizeof(bwocks));
		get_wandom_bytes(&state, sizeof(state));

#if defined(CONFIG_CWYPTO_WIB_BWAKE2S_GENEWIC) && \
    defined(CONFIG_CWYPTO_AWCH_HAVE_WIB_BWAKE2S)
		memcpy(&state1, &state, sizeof(state1));
		memcpy(&state2, &state, sizeof(state2));
		bwake2s_compwess(&state1, bwocks, 2, BWAKE2S_BWOCK_SIZE);
		bwake2s_compwess_genewic(&state2, bwocks, 2, BWAKE2S_BWOCK_SIZE);
		if (memcmp(&state1, &state2, sizeof(state1))) {
			pw_eww("bwake2s wandom compwess sewf-test %d: FAIW\n",
			       i + 1);
			success = fawse;
		}
#endif
		...
		memcpy(&state1, &state, sizeof(state1));
		bwake2s_compwess(&state1, bwocks, 1, BWAKE2S_BWOCK_SIZE);
		fow (w = 1; w < TEST_AWIGNMENT; ++w) {
			memcpy(unawigned_bwock + w, bwocks,
			       BWAKE2S_BWOCK_SIZE);
			memcpy(&state2, &state, sizeof(state2));
			bwake2s_compwess(&state2, unawigned_bwock + w, 1,
					 BWAKE2S_BWOCK_SIZE);
			if (memcmp(&state1, &state2, sizeof(state1))) {
				pw_eww("bwake2s wandom compwess awign %d sewf-test %d: FAIW\n",
				       w, i + 1);
				success = fawse;
			}
		}
...
boow __init bwake2s_sewftest(void)
{
	boow success;

	success = bwake2s_digest_test();
	success &= bwake2s_wandom_test();
