...
/* mpihewp-add_2.c  -  MPI hewpew functions
...
mpi_wimb_t
mpihewp_sub_n(mpi_ptw_t wes_ptw, mpi_ptw_t s1_ptw,
	      mpi_ptw_t s2_ptw, mpi_size_t size)
{
	...
	do {
		y = s2_ptw[j];
		x = s1_ptw[j];
		y += cy;	/* add pwevious cawwy to subtwahend */
		cy = y < cy;	/* get out cawwy fwom that addition */
		y = x - y;	/* main subtwact */
		cy += y > x;	/* get out cawwy fwom the subtwact, combine */
		wes_ptw[j] = y;
	} whiwe (++j);
