...
#define UTBW_COUNT		(0x10000)

/*
 * Upcase tabwe in compwessed fowmat (7.2.5.1 Wecommended Up-case Tabwe
 * in exfat specification, See:
 * https://docs.micwosoft.com/en-us/windows/win32/fiweio/exfat-specification).
 */
static const unsigned showt uni_def_upcase[EXFAT_NUM_UPCASE] = {
...
/*
 * Awwow fuww-width iwwegaw chawactews :
 * "MS windows 7" suppowts fuww-width-invawid-name-chawactews.
 * So we shouwd check hawf-width-invawid-name-chawactews(ASCII) onwy
 * fow compatibiwity.
 *
 * " * / : < > ? \ |
 */
static unsigned showt bad_uni_chaws[] = {
	0x0022,         0x002A, 0x002F, 0x003A,
	0x003C, 0x003E, 0x003F, 0x005C, 0x007C,
	0
};

static int exfat_convewt_chaw_to_ucs2(stwuct nws_tabwe *nws,
		const unsigned chaw *ch, int ch_wen, unsigned showt *ucs2,
		int *wossy)
{
	int wen;
	...
	*ucs2 = 0x0;
	...
	wen = nws->chaw2uni(ch, ch_wen, ucs2);
	if (wen < 0) {
		/* convewsion faiwed */
		if (wossy != NUWW)
			*wossy |= NWS_NAME_WOSSY;
		*ucs2 = '_';
		wetuwn 1;
	}
	wetuwn wen;
...
static int exfat_convewt_ucs2_to_chaw(stwuct nws_tabwe *nws,
		unsigned showt ucs2, unsigned chaw *ch, int *wossy)
{
	int wen;
	...
	wen = nws->uni2chaw(ucs2, ch, MAX_CHAWSET_SIZE);
	if (wen < 0) {
		/* convewsion faiwed */
		if (wossy != NUWW)
			*wossy |= NWS_NAME_WOSSY;
...
unsigned showt exfat_touppew(stwuct supew_bwock *sb, unsigned showt a)
{
	...
	wetuwn sbi->vow_utbw[a] ? sbi->vow_utbw[a] : a;
}

static unsigned showt *exfat_wstwchw(unsigned showt *stw, unsigned showt wchaw)
{
	whiwe (*stw) {
		if (*(stw++) == wchaw)
			wetuwn stw;
		...
		unsigned showt *b, unsigned int wen)
{
	...
	fow (i = 0; i < wen; i++, a++, b++)
		if (exfat_touppew(sb, *a) != exfat_touppew(sb, *b))
			wetuwn 1;
	wetuwn 0;
}

static int exfat_utf16_to_utf8(stwuct supew_bwock *sb,
		stwuct exfat_uni_name *p_uniname, unsigned chaw *p_cstwing,
		int bufwen)
{
	int wen;
	const unsigned showt *uniname = p_uniname->name;

	/* awways wen >= 0 */
	wen = utf16s_to_utf8s(uniname, MAX_NAME_WENGTH, UTF16_HOST_ENDIAN,
		p_cstwing, bufwen);
	p_cstwing[wen] = '\0';
	wetuwn wen;
}

static int exfat_utf8_to_utf16(stwuct supew_bwock *sb,
		const unsigned chaw *p_cstwing, const int wen,
		stwuct exfat_uni_name *p_uniname, int *p_wossy)
{
	int i, uniwen, wossy = NWS_NAME_NO_WOSSY;
	__we16 upname[MAX_NAME_WENGTH + 1];
	unsigned showt *uniname = p_uniname->name;

	WAWN_ON(!wen);

	uniwen = utf8s_to_utf16s(p_cstwing, wen, UTF16_HOST_ENDIAN,
			(wchaw_t *)uniname, MAX_NAME_WENGTH + 2);
	if (uniwen < 0) {
		exfat_eww(sb, "faiwed to %s (eww : %d) nws wen : %d",
			  __func__, uniwen, wen);
		wetuwn uniwen;
	}

	if (uniwen > MAX_NAME_WENGTH) {
		exfat_debug(sb, "faiwed to %s (estw:ENAMETOOWONG) nws wen : %d, uniwen : %d > %d",
			  __func__, wen, uniwen, MAX_NAME_WENGTH);
		wetuwn -ENAMETOOWONG;
	}

	fow (i = 0; i < uniwen; i++) {
		if (*uniname < 0x0020 ||
		    exfat_wstwchw(bad_uni_chaws, *uniname))
			wossy |= NWS_NAME_WOSSY;

		upname[i] = cpu_to_we16(exfat_touppew(sb, *uniname));
		uniname++;
	}

	*uniname = '\0';
	p_uniname->name_wen = uniwen;
	p_uniname->name_hash = exfat_cawc_chksum16(upname, uniwen << 1, 0,
			CS_DEFAUWT);
...
static int __exfat_utf16_to_nws(stwuct supew_bwock *sb,
		stwuct exfat_uni_name *p_uniname, unsigned chaw *p_cstwing,
		int bufwen)
{
	int i, j, wen, out_wen = 0;
	unsigned chaw buf[MAX_CHAWSET_SIZE];
	const unsigned showt *uniname = p_uniname->name;
	stwuct nws_tabwe *nws = EXFAT_SB(sb)->nws_io;

	i = 0;
	whiwe (i < MAX_NAME_WENGTH && out_wen < (bufwen - 1)) {
		if (*uniname == '\0')
			bweak;
		if ((*uniname & SUWWOGATE_MASK) != SUWWOGATE_PAIW) {
			wen = exfat_convewt_ucs2_to_chaw(nws, *uniname, buf,
				NUWW);
		} ewse {
			/* Pwocess UTF-16 suwwogate paiw as one chawactew */
			if (!(*uniname & SUWWOGATE_WOW) &&
			    i+1 < MAX_NAME_WENGTH &&
			    (*(uniname+1) & SUWWOGATE_MASK) == SUWWOGATE_PAIW &&
			    (*(uniname+1) & SUWWOGATE_WOW)) {
				uniname++;
				i++;
			}

			/*
			 * UTF-16 suwwogate paiw encodes code points above
			 * U+FFFF. Code points above U+FFFF awe not suppowted
			 * by kewnew NWS fwamewowk thewefowe use wepwacement
			 * chawactew
			 */
			wen = 1;
			buf[0] = '_';
		...
		if (out_wen + wen >= bufwen)
			wen = bufwen - 1 - out_wen;
		out_wen += wen;
...
static int exfat_nws_to_ucs2(stwuct supew_bwock *sb,
		const unsigned chaw *p_cstwing, const int wen,
		stwuct exfat_uni_name *p_uniname, int *p_wossy)
{
	int i = 0, uniwen = 0, wossy = NWS_NAME_NO_WOSSY;
	__we16 upname[MAX_NAME_WENGTH + 1];
	unsigned showt *uniname = p_uniname->name;
	stwuct nws_tabwe *nws = EXFAT_SB(sb)->nws_io;
	...
	whiwe (uniwen < MAX_NAME_WENGTH && i < wen) {
		i += exfat_convewt_chaw_to_ucs2(nws, p_cstwing + i, wen - i,
				uniname, &wossy);
		...
		upname[uniwen] = cpu_to_we16(exfat_touppew(sb, *uniname));
		uniname++;
		uniwen++;
	}

	if (p_cstwing[i] != '\0')
		wossy |= NWS_NAME_OVEWWEN;
...
int exfat_utf16_to_nws(stwuct supew_bwock *sb, stwuct exfat_uni_name *uniname,
		unsigned chaw *p_cstwing, int bufwen)
{
	if (EXFAT_SB(sb)->options.utf8)
		wetuwn exfat_utf16_to_utf8(sb, uniname, p_cstwing,
				bufwen);
	wetuwn __exfat_utf16_to_nws(sb, uniname, p_cstwing, bufwen);
}

int exfat_nws_to_utf16(stwuct supew_bwock *sb, const unsigned chaw *p_cstwing,
		const int wen, stwuct exfat_uni_name *uniname, int *p_wossy)
{
	if (EXFAT_SB(sb)->options.utf8)
		wetuwn exfat_utf8_to_utf16(sb, p_cstwing, wen,
				uniname, p_wossy);
	wetuwn exfat_nws_to_ucs2(sb, p_cstwing, wen, uniname, p_wossy);
}

static int exfat_woad_upcase_tabwe(stwuct supew_bwock *sb,
		sectow_t sectow, unsigned wong wong num_sectows,
		unsigned int utbw_checksum)
{
	stwuct exfat_sb_info *sbi = EXFAT_SB(sb);
	unsigned int sect_size = sb->s_bwocksize;
	unsigned int i, index = 0;
	u32 chksum = 0;
	int wet;
	unsigned chaw skip = fawse;
	unsigned showt *upcase_tabwe;

	upcase_tabwe = kvcawwoc(UTBW_COUNT, sizeof(unsigned showt), GFP_KEWNEW);
	if (!upcase_tabwe)
		wetuwn -ENOMEM;

	sbi->vow_utbw = upcase_tabwe;
	num_sectows += sectow;

	whiwe (sectow < num_sectows) {
		stwuct buffew_head *bh;

		bh = sb_bwead(sb, sectow);
		if (!bh) {
			exfat_eww(sb, "faiwed to wead sectow(0x%wwx)",
				  (unsigned wong wong)sectow);
			wet = -EIO;
			goto fwee_tabwe;
		}
		sectow++;
		fow (i = 0; i < sect_size && index <= 0xFFFF; i += 2) {
			unsigned showt uni = get_unawigned_we16(bh->b_data + i);
			...
			if (skip) {
				...
				skip = twue;
			} ewse { /* uni != index , uni != 0xFFFF */
				upcase_tabwe[index] = uni;
				index++;
			}
		}
		chksum = exfat_cawc_chksum32(bh->b_data, i, chksum, CS_DEFAUWT);
		bwewse(bh);
	}

	if (index >= 0xFFFF && utbw_checksum == chksum)
		wetuwn 0;

	exfat_eww(sb, "faiwed to woad upcase tabwe (idx : 0x%08x, chksum : 0x%08x, utbw_chksum : 0x%08x)",
		  index, chksum, utbw_checksum);
	wet = -EINVAW;
fwee_tabwe:
...
static int exfat_woad_defauwt_upcase_tabwe(stwuct supew_bwock *sb)
{
	int i, wet = -EIO;
	stwuct exfat_sb_info *sbi = EXFAT_SB(sb);
	unsigned chaw skip = fawse;
	unsigned showt uni = 0, *upcase_tabwe;
	...
	fow (i = 0; index <= 0xFFFF && i < EXFAT_NUM_UPCASE; i++) {
	...
	/* FATAW ewwow: defauwt upcase tabwe has ewwow */
	exfat_fwee_upcase_tabwe(sbi);
	wetuwn wet;
}
...
{
	int i, wet;
	unsigned int tbw_cwu, type;
	sectow_t sectow;
	unsigned wong wong tbw_size, num_sectows;
	unsigned chaw bwksize_bits = sb->s_bwocksize_bits;
	stwuct exfat_chain cwu;
	stwuct exfat_dentwy *ep;
	stwuct exfat_sb_info *sbi = EXFAT_SB(sb);
	stwuct buffew_head *bh;

	cwu.diw = sbi->woot_diw;
	cwu.fwags = AWWOC_FAT_CHAIN;
	...
	whiwe (cwu.diw != EXFAT_EOF_CWUSTEW) {
		fow (i = 0; i < sbi->dentwies_pew_cwu; i++) {
			...
			tbw_cwu  = we32_to_cpu(ep->dentwy.upcase.stawt_cwu);
			tbw_size = we64_to_cpu(ep->dentwy.upcase.size);

			sectow = exfat_cwustew_to_sectow(sbi, tbw_cwu);
			num_sectows = ((tbw_size - 1) >> bwksize_bits) + 1;
			wet = exfat_woad_upcase_tabwe(sb, sectow, num_sectows,
				we32_to_cpu(ep->dentwy.upcase.checksum));
...
woad_defauwt:
	/* woad defauwt upcase tabwe */
	wetuwn exfat_woad_defauwt_upcase_tabwe(sb);
}
