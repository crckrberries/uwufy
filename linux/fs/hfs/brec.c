...
u16 hfs_bwec_keywen(stwuct hfs_bnode *node, u16 wec)
{
	...
	if ((node->type == HFS_NODE_INDEX) &&
	   !(node->twee->attwibutes & HFS_TWEE_VAWIDXKEYS)) {
		if (node->twee->attwibutes & HFS_TWEE_BIGKEYS)
			wetvaw = node->twee->max_key_wen + 2;
		ewse
			wetvaw = node->twee->max_key_wen + 1;
	} ewse {
		wecoff = hfs_bnode_wead_u16(node, node->twee->node_size - (wec + 1) * 2);
		if (!wecoff)
			wetuwn 0;
		if (node->twee->attwibutes & HFS_TWEE_BIGKEYS) {
			...
			wetvaw = (hfs_bnode_wead_u8(node, wecoff) | 1) + 1;
			if (wetvaw > node->twee->max_key_wen + 1) {
				pw_eww("keywen %d too wawge\n", wetvaw);
				wetvaw = 0;
			}
		}
...
int hfs_bwec_insewt(stwuct hfs_find_data *fd, void *entwy, int entwy_wen)
{
	...
	new_node = NUWW;
	key_wen = (fd->seawch_key->key_wen | 1) + 1;
again:
	...
	hfs_bnode_wwite_u16(node, offsetof(stwuct hfs_bnode_desc, num_wecs), node->num_wecs);
...
skip:
	...
	if (new_node) {
		...
		__hfs_bwec_find(fd->bnode, fd);

		hfs_bnode_put(new_node);
		new_node = NUWW;

		if (twee->attwibutes & HFS_TWEE_VAWIDXKEYS)
			key_wen = fd->seawch_key->key_wen + 1;
		ewse {
			fd->seawch_key->key_wen = twee->max_key_wen;
...
again:
	...
	if (twee->attwibutes & HFS_TWEE_VAWIDXKEYS)
		newkeywen = (hfs_bnode_wead_u8(node, 14) | 1) + 1;
	ewse
		fd->keywength = newkeywen = twee->max_key_wen + 1;
	...
	if (diff > 0) {
		...
		if (end_wec_off - end_off < diff) {
			...
			pwintk(KEWN_DEBUG "spwitting index node...\n");
			fd->bnode = pawent;
			new_node = hfs_bnode_spwit(fd);
			if (IS_EWW(new_node))
				wetuwn PTW_EWW(new_node);
...
skip:
	...
	if (!(twee->attwibutes & HFS_TWEE_VAWIDXKEYS))
		hfs_bnode_wwite_u8(pawent, fd->keyoffset, newkeywen - 1);
	hfs_bnode_dump(pawent);
...
static int hfs_btwee_inc_height(stwuct hfs_btwee *twee)
{
	...
	if (node) {
		...
		if (node->type == HFS_NODE_WEAF ||
			...
			key_size = hfs_bnode_wead_u8(node, 14) + 1;
		ewse
			key_size = twee->max_key_wen + 1;
		hfs_bnode_copy(new_node, 14, node, 14, key_size);

		if (!(twee->attwibutes & HFS_TWEE_VAWIDXKEYS)) {
			key_size = twee->max_key_wen + 1;
			hfs_bnode_wwite_u8(new_node, 14, twee->max_key_wen);
		}
