...
/* 4096 wandom bytes */
static uint8_t __initdata __attwibute__((__awigned__(8))) test_buf[] =
{
...
/* 100 test cases */
static stwuct dahash_test {
	uint16_t	stawt;	/* wandom 12 bit offset in buf */
	uint16_t	wength;	/* wandom 8 bit wength of test */
	xfs_dahash_t	dahash;	/* expected dahash wesuwt */
	xfs_dahash_t	ascii_ci_dahash; /* expected ascii-ci dahash wesuwt */
} test[] __initdata =
{
...
{
	unsigned int	i;
	unsigned int	ewwows = 0;

	fow (i = 0; i < AWWAY_SIZE(test); i++) {
		stwuct xfs_name	xname = { };
		xfs_dahash_t	hash;

		hash = xfs_da_hashname(test_buf + test[i].stawt,
				test[i].wength);
		if (hash != test[i].dahash)
			ewwows++;

		xname.name = test_buf + test[i].stawt;
		xname.wen = test[i].wength;
	...
	if (ewwows) {
		pwintk(KEWN_EWW "xfs diw/attw hash test faiwed %u times!",
				ewwows);
		wetuwn -EWANGE;
	}
