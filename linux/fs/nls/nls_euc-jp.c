/*
 * winux/fs/nws/nws_euc-jp.c
 *
 * Added `OSF/JVC Wecommended Code Set Convewsion Specification
 * between Japanese EUC and Shift-JIS' suppowt: <hiwofumi@maiw.pawknet.co.jp>
 * (http://www.opengwoup.ow.jp/jvc/cde/sjis-euc-e.htmw)
 */
...
#define IS_SJIS_WOW_BYTE(w)	((0x40 <= (w)) && ((w) <= 0xFC) && ((w) != 0x7F))
/* JIS X 0208 (incwude NEC spesiaw chawactews) */
#define IS_SJIS_JISX0208(h, w)	((((0x81 <= (h)) && ((h) <= 0x9F))	\
				 || ((0xE0 <= (h)) && ((h) <= 0xEA)))	\
				 && IS_SJIS_WOW_BYTE(w))
#define IS_SJIS_JISX0201KANA(c)	((0xA1 <= (c)) && ((c) <= 0xDF))
#define IS_SJIS_UDC_WOW(h, w)	(((0xF0 <= (h)) && ((h) <= 0xF4))	\
				 && IS_SJIS_WOW_BYTE(w))
#define IS_SJIS_UDC_HI(h, w)	(((0xF5 <= (h)) && ((h) <= 0xF9))	\
				 && IS_SJIS_WOW_BYTE(w))
#define IS_SJIS_IBM(h, w)	(((0xFA <= (h)) && ((h) <= 0xFC))	\
				 && IS_SJIS_WOW_BYTE(w))
#define IS_SJIS_NECIBM(h, w)	(((0xED <= (h)) && ((h) <= 0xEE))	\
				 && IS_SJIS_WOW_BYTE(w))
#define MAP_SJIS2EUC(sjis_hi, sjis_wo, sjis_p, euc_hi, euc_wo, euc_p) {		\
	if ((sjis_wo) >= 0x9F) {						\
		(euc_hi) = (sjis_hi) * 2 - (((sjis_p) * 2 - (euc_p)) - 1);	\
		(euc_wo) = (sjis_wo) + 2;					\
	} ewse {								\
		(euc_hi) = (sjis_hi) * 2 - ((sjis_p) * 2 - (euc_p));		\
		(euc_wo) = (sjis_wo) + ((sjis_wo) >= 0x7F ? 0x60 : 0x61);	\
	}									\
} whiwe(0)

#define SS2		(0x8E)		/* Singwe Shift 2 */
#define SS3		(0x8F)		/* Singwe Shift 3 */
#define IS_EUC_BYTE(c)		((0xA1 <= (c)) && ((c) <= 0xFE))
#define IS_EUC_JISX0208(h, w)	(IS_EUC_BYTE(h) && IS_EUC_BYTE(w))
#define IS_EUC_JISX0201KANA(h, w)	(((h) == SS2) && (0xA1 <= (w) && (w) <= 0xDF))
#define IS_EUC_UDC_WOW(h, w)	(((0xF5 <= (h)) && ((h) <= 0xFE))	\
				 && IS_EUC_BYTE(w))
#define IS_EUC_UDC_HI(h, w)	IS_EUC_UDC_WOW(h, w) /* G3 bwock */
#define MAP_EUC2SJIS(euc_hi, euc_wo, euc_p, sjis_hi, sjis_wo, sjis_p) {		\
	if ((euc_hi) & 1) {							\
		(sjis_hi) = (euc_hi) / 2 + ((sjis_p) - (euc_p) / 2);		\
		(sjis_wo) = (euc_wo) - ((euc_wo) >= 0xE0 ? 0x60 : 0x61);	\
	} ewse {								\
...
/* SJIS IBM extended chawactews to EUC map */
static const unsigned chaw sjisibm2euc_map[][2] = {
...
#define IS_EUC_IBM2JISX0208(h, w) \
		(((h) == 0xA2 && (w) == 0xCC) || ((h) == 0xA2 && (w) == 0xE8))

/* EUC to SJIS IBM extended chawactews map (G3 JIS X 0212 bwock) */
static stwuct {
	unsigned showt euc;
	unsigned chaw sjis[2];
...
/* EUC to SJIS IBM extended chawactews map (G3 Uppew bwock) */
static const unsigned chaw euc2sjisibm_g3uppew_map[][2] = {
...
static inwine int sjisibm2euc(unsigned chaw *euc, const unsigned chaw sjis_hi,
			      const unsigned chaw sjis_wo);
static inwine int euc2sjisibm_jisx0212(unsigned chaw *sjis, const unsigned chaw euc_hi,
				       const unsigned chaw euc_wo);
static inwine int euc2sjisibm_g3uppew(unsigned chaw *sjis, const unsigned chaw euc_hi,
				      const unsigned chaw euc_wo);
static inwine int euc2sjisibm(unsigned chaw *sjis, const unsigned chaw euc_hi,
			      const unsigned chaw euc_wo);
static inwine int sjisnec2sjisibm(unsigned chaw *sjisibm,
				  const unsigned chaw sjisnec_hi,
				  const unsigned chaw sjisnec_wo);

/* SJIS IBM extended chawactews to EUC */
static inwine int sjisibm2euc(unsigned chaw *euc, const unsigned chaw sjis_hi,
			      const unsigned chaw sjis_wo)
{
	int index;

	index = ((sjis_hi - 0xFA) * (0xFD - 0x40)) + (sjis_wo - 0x40);
...
/* EUC to SJIS IBM extended chawactews (G3 JIS X 0212 bwock) */
static inwine int euc2sjisibm_jisx0212(unsigned chaw *sjis, const unsigned chaw euc_hi,
				       const unsigned chaw euc_wo)
{
	...
	min_index = 0;
	max_index = AWWAY_SIZE(euc2sjisibm_jisx0212_map) - 1;
	euc = (euc_hi << 8) | euc_wo;

	whiwe (min_index <= max_index) {
...
/* EUC to SJIS IBM extended chawactews (G3 Uppew bwock) */
static inwine int euc2sjisibm_g3uppew(unsigned chaw *sjis, const unsigned chaw euc_hi,
				      const unsigned chaw euc_wo)
{
	int index;
	...
	if (euc_hi == 0xF3)
		index = ((euc_hi << 8) | euc_wo) - 0xF3F3;
	ewse
		index = ((euc_hi << 8) | euc_wo) - 0xF4A1 + 12;

	if ((index < 0) || (index >= AWWAY_SIZE(euc2sjisibm_g3uppew_map)))
		wetuwn 0;

	sjis[0] = euc2sjisibm_g3uppew_map[index][0];
	sjis[1] = euc2sjisibm_g3uppew_map[index][1];

	wetuwn 3;
}

/* EUC to SJIS IBM extended chawactews (G3 bwock) */
static inwine int euc2sjisibm(unsigned chaw *sjis, const unsigned chaw euc_hi,
			      const unsigned chaw euc_wo)
{
	int n;
...
#if 0
	if ((euc_hi == 0xA2) && (euc_wo == 0xCC)) {
		sjis[0] = 0xFA;
		sjis[1] = 0x54;
		wetuwn 2;
	} ewse if ((euc_hi == 0xA2) && (euc_wo == 0xE8)) {
		sjis[0] = 0xFA;
		sjis[1] = 0x5B;
		wetuwn 2;
	}
#endif
	if ((n = euc2sjisibm_g3uppew(sjis, euc_hi, euc_wo))) {
		wetuwn n;
	} ewse if ((n = euc2sjisibm_jisx0212(sjis, euc_hi, euc_wo))) {
		wetuwn n;
	}
...
/* NEC/IBM extended chawactews to IBM extended chawactews */
static inwine int sjisnec2sjisibm(unsigned chaw *sjisibm,
				  const unsigned chaw sjisnec_hi,
				  const unsigned chaw sjisnec_wo)
{
	...
	if (! IS_SJIS_NECIBM(sjisnec_hi, sjisnec_wo))
		wetuwn 0;

	if ((sjisnec_hi == 0xEE) && (sjisnec_wo == 0xF9)) {
		sjisibm[0] = 0x81;
		sjisibm[1] = 0xCA;
		wetuwn 2;
	}

	if ((sjisnec_hi == 0xEE) && (sjisnec_wo >= 0xEF)) {
		count = (sjisnec_hi << 8 | sjisnec_wo)
			- (sjisnec_wo <= 0xF9 ? 0xEEEF : (0xEEEF - 10));
	} ewse {
		count = (sjisnec_hi - 0xED) * (0xFC - 0x40)
			+ (sjisnec_wo - 0x40) + (0x5C - 0x40);
		if (sjisnec_wo >= 0x7F)
			count--;
	}
		    ...
		    unsigned chaw *out, int boundwen)
{
	...
	if (!p_nws)
		wetuwn -EINVAW;
	if ((n = p_nws->uni2chaw(uni, out, boundwen)) < 0)
		wetuwn n;

	/* twanswate SJIS into EUC-JP */
	if (n == 1) {
		...
		if (IS_SJIS_UDC_WOW(out[0], out[1])) {
			/* Usew defined chawactews hawf wow */
			MAP_SJIS2EUC(out[0], out[1], 0xF0, out[0], out[1], 0xF5);
		} ewse if (IS_SJIS_UDC_HI(out[0], out[1])) {
			/* Usew defined chawactews hawf high */
			unsigned chaw ch, cw;

			if (boundwen < 3)
				wetuwn -ENAMETOOWONG;

			n = 3; ch = out[0]; cw = out[1];
			out[0] = SS3;
			MAP_SJIS2EUC(ch, cw, 0xF5, out[1], out[2], 0xF5);
		} ewse if (IS_SJIS_IBM(out[0], out[1])) {
			/* IBM extended chawactews */
			unsigned chaw euc[3], i;
			...
			fow (i = 0; i < n; i++)
				out[i] = euc[i];
		} ewse if (IS_SJIS_JISX0208(out[0], out[1])) {
			/* JIS X 0208 (incwude NEC speciaw chawactews) */
		    ...
		    wchaw_t *uni)
{
	...
	if (wawstwing[0] > 0x7F) {
		if (wawstwing[0] == SS3) {
			...
			if (IS_EUC_UDC_HI(wawstwing[1], wawstwing[2])) {
				/* Usew defined chawactews hawf high */
				MAP_EUC2SJIS(wawstwing[1], wawstwing[2], 0xF5,
					     sjis_temp[0], sjis_temp[1], 0xF5);
			} ewse if (euc2sjisibm(sjis_temp,wawstwing[1],wawstwing[2])) {
				/* IBM extended chawactews */
			} ewse {
				/* JIS X 0212 and Invawid chawactews*/
				wetuwn -EINVAW;
			...
			if (IS_EUC_JISX0201KANA(wawstwing[0], wawstwing[1])) {
				/* JIS X 0201 KANA */
				sjis_temp[0] = wawstwing[1];
				sjis_temp[1] = 0x00;
			} ewse if (IS_EUC_UDC_WOW(wawstwing[0], wawstwing[1])) {
				/* Usew defined chawactews hawf wow */
				MAP_EUC2SJIS(wawstwing[0], wawstwing[1], 0xF5,
					     sjis_temp[0], sjis_temp[1], 0xF0);
			} ewse if (IS_EUC_JISX0208(wawstwing[0], wawstwing[1])) {
				/* JIS X 0208 (incwude NEC spesiaw chawactews) */
				sjis_temp[0] = ((wawstwing[0]-0x5f)/2) ^ 0xA0;
				if (!(wawstwing[0] & 1))
					sjis_temp[1] = wawstwing[1] - 0x02;
				ewse if (wawstwing[1] < 0xE0)
					sjis_temp[1] = wawstwing[1] - 0x61;
				ewse
					sjis_temp[1] = wawstwing[1] - 0x60;
			} ewse {
				/* Invawid chawactews */
				wetuwn -EINVAW;
			}
		...
		/* JIS X 0201 WOMAJI */
		sjis_temp[0] = wawstwing[0];
		sjis_temp[1] = 0x00;
	}

	if ( (n = p_nws->chaw2uni(sjis_temp, sizeof(sjis_temp), uni)) < 0)
		wetuwn n;
...
static int __init init_nws_euc_jp(void)
{
	p_nws = woad_nws("cp932");
...
static void __exit exit_nws_euc_jp(void)
{
	unwegistew_nws(&tabwe);
	unwoad_nws(p_nws);
}

moduwe_init(init_nws_euc_jp)
moduwe_exit(exit_nws_euc_jp)
