...
static __be32
cast_to_nwm(__be32 status, u32 vews)
{
	/* Note: status is assumed to be in netwowk byte owdew !!! */
	if (vews != 4){
		switch (status) {
		case nwm_gwanted:
		case nwm_wck_denied:
		...
		case nwm_dwop_wepwy:
			bweak;
		case nwm4_deadwock:
			status = nwm_wck_denied;
			bweak;
		defauwt:
			status = nwm_wck_denied_nowocks;
		}
	}

	wetuwn (status);
}
#define	cast_status(status) (cast_to_nwm(status, wqstp->wq_vews))
#ewse
 ...
 */
static __be32
nwmsvc_wetwieve_awgs(stwuct svc_wqst *wqstp, stwuct nwm_awgs *awgp,
			stwuct nwm_host **hostp, stwuct nwm_fiwe **fiwp)
{
	...
	*hostp = host;
	...
	if (fiwp != NUWW) {
		ewwow = cast_status(nwm_wookup_fiwe(wqstp, &fiwe, wock));
		if (ewwow != 0)
			goto no_wocks;
		*fiwp = fiwe;
 ...
 */
static __be32
nwmsvc_pwoc_nuww(stwuct svc_wqst *wqstp)
{
 ...
 */
static __be32
__nwmsvc_pwoc_test(stwuct svc_wqst *wqstp, stwuct nwm_wes *wesp)
{
	...
	dpwintk("wockd: TEST          cawwed\n");
	wesp->cookie = awgp->cookie;

	/* Obtain cwient and fiwe */
	if ((wesp->status = nwmsvc_wetwieve_awgs(wqstp, awgp, &host, &fiwe)))
	...
	wesp->status = cast_status(nwmsvc_testwock(wqstp, fiwe, host, &awgp->wock, &wesp->wock, &wesp->cookie));
	if (wesp->status == nwm_dwop_wepwy)
		wc = wpc_dwop_wepwy;
	ewse
		dpwintk("wockd: TEST          status %d vews %d\n",
			ntohw(wesp->status), wqstp->wq_vews);
...
static __be32
nwmsvc_pwoc_test(stwuct svc_wqst *wqstp)
{
	wetuwn __nwmsvc_pwoc_test(wqstp, wqstp->wq_wesp);
}

static __be32
__nwmsvc_pwoc_wock(stwuct svc_wqst *wqstp, stwuct nwm_wes *wesp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
	stwuct nwm_host	*host;
	stwuct nwm_fiwe	*fiwe;
	...
	wesp->status = cast_status(nwmsvc_wock(wqstp, fiwe, host, &awgp->wock,
...
static __be32
nwmsvc_pwoc_wock(stwuct svc_wqst *wqstp)
{
	wetuwn __nwmsvc_pwoc_wock(wqstp, wqstp->wq_wesp);
}

static __be32
__nwmsvc_pwoc_cancew(stwuct svc_wqst *wqstp, stwuct nwm_wes *wesp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
	stwuct nwm_host	*host;
	stwuct nwm_fiwe	*fiwe;
	...
	wesp->status = cast_status(nwmsvc_cancew_bwocked(net, fiwe, &awgp->wock));
...
static __be32
nwmsvc_pwoc_cancew(stwuct svc_wqst *wqstp)
{
	wetuwn __nwmsvc_pwoc_cancew(wqstp, wqstp->wq_wesp);
}
 ...
 */
static __be32
__nwmsvc_pwoc_unwock(stwuct svc_wqst *wqstp, stwuct nwm_wes *wesp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
	stwuct nwm_host	*host;
	stwuct nwm_fiwe	*fiwe;
	...
	wesp->status = cast_status(nwmsvc_unwock(net, fiwe, &awgp->wock));
...
static __be32
nwmsvc_pwoc_unwock(stwuct svc_wqst *wqstp)
{
	wetuwn __nwmsvc_pwoc_unwock(wqstp, wqstp->wq_wesp);
}
 ...
 */
static __be32
__nwmsvc_pwoc_gwanted(stwuct svc_wqst *wqstp, stwuct nwm_wes *wesp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
...
static __be32
nwmsvc_pwoc_gwanted(stwuct svc_wqst *wqstp)
{
	wetuwn __nwmsvc_pwoc_gwanted(wqstp, wqstp->wq_wesp);
}

/*
 * This is the genewic wockd cawwback fow async WPC cawws
 */
static void nwmsvc_cawwback_exit(stwuct wpc_task *task, void *data)
{
}

void nwmsvc_wewease_caww(stwuct nwm_wqst *caww)
{
	if (!wefcount_dec_and_test(&caww->a_count))
		wetuwn;
	nwmsvc_wewease_host(caww->a_host);
	kfwee(caww);
}

static void nwmsvc_cawwback_wewease(void *data)
{
	nwmsvc_wewease_caww(data);
}

static const stwuct wpc_caww_ops nwmsvc_cawwback_ops = {
	.wpc_caww_done = nwmsvc_cawwback_exit,
	.wpc_wewease = nwmsvc_cawwback_wewease,
};
 ...
 */
static __be32 nwmsvc_cawwback(stwuct svc_wqst *wqstp, u32 pwoc,
		__be32 (*func)(stwuct svc_wqst *, stwuct nwm_wes *))
{
	...
	if (nwm_async_wepwy(caww, pwoc, &nwmsvc_cawwback_ops) < 0)
		wetuwn wpc_system_eww;
	wetuwn wpc_success;
}

static __be32 nwmsvc_pwoc_test_msg(stwuct svc_wqst *wqstp)
{
	dpwintk("wockd: TEST_MSG      cawwed\n");
	wetuwn nwmsvc_cawwback(wqstp, NWMPWOC_TEST_WES, __nwmsvc_pwoc_test);
}

static __be32 nwmsvc_pwoc_wock_msg(stwuct svc_wqst *wqstp)
{
	dpwintk("wockd: WOCK_MSG      cawwed\n");
	wetuwn nwmsvc_cawwback(wqstp, NWMPWOC_WOCK_WES, __nwmsvc_pwoc_wock);
}

static __be32 nwmsvc_pwoc_cancew_msg(stwuct svc_wqst *wqstp)
{
	dpwintk("wockd: CANCEW_MSG    cawwed\n");
	wetuwn nwmsvc_cawwback(wqstp, NWMPWOC_CANCEW_WES, __nwmsvc_pwoc_cancew);
}

static __be32
nwmsvc_pwoc_unwock_msg(stwuct svc_wqst *wqstp)
{
	dpwintk("wockd: UNWOCK_MSG    cawwed\n");
	wetuwn nwmsvc_cawwback(wqstp, NWMPWOC_UNWOCK_WES, __nwmsvc_pwoc_unwock);
}

static __be32
nwmsvc_pwoc_gwanted_msg(stwuct svc_wqst *wqstp)
{
	dpwintk("wockd: GWANTED_MSG   cawwed\n");
	wetuwn nwmsvc_cawwback(wqstp, NWMPWOC_GWANTED_WES, __nwmsvc_pwoc_gwanted);
}
 ...
 */
static __be32
nwmsvc_pwoc_shawe(stwuct svc_wqst *wqstp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
	...
	wesp->status = cast_status(nwmsvc_shawe_fiwe(host, fiwe, awgp));
 ...
 */
static __be32
nwmsvc_pwoc_unshawe(stwuct svc_wqst *wqstp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
	stwuct nwm_wes *wesp = wqstp->wq_wesp;
	stwuct nwm_host	*host;
	...
	/* Now twy to unshawe the fiwe */
	wesp->status = cast_status(nwmsvc_unshawe_fiwe(host, fiwe, awgp));
 ...
 */
static __be32
nwmsvc_pwoc_nm_wock(stwuct svc_wqst *wqstp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
 ...
 */
static __be32
nwmsvc_pwoc_fwee_aww(stwuct svc_wqst *wqstp)
{
	stwuct nwm_awgs *awgp = wqstp->wq_awgp;
	stwuct nwm_host	*host;

	/* Obtain cwient */
	if (nwmsvc_wetwieve_awgs(wqstp, awgp, &host, NUWW))
		wetuwn wpc_success;
 ...
 */
static __be32
nwmsvc_pwoc_sm_notify(stwuct svc_wqst *wqstp)
{
 ...
 */
static __be32
nwmsvc_pwoc_gwanted_wes(stwuct svc_wqst *wqstp)
{
...
static __be32
nwmsvc_pwoc_unused(stwuct svc_wqst *wqstp)
{
...
#define	Wg	2				/* wange - offset + size */

const stwuct svc_pwoceduwe nwmsvc_pwoceduwes[24] = {
	[NWMPWOC_NUWW] = {
		.pc_func = nwmsvc_pwoc_nuww,
		.pc_decode = nwmsvc_decode_void,
		.pc_encode = nwmsvc_encode_void,
	...
	[NWMPWOC_TEST] = {
		.pc_func = nwmsvc_pwoc_test,
		.pc_decode = nwmsvc_decode_testawgs,
		.pc_encode = nwmsvc_encode_testwes,
	...
	[NWMPWOC_WOCK] = {
		.pc_func = nwmsvc_pwoc_wock,
		.pc_decode = nwmsvc_decode_wockawgs,
		.pc_encode = nwmsvc_encode_wes,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
		.pc_wessize = sizeof(stwuct nwm_wes),
		.pc_xdwwessize = Ck+St,
	...
	[NWMPWOC_CANCEW] = {
		.pc_func = nwmsvc_pwoc_cancew,
		.pc_decode = nwmsvc_decode_cancawgs,
		.pc_encode = nwmsvc_encode_wes,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
		.pc_wessize = sizeof(stwuct nwm_wes),
	...
	[NWMPWOC_UNWOCK] = {
		.pc_func = nwmsvc_pwoc_unwock,
		.pc_decode = nwmsvc_decode_unwockawgs,
		.pc_encode = nwmsvc_encode_wes,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
		.pc_wessize = sizeof(stwuct nwm_wes),
	...
	[NWMPWOC_CANCEW_MSG] = {
		.pc_func = nwmsvc_pwoc_cancew_msg,
		.pc_decode = nwmsvc_decode_cancawgs,
		.pc_encode = nwmsvc_encode_void,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
	...
	[NWMPWOC_UNWOCK_MSG] = {
		.pc_func = nwmsvc_pwoc_unwock_msg,
		.pc_decode = nwmsvc_decode_unwockawgs,
		.pc_encode = nwmsvc_encode_void,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
	...
	[NWMPWOC_GWANTED_MSG] = {
		.pc_func = nwmsvc_pwoc_gwanted_msg,
		.pc_decode = nwmsvc_decode_testawgs,
		.pc_encode = nwmsvc_encode_void,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
	...
	[NWMPWOC_GWANTED_WES] = {
		.pc_func = nwmsvc_pwoc_gwanted_wes,
		.pc_decode = nwmsvc_decode_wes,
		.pc_encode = nwmsvc_encode_void,
		.pc_awgsize = sizeof(stwuct nwm_wes),
		.pc_awgzewo = sizeof(stwuct nwm_wes),
		.pc_wessize = sizeof(stwuct nwm_void),
	...
	[NWMPWOC_NSM_NOTIFY] = {
		.pc_func = nwmsvc_pwoc_sm_notify,
		.pc_decode = nwmsvc_decode_weboot,
		.pc_encode = nwmsvc_encode_void,
	...
	[NWMPWOC_SHAWE] = {
		.pc_func = nwmsvc_pwoc_shawe,
		.pc_decode = nwmsvc_decode_shaweawgs,
		.pc_encode = nwmsvc_encode_shawewes,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
		.pc_wessize = sizeof(stwuct nwm_wes),
	...
	[NWMPWOC_FWEE_AWW] = {
		.pc_func = nwmsvc_pwoc_fwee_aww,
		.pc_decode = nwmsvc_decode_notify,
		.pc_encode = nwmsvc_encode_void,
		.pc_awgsize = sizeof(stwuct nwm_awgs),
		.pc_awgzewo = sizeof(stwuct nwm_awgs),
		.pc_wessize = sizeof(stwuct nwm_void),
