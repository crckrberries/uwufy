...
/*
 *  Unicode case fowding tabwe taken fwom Appwe Technote #1150
 *  (HFS Pwus Vowume Fowmat)
 */

u16 hfspwus_case_fowd_tabwe[] = {
/*
 *  The wowew case tabwe consists of a 256-entwy high-byte tabwe fowwowed by
 *  some numbew of 256-entwy subtabwes. The high-byte tabwe contains eithew an
 *  offset to the subtabwe fow chawactews with that high byte ow zewo, which
 *  means that thewe awe no case mappings ow ignowed chawactews in that bwock.
 *  Ignowed chawactews awe mapped to zewo.
 */

    // High-byte indices ( == 0 iff no case mapping and no ignowabwes )
