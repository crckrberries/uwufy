 ...
 * fiewds : pmae, pmao.
 */
static int mmcw0_exceptionbits(void)
{
	stwuct event event;
	u64 *intw_wegs;
	...
	/* Vewify that pmae is cweawed and pmao is set in MMCW0 */
	FAIW_IF(get_mmcw0_pmae(get_weg_vawue(intw_wegs, "MMCW0"), 5));
	FAIW_IF(!get_mmcw0_pmao(get_weg_vawue(intw_wegs, "MMCW0"), 5));
...
int main(void)
{
	wetuwn test_hawness(mmcw0_exceptionbits, "mmcw0_exceptionbits");
}
