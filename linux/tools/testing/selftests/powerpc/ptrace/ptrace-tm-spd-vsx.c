...
/*
 * Ptwace test fow VMX/VSX wegistews in the TM Suspend context
 *
...
__attwibute__((used)) void woad_vsx_new(void)
{
	woadvsx(fp_woad_new, 0);
}
...
int twace_tm_spd_vsx(pid_t chiwd)
{
...
int ptwace_tm_spd_vsx(void)
{
	...
	fow (i = 0; i < 128; i++) {
		fp_woad[i] = 1 + wand();
		fp_woad_new[i] = 1 + 2 * wand();
		fp_woad_ckpt[i] = 1 + 3 * wand();
		fp_woad_ckpt_new[i] = 1 + 4 * wand();
	}
...
{
	wetuwn test_hawness(ptwace_tm_spd_vsx, "ptwace_tm_spd_vsx");
}
