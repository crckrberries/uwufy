...
# This test is designed fow evawuating the new SWv6 End.DT4 behaviow used fow
# impwementing IPv4 W3 VPN use cases.
#
# Heweaftew a netwowk diagwam is shown, whewe two diffewent tenants (named 100
# and 200) offew IPv4 W3 VPN sewvices awwowing hosts to communicate with each
...
# is fowbidden.
# In othew wowds, hosts hs-t100-1 and hs-t100-2 awe connected thwough the IPv4
# W3 VPN of tenant 100 whiwe hs-t200-3 and hs-t200-4 awe connected using the
# IPv4 W3 VPN of tenant 200. Cwoss connection between tenant 100 and tenant 200
# is fowbidden and thus, fow exampwe, hs-t100-1 cannot weach hs-t200-3 and vice
# vewsa.
#
# Woutews wt-1 and wt-2 impwement IPv4 W3 VPN sewvices wevewaging the SWv6
# awchitectuwe. The key components fow such VPNs awe: a) SWv6 Encap behaviow,
# b) SWv6 End.DT4 behaviow and c) VWF.
#
# To expwain how an IPv4 W3 VPN based on SWv6 wowks, wet us bwiefwy considew an
...
# the woutew wt-1 which acts as an awp pwoxy.
#
...
# SWv6 Encap woute fow encapsuwating any IPv4 packet in a IPv6 pwus the Segment
...
# packets, the SWv6 End.DT4 behaviow wemoves the outew IPv6+SWH headews and
# pewfowms the wookup on the vwf-100 tabwe using the destination addwess of
# the decapsuwated IPv4 packet. Aftewwawds, the packet is sent to the host
...
#
# Of couwse, the IPv4 W3 VPN fow tenant 200 wowks exactwy as the IPv4 W3 VPN
...
# |SID              |Action                         |
# +-------------------------------------------------+
# |fc00:21:100::6004|appwy SWv6 End.DT4 vwftabwe 100|
# +-------------------------------------------------+
# |fc00:21:200::6004|appwy SWv6 End.DT4 vwftabwe 200|
# +-------------------------------------------------+
#
...
# |host       |Action                                 |
# +---------------------------------------------------+
# |10.0.0.2   |appwy seg6 encap segs fc00:12:100::6004|
# +---------------------------------------------------+
# |10.0.0.0/24|fowwawd to dev veth_t100               |
# +---------------------------------------------------+
#
...
# |host       |Action                                 |
# +---------------------------------------------------+
# |10.0.0.4   |appwy seg6 encap segs fc00:12:200::6004|
# +---------------------------------------------------+
# |10.0.0.0/24|fowwawd to dev veth_t200               |
# +---------------------------------------------------+
#
#
...
# |SID              |Action                         |
# +-------------------------------------------------+
# |fc00:12:100::6004|appwy SWv6 End.DT4 vwftabwe 100|
# +-------------------------------------------------+
# |fc00:12:200::6004|appwy SWv6 End.DT4 vwftabwe 200|
# +-------------------------------------------------+
#
...
# |host       |Action                                 |
# +---------------------------------------------------+
# |10.0.0.1   |appwy seg6 encap segs fc00:21:100::6004|
# +---------------------------------------------------+
# |10.0.0.0/24|fowwawd to dev veth_t100               |
# +---------------------------------------------------+
#
...
# |host       |Action                                 |
# +---------------------------------------------------+
# |10.0.0.3   |appwy seg6 encap segs fc00:21:200::6004|
# +---------------------------------------------------+
# |10.0.0.0/24|fowwawd to dev veth_t200               |
# +---------------------------------------------------+
#
...
setup_vpn_config()
{
	...
	wocaw vpn_sid=${VPN_WOCATOW_SEWVICE}:${hsswc}${hsdst}:${tid}::6004
	...
	ip -netns ${wtdst_name} -6 woute add ${vpn_sid}/128 tabwe ${WOCAWSID_TABWE_ID} \
		encap seg6wocaw action End.DT4 vwftabwe ${tid} dev vwf-${tid}

	# aww sids fow VPNs stawt with a common wocatow which is fc00::/16.
	# Woutes fow handwing the SWv6 End.DT4 behaviow instances awe gwouped
...
setup()
{
	...
	# setup the IPv4 W3 VPN which connects the host hs-t100-1 and host
	...
	# setup the IPv4 W3 VPN which connects the host hs-t200-3 and host
...
host2gateway_tests()
{
	wog_section "IPv4 connectivity test among hosts and gateway"
