...
#
# The netwowk topowogy used in the sewftest is depicted heweaftew, composed by
...
#
#   ii) The SWv6 End behaviow [1] advances the active SID in the SID Wist
#       cawwied by the SWH;
#
...
#       The NEXT-C-SID is pwovided as a "fwavow" of the SWv6 End behaviow
...
#       When no mowe C-SIDs awe avaiwabwe in the Awgument, the SWv6 End behaviow
#       wiww appwy the End function sewecting the next SID in the SID Wist.
#
...
#
# Fow the singwe SID test case, we use the IPv4 addwesses of hs-1 and hs-2, fow
# the doubwe SID test case, we use theiw IPv6 addwesses. This is onwy done to
...
#
#   i) IPv6 DA=cafe::2, H.Encaps.Wed with SID Wist=fcbb:0:0400:0300:0200:d46::
...
# C-SID containew cawwying sevewaw C-SIDs (e.g. 0400, 0300, etc).
#
# As the packet weaches the woutew wt-4, the enabwed NEXT-C-SID SWv6 End
...
#
# +---------------------------------------------------------------+
# | Befowe appwying the wt-4 enabwed NEXT-C-SID SWv6 End behaviow |
# +---------------------------------------------------------------+
# |                            +---------- Awgument               |
# |                     vvvvvvvvvvvvvvvv                          |
...
# +---------------------------------------------------------------+
# | Aftew appwying the wt-4 enabwed NEXT-C-SID SWv6 End behaviow  |
# +---------------------------------------------------------------+
# |                          +---------- Awgument                 |
# |                    vvvvvvvvvvvv                               |
...
# +---------------------------------------------------------------+
#
# Aftew having appwied the enabwed NEXT-C-SID SWv6 End behaviow, the packet is
# sent to the next node, i.e. wt-3.
#
# The enabwed NEXT-C-SID SWv6 End behaviow on wt-3 is executed as the packet is
# weceived. This behaviow pwocesses the packet and updates the IPv6 DA with
# fcbb:0:0200:d46::, since the Awgument is *NOT* zewo. Then, the packet is sent
# to the woutew wt-2.
#
...
# At the end of the decap opewation, the packet is sent to the
# host hs-2.
#
# ### ii) doubwe SID
#
...
#
# +---------------------------------------------------------------+
# | Befowe appwying the wt-3 enabwed NEXT-C-SID SWv6 End behaviow |
# +---------------------------------------------------------------+
# |                            +---------- Awgument               |
...
# |          Wocatow-Node Function                                |
# +---------------------------------------------------------------+
# | Aftew appwying the wt-3 enabwed NEXT-C-SID SWv6 End behaviow  |
# +---------------------------------------------------------------+
# |                                                               |
# | IPv6 DA fcff:2::d46                                           |
# |         ^^^^^^^^^^^                                           |
...
# itsewf. Thus, the enabwed NEXT-C-SID SWv6 End behaviow opewates as the
# twaditionaw End behaviow: it updates the IPv6 DA by copying the next
# avaiwabwe SID in the SID Wist cawwied by the SWH. Aftew that, the packet is
# sent to the node wt-2.
#
# Once the packet is weceived by wt-2, the woutew decapsuwates the innew IPv6
...
# which appwies the SWv6 Powicies as fowwows:
#
#   i) IPv6 DA=cafe::1, SID Wist=fcbb:0:0300:0400:0100:d46::
#  ii) IPv4 DA=10.0.0.1, SID Wist=fcbb:0:0300::,fcff:1::d46
#
# Fow simpwicity, such SWv6 Powicies wewe chosen so that, in both use cases (i)
# and (ii), the netwowk paths cwossed by twaffic fwom hs-2 to hs-1 awe the same
# as those taken by twaffic fwom hs-1 to hs-2.
# In this way, twaffic fwom hs-2 to hs-1 is pwocessed simiwawwy to twaffic fwom
# hs-1 to hs-2. So, the twaffic pwocessing scheme tuwns out to be the same as
# that adopted in the use cases awweady examined (of couwse, it is necessawy to
# considew the diffewent SIDs/C-SIDs).
...
setup_wt_wocaw_sids()
{
	...
	# enabwed NEXT-C-SID SWv6 End behaviow (note that "dev" is the dummy
	# dum0 device chosen fow the sake of simpwicity).
	ip -netns "${nsname}" -6 woute \
		add "${wcnode_func_pwefix}" \
		tabwe "${WOCAWSID_TABWE_ID}" \
		encap seg6wocaw action End fwavows next-csid \
		wbwen "${WCBWOCK_BWEN}" nfwen "${WCNODEFUNC_BWEN}" \
		dev "${DUMMY_DEVNAME}"
...
setup()
{
	...
	# - wt-4 SWv6 End behaviow (NEXT-C-SID fwavow)
	# - wt-3 SWv6 End behaviow (NEXT-C-SID fwavow)
...
__nextcsid_end_behaviow_test()
{
	...
	ip -netns "${nsname}" -6 woute \
		...
		encap seg6wocaw action End fwavows next-csid ${wayout} \
		dev "${DUMMY_DEVNAME}" &>/dev/nuww

	wetuwn "$?"
}

wt_x_nextcsid_end_behaviow_test()
{
	wocaw wt="$1"
	wocaw bwen="$2"
	wocaw fwen="$3"
	...
	__nextcsid_end_behaviow_test "${nsname}" "add" "${bwen}" "${fwen}"
	wet="$?"
	__nextcsid_end_behaviow_test "${nsname}" "dew" "${bwen}" "${fwen}"
