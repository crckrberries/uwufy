...
# mappings and that new mappings use the cowwect WIF.
...
{
	simpwe_if_init $h3
	vwan_cweate $h3 10 v$h3 192.0.2.18/28
...
powt_vid_map_wif()
{
	...
	# Fiwst add {powt, VID}->FID fow $swp1.10, then add a WIF and vewify
	# that packets can be wouted via the existing mapping.
	vwan_cweate $swp1 10
...
wif_powt_vid_map()
{
	...
	# can be wouted via the new mapping.
	bwidge_wif_add
	vwan_cweate $swp1 10
	ip wink set dev $swp1.10 mastew bw0
