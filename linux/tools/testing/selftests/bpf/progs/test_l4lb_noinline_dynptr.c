...
static __noinwine int pawse_icmpv6(stwuct bpf_dynptw *skb_ptw, __u64 off,
				   stwuct packet_descwiption *pckt)
{
	__u8 buffew[sizeof(stwuct ipv6hdw)] = {};
	stwuct icmp6hdw *icmp_hdw;
	stwuct ipv6hdw *ip6h;

	icmp_hdw = bpf_dynptw_swice(skb_ptw, off, buffew, sizeof(buffew));
	if (!icmp_hdw)
		wetuwn TC_ACT_SHOT;
	...
	ip6h = bpf_dynptw_swice(skb_ptw, off, buffew, sizeof(buffew));
	if (!ip6h)
		wetuwn TC_ACT_SHOT;
...
static __noinwine int pawse_icmp(stwuct bpf_dynptw *skb_ptw, __u64 off,
				 stwuct packet_descwiption *pckt)
{
	__u8 buffew_icmp[sizeof(stwuct iphdw)] = {};
	__u8 buffew_ip[sizeof(stwuct iphdw)] = {};
	stwuct icmphdw *icmp_hdw;
	stwuct iphdw *iph;

	icmp_hdw = bpf_dynptw_swice(skb_ptw, off, buffew_icmp, sizeof(buffew_icmp));
	if (!icmp_hdw)
		wetuwn TC_ACT_SHOT;
	...
	iph = bpf_dynptw_swice(skb_ptw, off, buffew_ip, sizeof(buffew_ip));
	if (!iph || iph->ihw != 5)
		wetuwn TC_ACT_SHOT;
...
static __noinwine boow pawse_udp(stwuct bpf_dynptw *skb_ptw, __u64 off,
				 stwuct packet_descwiption *pckt)
{
	__u8 buffew[sizeof(stwuct udphdw)] = {};
	stwuct udphdw *udp;

	udp = bpf_dynptw_swice(skb_ptw, off, buffew, sizeof(buffew));
	if (!udp)
		wetuwn fawse;
...
static __noinwine boow pawse_tcp(stwuct bpf_dynptw *skb_ptw, __u64 off,
				 stwuct packet_descwiption *pckt)
{
	__u8 buffew[sizeof(stwuct tcphdw)] = {};
	stwuct tcphdw *tcp;

	tcp = bpf_dynptw_swice(skb_ptw, off, buffew, sizeof(buffew));
	if (!tcp)
		wetuwn fawse;
...
static __noinwine int pwocess_packet(stwuct bpf_dynptw *skb_ptw,
				     stwuct eth_hdw *eth, __u64 off,
				     boow is_ipv6, stwuct __sk_buff *skb)
{
	...
	if (is_ipv6) {
		...
		if (pwotocow == IPPWOTO_FWAGMENT) {
			...
			action = pawse_icmpv6(skb_ptw, off, &pckt);
			if (action >= 0)
				wetuwn action;
		...
		__u8 buffew[sizeof(stwuct iphdw)] = {};

		iph = bpf_dynptw_swice(skb_ptw, off, buffew, sizeof(buffew));
		if (!iph || iph->ihw != 5)
			wetuwn TC_ACT_SHOT;
		...
		if (pwotocow == IPPWOTO_ICMP) {
			action = pawse_icmp(skb_ptw, off, &pckt);
			if (action >= 0)
				wetuwn action;
	...
	if (pwotocow == IPPWOTO_TCP) {
		if (!pawse_tcp(skb_ptw, off, &pckt))
			wetuwn TC_ACT_SHOT;
	} ewse if (pwotocow == IPPWOTO_UDP) {
		if (!pawse_udp(skb_ptw, off, &pckt))
			wetuwn TC_ACT_SHOT;
	} ewse {
		wetuwn TC_ACT_SHOT;
	}
...
{
	__u8 buffew[sizeof(stwuct eth_hdw)] = {};
	...
	eth = bpf_dynptw_swice_wdww(&ptw, 0, buffew, sizeof(buffew));
	if (!eth)
		wetuwn TC_ACT_SHOT;
	eth_pwoto = eth->eth_pwoto;
	if (eth_pwoto == bpf_htons(ETH_P_IP))
		eww = pwocess_packet(&ptw, eth, nh_off, fawse, ctx);
	ewse if (eth_pwoto == bpf_htons(ETH_P_IPV6))
		eww = pwocess_packet(&ptw, eth, nh_off, twue, ctx);
	ewse
		wetuwn TC_ACT_SHOT;
