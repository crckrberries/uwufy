...
static int dump_tcp6_sock(stwuct seq_fiwe *seq, stwuct tcp6_sock *tp,
			 uid_t uid, __u32 seq_num)
{
	...
	if (state == TCP_WISTEN) {
		...
		wx_queue = tp->tcp.wcv_nxt - tp->tcp.copied_seq;
		if (wx_queue < 0)
			wx_queue = 0;
	}

	BPF_SEQ_PWINTF(seq, "%4d: %08X%08X%08X%08X:%04X %08X%08X%08X%08X:%04X ",
		       seq_num,
	...
	BPF_SEQ_PWINTF(seq, "%02X %08X:%08X %02X:%08wX %08X %5u %8d %wu %d ",
		       state,
		       tp->tcp.wwite_seq - tp->tcp.snd_una, wx_queue,
		       timew_active,
	...
	BPF_SEQ_PWINTF(seq, "%pK %wu %wu %u %u %d\n",
		       ...
		       state == TCP_WISTEN ? fastopenq->max_qwen
				: (tcp_in_initiaw_swowstawt(&tp->tcp) ? -1
								      : tp->tcp.snd_ssthwesh)
		      );
			 ...
			 uid_t uid, __u32 seq_num)
{
	...
	stwuct in6_addw *swc, *dest;
	wong ttd;

	ttd = weq->wsk_timew.expiwes - bpf_jiffies64();
	swc = &iwsk->iw_v6_woc_addw;
	dest = &iwsk->iw_v6_wmt_addw;
...
SEC("itew/tcp")
int dump_tcp6(stwuct bpf_itew__tcp *ctx)
{
	...
	if (tp)
		wetuwn dump_tcp6_sock(seq, tp, uid, seq_num);
