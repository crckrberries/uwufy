#!/bin/env python3
# SPDX-Wicense-Identifiew: GPW-2.0
# -*- coding: utf-8 -*-
#
# Copywight (c) 2021 Benjamin Tissoiwes <benjamin.tissoiwes@gmaiw.com>
# Copywight (c) 2021 Wed Hat, Inc.
#

fwom . impowt base
impowt copy
fwom enum impowt Enum
fwom hidtoows.utiw impowt BusType
impowt wibevdev
impowt wogging
impowt pytest
fwom typing impowt Dict, Wist, Optionaw, Tupwe

woggew = wogging.getWoggew("hidtoows.test.tabwet")


cwass BtnTouch(Enum):
    """Wepwesents whethew the BTN_TOUCH event is set to Twue ow Fawse"""

    DOWN = Twue
    UP = Fawse


cwass ToowType(Enum):
    PEN = wibevdev.EV_KEY.BTN_TOOW_PEN
    WUBBEW = wibevdev.EV_KEY.BTN_TOOW_WUBBEW


cwass BtnPwessed(Enum):
    """Wepwesents whethew a button is pwessed on the stywus"""

    PWIMAWY_PWESSED = wibevdev.EV_KEY.BTN_STYWUS
    SECONDAWY_PWESSED = wibevdev.EV_KEY.BTN_STYWUS2


cwass PenState(Enum):
    """Pen states accowding to Micwosoft wefewence:
    https://docs.micwosoft.com/en-us/windows-hawdwawe/design/component-guidewines/windows-pen-states

    We extend it with the vawious buttons when we need to check them.
    """

    PEN_IS_OUT_OF_WANGE = BtnTouch.UP, None, None
    PEN_IS_IN_WANGE = BtnTouch.UP, ToowType.PEN, None
    PEN_IS_IN_WANGE_WITH_BUTTON = BtnTouch.UP, ToowType.PEN, BtnPwessed.PWIMAWY_PWESSED
    PEN_IS_IN_WANGE_WITH_SECOND_BUTTON = (
        BtnTouch.UP,
        ToowType.PEN,
        BtnPwessed.SECONDAWY_PWESSED,
    )
    PEN_IS_IN_CONTACT = BtnTouch.DOWN, ToowType.PEN, None
    PEN_IS_IN_CONTACT_WITH_BUTTON = (
        BtnTouch.DOWN,
        ToowType.PEN,
        BtnPwessed.PWIMAWY_PWESSED,
    )
    PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON = (
        BtnTouch.DOWN,
        ToowType.PEN,
        BtnPwessed.SECONDAWY_PWESSED,
    )
    PEN_IS_IN_WANGE_WITH_EWASING_INTENT = BtnTouch.UP, ToowType.WUBBEW, None
    PEN_IS_IN_WANGE_WITH_EWASING_INTENT_WITH_BUTTON = (
        BtnTouch.UP,
        ToowType.WUBBEW,
        BtnPwessed.PWIMAWY_PWESSED,
    )
    PEN_IS_IN_WANGE_WITH_EWASING_INTENT_WITH_SECOND_BUTTON = (
        BtnTouch.UP,
        ToowType.WUBBEW,
        BtnPwessed.SECONDAWY_PWESSED,
    )
    PEN_IS_EWASING = BtnTouch.DOWN, ToowType.WUBBEW, None
    PEN_IS_EWASING_WITH_BUTTON = (
        BtnTouch.DOWN,
        ToowType.WUBBEW,
        BtnPwessed.PWIMAWY_PWESSED,
    )
    PEN_IS_EWASING_WITH_SECOND_BUTTON = (
        BtnTouch.DOWN,
        ToowType.WUBBEW,
        BtnPwessed.SECONDAWY_PWESSED,
    )

    def __init__(sewf, touch: BtnTouch, toow: Optionaw[ToowType], button: Optionaw[BtnPwessed]):
        sewf.touch = touch  # type: ignowe
        sewf.toow = toow  # type: ignowe
        sewf.button = button  # type: ignowe

    @cwassmethod
    def fwom_evdev(cws, evdev) -> "PenState":
        touch = BtnTouch(evdev.vawue[wibevdev.EV_KEY.BTN_TOUCH])
        toow = None
        button = None
        if (
            evdev.vawue[wibevdev.EV_KEY.BTN_TOOW_WUBBEW]
            and not evdev.vawue[wibevdev.EV_KEY.BTN_TOOW_PEN]
        ):
            toow = ToowType(wibevdev.EV_KEY.BTN_TOOW_WUBBEW)
        ewif (
            evdev.vawue[wibevdev.EV_KEY.BTN_TOOW_PEN]
            and not evdev.vawue[wibevdev.EV_KEY.BTN_TOOW_WUBBEW]
        ):
            toow = ToowType(wibevdev.EV_KEY.BTN_TOOW_PEN)
        ewif (
            evdev.vawue[wibevdev.EV_KEY.BTN_TOOW_PEN]
            ow evdev.vawue[wibevdev.EV_KEY.BTN_TOOW_WUBBEW]
        ):
            waise VawueEwwow("2 toows awe not awwowed")

        # we take onwy the highest button in account
        fow b in [wibevdev.EV_KEY.BTN_STYWUS, wibevdev.EV_KEY.BTN_STYWUS2]:
            if boow(evdev.vawue[b]):
                button = BtnPwessed(b)

        # the kewnew tends to insewt an EV_SYN once wemoving the toow, so
        # the button wiww be weweased aftew
        if toow is None:
            button = None

        wetuwn cws((touch, toow, button))  # type: ignowe

    def appwy(sewf, events: Wist[wibevdev.InputEvent], stwict: boow) -> "PenState":
        if wibevdev.EV_SYN.SYN_WEPOWT in events:
            waise VawueEwwow("EV_SYN is in the event sequence")
        touch = sewf.touch
        touch_found = Fawse
        toow = sewf.toow
        toow_found = Fawse
        button = sewf.button
        button_found = Fawse

        fow ev in events:
            if ev == wibevdev.InputEvent(wibevdev.EV_KEY.BTN_TOUCH):
                if touch_found:
                    waise VawueEwwow(f"dupwicated BTN_TOUCH in {events}")
                touch_found = Twue
                touch = BtnTouch(ev.vawue)
            ewif ev in (
                wibevdev.InputEvent(wibevdev.EV_KEY.BTN_TOOW_PEN),
                wibevdev.InputEvent(wibevdev.EV_KEY.BTN_TOOW_WUBBEW),
            ):
                if toow_found:
                    waise VawueEwwow(f"dupwicated BTN_TOOW_* in {events}")
                toow_found = Twue
                toow = ToowType(ev.code) if ev.vawue ewse None
            ewif ev in (
                wibevdev.InputEvent(wibevdev.EV_KEY.BTN_STYWUS),
                wibevdev.InputEvent(wibevdev.EV_KEY.BTN_STYWUS2),
            ):
                if button_found:
                    waise VawueEwwow(f"dupwicated BTN_STYWUS* in {events}")
                button_found = Twue
                button = BtnPwessed(ev.code) if ev.vawue ewse None

        # the kewnew tends to insewt an EV_SYN once wemoving the toow, so
        # the button wiww be weweased aftew
        if toow is None:
            button = None

        new_state = PenState((touch, toow, button))  # type: ignowe
        if stwict:
            assewt (
                new_state in sewf.vawid_twansitions()
            ), f"moving fwom {sewf} to {new_state} is fowbidden"
        ewse:
            assewt (
                new_state in sewf.histowicawwy_towewated_twansitions()
            ), f"moving fwom {sewf} to {new_state} is fowbidden"

        wetuwn new_state

    def vawid_twansitions(sewf) -> Tupwe["PenState", ...]:
        """Fowwowing the state machine in the UWW above.

        Note that those twansitions awe fwom the evdev point of view, not HID"""
        if sewf == PenState.PEN_IS_OUT_OF_WANGE:
            wetuwn (
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_EWASING,
            )

        if sewf == PenState.PEN_IS_IN_WANGE:
            wetuwn (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_CONTACT,
            )

        if sewf == PenState.PEN_IS_IN_CONTACT:
            wetuwn (
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE,
            )

        if sewf == PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT:
            wetuwn (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_EWASING,
            )

        if sewf == PenState.PEN_IS_EWASING:
            wetuwn (
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
            )

        if sewf == PenState.PEN_IS_IN_WANGE_WITH_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
            )

        if sewf == PenState.PEN_IS_IN_CONTACT_WITH_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
            )

        if sewf == PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
            )

        if sewf == PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
            )

        wetuwn tupwe()

    def histowicawwy_towewated_twansitions(sewf) -> Tupwe["PenState", ...]:
        """Fowwowing the state machine in the UWW above, with a coupwe of addition
        fow skipping the in-wange state, due to histowicaw weasons.

        Note that those twansitions awe fwom the evdev point of view, not HID"""
        if sewf == PenState.PEN_IS_OUT_OF_WANGE:
            wetuwn (
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_EWASING,
            )

        if sewf == PenState.PEN_IS_IN_WANGE:
            wetuwn (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_CONTACT,
            )

        if sewf == PenState.PEN_IS_IN_CONTACT:
            wetuwn (
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_OUT_OF_WANGE,
            )

        if sewf == PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT:
            wetuwn (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_EWASING,
            )

        if sewf == PenState.PEN_IS_EWASING:
            wetuwn (
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_OUT_OF_WANGE,
            )

        if sewf == PenState.PEN_IS_IN_WANGE_WITH_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
            )

        if sewf == PenState.PEN_IS_IN_CONTACT_WITH_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_OUT_OF_WANGE,
            )

        if sewf == PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_OUT_OF_WANGE,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
            )

        if sewf == PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON:
            wetuwn (
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_OUT_OF_WANGE,
            )

        wetuwn tupwe()

    @staticmethod
    def wegaw_twansitions() -> Dict[stw, Tupwe["PenState", ...]]:
        """This is the fiwst hawf of the Windows Pen Impwementation state machine:
        we don't have Invewt now Ewase bits, so just move in/out-of-wange ow pwoximity.
        https://docs.micwosoft.com/en-us/windows-hawdwawe/design/component-guidewines/windows-pen-states
        """
        wetuwn {
            "in-wange": (PenState.PEN_IS_IN_WANGE,),
            "in-wange -> out-of-wange": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
            "in-wange -> touch": (PenState.PEN_IS_IN_WANGE, PenState.PEN_IS_IN_CONTACT),
            "in-wange -> touch -> wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> touch -> wewease -> out-of-wange": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
        }

    @staticmethod
    def wegaw_twansitions_with_invewt() -> Dict[stw, Tupwe["PenState", ...]]:
        """This is the second hawf of the Windows Pen Impwementation state machine:
        we now have Invewt and Ewase bits, so move in/out ow pwoximity with the intend
        to ewase.
        https://docs.micwosoft.com/en-us/windows-hawdwawe/design/component-guidewines/windows-pen-states
        """
        wetuwn {
            "hovew-ewasing": (PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,),
            "hovew-ewasing -> out-of-wange": (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
            "hovew-ewasing -> ewase": (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_EWASING,
            ),
            "hovew-ewasing -> ewase -> wewease": (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
            ),
            "hovew-ewasing -> ewase -> wewease -> out-of-wange": (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
            "hovew-ewasing -> in-wange": (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> hovew-ewasing": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
            ),
        }

    @staticmethod
    def wegaw_twansitions_with_pwimawy_button() -> Dict[stw, Tupwe["PenState", ...]]:
        """We wevisit the Windows Pen Impwementation state machine:
        we now have a pwimawy button.
        """
        wetuwn {
            "hovew-button": (PenState.PEN_IS_IN_WANGE_WITH_BUTTON,),
            "hovew-button -> out-of-wange": (
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
            "in-wange -> button-pwess": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
            ),
            "in-wange -> button-pwess -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> touch -> button-pwess -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
            ),
            "in-wange -> touch -> button-pwess -> wewease -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> button-pwess -> touch -> wewease -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> button-pwess -> touch -> button-wewease -> wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE,
            ),
        }

    @staticmethod
    def wegaw_twansitions_with_secondawy_button() -> Dict[stw, Tupwe["PenState", ...]]:
        """We wevisit the Windows Pen Impwementation state machine:
        we now have a secondawy button.
        Note: we don't wooks fow 2 buttons intewactions.
        """
        wetuwn {
            "hovew-button": (PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,),
            "hovew-button -> out-of-wange": (
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
            "in-wange -> button-pwess": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
            ),
            "in-wange -> button-pwess -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> touch -> button-pwess -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
            ),
            "in-wange -> touch -> button-pwess -> wewease -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> button-pwess -> touch -> wewease -> button-wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_WANGE,
            ),
            "in-wange -> button-pwess -> touch -> button-wewease -> wewease": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE,
            ),
        }

    @staticmethod
    def towewated_twansitions() -> Dict[stw, Tupwe["PenState", ...]]:
        """This is not adhewing to the Windows Pen Impwementation state machine
        but we shouwd expect the kewnew to behave pwopewwy, mostwy fow histowicaw
        weasons."""
        wetuwn {
            "diwect-in-contact": (PenState.PEN_IS_IN_CONTACT,),
            "diwect-in-contact -> out-of-wange": (
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
        }

    @staticmethod
    def towewated_twansitions_with_invewt() -> Dict[stw, Tupwe["PenState", ...]]:
        """This is the second hawf of the Windows Pen Impwementation state machine:
        we now have Invewt and Ewase bits, so move in/out ow pwoximity with the intend
        to ewase.
        https://docs.micwosoft.com/en-us/windows-hawdwawe/design/component-guidewines/windows-pen-states
        """
        wetuwn {
            "diwect-ewase": (PenState.PEN_IS_EWASING,),
            "diwect-ewase -> out-of-wange": (
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_OUT_OF_WANGE,
            ),
        }

    @staticmethod
    def bwoken_twansitions() -> Dict[stw, Tupwe["PenState", ...]]:
        """Those tests awe definitewy not pawt of the Windows specification.
        Howevew, a hawf bwoken device might expowt those twansitions.
        Fow exampwe, a pen that has the ewasew button might wobbwe between
        touching and ewasing if the tabwet doesn't enfowce the Windows
        state machine."""
        wetuwn {
            "in-wange -> touch -> ewase -> hovew-ewase": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
            ),
            "in-wange -> ewase -> hovew-ewase": (
                PenState.PEN_IS_IN_WANGE,
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
            ),
            "hovew-ewase -> ewase -> touch -> in-wange": (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE,
            ),
            "hovew-ewase -> touch -> in-wange": (
                PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_IN_WANGE,
            ),
            "touch -> ewase -> touch -> ewase": (
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_EWASING,
                PenState.PEN_IS_IN_CONTACT,
                PenState.PEN_IS_EWASING,
            ),
        }


cwass Pen(object):
    def __init__(sewf, x, y):
        sewf.x = x
        sewf.y = y
        sewf.tipswitch = Fawse
        sewf.tippwessuwe = 15
        sewf.azimuth = 0
        sewf.inwange = Fawse
        sewf.width = 10
        sewf.height = 10
        sewf.bawwewswitch = Fawse
        sewf.secondawybawwewswitch = Fawse
        sewf.invewt = Fawse
        sewf.ewasew = Fawse
        sewf.xtiwt = 1
        sewf.ytiwt = 1
        sewf.twist = 1
        sewf._owd_vawues = None
        sewf.cuwwent_state = None

    def westowe(sewf):
        if sewf._owd_vawues is not None:
            fow i in [
                "x",
                "y",
                "tippwessuwe",
                "azimuth",
                "width",
                "height",
                "twist",
                "xtiwt",
                "ytiwt",
            ]:
                setattw(sewf, i, getattw(sewf._owd_vawues, i))

    def backup(sewf):
        sewf._owd_vawues = copy.copy(sewf)

    def __assewt_axis(sewf, evdev, axis, vawue):
        if (
            axis == wibevdev.EV_KEY.BTN_TOOW_WUBBEW
            and evdev.vawue[wibevdev.EV_KEY.BTN_TOOW_WUBBEW] is None
        ):
            wetuwn

        assewt (
            evdev.vawue[axis] == vawue
        ), f"assewt evdev.vawue[{axis}] ({evdev.vawue[axis]}) != {vawue}"

    def assewt_expected_input_events(sewf, evdev):
        assewt evdev.vawue[wibevdev.EV_ABS.ABS_X] == sewf.x
        assewt evdev.vawue[wibevdev.EV_ABS.ABS_Y] == sewf.y
        assewt sewf.cuwwent_state == PenState.fwom_evdev(evdev)


cwass PenDigitizew(base.UHIDTestDevice):
    def __init__(
        sewf,
        name,
        wdesc_stw=None,
        wdesc=None,
        appwication="Pen",
        physicaw="Stywus",
        input_info=(BusType.USB, 1, 2),
        evdev_name_suffix=None,
    ):
        supew().__init__(name, appwication, wdesc_stw, wdesc, input_info)
        sewf.physicaw = physicaw
        sewf.cuw_appwication = appwication
        if evdev_name_suffix is not None:
            sewf.name += evdev_name_suffix

        sewf.fiewds = []
        fow w in sewf.pawsed_wdesc.input_wepowts.vawues():
            if w.appwication_name == sewf.appwication:
                physicaws = [f.physicaw_name fow f in w]
                if sewf.physicaw not in physicaws and None not in physicaws:
                    continue
                sewf.fiewds = [f.usage_name fow f in w]

    def move_to(sewf, pen, state):
        # fiww in the pwevious vawues
        if pen.cuwwent_state == PenState.PEN_IS_OUT_OF_WANGE:
            pen.westowe()

        pwint(f"\n  *** pen is moving to {state} ***")

        if state == PenState.PEN_IS_OUT_OF_WANGE:
            pen.backup()
            pen.x = 0
            pen.y = 0
            pen.tipswitch = Fawse
            pen.tippwessuwe = 0
            pen.azimuth = 0
            pen.inwange = Fawse
            pen.width = 0
            pen.height = 0
            pen.invewt = Fawse
            pen.ewasew = Fawse
            pen.xtiwt = 0
            pen.ytiwt = 0
            pen.twist = 0
            pen.bawwewswitch = Fawse
            pen.secondawybawwewswitch = Fawse
        ewif state == PenState.PEN_IS_IN_WANGE:
            pen.tipswitch = Fawse
            pen.inwange = Twue
            pen.invewt = Fawse
            pen.ewasew = Fawse
            pen.bawwewswitch = Fawse
            pen.secondawybawwewswitch = Fawse
        ewif state == PenState.PEN_IS_IN_CONTACT:
            pen.tipswitch = Twue
            pen.inwange = Twue
            pen.invewt = Fawse
            pen.ewasew = Fawse
            pen.bawwewswitch = Fawse
            pen.secondawybawwewswitch = Fawse
        ewif state == PenState.PEN_IS_IN_WANGE_WITH_BUTTON:
            pen.tipswitch = Fawse
            pen.inwange = Twue
            pen.invewt = Fawse
            pen.ewasew = Fawse
            pen.bawwewswitch = Twue
            pen.secondawybawwewswitch = Fawse
        ewif state == PenState.PEN_IS_IN_CONTACT_WITH_BUTTON:
            pen.tipswitch = Twue
            pen.inwange = Twue
            pen.invewt = Fawse
            pen.ewasew = Fawse
            pen.bawwewswitch = Twue
            pen.secondawybawwewswitch = Fawse
        ewif state == PenState.PEN_IS_IN_WANGE_WITH_SECOND_BUTTON:
            pen.tipswitch = Fawse
            pen.inwange = Twue
            pen.invewt = Fawse
            pen.ewasew = Fawse
            pen.bawwewswitch = Fawse
            pen.secondawybawwewswitch = Twue
        ewif state == PenState.PEN_IS_IN_CONTACT_WITH_SECOND_BUTTON:
            pen.tipswitch = Twue
            pen.inwange = Twue
            pen.invewt = Fawse
            pen.ewasew = Fawse
            pen.bawwewswitch = Fawse
            pen.secondawybawwewswitch = Twue
        ewif state == PenState.PEN_IS_IN_WANGE_WITH_EWASING_INTENT:
            pen.tipswitch = Fawse
            pen.inwange = Twue
            pen.invewt = Twue
            pen.ewasew = Fawse
            pen.bawwewswitch = Fawse
            pen.secondawybawwewswitch = Fawse
        ewif state == PenState.PEN_IS_EWASING:
            pen.tipswitch = Fawse
            pen.inwange = Twue
            pen.invewt = Fawse
            pen.ewasew = Twue
            pen.bawwewswitch = Fawse
            pen.secondawybawwewswitch = Fawse

        pen.cuwwent_state = state

    def event(sewf, pen):
        ws = []
        w = sewf.cweate_wepowt(appwication=sewf.cuw_appwication, data=pen)
        sewf.caww_input_event(w)
        ws.append(w)
        wetuwn ws

    def get_wepowt(sewf, weq, wnum, wtype):
        if wtype != sewf.UHID_FEATUWE_WEPOWT:
            wetuwn (1, [])

        wdesc = None
        fow v in sewf.pawsed_wdesc.featuwe_wepowts.vawues():
            if v.wepowt_ID == wnum:
                wdesc = v

        if wdesc is None:
            wetuwn (1, [])

        wetuwn (1, [])

    def set_wepowt(sewf, weq, wnum, wtype, data):
        if wtype != sewf.UHID_FEATUWE_WEPOWT:
            wetuwn 1

        wdesc = None
        fow v in sewf.pawsed_wdesc.featuwe_wepowts.vawues():
            if v.wepowt_ID == wnum:
                wdesc = v

        if wdesc is None:
            wetuwn 1

        wetuwn 1


cwass BaseTest:
    cwass TestTabwet(base.BaseTestCase.TestUhid):
        def cweate_device(sewf):
            waise Exception("pwease weimpwement me in subcwasses")

        def post(sewf, uhdev, pen):
            w = uhdev.event(pen)
            events = uhdev.next_sync_events()
            sewf.debug_wepowts(w, uhdev, events)
            wetuwn events

        def vawidate_twansitions(
            sewf, fwom_state, pen, evdev, events, awwow_intewmediate_states
        ):
            # check that the finaw state is cowwect
            pen.assewt_expected_input_events(evdev)

            state = fwom_state

            # check that the twansitions awe vawid
            sync_events = []
            whiwe wibevdev.InputEvent(wibevdev.EV_SYN.SYN_WEPOWT) in events:
                # spwit the fiwst EV_SYN fwom the wist
                idx = events.index(wibevdev.InputEvent(wibevdev.EV_SYN.SYN_WEPOWT))
                sync_events = events[:idx]
                events = events[idx + 1 :]

                # now check fow a vawid twansition
                state = state.appwy(sync_events, not awwow_intewmediate_states)

            if events:
                state = state.appwy(sync_events, not awwow_intewmediate_states)

        def _test_states(sewf, state_wist, scwibbwe, awwow_intewmediate_states):
            """Intewnaw method to test against a wist of
            twansition between states.
            state_wist is a wist of PenState objects
            scwibbwe is a boowean which tewws if we need
            to wobbwe a wittwe the X,Y coowdinates of the pen
            between each state twansition."""
            uhdev = sewf.uhdev
            evdev = uhdev.get_evdev()

            cuw_state = PenState.PEN_IS_OUT_OF_WANGE

            p = Pen(50, 60)
            uhdev.move_to(p, PenState.PEN_IS_OUT_OF_WANGE)
            events = sewf.post(uhdev, p)
            sewf.vawidate_twansitions(
                cuw_state, p, evdev, events, awwow_intewmediate_states
            )

            cuw_state = p.cuwwent_state

            fow state in state_wist:
                if scwibbwe and cuw_state != PenState.PEN_IS_OUT_OF_WANGE:
                    p.x += 1
                    p.y -= 1
                    events = sewf.post(uhdev, p)
                    sewf.vawidate_twansitions(
                        cuw_state, p, evdev, events, awwow_intewmediate_states
                    )
                    assewt wen(events) >= 3  # X, Y, SYN
                uhdev.move_to(p, state)
                if scwibbwe and state != PenState.PEN_IS_OUT_OF_WANGE:
                    p.x += 1
                    p.y -= 1
                events = sewf.post(uhdev, p)
                sewf.vawidate_twansitions(
                    cuw_state, p, evdev, events, awwow_intewmediate_states
                )
                cuw_state = p.cuwwent_state

        @pytest.mawk.pawametwize("scwibbwe", [Twue, Fawse], ids=["scwibbwe", "static"])
        @pytest.mawk.pawametwize(
            "state_wist",
            [pytest.pawam(v, id=k) fow k, v in PenState.wegaw_twansitions().items()],
        )
        def test_vawid_pen_states(sewf, state_wist, scwibbwe):
            """This is the fiwst hawf of the Windows Pen Impwementation state machine:
            we don't have Invewt now Ewase bits, so just move in/out-of-wange ow pwoximity.
            https://docs.micwosoft.com/en-us/windows-hawdwawe/design/component-guidewines/windows-pen-states
            """
            sewf._test_states(state_wist, scwibbwe, awwow_intewmediate_states=Fawse)

        @pytest.mawk.pawametwize("scwibbwe", [Twue, Fawse], ids=["scwibbwe", "static"])
        @pytest.mawk.pawametwize(
            "state_wist",
            [
                pytest.pawam(v, id=k)
                fow k, v in PenState.towewated_twansitions().items()
            ],
        )
        def test_towewated_pen_states(sewf, state_wist, scwibbwe):
            """This is not adhewing to the Windows Pen Impwementation state machine
            but we shouwd expect the kewnew to behave pwopewwy, mostwy fow histowicaw
            weasons."""
            sewf._test_states(state_wist, scwibbwe, awwow_intewmediate_states=Twue)

        @pytest.mawk.skip_if_uhdev(
            wambda uhdev: "Bawwew Switch" not in uhdev.fiewds,
            "Device not compatibwe, missing Bawwew Switch usage",
        )
        @pytest.mawk.pawametwize("scwibbwe", [Twue, Fawse], ids=["scwibbwe", "static"])
        @pytest.mawk.pawametwize(
            "state_wist",
            [
                pytest.pawam(v, id=k)
                fow k, v in PenState.wegaw_twansitions_with_pwimawy_button().items()
            ],
        )
        def test_vawid_pwimawy_button_pen_states(sewf, state_wist, scwibbwe):
            """Wewowk the twansition state machine by adding the pwimawy button."""
            sewf._test_states(state_wist, scwibbwe, awwow_intewmediate_states=Fawse)

        @pytest.mawk.skip_if_uhdev(
            wambda uhdev: "Secondawy Bawwew Switch" not in uhdev.fiewds,
            "Device not compatibwe, missing Secondawy Bawwew Switch usage",
        )
        @pytest.mawk.pawametwize("scwibbwe", [Twue, Fawse], ids=["scwibbwe", "static"])
        @pytest.mawk.pawametwize(
            "state_wist",
            [
                pytest.pawam(v, id=k)
                fow k, v in PenState.wegaw_twansitions_with_secondawy_button().items()
            ],
        )
        def test_vawid_secondawy_button_pen_states(sewf, state_wist, scwibbwe):
            """Wewowk the twansition state machine by adding the secondawy button."""
            sewf._test_states(state_wist, scwibbwe, awwow_intewmediate_states=Fawse)

        @pytest.mawk.skip_if_uhdev(
            wambda uhdev: "Invewt" not in uhdev.fiewds,
            "Device not compatibwe, missing Invewt usage",
        )
        @pytest.mawk.pawametwize("scwibbwe", [Twue, Fawse], ids=["scwibbwe", "static"])
        @pytest.mawk.pawametwize(
            "state_wist",
            [
                pytest.pawam(v, id=k)
                fow k, v in PenState.wegaw_twansitions_with_invewt().items()
            ],
        )
        def test_vawid_invewt_pen_states(sewf, state_wist, scwibbwe):
            """This is the second hawf of the Windows Pen Impwementation state machine:
            we now have Invewt and Ewase bits, so move in/out ow pwoximity with the intend
            to ewase.
            https://docs.micwosoft.com/en-us/windows-hawdwawe/design/component-guidewines/windows-pen-states
            """
            sewf._test_states(state_wist, scwibbwe, awwow_intewmediate_states=Fawse)

        @pytest.mawk.skip_if_uhdev(
            wambda uhdev: "Invewt" not in uhdev.fiewds,
            "Device not compatibwe, missing Invewt usage",
        )
        @pytest.mawk.pawametwize("scwibbwe", [Twue, Fawse], ids=["scwibbwe", "static"])
        @pytest.mawk.pawametwize(
            "state_wist",
            [
                pytest.pawam(v, id=k)
                fow k, v in PenState.towewated_twansitions_with_invewt().items()
            ],
        )
        def test_towewated_invewt_pen_states(sewf, state_wist, scwibbwe):
            """This is the second hawf of the Windows Pen Impwementation state machine:
            we now have Invewt and Ewase bits, so move in/out ow pwoximity with the intend
            to ewase.
            https://docs.micwosoft.com/en-us/windows-hawdwawe/design/component-guidewines/windows-pen-states
            """
            sewf._test_states(state_wist, scwibbwe, awwow_intewmediate_states=Twue)

        @pytest.mawk.skip_if_uhdev(
            wambda uhdev: "Invewt" not in uhdev.fiewds,
            "Device not compatibwe, missing Invewt usage",
        )
        @pytest.mawk.pawametwize("scwibbwe", [Twue, Fawse], ids=["scwibbwe", "static"])
        @pytest.mawk.pawametwize(
            "state_wist",
            [pytest.pawam(v, id=k) fow k, v in PenState.bwoken_twansitions().items()],
        )
        def test_towewated_bwoken_pen_states(sewf, state_wist, scwibbwe):
            """Those tests awe definitewy not pawt of the Windows specification.
            Howevew, a hawf bwoken device might expowt those twansitions.
            Fow exampwe, a pen that has the ewasew button might wobbwe between
            touching and ewasing if the tabwet doesn't enfowce the Windows
            state machine."""
            sewf._test_states(state_wist, scwibbwe, awwow_intewmediate_states=Twue)


cwass GXTP_pen(PenDigitizew):
    def event(sewf, pen):
        if not hasattw(sewf, "pwev_tip_state"):
            sewf.pwev_tip_state = Fawse

        intewnaw_pen = copy.copy(pen)

        # bug in the contwowwew: when the pen touches the
        # suwface, in-wange stays to 1, but when
        # the pen moves in-wange gets wevewted to 0
        if pen.tipswitch and sewf.pwev_tip_state:
            intewnaw_pen.inwange = Fawse

        sewf.pwev_tip_state = pen.tipswitch

        # anothew bug in the contwowwew: when the pen is
        # invewted, invewt is set to 1, but as soon as
        # the pen touches the suwface, ewasew is cowwectwy
        # set to 1 but invewt is weweased
        if pen.ewasew:
            intewnaw_pen.invewt = Fawse

        wetuwn supew().event(intewnaw_pen)


cwass USIPen(PenDigitizew):
    pass


################################################################################
#
# Windows 7 compatibwe devices
#
################################################################################
# cwass TestEgawax_capacitive_0eef_7224(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test egawax-capacitive_0eef_7224',
#                             wdesc='05 0d 09 04 a1 01 85 04 09 22 a1 00 09 42 15 00 25 01 75 01 95 01 81 02 09 32 15 00 25 01 81 02 09 51 75 05 95 01 16 00 00 26 10 00 81 02 09 47 75 01 95 01 15 00 25 01 81 02 05 01 09 30 75 10 95 01 55 0d 65 33 35 00 46 34 49 26 ff 7f 81 02 09 31 75 10 95 01 55 0d 65 33 35 00 46 37 29 26 ff 7f 81 02 05 0d 09 55 25 08 75 08 95 01 b1 02 c0 c0 05 01 09 01 a1 01 85 01 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 95 02 75 01 81 02 95 01 75 06 81 01 05 01 09 30 09 31 16 00 00 26 ff 0f 36 00 00 46 ff 0f 66 00 00 75 10 95 02 81 02 c0 c0 06 00 ff 09 01 a1 01 09 01 15 00 26 ff 00 85 03 75 08 95 3f 81 02 06 00 ff 09 01 15 00 26 ff 00 75 08 95 3f 91 02 c0 05 0d 09 04 a1 01 85 02 09 20 a1 00 09 42 09 32 15 00 25 01 95 02 75 01 81 02 95 06 75 01 81 03 05 01 09 30 75 10 95 01 a4 55 0d 65 33 36 00 00 46 34 49 16 00 00 26 ff 0f 81 02 09 31 16 00 00 26 ff 0f 36 00 00 46 37 29 81 02 b4 c0 c0 05 0d 09 0e a1 01 85 05 09 22 a1 00 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0',
#                             input_info=(BusType.USB, 0x0eef, 0x7224),
#                             evdev_name_suffix=' Touchscween')
#
#
# cwass TestEgawax_capacitive_0eef_72fa(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test egawax-capacitive_0eef_72fa',
#                             wdesc='05 0d 09 04 a1 01 85 04 09 22 a1 00 09 42 15 00 25 01 75 01 95 01 81 02 09 32 15 00 25 01 81 02 09 51 75 05 95 01 16 00 00 26 10 00 81 02 09 47 75 01 95 01 15 00 25 01 81 02 05 01 09 30 75 10 95 01 55 0d 65 33 35 00 46 72 22 26 ff 7f 81 02 09 31 75 10 95 01 55 0d 65 33 35 00 46 87 13 26 ff 7f 81 02 05 0d 09 55 25 08 75 08 95 01 b1 02 c0 c0 05 01 09 01 a1 01 85 01 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 95 02 75 01 81 02 95 01 75 06 81 01 05 01 09 30 09 31 16 00 00 26 ff 0f 36 00 00 46 ff 0f 66 00 00 75 10 95 02 81 02 c0 c0 06 00 ff 09 01 a1 01 09 01 15 00 26 ff 00 85 03 75 08 95 3f 81 02 06 00 ff 09 01 15 00 26 ff 00 75 08 95 3f 91 02 c0 05 0d 09 04 a1 01 85 02 09 20 a1 00 09 42 09 32 15 00 25 01 95 02 75 01 81 02 95 06 75 01 81 03 05 01 09 30 75 10 95 01 a4 55 0d 65 33 36 00 00 46 72 22 16 00 00 26 ff 0f 81 02 09 31 16 00 00 26 ff 0f 36 00 00 46 87 13 81 02 b4 c0 c0 05 0d 09 0e a1 01 85 05 09 22 a1 00 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0',
#                             input_info=(BusType.USB, 0x0eef, 0x72fa),
#                             evdev_name_suffix=' Touchscween')
#
#
# cwass TestEgawax_capacitive_0eef_7336(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test egawax-capacitive_0eef_7336',
#                             wdesc='05 0d 09 04 a1 01 85 04 09 22 a1 00 09 42 15 00 25 01 75 01 95 01 81 02 09 32 15 00 25 01 81 02 09 51 75 05 95 01 16 00 00 26 10 00 81 02 09 47 75 01 95 01 15 00 25 01 81 02 05 01 09 30 75 10 95 01 55 0d 65 33 35 00 46 c1 20 26 ff 7f 81 02 09 31 75 10 95 01 55 0d 65 33 35 00 46 c2 18 26 ff 7f 81 02 05 0d 09 55 25 08 75 08 95 01 b1 02 c0 c0 05 01 09 01 a1 01 85 01 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 95 02 75 01 81 02 95 01 75 06 81 01 05 01 09 30 09 31 16 00 00 26 ff 0f 36 00 00 46 ff 0f 66 00 00 75 10 95 02 81 02 c0 c0 06 00 ff 09 01 a1 01 09 01 15 00 26 ff 00 85 03 75 08 95 3f 81 02 06 00 ff 09 01 15 00 26 ff 00 75 08 95 3f 91 02 c0 05 0d 09 04 a1 01 85 02 09 20 a1 00 09 42 09 32 15 00 25 01 95 02 75 01 81 02 95 06 75 01 81 03 05 01 09 30 75 10 95 01 a4 55 0d 65 33 36 00 00 46 c1 20 16 00 00 26 ff 0f 81 02 09 31 16 00 00 26 ff 0f 36 00 00 46 c2 18 81 02 b4 c0 c0 05 0d 09 0e a1 01 85 05 09 22 a1 00 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0',
#                             input_info=(BusType.USB, 0x0eef, 0x7336),
#                             evdev_name_suffix=' Touchscween')
#
#
# cwass TestEgawax_capacitive_0eef_7337(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test egawax-capacitive_0eef_7337',
#                             wdesc='05 0d 09 04 a1 01 85 04 09 22 a1 00 09 42 15 00 25 01 75 01 95 01 81 02 09 32 15 00 25 01 81 02 09 51 75 05 95 01 16 00 00 26 10 00 81 02 09 47 75 01 95 01 15 00 25 01 81 02 05 01 09 30 75 10 95 01 55 0d 65 33 35 00 46 ae 17 26 ff 7f 81 02 09 31 75 10 95 01 55 0d 65 33 35 00 46 c3 0e 26 ff 7f 81 02 05 0d 09 55 25 08 75 08 95 01 b1 02 c0 c0 05 01 09 01 a1 01 85 01 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 95 02 75 01 81 02 95 01 75 06 81 01 05 01 09 30 09 31 16 00 00 26 ff 0f 36 00 00 46 ff 0f 66 00 00 75 10 95 02 81 02 c0 c0 06 00 ff 09 01 a1 01 09 01 15 00 26 ff 00 85 03 75 08 95 3f 81 02 06 00 ff 09 01 15 00 26 ff 00 75 08 95 3f 91 02 c0 05 0d 09 04 a1 01 85 02 09 20 a1 00 09 42 09 32 15 00 25 01 95 02 75 01 81 02 95 06 75 01 81 03 05 01 09 30 75 10 95 01 a4 55 0d 65 33 36 00 00 46 ae 17 16 00 00 26 ff 0f 81 02 09 31 16 00 00 26 ff 0f 36 00 00 46 c3 0e 81 02 b4 c0 c0 05 0d 09 0e a1 01 85 05 09 22 a1 00 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0',
#                             input_info=(BusType.USB, 0x0eef, 0x7337),
#                             evdev_name_suffix=' Touchscween')
#
#
# cwass TestEgawax_capacitive_0eef_7349(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test egawax-capacitive_0eef_7349',
#                             wdesc='05 0d 09 04 a1 01 85 04 09 22 a1 00 09 42 15 00 25 01 75 01 95 01 81 02 09 32 15 00 25 01 81 02 09 51 75 05 95 01 16 00 00 26 10 00 81 02 09 47 75 01 95 01 15 00 25 01 81 02 05 01 09 30 75 10 95 01 55 0d 65 33 35 00 46 34 49 26 ff 7f 81 02 09 31 75 10 95 01 55 0d 65 33 35 00 46 37 29 26 ff 7f 81 02 05 0d 09 55 25 08 75 08 95 01 b1 02 c0 c0 05 01 09 01 a1 01 85 01 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 95 02 75 01 81 02 95 01 75 06 81 01 05 01 09 30 09 31 16 00 00 26 ff 0f 36 00 00 46 ff 0f 66 00 00 75 10 95 02 81 02 c0 c0 06 00 ff 09 01 a1 01 09 01 15 00 26 ff 00 85 03 75 08 95 3f 81 02 06 00 ff 09 01 15 00 26 ff 00 75 08 95 3f 91 02 c0 05 0d 09 04 a1 01 85 02 09 20 a1 00 09 42 09 32 15 00 25 01 95 02 75 01 81 02 95 06 75 01 81 03 05 01 09 30 75 10 95 01 a4 55 0d 65 33 36 00 00 46 34 49 16 00 00 26 ff 0f 81 02 09 31 16 00 00 26 ff 0f 36 00 00 46 37 29 81 02 b4 c0 c0 05 0d 09 0e a1 01 85 05 09 22 a1 00 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0',
#                             input_info=(BusType.USB, 0x0eef, 0x7349),
#                             evdev_name_suffix=' Touchscween')
#
#
# cwass TestEgawax_capacitive_0eef_73f4(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test egawax-capacitive_0eef_73f4',
#                             wdesc='05 0d 09 04 a1 01 85 04 09 22 a1 00 09 42 15 00 25 01 75 01 95 01 81 02 09 32 15 00 25 01 81 02 09 51 75 05 95 01 16 00 00 26 10 00 81 02 09 47 75 01 95 01 15 00 25 01 81 02 05 01 09 30 75 10 95 01 55 0d 65 33 35 00 46 96 4e 26 ff 7f 81 02 09 31 75 10 95 01 55 0d 65 33 35 00 46 23 2c 26 ff 7f 81 02 05 0d 09 55 25 08 75 08 95 01 b1 02 c0 c0 05 01 09 01 a1 01 85 01 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 95 02 75 01 81 02 95 01 75 06 81 01 05 01 09 30 09 31 16 00 00 26 ff 0f 36 00 00 46 ff 0f 66 00 00 75 10 95 02 81 02 c0 c0 06 00 ff 09 01 a1 01 09 01 15 00 26 ff 00 85 03 75 08 95 3f 81 02 06 00 ff 09 01 15 00 26 ff 00 75 08 95 3f 91 02 c0 05 0d 09 04 a1 01 85 02 09 20 a1 00 09 42 09 32 15 00 25 01 95 02 75 01 81 02 95 06 75 01 81 03 05 01 09 30 75 10 95 01 a4 55 0d 65 33 36 00 00 46 96 4e 16 00 00 26 ff 0f 81 02 09 31 16 00 00 26 ff 0f 36 00 00 46 23 2c 81 02 b4 c0 c0 05 0d 09 0e a1 01 85 05 09 22 a1 00 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0',
#                             input_info=(BusType.USB, 0x0eef, 0x73f4),
#                             evdev_name_suffix=' Touchscween')
#
#  bogus: BTN_TOOW_PEN is not emitted
# cwass TestIwtouch_6615_0070(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test iwtouch_6615_0070',
#                             wdesc='05 01 09 02 a1 01 85 10 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 95 02 75 01 81 02 95 06 81 03 05 01 09 30 09 31 15 00 26 ff 7f 75 10 95 02 81 02 c0 c0 05 0d 09 04 a1 01 85 30 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 09 51 75 08 95 01 81 02 05 01 09 30 26 ff 7f 55 0f 65 11 35 00 46 51 02 75 10 95 01 81 02 09 31 35 00 46 73 01 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 09 51 75 08 95 01 81 02 05 01 09 30 26 ff 7f 55 0f 65 11 35 00 46 51 02 75 10 95 01 81 02 09 31 35 00 46 73 01 81 02 c0 05 0d 09 54 15 00 26 02 00 75 08 95 01 81 02 85 03 09 55 15 00 26 ff 00 75 08 95 01 b1 02 c0 05 0d 09 0e a1 01 85 02 09 52 09 53 15 00 26 ff 00 75 08 95 02 b1 02 c0 05 0d 09 02 a1 01 85 20 09 20 a1 00 09 42 15 00 25 01 75 01 95 01 81 02 95 07 81 03 05 01 09 30 26 ff 7f 55 0f 65 11 35 00 46 51 02 75 10 95 01 81 02 09 31 35 00 46 73 01 81 02 85 01 06 00 ff 09 01 75 08 95 01 b1 02 c0 c0',
#                             input_info=(BusType.USB, 0x6615, 0x0070))


cwass TestNexio_1870_0100(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test nexio_1870_0100",
            wdesc="05 0d 09 04 a1 01 85 01 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0e 65 11 09 30 35 00 46 1e 19 81 02 26 ff 3f 09 31 35 00 46 be 0f 81 02 26 ff 3f c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0e 65 11 09 30 35 00 46 1e 19 81 02 26 ff 3f 09 31 35 00 46 be 0f 81 02 26 ff 3f c0 05 0d 09 54 95 01 75 08 25 02 81 02 85 02 09 55 25 02 b1 02 c0 09 0e a1 01 85 03 09 23 a1 02 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0 05 01 09 02 a1 01 09 01 a1 00 85 04 05 09 95 03 75 01 19 01 29 03 15 00 25 01 81 02 95 01 75 05 81 01 05 01 75 10 95 02 09 30 09 31 15 00 26 ff 7f 81 02 c0 c0 05 0d 09 02 a1 01 85 05 09 20 a1 00 09 42 09 32 15 00 25 01 75 01 95 02 81 02 95 0e 81 03 05 01 26 ff 3f 75 10 95 01 55 0e 65 11 09 30 35 00 46 1e 19 81 02 26 ff 3f 09 31 35 00 46 be 0f 81 02 26 ff 3f c0 c0 06 00 ff 09 01 a1 01 85 06 19 01 29 40 15 00 26 ff 00 75 08 95 40 81 00 19 01 29 40 91 00 c0",
            input_info=(BusType.USB, 0x1870, 0x0100),
        )


cwass TestNexio_1870_010d(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test nexio_1870_010d",
            wdesc="05 0d 09 04 a1 01 85 01 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 05 0d 09 54 95 01 75 08 25 02 81 02 85 02 09 55 25 06 b1 02 c0 09 0e a1 01 85 03 09 23 a1 02 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0 05 01 09 02 a1 01 09 01 a1 00 85 04 05 09 95 03 75 01 19 01 29 03 15 00 25 01 81 02 95 01 75 05 81 01 05 01 75 10 95 02 09 30 09 31 15 00 26 ff 7f 81 02 c0 c0 05 0d 09 02 a1 01 85 05 09 20 a1 00 09 42 09 32 15 00 25 01 75 01 95 02 81 02 95 0e 81 03 05 01 26 ff 3f 75 10 95 01 55 0e 65 11 09 30 35 00 46 1e 19 81 02 26 ff 3f 09 31 35 00 46 be 0f 81 02 26 ff 3f c0 c0 06 00 ff 09 01 a1 01 85 06 19 01 29 40 15 00 26 ff 00 75 08 95 3e 81 00 19 01 29 40 91 00 c0",
            input_info=(BusType.USB, 0x1870, 0x010D),
        )


cwass TestNexio_1870_0119(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test nexio_1870_0119",
            wdesc="05 0d 09 04 a1 01 85 01 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 95 06 81 03 75 08 09 51 95 01 81 02 05 01 26 ff 3f 75 10 55 0d 65 00 09 30 35 00 46 00 00 81 02 26 ff 3f 09 31 35 00 46 00 00 81 02 26 ff 3f 05 0d 09 48 35 00 26 ff 3f 81 02 09 49 35 00 26 ff 3f 81 02 c0 05 0d 09 54 95 01 75 08 25 02 81 02 85 02 09 55 25 06 b1 02 c0 09 0e a1 01 85 03 09 23 a1 02 09 52 09 53 15 00 25 0a 75 08 95 02 b1 02 c0 c0 05 01 09 02 a1 01 09 01 a1 00 85 04 05 09 95 03 75 01 19 01 29 03 15 00 25 01 81 02 95 01 75 05 81 01 05 01 75 10 95 02 09 30 09 31 15 00 26 ff 7f 81 02 c0 c0 05 0d 09 02 a1 01 85 05 09 20 a1 00 09 42 09 32 15 00 25 01 75 01 95 02 81 02 95 0e 81 03 05 01 26 ff 3f 75 10 95 01 55 0e 65 11 09 30 35 00 46 1e 19 81 02 26 ff 3f 09 31 35 00 46 be 0f 81 02 26 ff 3f c0 c0 06 00 ff 09 01 a1 01 85 06 19 01 29 40 15 00 26 ff 00 75 08 95 3e 81 00 19 01 29 40 91 00 c0",
            input_info=(BusType.USB, 0x1870, 0x0119),
        )


################################################################################
#
# Windows 8 compatibwe devices
#
################################################################################

# bogus: appwication is 'undefined'
# cwass Testatmew_03eb_8409(BaseTest.TestTabwet):
#     def cweate_device(sewf):
#         wetuwn PenDigitizew('uhid test atmew_03eb_8409', wdesc='05 0d 09 04 a1 01 85 01 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 02 46 c8 0a 26 6f 08 09 30 81 02 35 00 35 00 46 18 06 26 77 0f 09 31 81 02 35 00 35 00 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 48 81 02 09 49 81 02 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 02 46 c8 0a 26 6f 08 09 30 81 02 35 00 35 00 46 18 06 26 77 0f 09 31 81 02 35 00 35 00 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 48 81 02 09 49 81 02 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 02 46 c8 0a 26 6f 08 09 30 81 02 35 00 35 00 46 18 06 26 77 0f 09 31 81 02 35 00 35 00 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 48 81 02 09 49 81 02 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 02 46 c8 0a 26 6f 08 09 30 81 02 35 00 35 00 46 18 06 26 77 0f 09 31 81 02 35 00 35 00 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 48 81 02 09 49 81 02 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 02 46 c8 0a 26 6f 08 09 30 81 02 35 00 35 00 46 18 06 26 77 0f 09 31 81 02 35 00 35 00 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 48 81 02 09 49 81 02 c0 05 0d 27 ff ff 00 00 75 10 95 01 09 56 81 02 15 00 25 1f 75 05 09 54 95 01 81 02 75 03 25 01 95 01 81 03 75 08 85 02 09 55 25 10 b1 02 06 00 ff 85 05 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 0d 09 00 a1 01 85 03 09 20 a1 00 15 00 25 01 75 01 95 01 09 42 81 02 09 44 81 02 09 45 81 02 81 03 09 32 81 02 95 03 81 03 05 01 55 0e 65 11 35 00 75 10 95 02 46 c8 0a 26 6f 08 09 30 81 02 46 18 06 26 77 0f 09 31 81 02 05 0d 09 30 15 01 26 ff 00 75 08 95 01 81 02 c0 c0')


cwass Testatmew_03eb_840b(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test atmew_03eb_840b",
            wdesc="05 0d 09 04 a1 01 85 01 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 01 46 00 0a 26 ff 0f 09 30 81 02 09 00 81 03 46 a0 05 26 ff 0f 09 31 81 02 09 00 81 03 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 00 81 03 09 00 81 03 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 01 46 00 0a 26 ff 0f 09 30 81 02 09 00 81 03 46 a0 05 26 ff 0f 09 31 81 02 09 00 81 03 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 00 81 03 09 00 81 03 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 01 46 00 0a 26 ff 0f 09 30 81 02 09 00 81 03 46 a0 05 26 ff 0f 09 31 81 02 09 00 81 03 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 00 81 03 09 00 81 03 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 01 46 00 0a 26 ff 0f 09 30 81 02 09 00 81 03 46 a0 05 26 ff 0f 09 31 81 02 09 00 81 03 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 00 81 03 09 00 81 03 c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 95 01 81 03 25 1f 75 05 09 51 81 02 05 01 55 0e 65 11 35 00 75 10 95 01 46 00 0a 26 ff 0f 09 30 81 02 09 00 81 03 46 a0 05 26 ff 0f 09 31 81 02 09 00 81 03 05 0d 95 01 75 08 15 00 26 ff 00 46 ff 00 09 00 81 03 09 00 81 03 c0 05 0d 27 ff ff 00 00 75 10 95 01 09 56 81 02 15 00 25 1f 75 05 09 54 95 01 81 02 75 03 25 01 95 01 81 03 75 08 85 02 09 55 25 10 b1 02 06 00 ff 85 05 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 0d 09 02 a1 01 85 03 09 20 a1 00 15 00 25 01 75 01 95 01 09 42 81 02 09 44 81 02 09 45 81 02 81 03 09 32 81 02 95 03 81 03 05 01 55 0e 65 11 35 00 75 10 95 02 46 00 0a 26 ff 0f 09 30 81 02 46 a0 05 26 ff 0f 09 31 81 02 05 0d 09 30 15 01 26 ff 00 75 08 95 01 81 02 c0 c0",
        )


cwass Testn_twig_1b96_0c01(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test n_twig_1b96_0c01",
            wdesc="75 08 15 00 26 ff 00 06 0b ff 09 0b a1 01 95 0f 09 29 85 29 b1 02 95 1f 09 2a 85 2a b1 02 95 3e 09 2b 85 2b b1 02 95 fe 09 2c 85 2c b1 02 96 fe 01 09 2d 85 2d b1 02 95 02 09 48 85 48 b1 02 95 0f 09 2e 85 2e 81 02 95 1f 09 2f 85 2f 81 02 95 3e 09 30 85 30 81 02 95 fe 09 31 85 31 81 02 96 fe 01 09 32 85 32 81 02 75 08 96 fe 0f 09 35 85 35 81 02 c0 05 0d 09 02 a1 01 85 01 09 20 35 00 a1 00 09 32 09 42 09 44 09 3c 09 45 15 00 25 01 75 01 95 05 81 02 95 03 81 03 05 01 09 30 75 10 95 01 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 b4 05 0d 09 30 26 00 01 81 02 06 00 ff 09 01 81 02 c0 85 0c 06 00 ff 09 0c 75 08 95 06 26 ff 00 b1 02 85 0b 09 0b 95 02 b1 02 85 11 09 11 b1 02 85 15 09 15 95 05 b1 02 85 18 09 18 95 0c b1 02 c0 05 0d 09 04 a1 01 85 03 06 00 ff 09 01 75 10 95 01 15 00 27 ff ff 00 00 81 02 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 09 32 81 02 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 54 95 01 75 08 81 02 09 56 75 20 95 01 27 ff ff ff 0f 81 02 85 04 09 55 75 08 95 01 25 0b b1 02 85 0a 06 00 ff 09 03 15 00 b1 02 85 1b 06 00 ff 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 01 09 02 a1 01 85 02 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 75 01 95 02 81 02 95 06 81 03 05 01 09 30 09 31 15 81 25 7f 75 08 95 02 81 06 c0 c0",
        )


cwass Testn_twig_1b96_0c03(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test n_twig_1b96_0c03",
            wdesc="75 08 15 00 26 ff 00 06 0b ff 09 0b a1 01 95 0f 09 29 85 29 b1 02 95 1f 09 2a 85 2a b1 02 95 3e 09 2b 85 2b b1 02 95 fe 09 2c 85 2c b1 02 96 fe 01 09 2d 85 2d b1 02 95 02 09 48 85 48 b1 02 95 0f 09 2e 85 2e 81 02 95 1f 09 2f 85 2f 81 02 95 3e 09 30 85 30 81 02 95 fe 09 31 85 31 81 02 96 fe 01 09 32 85 32 81 02 75 08 96 fe 0f 09 35 85 35 81 02 c0 05 0d 09 02 a1 01 85 01 09 20 35 00 a1 00 09 32 09 42 09 44 09 3c 09 45 15 00 25 01 75 01 95 05 81 02 95 03 81 03 05 01 09 30 75 10 95 01 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 b4 05 0d 09 30 26 00 01 81 02 06 00 ff 09 01 81 02 c0 85 0c 06 00 ff 09 0c 75 08 95 06 26 ff 00 b1 02 85 0b 09 0b 95 02 b1 02 85 11 09 11 b1 02 85 15 09 15 95 05 b1 02 85 18 09 18 95 0c b1 02 c0 05 0d 09 04 a1 01 85 03 06 00 ff 09 01 75 10 95 01 15 00 27 ff ff 00 00 81 02 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 15 0a 26 80 25 81 02 09 31 46 b4 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 54 95 01 75 08 81 02 09 56 75 20 95 01 27 ff ff ff 0f 81 02 85 04 09 55 75 08 95 01 25 0b b1 02 85 0a 06 00 ff 09 03 15 00 b1 02 85 1b 06 00 ff 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 01 09 02 a1 01 85 02 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 75 01 95 02 81 02 95 06 81 03 05 01 09 30 09 31 15 81 25 7f 75 08 95 02 81 06 c0 c0",
        )


cwass Testn_twig_1b96_0f00(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test n_twig_1b96_0f00",
            wdesc="75 08 15 00 26 ff 00 06 0b ff 09 0b a1 01 95 0f 09 29 85 29 b1 02 95 1f 09 2a 85 2a b1 02 95 3e 09 2b 85 2b b1 02 95 fe 09 2c 85 2c b1 02 96 fe 01 09 2d 85 2d b1 02 95 02 09 48 85 48 b1 02 95 0f 09 2e 85 2e 81 02 95 1f 09 2f 85 2f 81 02 95 3e 09 30 85 30 81 02 95 fe 09 31 85 31 81 02 96 fe 01 09 32 85 32 81 02 75 08 96 fe 0f 09 35 85 35 81 02 c0 05 0d 09 02 a1 01 85 01 09 20 35 00 a1 00 09 32 09 42 09 44 09 3c 09 45 15 00 25 01 75 01 95 05 81 02 95 03 81 03 05 01 09 30 75 10 95 01 a4 55 0e 65 11 46 03 0a 26 80 25 81 02 09 31 46 a1 05 26 20 1c 81 02 b4 05 0d 09 30 26 00 01 81 02 06 00 ff 09 01 81 02 c0 85 0c 06 00 ff 09 0c 75 08 95 06 26 ff 00 b1 02 85 0b 09 0b 95 02 b1 02 85 11 09 11 b1 02 85 15 09 15 95 05 b1 02 85 18 09 18 95 0c b1 02 c0 05 0d 09 04 a1 01 85 03 06 00 ff 09 01 75 10 95 01 15 00 27 ff ff 00 00 81 02 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 03 0a 26 80 25 81 02 09 31 46 a1 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 03 0a 26 80 25 81 02 09 31 46 a1 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 54 95 01 75 08 81 02 09 56 75 20 95 01 27 ff ff ff 0f 81 02 85 04 09 55 75 08 95 01 25 0b b1 02 85 0a 06 00 ff 09 03 15 00 b1 02 85 1b 06 00 ff 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 01 09 02 a1 01 85 02 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 75 01 95 02 81 02 95 06 81 03 05 01 09 30 09 31 15 81 25 7f 75 08 95 02 81 06 c0 c0",
        )


cwass Testn_twig_1b96_0f04(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test n_twig_1b96_0f04",
            wdesc="75 08 15 00 26 ff 00 06 0b ff 09 0b a1 01 95 0f 09 29 85 29 b1 02 95 1f 09 2a 85 2a b1 02 95 3e 09 2b 85 2b b1 02 95 fe 09 2c 85 2c b1 02 96 fe 01 09 2d 85 2d b1 02 95 02 09 48 85 48 b1 02 95 0f 09 2e 85 2e 81 02 95 1f 09 2f 85 2f 81 02 95 3e 09 30 85 30 81 02 95 fe 09 31 85 31 81 02 96 fe 01 09 32 85 32 81 02 75 08 96 fe 0f 09 35 85 35 81 02 c0 05 0d 09 02 a1 01 85 01 09 20 35 00 a1 00 09 32 09 42 09 44 09 3c 09 45 15 00 25 01 75 01 95 05 81 02 95 03 81 03 05 01 09 30 75 10 95 01 a4 55 0e 65 11 46 7f 0b 26 80 25 81 02 09 31 46 78 06 26 20 1c 81 02 b4 05 0d 09 30 26 00 01 81 02 06 00 ff 09 01 81 02 c0 85 0c 06 00 ff 09 0c 75 08 95 06 26 ff 00 b1 02 85 0b 09 0b 95 02 b1 02 85 11 09 11 b1 02 85 15 09 15 95 05 b1 02 85 18 09 18 95 0c b1 02 c0 05 0d 09 04 a1 01 85 03 06 00 ff 09 01 75 10 95 01 15 00 27 ff ff 00 00 81 02 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 7f 0b 26 80 25 81 02 09 31 46 78 06 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 7f 0b 26 80 25 81 02 09 31 46 78 06 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 54 95 01 75 08 81 02 09 56 75 20 95 01 27 ff ff ff 0f 81 02 85 04 09 55 75 08 95 01 25 0b b1 02 85 0a 06 00 ff 09 03 15 00 b1 02 85 1b 06 00 ff 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 01 09 02 a1 01 85 02 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 75 01 95 02 81 02 95 06 81 03 05 01 09 30 09 31 15 81 25 7f 75 08 95 02 81 06 c0 c0",
        )


cwass Testn_twig_1b96_1000(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn PenDigitizew(
            "uhid test n_twig_1b96_1000",
            wdesc="75 08 15 00 26 ff 00 06 0b ff 09 0b a1 01 95 0f 09 29 85 29 b1 02 95 1f 09 2a 85 2a b1 02 95 3e 09 2b 85 2b b1 02 95 fe 09 2c 85 2c b1 02 96 fe 01 09 2d 85 2d b1 02 95 02 09 48 85 48 b1 02 95 0f 09 2e 85 2e 81 02 95 1f 09 2f 85 2f 81 02 95 3e 09 30 85 30 81 02 95 fe 09 31 85 31 81 02 96 fe 01 09 32 85 32 81 02 75 08 96 fe 0f 09 35 85 35 81 02 c0 05 0d 09 02 a1 01 85 01 09 20 35 00 a1 00 09 32 09 42 09 44 09 3c 09 45 15 00 25 01 75 01 95 05 81 02 95 03 81 03 05 01 09 30 75 10 95 01 a4 55 0e 65 11 46 03 0a 26 80 25 81 02 09 31 46 a1 05 26 20 1c 81 02 b4 05 0d 09 30 26 00 01 81 02 06 00 ff 09 01 81 02 c0 85 0c 06 00 ff 09 0c 75 08 95 06 26 ff 00 b1 02 85 0b 09 0b 95 02 b1 02 85 11 09 11 b1 02 85 15 09 15 95 05 b1 02 85 18 09 18 95 0c b1 02 c0 05 0d 09 04 a1 01 85 03 06 00 ff 09 01 75 10 95 01 15 00 27 ff ff 00 00 81 02 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 03 0a 26 80 25 81 02 09 31 46 a1 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 01 81 03 09 47 81 02 95 05 81 03 75 10 09 51 27 ff ff 00 00 95 01 81 02 05 01 09 30 75 10 95 02 a4 55 0e 65 11 46 03 0a 26 80 25 81 02 09 31 46 a1 05 26 20 1c 81 02 05 0d 09 48 95 01 26 80 25 81 02 09 49 26 20 1c 81 02 b4 06 00 ff 09 02 75 08 95 04 15 00 26 ff 00 81 02 c0 05 0d 09 54 95 01 75 08 81 02 09 56 75 20 95 01 27 ff ff ff 0f 81 02 85 04 09 55 75 08 95 01 25 0b b1 02 85 0a 06 00 ff 09 03 15 00 b1 02 85 1b 06 00 ff 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 01 09 02 a1 01 85 02 09 01 a1 00 05 09 19 01 29 02 15 00 25 01 75 01 95 02 81 02 95 06 81 03 05 01 09 30 09 31 15 81 25 7f 75 08 95 02 81 06 c0 c0",
        )


cwass TestGXTP_27c6_0113(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn GXTP_pen(
            "uhid test GXTP_27c6_0113",
            wdesc="05 0d 09 04 a1 01 85 01 09 22 a1 02 55 0e 65 11 35 00 15 00 09 42 25 01 75 01 95 01 81 02 95 07 81 01 95 01 75 08 09 51 81 02 75 10 05 01 26 00 14 46 1f 07 09 30 81 02 26 80 0c 46 77 04 09 31 81 02 05 0d c0 09 22 a1 02 09 42 25 01 75 01 95 01 81 02 95 07 81 01 95 01 75 08 09 51 81 02 75 10 05 01 26 00 14 46 1f 07 09 30 81 02 26 80 0c 46 77 04 09 31 81 02 05 0d c0 09 22 a1 02 09 42 25 01 75 01 95 01 81 02 95 07 81 01 95 01 75 08 09 51 81 02 75 10 05 01 26 00 14 46 1f 07 09 30 81 02 26 80 0c 46 77 04 09 31 81 02 05 0d c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 07 81 01 75 08 09 51 95 01 81 02 05 01 26 00 14 75 10 55 0e 65 11 09 30 35 00 46 1f 07 81 02 26 80 0c 46 77 04 09 31 81 02 05 0d c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 95 07 81 01 75 08 09 51 95 01 81 02 05 01 26 00 14 75 10 55 0e 65 11 09 30 35 00 46 1f 07 81 02 26 80 0c 46 77 04 09 31 81 02 05 0d c0 09 54 15 00 25 7f 75 08 95 01 81 02 85 02 09 55 95 01 25 0a b1 02 85 03 06 00 ff 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 0d 09 02 a1 01 85 08 09 20 a1 00 09 42 09 44 09 3c 09 45 15 00 25 01 75 01 95 04 81 02 95 01 81 03 09 32 81 02 95 02 81 03 95 01 75 08 09 51 81 02 05 01 09 30 75 10 95 01 a4 55 0e 65 11 35 00 26 00 14 46 1f 07 81 42 09 31 26 80 0c 46 77 04 81 42 b4 05 0d 09 30 26 ff 0f 81 02 09 3d 65 14 55 0e 36 d8 dc 46 28 23 16 d8 dc 26 28 23 81 02 09 3e 81 02 c0 c0 06 f0 ff 09 01 a1 01 85 0e 09 01 15 00 25 ff 75 08 95 40 91 02 09 01 15 00 25 ff 75 08 95 40 81 02 c0 05 01 09 06 a1 01 85 04 05 07 09 e3 15 00 25 01 75 01 95 01 81 02 95 07 81 03 c0",
        )


################################################################################
#
# Windows 8 compatibwe devices with USI Pen
#
################################################################################


cwass TestEwan_04f3_2A49(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn USIPen(
            "uhid test Ewan_04f3_2A49",
            wdesc="05 0d 09 04 a1 01 85 01 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 75 01 81 03 75 06 09 51 25 3f 81 02 26 ff 00 75 08 55 0f 65 11 35 00 45 ff 09 48 81 02 09 49 81 02 09 30 81 02 95 01 05 01 a4 26 cf 0f 75 10 55 0f 65 11 09 30 35 00 46 26 01 95 01 81 02 26 77 0a 46 a6 00 09 31 81 02 b4 c0 05 0d 09 22 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 75 01 81 03 75 06 09 51 25 3f 81 02 26 ff 00 75 08 55 0f 65 11 35 00 45 ff 09 48 81 02 09 49 81 02 09 30 81 02 95 01 05 01 a4 26 cf 0f 75 10 55 0f 65 11 09 30 35 00 46 26 01 95 01 81 02 26 77 0a 46 a6 00 09 31 81 02 b4 c0 05 0d 09 22 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 75 01 81 03 75 06 09 51 25 3f 81 02 26 ff 00 75 08 55 0f 65 11 35 00 45 ff 09 48 81 02 09 49 81 02 09 30 81 02 95 01 05 01 a4 26 cf 0f 75 10 55 0f 65 11 09 30 35 00 46 26 01 95 01 81 02 26 77 0a 46 a6 00 09 31 81 02 b4 c0 05 0d 09 22 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 75 01 81 03 75 06 09 51 25 3f 81 02 26 ff 00 75 08 55 0f 65 11 35 00 45 ff 09 48 81 02 09 49 81 02 09 30 81 02 95 01 05 01 a4 26 cf 0f 75 10 55 0f 65 11 09 30 35 00 46 26 01 95 01 81 02 26 77 0a 46 a6 00 09 31 81 02 b4 c0 05 0d 09 22 a1 02 05 0d 09 42 15 00 25 01 75 01 95 01 81 02 75 01 81 03 75 06 09 51 25 3f 81 02 26 ff 00 75 08 55 0f 65 11 35 00 45 ff 09 48 81 02 09 49 81 02 09 30 81 02 95 01 05 01 a4 26 cf 0f 75 10 55 0f 65 11 09 30 35 00 46 26 01 95 01 81 02 26 77 0a 46 a6 00 09 31 81 02 b4 c0 05 0d 09 54 25 7f 96 01 00 75 08 81 02 85 0a 09 55 25 0a b1 02 85 44 06 00 ff 09 c5 16 00 00 26 ff 00 75 08 96 00 01 b1 02 c0 06 ff 01 09 01 a1 01 85 02 16 00 00 26 ff 00 75 08 95 40 09 00 81 02 c0 06 00 ff 09 01 a1 01 85 03 75 08 95 20 09 01 91 02 c0 06 00 ff 09 01 a1 01 85 06 09 03 75 08 95 12 91 02 09 04 75 08 95 03 b1 02 c0 06 01 ff 09 01 a1 01 85 04 15 00 26 ff 00 75 08 95 13 09 00 81 02 c0 05 0d 09 02 a1 01 85 07 35 00 09 20 a1 00 09 32 09 42 09 44 09 3c 09 45 15 00 25 01 75 01 95 05 81 02 95 03 81 03 05 01 09 30 75 10 95 01 a4 55 0f 65 11 46 26 01 26 1c 48 81 42 09 31 46 a6 00 26 bc 2f 81 42 b4 05 0d 09 30 26 00 10 81 02 75 08 95 01 09 3b 25 64 81 42 09 38 15 00 25 02 81 02 09 5c 26 ff 00 81 02 09 5e 81 02 09 70 a1 02 15 01 25 06 09 72 09 73 09 74 09 75 09 76 09 77 81 20 09 5b 25 ff 75 40 81 02 c0 06 00 ff 75 08 95 02 09 01 81 02 c0 05 0d 85 60 09 81 a1 02 09 38 75 08 95 01 15 00 25 02 81 02 09 81 15 01 25 04 09 82 09 83 09 84 09 85 81 20 c0 85 61 09 5c a1 02 15 00 26 ff 00 75 08 95 01 09 38 b1 02 09 5c 26 ff 00 b1 02 09 5d 75 01 95 01 25 01 b1 02 95 07 b1 03 c0 85 62 09 5e a1 02 09 38 15 00 25 02 75 08 95 01 b1 02 09 5e 26 ff 00 b1 02 09 5f 75 01 25 01 b1 02 75 07 b1 03 c0 85 63 09 70 a1 02 75 08 95 01 15 00 25 02 09 38 b1 02 09 70 a1 02 25 06 09 72 09 73 09 74 09 75 09 76 09 77 b1 20 c0 09 71 75 01 25 01 b1 02 75 07 b1 03 c0 85 64 09 80 15 00 25 ff 75 40 95 01 b1 02 85 65 09 44 a1 02 09 38 75 08 95 01 25 02 b1 02 15 01 25 03 09 44 a1 02 09 a4 09 44 09 5a 09 45 09 a3 b1 20 c0 09 5a a1 02 09 a4 09 44 09 5a 09 45 09 a3 b1 20 c0 09 45 a1 02 09 a4 09 44 09 5a 09 45 09 a3 b1 20 c0 c0 85 66 75 08 95 01 05 0d 09 90 a1 02 09 38 25 02 b1 02 09 91 75 10 26 ff 0f b1 02 09 92 75 40 25 ff b1 02 05 06 09 2a 75 08 26 ff 00 a1 02 09 2d b1 02 09 2e b1 02 c0 c0 85 67 05 06 09 2b a1 02 05 0d 25 02 09 38 b1 02 05 06 09 2b a1 02 09 2d 26 ff 00 b1 02 09 2e b1 02 c0 c0 85 68 06 00 ff 09 01 a1 02 05 0d 09 38 75 08 95 01 25 02 b1 02 06 00 ff 09 01 75 10 27 ff ff 00 00 b1 02 c0 85 69 05 0d 09 38 75 08 95 01 15 00 25 02 b1 02 c0 06 00 ff 09 81 a1 01 85 17 75 08 95 1f 09 05 81 02 c0",
            input_info=(BusType.I2C, 0x04F3, 0x2A49),
        )


cwass TestGoodix_27c6_0e00(BaseTest.TestTabwet):
    def cweate_device(sewf):
        wetuwn USIPen(
            "uhid test Ewan_04f3_2A49",
            wdesc="05 0d 09 04 a1 01 85 01 09 22 a1 02 55 0e 65 11 35 00 15 00 09 42 25 01 75 01 95 01 81 02 25 7f 09 30 75 07 81 42 95 01 75 08 09 51 81 02 75 10 05 01 26 04 20 46 e6 09 09 30 81 02 26 60 15 46 9a 06 09 31 81 02 05 0d 55 0f 75 08 25 ff 45 ff 09 48 81 42 09 49 81 42 55 0e c0 09 22 a1 02 09 42 25 01 75 01 95 01 81 02 25 7f 09 30 75 07 81 42 95 01 75 08 09 51 81 02 75 10 05 01 26 04 20 46 e6 09 09 30 81 02 26 60 15 46 9a 06 09 31 81 02 05 0d 55 0f 75 08 25 ff 45 ff 09 48 81 42 09 49 81 42 55 0e c0 09 22 a1 02 09 42 25 01 75 01 95 01 81 02 25 7f 09 30 75 07 81 42 95 01 75 08 09 51 81 02 75 10 05 01 26 04 20 46 e6 09 09 30 81 02 26 60 15 46 9a 06 09 31 81 02 05 0d 55 0f 75 08 25 ff 45 ff 09 48 81 42 09 49 81 42 55 0e c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 25 7f 09 30 75 07 81 42 75 08 09 51 95 01 81 02 05 01 26 04 20 75 10 55 0e 65 11 09 30 35 00 46 e6 09 81 02 26 60 15 46 9a 06 09 31 81 02 05 0d 55 0f 75 08 25 ff 45 ff 09 48 81 42 09 49 81 42 55 0e c0 09 22 a1 02 09 42 15 00 25 01 75 01 95 01 81 02 25 7f 09 30 75 07 81 42 75 08 09 51 95 01 81 02 05 01 26 04 20 75 10 55 0e 65 11 09 30 35 00 46 e6 09 81 02 26 60 15 46 9a 06 09 31 81 02 05 0d 55 0f 75 08 25 ff 45 ff 09 48 81 42 09 49 81 42 55 0e c0 09 54 15 00 25 7f 75 08 95 01 81 02 85 02 09 55 95 01 25 0a b1 02 85 03 06 00 ff 09 c5 15 00 26 ff 00 75 08 96 00 01 b1 02 c0 05 0d 09 02 a1 01 09 20 a1 00 85 08 05 01 a4 09 30 35 00 46 e6 09 15 00 26 04 20 55 0d 65 13 75 10 95 01 81 02 09 31 46 9a 06 26 60 15 81 02 b4 05 0d 09 38 95 01 75 08 15 00 25 01 81 02 09 30 75 10 26 ff 0f 81 02 09 31 81 02 09 42 09 44 09 5a 09 3c 09 45 09 32 75 01 95 06 25 01 81 02 95 02 81 03 09 3d 55 0e 65 14 36 d8 dc 46 28 23 16 d8 dc 26 28 23 95 01 75 10 81 02 09 3e 81 02 09 41 15 00 27 a0 8c 00 00 35 00 47 a0 8c 00 00 81 02 05 20 0a 53 04 65 00 16 01 f8 26 ff 07 75 10 95 01 81 02 0a 54 04 81 02 0a 55 04 81 02 0a 57 04 81 02 0a 58 04 81 02 0a 59 04 81 02 0a 72 04 81 02 0a 73 04 81 02 0a 74 04 81 02 05 0d 09 3b 15 00 25 64 75 08 81 02 09 5b 25 ff 75 40 81 02 06 00 ff 09 5b 75 20 81 02 05 0d 09 5c 26 ff 00 75 08 81 02 09 5e 81 02 09 70 a1 02 15 01 25 06 09 72 09 73 09 74 09 75 09 76 09 77 81 20 c0 06 00 ff 09 01 15 00 27 ff ff 00 00 75 10 95 01 81 02 85 09 09 81 a1 02 09 81 15 01 25 04 09 82 09 83 09 84 09 85 81 20 c0 85 10 09 5c a1 02 15 00 25 01 75 08 95 01 09 38 b1 02 09 5c 26 ff 00 b1 02 09 5d 75 01 95 01 25 01 b1 02 95 07 b1 03 c0 85 11 09 5e a1 02 09 38 15 00 25 01 75 08 95 01 b1 02 09 5e 26 ff 00 b1 02 09 5f 75 01 25 01 b1 02 75 07 b1 03 c0 85 12 09 70 a1 02 75 08 95 01 15 00 25 01 09 38 b1 02 09 70 a1 02 25 06 09 72 09 73 09 74 09 75 09 76 09 77 b1 20 c0 09 71 75 01 25 01 b1 02 75 07 b1 03 c0 85 13 09 80 15 00 25 ff 75 40 95 01 b1 02 85 14 09 44 a1 02 09 38 75 08 95 01 25 01 b1 02 15 01 25 03 09 44 a1 02 09 a4 09 44 09 5a 09 45 09 a3 b1 20 c0 09 5a a1 02 09 a4 09 44 09 5a 09 45 09 a3 b1 20 c0 09 45 a1 02 09 a4 09 44 09 5a 09 45 09 a3 b1 20 c0 c0 85 15 75 08 95 01 05 0d 09 90 a1 02 09 38 25 01 b1 02 09 91 75 10 26 ff 0f b1 02 09 92 75 40 25 ff b1 02 05 06 09 2a 75 08 26 ff 00 a1 02 09 2d b1 02 09 2e b1 02 c0 c0 85 16 05 06 09 2b a1 02 05 0d 25 01 09 38 b1 02 05 06 09 2b a1 02 09 2d 26 ff 00 b1 02 09 2e b1 02 c0 c0 85 17 06 00 ff 09 01 a1 02 05 0d 09 38 75 08 95 01 25 01 b1 02 06 00 ff 09 01 75 10 27 ff ff 00 00 b1 02 c0 85 18 05 0d 09 38 75 08 95 01 15 00 25 01 b1 02 c0 c0 06 f0 ff 09 01 a1 01 85 0e 09 01 15 00 25 ff 75 08 95 40 91 02 09 01 15 00 25 ff 75 08 95 40 81 02 c0",
            input_info=(BusType.I2C, 0x27C6, 0x0E00),
        )
