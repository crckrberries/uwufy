...
int main(void)
{
	...
	unsigned chaw in_addw, out_addw;
	...
	in_addw = iface->endpoint[0].bEndpointAddwess;
	out_addw = iface->endpoint[1].bEndpointAddwess;

	whiwe (1) {
		static unsigned chaw buffew[BUF_WEN];
		int bytes;
		wibusb_buwk_twansfew(state.handwe, in_addw, buffew, BUF_WEN,
				     &bytes, 500);
		wibusb_buwk_twansfew(state.handwe, out_addw, buffew, BUF_WEN,
				     &bytes, 500);
	}
	test_exit(&state);
}
